...
nsFreshcare.admin.trainingpackage = 
{
	...
	home: 		function (oParam, oResponse)
	{
		if (oResponse == undefined)
		{
			...
			oSearch.addField('agritrainingpackage.membership,agritrainingpackage.membershiptext,agritrainingpackage.title,agritrainingpackage.availablefrom' +
							',agritrainingpackage.availableto,agritrainingpackage.segrouptype.title');
			...
			if (nsFreshcare.user.roleID != nsFreshcare.data.roles.admin) 
			{
				...
				oSearch.addBracket(')');
				// v3.4.004 SUP023891 Filter so that external users only see pcakages linked to Members
				oSearch.addFilter('segrouptype', 'EQUAL_TO', nsFreshcare.data.businessGroupGrowerID);
			}
		...
		else
		{
			...
			else
			{
				...
				aHTML.push('<div class="ns1blankspaceCaption"' +
							...
							(nsFreshcare.user.roleID === nsFreshcare.data.roles.admin ? 'MOST RECENTLY UPDATED' : '') + '</div>');
				...
				if (nsFreshcare.user.roleID == nsFreshcare.data.roles.admin)
				{
					aHTML.push('<td class="ns1blankspaceCaption">Relates To</td>');
				}
				...
				$.each(oResponse.data.rows, function()
				{
					...
					if (nsFreshcare.user.roleID == nsFreshcare.data.roles.admin)
					{
						aHTML.push('<td id="ns1blankspaceMostLikely_group-' + this.id + '" class="ns1blankspaceMostLikely' + ((bAvailable) ? '' : ' nsFreshcareInactive') + '">' +
												this['agritrainingpackage.segrouptype.title'] + '</td>');
					}
	...
	search: 	
	{
		send: 		function (sXHTMLElementId, oParam)
		{
			...
			if (sSearchContext != undefined && iSource != ns1blankspace.data.searchSource.browse)
			{
				...
				oSearch.method = 'AGRI_EDUCATION_TRAINING_PACKAGE_SEARCH';
				oSearch.addField('agritrainingpackage.reference,agritrainingpackage.title,agritrainingpackage.details,agritrainingpackage.segrouptype.title' +
								',agritrainingpackage.availablefrom,agritrainingpackage.availableto,agritrainingpackage.membership,agritrainingpackage.segrouptype' +
	...
	summary: function ()
	{
		...
		else
		{
			...
			if (nsFreshcare.user.roleID === nsFreshcare.data.roles.admin)
			{
				aHTML.push('<tr><td class="ns1blankspaceSummaryCaption">Relates To...</td></tr>' +
							'<tr><td class="ns1blankspaceSummary">' +
							ns1blankspace.objectContextData['agritrainingpackage.segrouptype.title'] +
							'</td></tr>');
			}
		...
		if ($('#ns1blankspaceMainDetails').attr('data-loading') == '1')
		{
			...
			if (nsFreshcare.user.roleID === nsFreshcare.data.roles.admin)
			{
				aHTML.push('<tr class="ns1blankspaceCaption">' +
							...
							'<input id="ns1blankspaceDetailsGroupType" class="ns1blankspaceSelect' + 
								((nsFreshcare.user.roleID != nsFreshcare.data.roles.admin) ? ' nsFreshcareDisabled' : '') + '"' +
								' data-mandatory="1" data-caption="Relates To.."' +
								' data-method="SETUP_CONTACT_BUSINESS_GROUP_SEARCH"' +
								' data-methodFilter="id-IN_LIST-' + nsFreshcare.data.businessGroupGrowerID + ',' +
			...
			aHTML.push('<tr class="ns1blankspaceCaption">' +
							...
							'<textarea rows="5" cols="35" id="ns1blankspaceDetailsDetails" class="ns1blankspaceTextMulti' + 
								((nsFreshcare.user.roleID != nsFreshcare.data.roles.admin) ? ' nsFreshcareDisabled' : '') + '"></textarea>' +
							'</td></tr>');							
			...
			if (ns1blankspace.objectContextData != undefined)
			{
				...
				$('#ns1blankspaceDetailsGroupType').val(ns1blankspace.objectContextData['agritrainingpackage.segrouptype.title'].formatXHTML());
				$('#ns1blankspaceDetailsGroupType').attr('data-id', ns1blankspace.objectContextData['agritrainingpackage.segrouptype']);
				...
				$('#ns1blankspaceDetailsGroupType').val(nsFreshcare.data.growerText);
				$('#ns1blankspaceDetailsGroupType').attr('data-id', nsFreshcare.data.businessGroupGrowerID);
	...
	trainingCourses: 
	{
			...
			if ($('#ns1blankspaceMainCourses').attr('data-loading') == '1')
			{
				...
				if (oResponse === undefined && (oParam.response === undefined)) {
					...
					aHTML.push('<tr><td class="ns1blankspaceHeaderCaption">Title</td>' + 
							        ...
							        '<td class="ns1blankspaceHeaderCaption ns1blankspacePackageCourseAdd"' +
							        	' title="Add Training Course"></td>' +
							    '</tr>');
					
					$.each(oResponse.data.rows, function() 
					{
						aHTML.push(nsFreshcare.admin.trainingpackage.trainingCourses.row(this, oParam));
					});
					...
					$.extend(true, oParam,
					{
						xhtmlElementID: 'ns1blankspaceCoursesColumn1',
						xhtmlContext: 'Courses',
						xhtml: aHTML.join(''),
						showMore: (oResponse.morerows === "true"),
						columns: oParam.columns,
						more: oResponse.moreid,
						rows: oResponse.rows,
						functionSearch: nsFreshcare.admin.trainingpackage.trainingCourses.search,
						functionShowRow: nsFreshcare.admin.trainingpackage.trainingCourses.row,
						functionOnNewPage: nsFreshcare.admin.trainingpackage.trainingCourses.bind,
						type: 'json'
					}); 	
		...
		row: function(oRow, oParam)
		{
			...
			aHTML.push('<tr id="ns1blankspaceCourseRow_' + oRow.id + '">');
			
			aHTML.push('<td class="ns1blankspaceRow ns1blankspaceRowPackageCourse"' + 
							' id="ns1blankspaceCourseTitle_' + oRow.id + 
							'">' +
								oRow["title"] + 
							'</td>');

			aHTML.push('<td class="ns1blankspaceRow ns1blankspaceRowPackageCourse"' + 
							' id="ns1blankspaceCourseDetail_' + oRow.id + 
							'">' +
								oRow["details"] + 
							'</td>');

			aHTML.push('<td class="ns1blankspaceRow ns1blankspaceRowPackageCourse"' + 
							' id="ns1blankspaceCourseDate_' + oRow.id + 
							'">' +
								oRow["coursedate"] + 
							'</td>');
			
			aHTML.push('<td class="ns1blankspaceRow ns1blankspaceRowPackageCourse"' + 
							' id="ns1blankspaceCourseTrainer_' + oRow.id + 
							'">' +
								oRow['trainercontactpersontext'] + 
							'</td>');

			aHTML.push('<td class="ns1blankspaceRow ns1blankspaceRowPackageCourse"' + 
							' id="ns1blankspaceCourseStatus_' + oRow.id + 
							'">' +
								oRow['statustext'] + 
							'</td>');
			...
			aHTML.push('<td id=ns1blankspaceCourseActions">' +
							'<span id="ns1blankspaceCourseShow-' + oRow.id + '" ' + 
							'class="ns1blankspaceAction ns1blankspaceCourseShow" ' + 
							'data-rowID="' + oRow.id + '" title="Open"></span>');

			if (oRow.status != nsFreshcare.data.trainingCourse.statusCompleted) 
			{
				aHTML.push('<span id="ns1blankspaceCourseRemove_' + oRow.id + '" ' + 
								'class="ns1blankspaceAction ns1blankspaceCourseRemove" ' + 
								'data-rowID="' + oRow.id + '" title="Remove"></span>');
			}
			aHTML.push('</td></tr>');
		...
		bind: function(oParam)
		{
			...
			$('ns1blankspacePackageCourseAdd:visible').button(
			{
			...
			$('.ns1blankspaceCourseShow:visible').click(function(event) {
			...
			// Remove Course 
			$('.ns1blankspaceCourseRemove:visible').button(
			{
				text: false,
	...
	{		
		...
		send: 	function(oParam) 
		{
			...
			else if (oParam.savePackageStep === 2)
			{
				...
				if (nsFreshcare.user.roleID === nsFreshcare.data.roles.admin)
				{
					sData += '&segrouptype=' + ns1blankspace.util.fs($('#ns1blankspaceDetailsGroupType').attr('data-id'));
				}
