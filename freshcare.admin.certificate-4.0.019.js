...
nsFreshcare.admin.certificate = 
{
	...
	checkForNewVersion: function(oParam)
	{
		...
		else if (oParam.checkNewVersionStep === 2)
		{
			...
			if (oPreviousCertificate != undefined)
			{
				...
				// v4.0.019 SUP024339 Added 
				if (oCertificate.ioasAccredited && oPreviousCertificate.ioasAccredited == undefined)
				{
					oPreviousCertificate.ioasAccredited = oCertificate.ioasAccredited;
				}
	...
	getDifferencesHTML: function(oBefore, oAfter, sKey)
	{
		...
		[
			...
			{name: 'ioasAccredited', caption: 'IOAS Certificate'},
	...
	certificateGetExpiry: function(oParam)
	{
		...
		else
		{
			...
			var dBefore = (new Date(dAnniversaryStart)).add({days: (iDaysBefore * -1)})
			if (dAudit >= dBefore)
	...
	certificateGetData: function(oParam) 
	{
		...
		if (iCertificate) 
		{
			...
			if (iCertificateGetDataStep === 1) 
			{
				...
				// v4.0.019 Added IOASAccredites & seioasaudit
				var oSearch = new AdvancedSearch();
				oSearch.method = 'AGRI_CERTIFICATE_SEARCH';
				oSearch.addField('agricertificate.certificatenumber,agricertificate.externalcertificatenumber,agricertificate.enddate,agricertificate.dateissued,agricertificate.audit' +
								...
								',agricertificate.audit.sefullcertdata,agricertificate.audit.seextensionmonths,agricertificate.subscription.membership.seioasaccredited' +
								',agricertificate.audit.seioasaudit');
					...
					if (oResponse.status === 'OK') 
					{
						if (oResponse.data.rows.length > 0)
						{
							...
							if (oParam.certificateData === undefined) 
							{
								...
								// v4.0.019 SUP024339 Added IOAS flag
								oParam.certificateData['jasanzAccredited'] = (oRow['agricertificate.audit.sejasanzaudit'] == "Y" ? 'Y' : 'N');
								oParam.certificateData['ioasAccredited'] = (oRow['agricertificate.audit.seioasaudit'] == "Y" ? 'Y' : 'N');
	...
	certificateGetTemplate: function(oParam)
	{
		...
		//    if membership if jasanz accredited and Audit's JASANZAudit flag is Y, use jasanz templates
		//    if membership if ioas accredited and Audit's IOASAudit flag is Y, use ioas templates
		//	  otherwise use fc templates

		// Mandatory parameters are:
		// membership (non-blank)
		// certificateData.jasanzAccredited
		// certificateData.ioasAccredited
		...
		if (oParam.certificateData.ioasAccredited == undefined) {bValidParms = false;}
		...
		if (bValidParms)
		{
			...
			if (dIssued < dSelfCertStart)
			{
				...
				oParam.certificateData.ioasAccredited = 'N';
			}
			...
			else 
			{
				...
				// If IOAS Audit (already set in certificateGetData), apply ioasCertificate templates
				else if (oParam.certificateData.ioasAccredited === 'Y')
				{
					oParam.template = oMembership.secertificatetextioas;
				}
				else
				{
