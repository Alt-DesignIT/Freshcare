...
nsFreshcare.auditor.grower = 
{
		...
		else
		{
			...
			if (oResponse.data.rows.length == 0)
			{
				aHTML.push('<table id="ns1blankspaceMostLikely">' +
								'<tr><td class="ns1blankspaceNothing">Click New to inform Freshcare of an unregistered ' + nsFreshcare.data.growerText + '.</td></tr>' +
								'</table>');
			}
			else
			{
	...
	search: 	
	{
		...
		process: 	function (oParam, oResponse)
		{
			...
			{	
				...
				// v3.2.005 SUP022879 1st & 2nd page of results now match formats
	...
	layout: 	function ()
	{
		...
		$('#ns1blankspaceControlContacts').click(function(event)
		{
			...
			// v3.2.005 Was showing Go To Person for CB's
			// v3.3.001 SUP023603 Set hidePrimaryContact to true to omit primary contacts from the list
	...
	summary: 	function ()
	{
		...
		else
		{
			...
			// v3.4.004 SUP023867 Removed - can only add from Membership tab
	...
	trainer: 	function ()
	{
		...
		if ($('#ns1blankspaceMainTrainer').attr('data-loading') == '1') {
			...
			// v3.2.016 SUP023490 Had random 'xeroSuperUser' after Trainer causing tab to not display
			aHTML.push('<tr class="ns1blankspaceCaption">' +
							'<td class="ns1blankspaceCaption">' +
							'Trainer (if known)' +
							'</td></tr>' +
			...
			// v4.0.006 SUP023894 Added new fields
			aHTML.push('<tr class="ns1blankspaceCaption">' +
							'<td class="ns1blankspaceCaption">' +
							'Trainee Name (if known)' +
							'</td></tr>' +
							'<tr class="ns1blankspace">' +
							'<td class="ns1blankspaceText">' +
							'<input id="ns1blankspaceTrainerTraineeName" class="ns1blankspaceText">' +
							'</td></tr>');			

			aHTML.push('<tr class="ns1blankspaceCaption">' +
							'<td class="ns1blankspaceCaption">' +
							'Training Location (if known)' +
							'</td></tr>' +
							'<tr class="ns1blankspace">' +
							'<td class="ns1blankspaceText">' +
							'<input id="ns1blankspaceTrainerTrainingLocation" class="ns1blankspaceText">' +
							'</td></tr>');			
	...
	reinstateWithdrawnMembership: function(oParam)
	{
		...
		var sMessage = nsFreshcare.data.growerText + ': ' + sContactBusinessText + ', ' + sContactPersonText +
						'<br /><br />' +
						'Membership: ' + sMembershipText +
						'<br /><br />';

		// v3.2.005 SUP023190 Now sends from and replyTo
	...
	save: 	
	{
		validate: 	function(oParam) 
		{
						...
						$.each(ns1blankspace.objectContextData.memberships, function(x)
						{
							if ($('#ns1blankspaceMembership' + this.id).html() != '')
							{
									...
									&& this['agrisubscription.expirymonth'] != $('#ns1blankspaceMembership' + this.id + 'ExpiryMonthUpdate').attr('data-id'))
								{
									...
									else if ($('input[name="radio' + this.id + 'UpdateCertificateExpiry"]').is('*')
										&& $('input[name="radio' + this.id + 'UpdateCertificateExpiry"]:checked').val() == undefined)
									{
			...
			if (ns1blankspace.objectContext === -1) 
			{
				...
				// v4.0.006 SUP023894 Audit Date must be entered if Audit has Occurred
				if ($('#radioAuditOccurred1').prop('checked') && $('#ns1blankspaceDetailsAuditDateUpdate').val() == '')
				{
					ns1blankspace.okToSave = false;
					ns1blankspace.status.message('If the audit has occurred, you must specify the Audit date.');
				}

				if (ns1blankspace.okToSave && $('#ns1blankspaceMainTrainer').html() === '')
				{
					ns1blankspace.status.message('Please enter Training details');
					ns1blankspace.okToSave = false;
				}
		...
		send: 		function (oParam, oResponse)
		{
			...
			if (oParam) 
			{
				...
				if (oParam.dataMembershipAudit) {aDataMembershipAudit = oParam.dataMembershipAudit;}
			...
			else if (oParam.step > 0) 
			{
				...
				if (ns1blankspace.okToSave) 
				{
					if (oParam.step === 1) 
					{
						...
						if ($('#ns1blankspaceMainDetails').html() != '') 
						{
							...
							if (ns1blankspace.objectContext === -1) 
							{
								...
								// v4.0.006 SUP023894 Added new fields
								if ($('#ns1blankspaceDetailsMembershipsUpdate_SelectRows .ns1blankspaceMultiSelect').length > 0) 
								{
									aMessageHTML.push(nsFreshcare.external.grower.save.addToEmail(
									{
										label: 'Memberships',
										oldValue: '',
										newValue: $.map($('#ns1blankspaceDetailsMembershipsUpdate_SelectRows .ns1blankspaceMultiSelect'), 
											function(x) {return $(x).html().formatXHTML()}).join(', ')
									}));
								}

								if ($('#ns1blankspaceDetailsCertificateNumberUpdate').val() != '') 
								{
									aMessageHTML.push(nsFreshcare.external.grower.save.addToEmail({label: 'Certificate Number',
													oldValue: '',
													newValue: $('#ns1blankspaceDetailsCertificateNumberUpdate').val().formatXHTML()}));
								}

								if ($('#ns1blankspaceDetailsStateUpdate').val() != '') 
								{
									aMessageHTML.push(nsFreshcare.external.grower.save.addToEmail({label: 'State',
													oldValue: '',
													newValue: $('#ns1blankspaceDetailsStateUpdate').val().formatXHTML()}));
								}

								if ($('#ns1blankspaceDetailsAuditDateUpdate').val() != '') 
								{
									aMessageHTML.push(nsFreshcare.external.grower.save.addToEmail({label: 'Audit Date',
													oldValue: '',
													newValue: $('#ns1blankspaceDetailsAuditDateUpdate').val().formatXHTML()}));
								}
							}
							else 
							{
						...
						if ($('#ns1blankspaceMainAddress').html() != '') 
						{
							...
							{
								sDataBusiness += '&mailingaddress1=' + ns1blankspace.util.fs($('#ns1blankspaceAddressMailingAddress1Update').val()); 
							...
							{
								sDataBusiness += '&mailingaddress2=' + ns1blankspace.util.fs($('#ns1blankspaceAddressMailingAddress2Update').val()); 
							...
							{
								sDataBusiness += '&mailingsuburb=' + ns1blankspace.util.fs($('#ns1blankspaceAddressMailingSuburbUpdate').val()); 
							...
							{
								sDataBusiness += '&mailingstate=' + ns1blankspace.util.fs($('#ns1blankspaceAddressMailingStateUpdate').val()); 
							...
							{
								sDataBusiness += '&mailingpostcode=' + ns1blankspace.util.fs($('#ns1blankspaceAddressMailingPostCodeUpdate').val()); 
							...
							{
								sDataBusiness += '&mailingcountry=' + ns1blankspace.util.fs($('#ns1blankspaceAddressMailingCountryUpdate').val()); 
							...
							if (bChangedAddress) {aChangedFields.push("Mailing Address");}
						...
						if ($('#ns1blankspaceMainTrainer').html() != '') 
						{
							...
							// v4.0.006 SUP023894 Added new fields
							if ($('#ns1blankspaceTrainerTraineeName').val() != '') 
							{
								aMessageHTML.push(nsFreshcare.external.grower.save.addToEmail({label: 'Trainee Name',
											oldValue: '', newValue: $('#ns1blankspaceTrainerTraineeName').val()}));
							}

							if ($('#ns1blankspaceTrainerTrainingLocation').val() != '') 
							{
								aMessageHTML.push(nsFreshcare.external.grower.save.addToEmail({label: 'Traininf Location',
											oldValue: '', newValue: $('#ns1blankspaceTrainerTrainingLocation').val()}));
							}
						}
							...
							$.each(ns1blankspace.objectContextData.memberships, function(index)
							{
								...
								if ($('#ns1blankspaceMainMembership' + this.id).html() != '')
								{
										...
										&& this['agrisubscription.expirymonth'] != $('#ns1blankspaceMembership' + iMembershipId + 'ExpiryMonthUpdate').attr('data-id'))
									{
										...
										aChangedFields.push("Re-Cert Audit Month");		// v3.3.001 SUP023584 Added as wasn't including in subject
										...
										if ($('input[name="radio' + iMembershipId + 'UpdateCertificateExpiry"]:checked').val() == 'Y')
										{
											...
											// v3.3.005 SUP023800 Update Re-Cert Audit date on audit if cert expiry date is changing as well
											if (this['agrisubscription.agricertificate.audit'])
											{
												aDataMembershipAudit.push( 
												{
													id: this['agrisubscription.agricertificate.audit'], 
													serecertauditmonth: $('#ns1blankspaceMembership' + iMembershipId + 'ExpiryMonthUpdate').attr('data-id')
												});
											}
										}
									}
									...
									// v3.4.010 SUP023755 Added created date & user to Scope, Category & Site data when removing
									...
									if (this['agrisubscription.agrilastaudit.type'] == nsFreshcare.data.audit.typeScopeExtension 
										&& this['agrisubscription.agrilastaudit.resultstatus'] != nsFreshcare.data.audit.resultStatusCompleted
						...
						oParam.dataMembershipAudit = aDataMembershipAudit;
						oParam.messageHTML = aMessageHTML;
						oParam.changedFields = aChangedFields.join(', ');
						oParam.step = (ns1blankspace.objectContext == -1) ? 100 : 2;	// v3.3.007 Was still sending to step 10

					}
						
					// We only do a post if we're updating - otherwise we're just sending an email
					if (ns1blankspace.objectContext != -1 && oParam.step < 100) 
					{
						...
						else if (oParam.step === 4)
						{
							if (aDataBusinessSites.length > 0)
							{
								...
								oParam.increment = (aDataBusinessSites.length === 0) 
						...
						else if (oParam.step === 6)
						{
							if (aDataProductGroups.length > 0) 
							{
								...
								// v3.4.010 If removing, add a history row
								if (aThisProductGroup.data.indexOf('remove=1') > -1)
								{
									oHistoryData = 
									{
										object: nsFreshcare.objectSubscription,
										objectcontext: aThisProductGroup.subscription,
										seauditobject: 'agriproductgroupcategory',
										description: aThisProductGroup.value,
										text: ns1blankspace.objectContextData['contactbusiness.tradename'],
										createddate: aThisProductGroup.createddate,
										createduser: aThisProductGroup.createduser
									};
								}
								aDataProductGroups.shift();
								oParam = ns1blankspace.util.setParam(oParam, 'dataProductGroups', aDataProductGroups);
								oParam.increment = (aDataProductGroups.length === 0);
								...
								$.ajax(
								{
									...
									success: function(oResponse) 
									{
										if (oResponse.status === 'OK')
										{
											...
											if (oHistoryData)
											{
												oParam.onComplete = nsFreshcare.auditor.grower.save.send;
												oParam.historyData = oHistoryData;
												nsFreshcare.util.system.actionHistory.save(oParam, oResponse);
											}
											else
											{
												nsFreshcare.auditor.grower.save.send(oParam, oResponse); 	
						...
						else if (oParam.step === 7) 
						{
							if (aDataScopes.length > 0) 
							{
								...
								// v3.4.010 If removing, add a history row
								if (aThisScope.data.indexOf('remove=1') > -1)
								{
									oHistoryData = 
									{
										object: nsFreshcare.objectSubscription,
										objectcontext: aThisScope.subscription,
										seauditobject: 'objectscope',
										description: aThisScope.value,
										text: ns1blankspace.objectContextData['contactbusiness.tradename'],
										createddate: aThisScope.createddate,
										createduser: aThisScope.createduser
									};
								}
								aDataScopes.shift();
								oParam = ns1blankspace.util.setParam(oParam, 'dataScopes', aDataScopes);
								oParam.increment = (aDataScopes.length == 0) 
						...
						else if (oParam.step === 9)
						{
							if (aDataMembershipSites.length > 0) 
							{
								...
								// v3.4.010 If removing, add a history row
								if (aThisMembershipSite.data.indexOf('remove=1') > -1)
								{
									oHistoryData = 
									{
										object: nsFreshcare.objectSubscription,
										objectcontext: aThisMembershipSite.subscription,
										seauditobject: "addresslink",
										description: aThisMembershipSite.value,
										text: ns1blankspace.objectContextData['contactbusiness.tradename'],
										createddate: aThisMembershipSite.createddate,
										createduser: aThisMembershipSite.createduser
									};
								}
								aDataMembershipSites.shift();
								oParam = ns1blankspace.util.setParam(oParam, 'dataMembershipSites', aDataMembershipSites);
								oParam.increment = (aDataMembershipSites.length === 0) 
						...
						{
							if (aDataMembershipAudit.length > 0) 
							{
								var oThisMembershipAudit = aDataMembershipAudit[0];

								// Each row contains the data payload for each AUDIT_MANAGE call already formatted
								oParam.updating = 'Membership Audit\'s Re-Cert Audit Month ';
								
								aDataMembershipAudit.shift();
								oParam = ns1blankspace.util.setParam(oParam, 'dataMembershipAudit', aDataMembershipAudit);
								oParam.increment = (aDataMembershipAudit.length === 0)
								oParam.updating = 'Audit';

								$.ajax(
								{
									type: 'POST',
									url: ns1blankspace.util.endpointURI('AUDIT_MANAGE'),
									data: oThisMembershipAudit,
									dataType: 'json',
									success: function(oResponse) 
									{
										oParam.step = 10;
					...
					// We're just emailing - go straight to step 100
					...
					else if (oParam.step === 100 && aMessageHTML.length > 0) 
					{
						// v3.2.001 SUP022943
						// v3.1.213 SUP023254 Conditionally sets subject
						sChangedFieldsList	= 	oParam.changedFields;
						var sSubject = (ns1blankspace.objectContext == -1) 
										? 'Certification Body ' + ns1blankspace.user.contactBusinessText.formatXHTML() + ' has added a notification of an unregistered grower'
										: 'CB Update : [Fields Updated] ' +  sChangedFieldsList;
						delete(oParam.step);
						...
						delete(oParam.dataMembershipAudit);
						...
						// v3.1.215 SUP023190 Needs to send from contactPerson to pick up correct email alias
						// v3.2.005 SUP023190 Added replyTo
						var oEmailParam = {
											updateRows: aMessageHTML,
											subject: sSubject,
											to: nsFreshcare.data.emailToAuditor,
											from: (nsFreshcare.user.roleID == nsFreshcare.data.roles.admin ? ns1blankspace.user.contactPerson : nsFreshcare.data.emailFromAuditor),
											replyTo: (nsFreshcare.user.roleID == nsFreshcare.data.roles.admin ? undefined : ns1blankspace.user.contactPerson)
										}
						// v3.1.0b Since now using rpc messaging, need to send from verified account if switched-in
						// v3.2.022 SUP023622 emailFromAuditor set when switching in so this no longer necessary
