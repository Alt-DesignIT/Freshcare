...
nsFreshcare.internal = 
{
		...
		invoice:
		{
			home: function(oParam, oResponse)
			{
				...
				if (oResponse == undefined)
				{
					...
					$('#ns1blankspaceControlRoyaltyPayments').click(function(event)
					{
						...
						oParam.accountCode = '5001'; 	// v4.0.004 Changed from 4-1200
						...
						oParam.columns = [
											...
											{title: nsFreshcare.data.growerText, name: 'GrowerText', field: 'contactbusinesstext'},
					...
					$('#ns1blankspaceControlCertificationFees').click(function(event)
					{
						...
						oParam.accountCode = '5005'; 	// v4.0.004 changed from 4-1300
					...
					oSearch.addField('reference,description,contactbusinesssenttotext,contactpersonsenttotext,duedate,preadjustmentamount,amount,sent');
					oSearch.rows = 30;
				...
				else
				{
					...
					else
					{
						...
						aHTML.push('<div class="ns1blankspaceCaption"' +
										'style="padding-left:8px; margin-bottom:10px; font-weight:500;">MOST RECENTLY UPDATED</div>');
						...
						{					
							...
							aHTML.push('<td id="ns1blankspaceMostLikely_Amount-' + this.id + '" class="ns1blankspaceMostLikelySub" style="width:50px;text-align:right;padding-left:10px;padding-right:10px;">' +
													'$' + this.preadjustmentamount + '</td>');		// v3.2.004 Changed to preadjustmentamount
																	
							aHTML.push('<td id="ns1blankspaceMostLikely_Sent-' + this.id + '" class="ns1blankspaceMostLikelySub" style="width:50px;text-align:right;padding-left:10px;padding-right:10px;">' +
													this.sent + '</td>');
			...
			certificationBodies: function(oParam)
			{
				if (nsFreshcare.internal.data.certificationBodies === undefined)
				{
					...
					// v3.2.001 SUP023329 Now gets correct accounts or primary contact person
					var oSearch = new AdvancedSearch();
					oSearch.method = 'CONTACT_BUSINESS_SEARCH';
					oSearch.addField('tradename,contactbusiness.contactperson.id,contactbusiness.contactperson.persongroup' +
										',contactbusiness.contactperson.firstname,primarycontactperson');
					oSearch.addFilter('contactbusiness.businessgroup', 'EQUAL_TO', nsFreshcare.data.businessGroupAuditor);
					oSearch.addBracket('(');
					oSearch.addFilter('primarycontactperson', 'EQUAL_TO', 'field:contactbusiness.contactperson.id');
					...
					oSearch.getResults(function(oResponse)
					{
						if (oResponse.status === 'OK')
						{
							...
							{
								var row = oResponse.data.rows[i];

								if (row.id != iThisCertBody)
								{
									// We want either the Primary Contact or the first Accounts Contact - preferably the Accounts Contact
									if (i > 0)
									{
										oResponse.data.rows[i-1].contactPerson = (iAccountsContactPerson ? iAccountsContactPerson : iPrimaryContactPerson);
									}
									var iAccountsContactPerson = undefined;
									var iPrimaryContactPerson = undefined;
								}

								if (iAccountsContactPerson == undefined && row['contactbusiness.contactperson.persongroup'] == nsFreshcare.data.groupAccountsContact)
								{
									iAccountsContactPerson = row['contactbusiness.contactperson.id'];
								}	
								else if (iPrimaryContactPerson == undefined && row['primarycontactperson'] == row['contactbusiness.contactperson.id'])
								{
									iPrimaryContactPerson = row['contactbusiness.contactperson.id']
								}

								iThisCertBody = row.id;
							}
							// v3.2.003 now updates length-1 row
							oResponse.data.rows[oResponse.data.rows.length-1].contactPerson = (iAccountsContactPerson ? iAccountsContactPerson : iPrimaryContactPerson);

							nsFreshcare.internal.data.certificationBodies = 
								$.map($.grep(oResponse.data.rows, function(y) {return y.contactPerson != undefined}), 
										function(x) {return {id: x.id, value: x.tradename, contactPerson: x.contactPerson}});
			...
			commissions: 	
			{
				search: function(oParam)
				{
					...
					sStartDate = (oParam.accountCode === '4-1300') ? nsFreshcare.internal.financial.data.certificationFeesFrom : nsFreshcare.internal.financial.data.royaltyFeesFrom;
				...
				show: function(oParam, oResponse)
				{
					...
					if ($('#' + sXHTMLElementID).attr('data-loading') === '1') 
					{
						...
						aHTML = [];
						aHTML.push('<span id="ns1blankspaceInvoice' + sXHTMLContext + 'SearchHandle" style="height:25px" title="Search Criteria"></span>');
						aHTML.push('<span id="ns1blankspaceInvoice' + sXHTMLContext + 'Process" style="height:25px" title="Create Invoices"></span>');
						...
						.click(function() 
						{
							// v3.1.209 Changed from confirm() to container.confirm()
							ns1blankspace.container.confirm(
							{
								html: 'Are you sure you want to create ' + sLabel + ' for ' 
										+ $.map($('.nsFreshcareSelected'), function(x) {return $(x).html()}).join(', ') + '?',
								buttons:
								[
									{
										text: 'Yes',
						...
						$('#ns1blankspaceInvoice' + sXHTMLContext + 'Search')
						.button()
						.click(function() 
						{
				...
				calculateLineItemAmount: function(oRow)
				{
					...
					{
						oRow.feeType = iFeeType;		// Certification Renewal Fee
			...
			unsentInvoices:
			{
				...
				show: function(oParam, oResponse)
				{
					...
					if ($('#' + sXHTMLElementID).attr('data-loading') === '1') 
					{
						...
						// Search criteria bar 'handle' - allows user to get the search criteria div back into view
						aHTML = [];
						aHTML.push('<span id="ns1blankspaceInvoice' + sXHTMLContext + 'SearchHandle"' +
									' style="height:25px" class="ns1blankspaceInvoiceAction" title="Search Criteria"></span>');
						aHTML.push('<span id="ns1blankspaceInvoice' + sXHTMLContext + 'Email" title="Email Invoices"' +
									' style="height:25px" class="ns1blankspaceInvoiceAction"></span>');
						aHTML.push('<span id="ns1blankspaceInvoice' + sXHTMLContext + 'Print" title="Print Invoices"' +
									' style="height:25px" class="ns1blankspaceInvoiceAction"></span>');
						aHTML.push('<span id="ns1blankspaceInvoice' + sXHTMLContext + 'Delete" title="Delete Invoices"' +
									' style="height:25px" class="ns1blankspaceInvoiceAction"></span>');
						aHTML.push('<span id="ns1blankspaceInvoice' + sXHTMLContext + 'Complete" title="Mark as Sent"' +
									' style="height:25px" class="ns1blankspaceInvoiceAction"></span>');
						$('#ns1blankspaceInvoice' + sXHTMLContext + 'SearchRibbon').html(aHTML.join(''));
						...
						aHTML.push('<tr><td id="ns1blankspaceInvoice' + sXHTMLContext + 'Search" class="ns1blankspaceAction" title="Search"></td></tr>');
						...
						$('#ns1blankspaceInvoice' + sXHTMLContext + 'Email').button({
							text: false,
							icons: { primary: 'glyphicon-envelope'}
						})
						.css('width', '12px')
						.css('height', '25px')
						.click(function() 
						...
						// Bind the Search button
						$('#ns1blankspaceInvoice' + sXHTMLContext + 'Search').button({label: "Search"})
						.click(function() 
						{
							ns1blankspace.okToSave = true;
					...
					{	
						...
						else
						{
							...
							// v3.1.208c SUP023195 showRow was calling commissions.row instead of unsentInvoices.row
							ns1blankspace.render.page.show(
							{
								xhtmlElementID: 'ns1blankspaceInvoice' + sXHTMLContext + 'Results',
								xhtmlContext: 'Invoices' + sXHTMLContext,
								...
								more: oResponse.moreid,
								rows: 20,
								functionShowRow: nsFreshcare.internal.financial.invoice.unsentInvoices.row,
	...
	entity:
	{
		search: 
		{
			...
			{
				// v3.2.005 SUP022879 Alternate sStatusColumn parameter now allowed
				var aHTML = [];

				var sStatusColumn = $('#ns1blankspaceViewControlSearch').attr('data-statuscolumn');
				sStatusColumn = sStatusColumn || oParam.statusColumn;
		...
		details: function(oParam)
		{
			...
			if ($('#ns1blankspaceMainDetails').attr('data-loading') == '1')
			{
				...
				// v3.3.005 SUP023801 Removed JASANZ Date from this tab as now whos on it's own tab
				if (sSourceNamespace == 'certificationbody')
				{
					if (nsFreshcare.option.certBodyNumber)
					{
						aHTML.push('<tr class="ns1blankspaceCaption">' +
										'<td class="ns1blankspaceCaption">' +
					...
					if (nsFreshcare.option.certBodySelfCertDate)
					{
						aHTML.push('<tr class="ns1blankspaceCaption">' +
										'<td class="ns1blankspaceCaption">' +
					...
					if (nsFreshcare.option.auditManualApprovalFlag)
					{					
						aHTML.push('<tr class="ns1blankspaceCaption">' +
										'<td class="ns1blankspaceCaption">' +
				...
				if (ns1blankspace.objectContext === -1)
				{
					aHTML.push('<tr><td>&nbsp;</td></tr>' +
								'<tr><td class="ns1blankspaceAction" style="text-align:right;">' +
									'<span id="ns1blankspaceDetailsNext" title="Next"></span>'+
								'</td></tr>');
				}
				...
				if (ns1blankspace.objectContextData != undefined)
				{
					...
					// v3.1.2 If we're looking at a CB, show CB Number and Self Cert start date & Manual Audit Aproval
					// v3.1.209 Conditionally populates
					if (sSourceNamespace == 'certificationbody')
					{
						...
						if ($('#ns1blankspaceDetailsSelfCertificationDate').is('*'))
						...
						if ($('[name="radioManualAuditApproval"]').is('*'))
		...
		people:
		{
			search: function(oParam)
			{
				...
				var sStatusColumn = ns1blankspace.util.getParam(oParam, 'statusColumn').value;
				var bHidePrimaryContact = ns1blankspace.util.getParam(oParam, 'hidePrimaryContact', {'default': false}).value;
				...
				oSearch.addField('firstname,surname,email,workphone,mobile,supplierstatus,supplierstatustext,customerstatus,customerstatustext,reference,' +
								'contactperson.contactbusiness.primarycontactperson,title,titletext,fax,position,' +
				...
				// v3.3.001 SUP023603 Also requires hidePrimaryContact to be true if not showing primary contacts in the list
				if (bHidePrimaryContact && iPrimaryContact)
				{
			...
			show: function(oParam)
			{
				...
				var bShowCompetencies = ((oParam.pageObject == 'trainer' || oParam.pageObject == 'certificationbody') 
										&& nsFreshcare.user.roleID == nsFreshcare.data.roles.admin)
				var bCanUpdate;
				var aHTML = [];

				// v3.2.025 Conditionally prevent Updates
				bCanAdd = (ns1blankspace.option.preventUpdates == true) ? false : bCanAdd;
				oParam.showCompetencies = bShowCompetencies;
				...
				oParam.statusColumn = sStatusColumn || undefined;
				oParam.xhtmlElementID = sXHTMLElementID || undefined;
				...
				else
				{
					...
					// v3.3.001 SUP023794 Show Trainer & Auditor standards
					if (bShowCompetencies)
					{
						aHTML.push('<td class="ns1blankspaceHeaderCaption">Standards</td>');
					}
					aHTML.push('<td class="ns1blankspaceHeaderCaption" width="65px">' + 
								(bCanAdd 
									? '<span id="ns1blankspaceBusinessContactAdd" title="Add Contact" data-toggle="tooltip">&nbsp;</span>' 
									: '') 
								+ '</td>');
					aHTML.push('</tr>');
			...
			{	// v3.1.205 SUP023021 Pending Approval now in different colour
				...
				// v3.2.025 Override update flags if preventUpdates
				if (ns1blankspace.option.preventUpdates == true)
				{
					bCanAddGroup = false;
					bCanRemove = false;
					bCanRemoveDirectly = false;
					bCanUpdate = false;
					bCanUpdateDirectly = false;
					bCanSetPrimary = false;
					bCanSetPrimaryDirectly = false;
				}
				...
				aHTML.push('<td id="ns1blankspaceEntityPeople_mobile-' + oRow.id + '" data-directupdate="' + bCanUpdateDirectly + '"' +
								' class="ns1blankspaceRow ns1blankspaceRowBusinessContact' + sClass + '">' +
								(oRow.mobile ? oRow.mobile : oRow.workphone) + 
								'</td>');
				...
				*/
				...
				if (bShowCompetencies)
				{
					aHTML.push('<td id="ns1blankspaceEntityPeople_standards-' + oRow.id + '" class="ns1blankspaceRow' + sClass + '"' +
									' style="font-size:0.75em;color:#A4A4A4;">' +
									ns1blankspace.xhtml.loadingSmall + '</td>');
				}
				...
				if (bCanAddGroup)
				{
					aHTML.push('<span class="ns1blankspaceBusinessContactGroupAdd" id="ns1blankspaceBusinessContactGroupAdd-' + oRow.id + '"' +
											' data-rowid="' + oRow.id + '" title="Add Group" data-toggle="tooltip"' +
											'></span>');
				}
					...
					&& oRow['contactperson.contactbusiness.primarycontactperson'] != oRow.id)		// Can't remove primary contact 
				{
					if (bCanRemove)
					{
						...
						else
						{
							aHTML.push('<span class="ns1blankspaceBusinessContactReactivate" id="ns1blankspaceBusinessContactReactivate-' + oRow.id + '"' +
												' data-rowid="' + oRow.id + '" title="Reactivate"' +
												'></span>');
						}
					}
					...
					{	// v3.1.205 SUP023015 Can only set Primary Contatct if Active 
						// v3.3.001 SUP023505 Added data-name attribute
						if (oRow[sStatusColumn] == nsFreshcare.data.contactStatusActive)
						{
							aHTML.push('<span class="ns1blankspaceBusinessContactPrimary" id="ns1blankspaceBusinessContactPrimary-' + oRow.id + '"' +
												...
												' data-name="' + oRow['firstname'] + ' ' + oRow['surname'] + '"' + 
												' title="Set as Primary Contact" data-toggle="tooltip"' +
												'></span>');
						}
					}
				}
			...
			newPage: function(oParam)
			{
				...
				{
					oParam.newPageStep = (bShowGroups) ? 1 : ((bShowUsers) ? 2 : 3);
				}
				...
				// v3.1.209b SUP022867 Search for trainer memberships if this is the Trainer page
				else if (oParam.newPageStep == 3)
				{
					oParam.newPageStep = 4;
					// v3.2.015 SUP023421 now uses roleID
					if (oParam.pageObject == 'trainer' && bShowCompetencies)
					{
						oParam.onComplete = nsFreshcare.internal.entity.people.newPage;
						oParam.trainerBusiness = ns1blankspace.objectContext;
						oParam.trainerPerson = $.map($('.nsFreshcareEntityPeopleRow:visible'), function(x) {return x.id.split('-').pop()}).join(',');
				...
				// v3.3.001 SUP023603 Search for auditor memberships if bShowCompetencies - uses same function/method as trainer memberships
				else if (oParam.newPageStep == 4)
				{
					oParam.newPageStep = 5;
					if (oParam.pageObject == 'certificationbody' && bShowCompetencies)
					{
						oParam.onComplete = nsFreshcare.internal.entity.people.newPage;
						oParam.trainerBusiness = ns1blankspace.objectContext;
						oParam.trainerPerson = $.map($('.nsFreshcareEntityPeopleRow:visible'), function(x) {return x.id.split('-').pop()}).join(',');
				...
				// indicate the person groups, Competencies and the contacts who are users on each row where applicable
				else if (oParam.newPageStep === 5)
				{
					// v3.1.214 SUP023270 Conditionally processes this section
					$.each($('.nsFreshcareEntityPeopleRow:visible'), function(i, oRowElement)
					{
						...
						// v3.3.001 SUP023794 Show Standards in list
						if (bShowCompetencies)
						{
							var oStandards = $.grep(nsFreshcare.data[sStoreObject], function(x) {return x.trainercontactperson === sThisPerson});
							...
							{
								$('#ns1blankspaceEntityPeople_standards-' + sThisPerson)
									.html(aStandardsText.join(', '));
							}
							else
							{
								$('#ns1blankspaceEntityPeople_standards-' + sThisPerson).html('');
							}
						}

						// v3.2.025 Conditionally add button 
						if (bShowGroups)
						{
							...
							$.each(oGroups, function()
							{
								...
								{
									if ($(oRowElement).attr('data-userid') === undefined && ns1blankspace.option.preventUpdates != true)
									{
										$('#ns1blankspaceEntityPeople_select-' + sThisPerson).html(sOptionsText +
													'<span class="ns1blankspaceBusinessContactCreateUser" id="ns1blankspaceBusinessContactCreateUser-' + sThisPerson + '"' +
														' data-rowid="' + sThisPerson + '" title="Create User"></span>');
									}
								}
								...
								aGroupText.push('<div id="nsFreshcareEntityPeopleGroupsDiv-' + this.id + '" class="nsFreshcareEntityPeopleGroupsDiv" data-id="' + this.id + '">' +
													...
													((oParam.canAddGroup === true)
														?'<span class="nsFreshcareEntityPeopleGroupsRemove"' +
														 ' data-id="' + this.id + '"' +
														 ' title="Remove Group" data-toggle="tooltip"' +
					...
					// v3.1.209b SUP022867 Assign memberships to each person on the list 
					// v3.1.210 SUP023225 Only applies to trainer page
					if (oParam.pageObject == 'trainer')
					{
						$.each(ns1blankspace.objectContextData.people, function(i, x)
						{
							var aTrainerMemberships = $.grep(nsFreshcare.data.trainerMemberships, function(y) {return y.trainercontactperson == x.id});
							if (aTrainerMemberships.length > 0 || x.trainerMemberships == undefined)
							{
								x.trainerMemberships = aTrainerMemberships;
							}
						});
					}

					// v3.3.001 SUP023603
					if (oParam.pageObject == 'certificationbody')
					{
						$.each(ns1blankspace.objectContextData.people, function(i, x)
						{
							var aAuditorMemberships = $.grep(nsFreshcare.data.auditorMemberships, function(y) {return y.trainercontactperson == x.id});
							if (aAuditorMemberships.length > 0 || x.auditorMemberships == undefined)
							{
								x.auditorMemberships = aAuditorMemberships;
							}
						});
					}

					oParam.newPageStep = (bShowTraining) ? 6 : 10;
					nsFreshcare.internal.entity.people.newPage(oParam);
				}

				// v3.1.0 Get most recent training course for each contact & Membership
				else if (oParam.newPageStep === 6)
				{
					var oSearch = new AdvancedSearch();
					...
					oSearch.getResults(function(oResponse)
					{
						if (oResponse.status == 'OK')
						{
							$.each(ns1blankspace.objectContextData.people, function()
							{
								...
								// v3.2.020 Was incorrectly only setting last course
								this.trainingCourses = aMembershipCourses;
			...
			bind: function(oParam)
			{
				...
				$('#' + sXHTMLContainerID + ' .ns1blankspaceRowBusinessContact').click(function(event) 
				{
					...
					if (nsFreshcare.internal.entity.people.manage(oParam) && oThisContact) 
					{ 
						$.extend(true, oParam,
						{
						...
						oParam.thisContact = oThisContact 		// v3.3.001 Had to separate out as was not updating sub-objects

						fFunctionDetails(oParam);
					}
					else if (oThisContact == undefined)
					{
				...
				// v3.2.015 SUP023422 Added COP Based Training
				// v4.0.001 Bootstrap - removed label
				$('#' + sXHTMLContainerID + ' .ns1blankspaceBusinessContactRemove')
					.button(
					{
						text: false,
						//label: ((nsFreshcare.user.roleID === nsFreshcare.data.roles.admin) ? 'Remove' : 'Inactivate'),
						icons: {primary: "ui-icon-close"}
					})
					.click(function(event) 
					{
						...
						{	// We remove if admin, otherwise, just inactivate 
							...
							$.extend(true, oParam,
							{
								...
								clickTitle: ((nsFreshcare.user.roleID === nsFreshcare.data.roles.admin) ? 'Remove' : 'Inactivate'),
								clickBind: ((nsFreshcare.user.roleID === nsFreshcare.data.roles.admin) 
								...
								confirmed: true, 
								onComplete: ((nsFreshcare.user.roleID === nsFreshcare.data.roles.admin) ? nsFreshcare.internal.entity.people.remove.send : undefined)
							});
				...
				if (bCanAdd)
				{
					$('#ns1blankspaceBusinessContactAdd')
						...
						.click(function() 
						{
							...
							delete(oParam.rowID) // v3.4.001 Was updating existing records instead of adding new
							nsFreshcare.internal.entity.people.manage(oParam);
				...
				$('#' + sXHTMLContainerID + ' .nsFreshcareEntityPeopleGroupsRemove')
					.button(
					{
						icons: {primary: 'glyphicon-remove-sign'}
					})
					//.css('width', '8px')
					//.css('height', '8px')
					//.css('margin', '0px')
					...
					show: function(oParam)
					{
						...
						else 
						{
							...
							{ 
								...
								left: $('#' + ns1blankspace.xhtml.divID).offset().left + 20
							});
			...
			manage: function(oParam)
			{
				...
				// v3.2.020 Was setting bAdd from oParam.canAdd - these are 2 different parameters
				var bContinue = true;
				...
				// v3.2.025 Conditionally allow updates
				bUpdate = (ns1blankspace.option.preventUpdates == true) ? false : bUpdate;
				bAdd = (ns1blankspace.option.preventUpdates == true) ? false : bAdd;
			...
			details: function(oParam)
			{
				...
				aHTML.push('<tr class="ns1blankspaceCaption">' +
								...
								'<span id="ns1blankspaceContactCopyStreetAddressFromBusiness"' +
									' style="text-align:right;font-size:0.625em;" class="ns1blankspaceAction"' +
									' title="Copy from Business"></span>' +
								'</td></tr>');
				...
				aHTML.push('<tr class="ns1blankspace ns1blankspaceHideNonAdmin">' +
								...
								'</td>' +
								'<td class="ns1blankspaceCaption">' +
								(oParam.showTrainingCourses === true 
									? 'Recent Training Courses' 
									: (oParam.showMemberships == true ? 'Standards' :'&nbsp;')) +
								'</td></tr>');								
				...
				else if (oParam.showMemberships == true)
				{
					aHTML.push(' nsFreshcareReadOnly" id="ns1blankspaceBusinessContactMemberships">');
				}
				else {aHTML.push('">&nbsp;')}
				...
				{	// v3.1.207 SUP023067 Added .formatXHTML() to non-address / name fields
					...
					else if (oParam.showMemberships)
					{
						// v3.3.001 SUP023603 Now uses this area interchangably for Trainer & Auditor memberships
						var oMembershipStore = oThisContact.trainerMemberships || oThisContact.auditorMemberships;
						aHTML = [];
						aHTML.push('<table id="ns1blankspaceContactMemberships">');
						$.each(nsFreshcare.data.memberships, function()
						{
							aHTML.push('<tr>' +
											'<td class="ns1blankspaceRow nsFreshcareSelectable"' +
												' id="ns1blankspaceContactMembership-' + this['id'] + '"' +
												' style="font-size:0.75em;">' + this['title'].formatXHTML() + '</td>' +
										'</tr>')
						});
						aHTML.push('</table>');

						$('#ns1blankspaceBusinessContactMemberships').html(aHTML.join(''));
						
						$.each(oMembershipStore, function()
						{
							$('#ns1blankspaceContactMembership-' + this.membership)
								.addClass('nsFreshcareSelected')
								.attr('data-id', this.id);
						});

						$('#ns1blankspaceContactMemberships .nsFreshcareSelectable')
							.css('cursor', 'pointer')
							.on('click', function()
							{
								if ($(this).hasClass('nsFreshcareSelected') && $(this).attr('data-id')) 
								{
									$(this).css('text-decoration', "line-through");
								}
								else if ($(this).css('text-decoration').indexOf('line-through') > -1 && $(this).attr('data-id'))
								{
				...
				send: function(oParam)
				{
					...
					if (sID && sXHTMLElementID)
					{
						...
						else if (oParam.savePrimaryContactStep === 3)
						{
							...
							// v3.2.015 now used roleID
							// v3.3.001 SUP023505 Now emails details of primary contacts pre-and-post
							if (nsFreshcare.user.roleID === nsFreshcare.data.roles.auditor || nsFreshcare.user.roleID === nsFreshcare.data.roles.grower || nsFreshcare.user.roleID === nsFreshcare.data.roles.trainer)
							{
								$.extend(oParam, false,
								{
									...
									from: nsFreshcare.data['emailFrom' + nsFreshcare.user.roleID],
									replyTo: (nsFreshcare.user.roleID == nsFreshcare.data.roles.auditor ? ns1blankspace.user.contactPerson : undefined),
									subject: nsFreshcare.user.role + ' ' + ns1blankspace.user.contactBusinessText + ' has changed a primary contact',
									message: nsFreshcare.user.role + ' ' + ns1blankspace.user.contactBusinessText + 
										' has changed the primary contact against ' + ns1blankspace.data.contactBusinessText + ' from ' +
										...
										' to ' + $('#' + sXHTMLElementID).attr('data-name'), 
									contactBusiness: ns1blankspace.objectContext,
			...
			remove:
			{
				validate: function(oParam)
				{
					...
					sID = sID || ns1blankspace.util.getParam(oParam, 'rowID').value;	// v3.4.001 Added for when id not passed
			...
			save:
			{
				...
				send: function(oParam)
				{
					...
					// v3.1.209b SUP022867 Add / remove Training Memberships as applicable
					else if (oParam.savePersonStep == 5)
					{
						if (oParam.showMemberships == true)
						{
							if (oParam.membershipUpdateIndex == undefined) 
							{
								oParam.membershipUpdates = [];
								$.each($('#ns1blankspaceBusinessContactMemberships td'), function()
								{
									if ($(this).attr('data-id') && $(this).css('text-decoration').indexOf('line-through') > -1)
									{
										oParam.membershipUpdates.push({data: 'id=' + $(this).attr('data-id') + '&remove=1', membership: this.id.split('-').pop()});
									}
									else if ($(this).attr('data-id') == undefined && $(this).hasClass('nsFreshcareSelected'))
									{
										oParam.membershipUpdates.push({membership: this.id.split('-').pop(), data: 'membership=' + this.id.split('-').pop() + 
																		'&contactbusiness=' + ns1blankspace.objectContext + '&contactperson=' + sID});
									}
								});
								oParam.membershipUpdateIndex = (oParam.membershipUpdates.length > 0) ? 0 : oParam.membershipUpdates.length;
								if (oParam.membershipUpdates.length == 0 && !ns1blankspace.inputDetected)
								{
									ns1blankspace.status.message('No change to Contact. Nothing to save!');
								}
							}

							if (oParam.membershipUpdateIndex < oParam.membershipUpdates.length)
							{
								$.ajax(
								{
									type: 'POST',
									url: ns1blankspace.util.endpointURI('AGRI_MEMBERSHIP_TRAINER_MANAGE'),
									data: oParam.membershipUpdates[oParam.membershipUpdateIndex].data,
									success: function(oResponse)
									{
										if (oResponse.status == 'OK')
										{
											...
											{
												$('#ns1blankspaceContactMembership-' + oParam.membershipUpdates[oParam.membershipUpdateIndex].membership).attr('data-id', oResponse.id)
											}
											else
											{
												$('#ns1blankspaceContactMembership-' + oParam.membershipUpdates[oParam.membershipUpdateIndex].membership).removeAttr('data-id');
											}
											oParam.membershipUpdateIndex += 1;
											nsFreshcare.internal.entity.people.save.send(oParam);
										}
										else
										{
											ns1blankspace.status.error('Error saving Trainer Membership: ' + oResponse.error.errornotes);
										}
									}
								});
							}
							...
							{
								delete(oParam.membershipUpdates);
								delete(oParam.membershipUpdateIndex);
								oParam.savePersonStep = 6;
								nsFreshcare.internal.entity.people.save.send(oParam);
							}
						}
						else
						{	// v3.2.005 Was notgetting to final step if not showing memberships
					...
					else if (oParam.savePersonStep === 6)
					{
					...
					else if (oParam.savePersonStep === 10)
					{
						...
						else if (nsFreshcare.user.roleID != nsFreshcare.data.roles.admin && sPageObject === nsFreshcare.data.roles.grower)
						{	// We're viewing a grower page
		...
		training:
		{
			...
			search: function(oParam)
			{
				...
				var sStoreColumn = ns1blankspace.util.getParam(oParam, 'storeColumn', {'default': 'training'}).value;
				var oAttachToObject = ns1blankspace.util.getParam(oParam, 'attachToObject', {'default': ns1blankspace.objectContextData}).value;
				var iContactBusiness = ns1blankspace.util.getParam(oParam, 'contactBusiness', {'default': ns1blankspace.objectContext}).value;
				var bLinkedOnly = ns1blankspace.util.getParam(oParam, 'linkedOnly', {'default': false}).value;
				...
				oSearch.addField('contactbusinesstext,contactpersontext,contactperson,contactbusiness' +
								',subscription,subscriptiontext' +
								',agritraineeattendancelink.attendee.course.coursedate,attendee' +
								...
								',agritraineeattendancelink.attendee.course.trainercontactbusiness' +
								',agritraineeattendancelink.attendee.course.trainercontactperson' +
								...
								',agritraineeattendancelink.attendee.course.package.membership' +
								',agritraineeattendancelink.attendee.course.package.title' +
								',agritraineeattendancelink.attendee.attendingtrainee' +
								',agritraineeattendancelink.attendee.course' +
								',agritraineeattendancelink.attendee.course.status' +
								',agritraineeattendancelink.attendee.course.statustext' +
								',agritraineeattendancelink.attendee.traineecontactbusiness' +
								',agritraineeattendancelink.attendee.traineecontactbusinesstext' +
								',agritraineeattendancelink.contactbusiness.tradename' +
								',agritraineeattendancelink.contactperson.firstname' +
								',agritraineeattendancelink.contactperson.surname,attendeetext' +
								',agritraineeattendancelink.subscription.membership' +
								',agritraineeattendancelink.subscription.membershiptext' +
								',agritraineeattendancelink.subscription.membership.code' +
								',agritraineeattendancelink.subscription.codeofpracticetext' +
								',agritraineeattendancelink.attendee.firstname' +
								',agritraineeattendancelink.attendee.surname' +
								',agritraineeattendancelink.attendee.traineecontactpersontext');	
				oSearch.addField(ns1blankspace.option.auditFields)	;

			
				if (iObject == nsFreshcare.objectBusiness)
				{
					oSearch.addFilter('contactbusiness', 'EQUAL_TO', iContactBusiness);
				}
				else if (iObject == nsFreshcare.objectTrainingCourseAttendee)
				{
					// v3.4.008 SUP023948 Filter on passed contactbusiness if grower.trainingcourse
					if (ns1blankspace.objectParentName == 'grower')
					{
						if (bLinkedOnly)
						{
							oSearch.addFilter('contactbusiness', 'EQUAL_TO', ns1blankspace.user.contactBusiness);
							oSearch.addFilter('agritraineeattendancelink.attendee.traineecontactbusiness', 'NOT_EQUAL_TO', ns1blankspace.user.contactBusiness);	
						}
						else if (iContactBusiness)
						{
							oSearch.addFilter('agritraineeattendancelink.attendee.traineecontactbusiness', 'EQUAL_TO', iContactBusiness);
							oSearch.addFilter('contactbusiness', 'EQUAL_TO', iContactBusiness);
						}
					}
					else
					{
						oSearch.addFilter('attendee', 'EQUAL_TO', ns1blankspace.objectContext);
					}
				}
				else if (iObject == nsFreshcare.objectAudit)
				{
					if (oParam.contactBusiness)
					{
						oSearch.addFilter('contactbusiness', 'EQUAL_TO', iContactBusiness);
						oSearch.addFilter('agritraineeattendancelink.attendee.course.coursedate', 'LESS_THAN_OR_EQUAL_TO', dToday.toString('dd MMM yyyy') + ' 23:59:59');
						// v3.4.001 SUP023829 Don't show cancelled coursed to non-admin
						if (nsFreshcare.user.roleID != nsFreshcare.data.roles.admin)
						{
							oSearch.addFilter('agritraineeattendancelink.attendee.course.status', 'NOT_EQUAL_TO', '3');
						}
					}
					else
					{
						ns1blankspace.status.error('Error searching for trainees: Member business unknown.');
						bContinue = false
					}
				}
				else
				...
				// v3.3.001 SUP023763 Added abilty to filter on qualifying COPs / Memberships
				if (oParam.qualifyingCOPs)
				{
					oSearch.addFilter('agritraineeattendancelink.attendee.course.package.codeofpractice', 'IN_LIST', oParam.qualifyingCOPs.join(','));
				}
				if (oParam.membership && iObject != nsFreshcare.objectBusiness)
				{
					oSearch.addFilter('agritraineeattendancelink.attendee.course.package.membership', 'EQUAL_TO', oParam.membership);
				}
				// v3.3.001 SUP023097 Added filter for when trainers see training courses their Trainees have attended
				if (oParam.trainerBusiness)
				{
					oSearch.addFilter('agritraineeattendancelink.attendee.course.trainercontactbusiness', 'EQUAL_TO', oParam.trainerBusiness)
				}
				...
				if (bContinue)
				{
					oSearch.getResults(function(oResponse)
					{
						if (oResponse.status === 'OK')
						{
							oAttachToObject[sStoreColumn] = oResponse.data.rows;	
							oAttachToObject.trainingCourses = oResponse.data.rows;
							oAttachToObject.links = oResponse.data.rows;
							...
							{
								ns1blankspace.status.error('Error finding Training: ' + oResponse.error.errornotes);
							}
						}
					});
				}
			...
			show: function(oParam)
			{
				...
				var sSortColumn = ns1blankspace.util.getParam(oParam, 'sortColumn', {'default': 'contactbusiness'}).value;
				var sSortDirection = ns1blankspace.util.getParam(oParam, 'sortDirection', {'default': 'desc'}).value;
				var fFunctionBind = ns1blankspace.util.getParam(oParam, 'functionBind', {'default': nsFreshcare.internal.entity.training.bind}).value;
				var oAttachToObject = ns1blankspace.util.getParam(oParam, 'attachToObject', {'default': ns1blankspace.objectContextData}).value;
				var sEmptyMessage = ns1blankspace.util.getParam(oParam, 'emptyMessage', {'default': "No Linked Training"}).value;
				var iObject = ns1blankspace.util.getParam(oParam, 'object', {'default': ns1blankspace.object}).value;
				var aHTML = [];

				if (oAttachToObject.training === undefined)
				{
				...
				else
				{
					...
					aHTML.push('<table class="ns1blankspaceContainer">' +
						'<tr class="ns1blankspaceContainer">' +
						'<td id="ns1blankspaceTrainingColumn1' + sXHTMLContext + '" class="ns1blankspaceColumn1Large">' +
						ns1blankspace.xhtml.loading +
						'</td>' +
						'</tr>' +
						'</table>');				
					...
					aHTML.push('<td class="ns1blankspaceHeaderCaption ns1blankspaceHeaderSort"' +
									...
									' data-sortdirection="' + ((sSortColumn == "agritraineeattendancelink.attendee.course.coursedate") 
										? ((sSortDirection === 'asc') ? "desc" : "asc") : 'asc') + '"' +
									'>Training Date</td>');

					aHTML.push('<td class="ns1blankspaceHeaderCaption ns1blankspaceHeaderSort"' +
									' data-column="agritraineeattendancelink.attendee.course.trainercontactbusinesstext"' +
									' data-sortdirection="' + ((sSortColumn == "agritraineeattendancelink.attendee.course.trainercontactbusinesstext") 
										? ((sSortDirection === 'asc') ? "desc" : "asc") : 'asc') + '"' +
									'>Trainer</td>');

					// v3.4.008 SUP023948 Package shows for everything except Audit
					if (iObject != nsFreshcare.objectAudit)
					{
						aHTML.push('<td class="ns1blankspaceHeaderCaption ns1blankspaceHeaderSort"' +
										' data-column="agritraineeattendancelink.attendee.course.package.title"' +
										' data-sortdirection="' + ((sSortColumn == "agritraineeattendancelink.attendee.course.package.title") 
											? ((sSortDirection === 'asc') ? "desc" : "asc") : 'asc') + '"' +
										'>Package</td>');
					}
					...
					aHTML.push('<td class="ns1blankspaceHeaderCaption ns1blankspaceHeaderSort"' +
									...
									' data-sortdirection="' + ((sSortColumn == "agritraineeattendancelink.attendee.course.package.membership.code") 
										? ((sSortDirection === 'asc') ? "desc" : "asc") : 'asc') + '"' +
									'>Trained Standard</td>');
					
					aHTML.push('<td class="ns1blankspaceHeaderCaption ns1blankspaceHeaderSort"' +
								' data-column="contactbusinesstext"' +
								' data-sortdirection="' + ((sSortColumn == "contactbusinesstext") 
									? ((sSortDirection === 'asc') ? "desc" : "asc") : 'asc') + '"' +
								'>Business</td>');	

					// v3.4.008 SUP023948 Show for all except Person
					if (iObject != nsFreshcare.objectPerson)
					{
						aHTML.push('<td class="ns1blankspaceHeaderCaption ns1blankspaceHeaderSort"' +
									' data-column="contactpersontext"' +
									' data-sortdirection="' + ((sSortColumn == "contactpersontext") 
										? ((sSortDirection === 'asc') ? "desc" : "asc") : 'asc') + '"' +
									'>Person</td>');
					}

					if (iObject == nsFreshcare.objectTrainingCourseAttendee 
						&& ((nsFreshcare.user.roleID === nsFreshcare.data.roles.admin) || (nsFreshcare.user.roleID === nsFreshcare.data.roles.auditor)))
					{
						aHTML.push('<td class="ns1blankspaceHeaderCaption" width="65px">' +
										'<span id="ns1blankspaceAttendanceLinkAdd" title="Add Link" data-toggle="tooltip">' +
										'</span></td>');
					}
					else if (iObject == nsFreshcare.objectAudit)
					{
						aHTML.push('<td class="ns1blankspaceHeaderCaption">' +
									'<span id="ns1blankspaceAttendanceLinkRefresh" title="Refresh"></span></td>');
					}
					else
					{
					...
					if (oAttachToObject.training.length == 0)
					{
						aHTML.push('<tr>');
						aHTML.push('<td class="ns1blankspaceNothing">' + sEmptyMessage + '.</td>');
						aHTML.push('</tr>');
					...
					else
					{
						...
						$.each(oAttachToObject.training, function()
						{
							aHTML.push(nsFreshcare.internal.entity.training.row(this, oParam));
						});
						...
						ns1blankspace.render.page.show(
						{
							xhtmlElementID: 'ns1blankspaceTrainingColumn1' + sXHTMLContext,
							xhtmlContext: 'Training',
							xhtml: aHTML.join(''),
							showMore: (oParam.morerows == "true"),
							more: oParam.moreid,
							rows: ns1blankspace.option.defaultRows,
							functionShowRow: nsFreshcare.internal.entity.training.row,
							functionNewPage: fFunctionBind,
							type: 'json'
						}); 	
			...
			row: function(oRow, oParam)
			{
				oParam = oParam || {};
				var aHTML = [];
				var sWarningStyle = ' style="color: #ee8f00;"';
				var iObject = ns1blankspace.util.getParam(oParam, 'object', {'default': ns1blankspace.object}).value;
				var bShowWarning = (oRow['agritraineeattendancelink.attendee.surname'] + ', ' + oRow['agritraineeattendancelink.attendee.firstname']) 
									!= oRow['agritraineeattendancelink.attendee.traineecontactpersontext']
									&& nsFreshcare.user.roleID == nsFreshcare.data.roles.admin;
				
				// v3.4.001 SUP023829 If course is cancelled show as greyed out
				aHTML.push('<tr id="ns1blankspaceAttendanceLink_' + oRow.id + '" class="ns1blankspaceRow">');

				aHTML.push('<td id="ns1blankspaceAttendanceLinkCourseDate_' + oRow.id + '" class="ns1blankspaceRow' +
							((iObject == nsFreshcare.objectTrainingCourseAttendee) ? ' ns1blankspaceRowAttendanceLink' : '') + 
							((oRow['agritraineeattendancelink.attendee.course.status'] == '3') ? ' nsFreshcareInactive' : '') + '">' +
								oRow['agritraineeattendancelink.attendee.course.coursedate'] + 
								'<br /><span class="ns1blankspaceSubNote">(' + oRow['agritraineeattendancelink.attendee.course.statustext'] + ')</span></td>');

				aHTML.push('<td id="ns1blankspaceAttendanceLinkTrainerBusinesstext_' + oRow.id + '" class="ns1blankspaceRow' +
							((oRow['agritraineeattendancelink.attendee.course.status'] == '3') ? ' nsFreshcareInactive' : '') + 
							((iObject == nsFreshcare.objectTrainingCourseAttendee) ? ' ns1blankspaceRowAttendanceLink' : '') + '">' +
								oRow['agritraineeattendancelink.attendee.course.trainercontactbusinesstext'] + 
								(oRow['agritraineeattendancelink.attendee.course.trainercontactpersontext'] != ''
									? '<br /><span class="ns1blankspaceSubNote">(' + oRow['agritraineeattendancelink.attendee.course.trainercontactpersontext'] + ')</span>'
									: '')
								 + '</td>');

				if (iObject != nsFreshcare.objectAudit)
				{
					aHTML.push('<td id="ns1blankspaceAttendanceLinkPackage_' + oRow.id + '" class="ns1blankspaceRow' +
							((oRow['agritraineeattendancelink.attendee.course.status'] == '3') ? ' nsFreshcareInactive' : '') + 
							((iObject == nsFreshcare.objectTrainingCourseAttendee) ? ' ns1blankspaceRowAttendanceLink' : '') + '">' +
								oRow['agritraineeattendancelink.attendee.course.package.title'] + '</td>');
				}

				aHTML.push('<td id="ns1blankspaceAttendanceLinkTrainedCode_' + oRow.id + '" class="ns1blankspaceRow' +
						((oRow['agritraineeattendancelink.attendee.course.status'] == '3') ? ' nsFreshcareInactive' : '') + 
						((iObject == nsFreshcare.objectTrainingCourseAttendee) ? ' ns1blankspaceRowAttendanceLink' : '') + '">' +
							oRow['agritraineeattendancelink.attendee.course.package.membership.code'] + ' ' + 
						 	oRow['agritraineeattendancelink.attendee.course.package.codeofpracticetext'] + '</td>');
				...
				// v3.3.003 SUP023800 Now shows Attending Trainee name in case name is different to contact on older Attendee records
				// v3.4.001 SUP023826 Show Attending Trainee in orange and add ns1blankspaceLinkedPerson class if different to contactperson and admin user
				// v3.4.008 SUP023948 Now shows for all except Person
				if (iObject != nsFreshcare.objectPerson)
				{
					aHTML.push('<td id="ns1blankspaceAttendanceLinkContactPersontext_' + oRow.id + '" class="ns1blankspaceRow' +
							...
							((iObject == nsFreshcare.objectTrainingCourseAttendee) ? ' ns1blankspaceRowAttendanceLink' : '') +
							((bShowWarning) 
								? ' ns1blankspaceLinkedPerson"' + sWarningStyle + ' data-contactpersontext="' + oRow['agritraineeattendancelink.attendee.traineecontactpersontext'] + '"' 
								: '"') + '>' +
							oRow['agritraineeattendancelink.attendee.firstname'] + ' ' + oRow['agritraineeattendancelink.attendee.surname'] + 
							(oRow['contactbusiness'] != oRow['agritraineeattendancelink.attendee.traineecontactbusiness'] 
									? '<span class="ns1blankspaceSubNote"><br/>(' + oRow['agritraineeattendancelink.attendee.traineecontactbusinesstext'] + ')<span>' 
									: '') +
							'</td>');
				}


				// v3.4.008 Don't show any actions if in external portals (v3.4.010 Updated to only show in admin)
				if (nsFreshcare.user.roleID == nsFreshcare.data.roles.admin)
				{
					if (iObject == nsFreshcare.objectTrainingCourseAttendee)
					{
					...
					else
					{
						aHTML.push('<td id=ns1blankspaceTrainingActions">' +
											...
											' title="Open" data-toggle="tooltip"' + 
											' data-rowID="' + oRow.id + '"></span>');
					}
				}
				aHTML.push('</tr>');
			...
			bind: function(oParam)
			{
				...
				$('.ns1blankspaceHeaderSort')
					.on('click', function(event)
					{
						...
						$(this).attr('data-sortdirection', (($(this).attr('data-sortdirection') === 'asc') ? 'desc' : 'asc'));
						nsFreshcare.internal.entity.training.show(oParam);
					})
					.css('cursor', 'pointer')
				...
				if (iObject != nsFreshcare.objectTrainingCourseAttendee)
				{
					// v4.0.001 Not sure this is actually binding to anything!!
					$('.ns1blankspaceSelect')
					.button(
					{
						text: false,
				...
				{	
					...
					$('.ns1blankspaceRowAttendanceLink').click(function(event) 
					{
						...
						{
							var oThisLink = $.grep(oAttachToObject.training, function(a) {return a.id == sElementId});
				...
				$('.ns1blankspaceLinkedPerson')
				.hover(
					function() 
					{
						var sLinkedPerson = $(this).attr('data-contactpersontext');
						$('#ns1blankspaceMultiUseDialog').show();
							
						ns1blankspace.container.position({xhtmlElementID: this.id, topOffset: -10, xhtmlElementContainerID: 'ns1blankspaceMultiUseDialog'});

						$('#ns1blankspaceMultiUseDialog').html('<table class="ns1blankspaceSearchMedium" style="width:150px;"><tr>' +
										'<td><span class="ns1blankspaceSearch" style="font-size:0.825em;"><b>Linked Person: </b>' + sLinkedPerson + '</span></td>' +
										'</tr></table>');
					},
					function() 
					{
			...
			add: 	function() 
			{
				...
				{
					nsFreshcare.internal.entity.training.details(
					{
						dataLinkID: -1,
			...
			manage: function(oParam) 
			{
				...
				if (bContinue) {
					...
					if (nsFreshcare.user.roleID === nsFreshcare.data.roles.admin ||
						...
						aHTML.push('<tr><td><span id="ns1blankspaceLinkAttendeeSave" class="ns1blankspaceAction" title="Save"></span></td></tr>');
						aHTML.push('<tr><td>&nbsp;</td></tr>');
						aHTML.push('<tr><td>' +
										'<input type="checkbox" id="ns1blankspaceLinkAttendeeNonGrower">' + 
					...
					// v4.0.002 incorreclty removed save label
			...
			details: function(oParam) 
			{
				...
				// v3.3.002 Removed businessgroup filter and replaced otherContact filter with eLearningTrainee
				aHTML.push('<tr class="ns1blankspaceCaption">' +
							'<td class="ns1blankspaceCaption ns1blankspaceCaptionMandatory">' +
							'Person</td></tr>' +
							'<tr class="ns1blankspace">' +
							...
							'<input id="ns1blankspaceLinksPerson" class="ns1blankspaceSelect"' +
								...
								' data-methodfilter="firstname-TEXT_IS_LIKE|surname-TEXT_IS_LIKE|' +
										'persongroup-IN_LIST-' + nsFreshcare.data.grower.categoryGrower + ',' + 
											nsFreshcare.data.groupTrainee  + ',' + nsFreshcare.data.groupELearningTrainee + '"' + 
								' data-click="">' +
							'</td></tr>');
				...
				$('#ns1blankspaceLinkAttendeeNonGrower')
				.on('click', function()
				{
					if ($('#ns1blankspaceLinkAttendeeNonGrower').prop('checked'))
					{
						// Move extra filters to data-extraFilters
				
						var aFilters = $('#ns1blankspaceLinksBusiness').attr('data-methodFilter').split('|');
						$('#ns1blankspaceLinksBusiness').attr('data-extraFilters', $.grep(aFilters, function(x)
						{
							var a = x.split('-');
							return a.length != 2;
						}).join('|'));
						$('#ns1blankspaceLinksBusiness').attr('data-methodFilter', $.grep(aFilters, function(x)
						{
							var a = x.split('-');
							return a.length == 2;
						}).join('|'));
						// Remove Mandatory for Business
						$('#ns1blankspaceLinksBusiness').removeAttr('data-mandatory');
					}
					else
					{
						// Add extra filters back to data-methodFilter
						if ($('#ns1blankspaceLinksBusiness').attr('data-extraFilters'))
						{
							$('#ns1blankspaceLinksBusiness')
							.attr('data-methodFilter', $('#ns1blankspaceLinksBusiness').attr('data-methodFilter') + '|' + 
														$('#ns1blankspaceLinksBusiness').attr('data-extraFilters'))
							.removeAttr('data-extraFilters');
						}
						$('#ns1blankspaceLinksBusiness').attr('data-mandatory', '1');
					}
				});
			...
			{ 
				show: 	function(oElement) 
				{
					...
					else 
					{
						...
						// v4.0.001 Bootstrap - removed label
						aHTML.push('<table class="ns1blankspaceSearchMedium" style="width:100px;"><tr>' +
									'<td><span id="ns1blankspaceLinksDelete_' + iRowID + '" class="ns1blankspaceSearch"'+
										'>Remove</span></td>' +
									'</tr></table>');
		...
		groups:
		{
			...
			{	
				...
				if (oResponse == undefined)
				{
					...
					// v3.2.025 COnditionally allow adding
					if (ns1blankspace.option.preventUpdates != true)
					{
						aHTML.push('<tr><td>' +
										'<span id="ns1blankspaceContactBusinessGroupsAdd" class="ns1blankspaceAction"' +
										' title="Add to group"></span>' +
										'</td></tr>');	
					}												
						...
						{	
							if (this.grouptext != '')
							{
								...
								aHTML.push('<td style="width:30px;text-align:right;" class="ns1blankspaceRow">' +
												...
												' data-groupid="' + this.group + '" title="Remove Group"' +
			...
			add: function (oParam, oResponse)
			{
				...
				var sSearchContext = (ns1blankspace.object == nsFreshcare.objectBusiness) ? "BUSINESS": 'PERSON';
				var sXHTMLContainerID = "ns1blankspaceContactPersonGroupsColumn2".replace('Person', (sSearchContext == 'BUSINESS' ? 'Business' : 'Person'));
				...
				// v3.2.025 COnditionally allow adding groups
				if (ns1blankspace.option.preventUpdates != true)
				{
					...
					else
					{
						if (oResponse == undefined)
						{
							ns1blankspace.status.clear();
							var oSearch = new AdvancedSearch();
							oSearch.method = 'SETUP_CONTACT_' + sSearchContext + '_GROUP_SEARCH';
							oSearch.addField('title');
							oSearch.sort("title", 'asc');
							oSearch.rows = 20;
							oSearch.getResults(function(oResponse)
								...
								if (oResponse.morerows == "true")
								{
									aHTML.push('<tr><td class="ns1blankspaceSelect">' +
													'<input class="ns1blankspaceSelect" id="ns1blankspaceGroupsAddSelect"' +
															...
															' data-method="SETUP_CONTACT_' + sSearchContext.toUpperCase() + '_GROUP_SEARCH"' +
				...
				{
					ns1blankspace.status.message('Adding groups not permitted');
				}
			},

			select: function(sXHTMLElementID)
			{
				// v3.2.020 SUP023347 Wasn't getting correct values when passed from ns1blankspaceSelect
				// v3.2.024 SUP023638 Wasn't getting correct value for xhtmlContext
				var sXHTMLContext = '';
				var sSearchContext = (ns1blankspace.object == nsFreshcare.objectBusiness) ? "Business": 'Person';
				...
				{
					var sXHTMLInputElementID = 'ns1blankspaceGroupsAddSelect';
					if ($.type(sXHTMLElementID) == 'object') 
					{
						sXHTMLInputElementID = sXHTMLElementID.xhtmlInputElementID
						sXHTMLElementID = sXHTMLElementID.xhtmlElementID.replace('-', '--')
					}
					sXHTMLContext = $('#' + sXHTMLInputElementID).attr('data-context');
					sXHTMLContext = sXHTMLContext || $('#' + sXHTMLElementID).attr('data-context');
				}
				ns1blankspace['contact' + sSearchContext].groups.select(sXHTMLElementID);
			}
		},
		...
		actions:
		{
			search: function(oParam)
			{
				...
				{
					// v3.4.007 Added auditFields & object/context, plus action.action.message
					oSearch.addField('subject,duedate,duedatetime,actiontype' +
									',actiontypetext,message,contactpersontext,contactbusiness,contactbusinesstext' +
									',actionby,actionbytext,emailsent,object,objectcontext' +
									',action.action.message');
					oSearch.addField(ns1blankspace.option.auditFields);
				}
				else
				{
					oSearch.addField('subject,duedate,actiontype,actiontypetext,description,actionby,actionbytext,text');
				}
				...
				if (oFilters.length > 0)
				{
					$.each(oFilters, function()
					{
						if (this.operation == 'addFilter')
						{
							...
							// 4.0.015 No longer processes applytosubsearch
							oSearch.addFilter(oThisFilter.name, oThisFilter.comparison, oThisFilter.value1, oThisFilter.value2, oThisFilter.value3);	
						}
			...
			show: function(oParam)
			{
				...
				{
					aHTML = [];
					aHTML.push('<span id="ns1blankspace' + sXHTMLContext + 'SearchHandle"' +
								' style="height:20px;" title="Search Criteria" data-toggle="tooltip"></span>');
					$('#ns1blankspace' + sXHTMLContext + 'SearchRibbon').html(aHTML.join(''));

					aHTML = [];
					aHTML.push('<table class="ns1blankspaceColumn2" style="width:100%">');

					aHTML.push('<tr><td id="ns1blankspace' + sXHTMLContext + 'Search" class="ns1blankspaceAction" title="Search"></td></tr>');
			...
			{	
				...
				// v3.2.025 React to preventUpdates option
				oParam.actions = (ns1blankspace.option.preventUpdates == true) ? {add: false} : oParam.actions;
				...
				if (!bEmailsOnly)
				{	
					aHTML.push('<td class="ns1blankspaceHeaderCaption" style="font-size:0.875em;">Type</td>');	
				}
				...
				// v3.2.001 SUP023329 Added Description header if applicable
				// v4.0.001 Bootstrap
				if (bShowDescription)
				{
					aHTML.push('<td class="ns1blankspaceHeaderCaption" style="font-size:0.875em;">Description</td>');	
				}
				aHTML.push('<td class="ns1blankspaceHeaderCaption" style="text-align:right;font-size=0.875em;">' +
								((oActions.add === true) 
									? '<span id="ns1blankspaceActionsAdd" title="Add" data-toggle="tooltip"></span>' 
									: '&nbsp;') +
							'</td>');
				aHTML.push('</tr>');
			...
			bind: function(oParam)
			{
				...
				.click(function()
				{
					...
					// v3.1.210 SUP023227 Needs all of the oParam values
					//var oParam = {};
			...
			row: function(oRow, oParam)
			{
				...
				// v3.4.001 Shows text or description - whichever is populated (these are mutually exclusive)
				...
				if (bShowDescription && (oRow.description || oRow.text))
				{
					aHTML.push('<td id="ns1blankspaceAction_description-' + oRow.id + '" style="font-size:0.75em;" class="ns1blankspaceRow">' +
									(oRow.description ? oRow.description : oRow.text) + '</td>');
				}					
				
				aHTML.push('<td style="width:30px;text-align:right;" class="ns1blankspaceRow">' + 			
								'<span id="ns1blankspaceAction_options_remove-' + oRow.id + '"' +
									' title="Remove" data-toggle="tooltip"' + 
			...
			edit: function(oParam, oResponse)
			{
				...
				else
				{
					...
					aHTML.push('<tr><td>' +
										'<span id="ns1blankspaceActionAddSave" class="ns1blankspaceAction" title="Save"></span>' +
										'<td></tr>');
					

					aHTML.push('<tr><td>' +
										'<span id="ns1blankspaceActionAddCancel" class="ns1blankspaceAction" title="Cancel"></span>' +
										'<td></tr>');
					...
					.click(function()
					{
						oParam.functionPostSave(oParam);	// v3.2.025 Was calling functionPostSave - it's undefined
					})
					.css('width', '75px');
					...
					// v3.4.008 Now uses util function
					...
					{	
						...
						{	
							...
							$('#ns1blankspaceActionAddDescription').val((oResponse.data.rows[0].description ? oResponse.data.rows[0].description : oResponse.data.rows[0].text).formatXHTML());
					...
					{
						$('#ns1blankspaceActionAddDate').val((new Date()).toString('dd MMM yyyy hh:mm tt'));
			...
			save: 		function (oParam, oData, oResponse)
			{
				if (oResponse === undefined)
				{
					// v3.1.210 SUP023227 Now uses fPostSave from parameters instead of hard-coding
					// v3.4.001 Now saves to text instead of description as it's longer
					var oSaveData = {};
					...
					var fPostSave = ns1blankspace.util.getParam(oParam, 'functionPostSave', {'default': nsFreshcare.internal.entity.actions.show}).value;
					...
					if (oData != undefined)
					{
						...
						oSaveData[ns1blankspace.action.data.blobField] = oData.description;
						...
						$.ajax(
						{
							...
							success: function(oResponse) 
							{
								if (oResponse.status == 'OK')
								{
									...
									fPostSave(oParam, oData, oResponse);
								}
								else
								{
									ns1blankspace.status.error('Action not saved: ' + oResponse.error.errornotes);
								}
							}
						});
					}
		...
		notes:
		{
			search: function(oParam)
			{
				var iObject = ns1blankspace.util.getParam(oParam, 'object', {'default': ns1blankspace.object}).value;
				var iObjectContext = ns1blankspace.util.getParam(oParam, 'objectContext', {'default': ns1blankspace.objectContext}).value;
				var aActionTypes = ns1blankspace.util.getParam(oParam, 'actionTypes', {'default': [nsFreshcare.data.actionTypeUserNotes]}).value;
				...
				oSearch.addFilter('actiontype', 'IN_LIST', aActionTypes.join(','));
			...
			show: function(oParam, oResponse)
			{
				...
				aHTML.push('<td class="ns1blankspaceHeaderCaption">' +
								((oActions.add) ? '<span id="ns1blankspaceNotesAdd" title="Add Note"></span>' : '&nbsp;') +
							'</td>');
				aHTML.push('</tr>');
			...
			bind: function(oParam)
			{
				...
				// v3.1.0i SUP022250 Now binds only visible elements
				// v3.2.025 Conditionally bind depending on preventUpdates option
				if (ns1blankspace.option.preventUpdates != true)
				{
			...
			add: function(oParam)
			{
				// v3.3.001 Added parameterisation for hidePriority, hideCancel, title, actionType, actionTypes and html
				oParam = oParam || {};
				...
				var bHidePriority = ns1blankspace.util.getParam(oParam, 'hidePriority', {'default': false}).value;
				var bHideCancel = ns1blankspace.util.getParam(oParam, 'hideCancel', {'default': false}).value;
				var sTitle = ns1blankspace.util.getParam(oParam, 'title', {'default': 'Add Note'}).value;
				var sHTML = ns1blankspace.util.getParam(oParam, 'html', {'default': 'Note'}).value;
				var bShowActionTypes = ns1blankspace.util.getParam(oParam, 'showActionTypes', {'default': oParam.actionType == undefined}).value;
				var sSortTypes = ns1blankspace.util.getParam(oParam, 'sortTypes').value;
				var iActionType = ns1blankspace.util.getParam(oParam, 'actionType', {'default': nsFreshcare.data.actionTypeUserNotes}).value;
				var aActionTypes = ns1blankspace.util.getParam(oParam, 'actionTypes', 
									{'default': [nsFreshcare.data.actionTypeUserNotes]}).value;
				// v3.4.010 SUP023939 was always defaulting to actionType
				if (oParam.actionType == undefined && oParam.actionTypes) {iActionType = undefined}
				...
				var aButtons = 
				{
					"Cancel": function() 
					{
						$('#ns1blankspaceMultiUseContainer').css( "z-index", "99" ).css('display', 'none');
						$( this ).dialog( "destroy" );
					},
					"Add Note": function(event) 
					{
						var bReminderValid = !bShowReminder;
						//var iActionType = iActionType;
						...
						if (bShowActionTypes)
						{
							iActionType = $('#ns1blankspaceAddNoteType').attr('data-id');
						}

						if ($('#ns1blankspaceAddNoteDescription').val() != '' && bReminderValid  
							&& (!bShowActionTypes || (bShowActionTypes && iActionType)))
						{
						...
						else
						{
							...
							else if (iActionType == undefined || iActionType == '')
							{
								$('#ns1blankspaceAddNoteStatus').html('Please choose a note type!');
							}
							else
							{
				...
				if (bHideCancel == false) {delete(aButtons['Cancel]'])}
				...
				if (bShowActionTypes)
				{
					aHTML.push('<tr><td><span>Note Type: </span>' +
									'<input class="ns1blankspaceSelect" id="ns1blankspaceAddNoteType"' +
											' data-method="SETUP_ACTION_TYPE_SEARCH"' +
											' data-methodFilter="id-IN_LIST-' + aActionTypes.join(',') + '"' +
									(sSortTypes ? ' data-sort="' + sSortTypes + '"' : '') +
									(iActionType && iActionType == nsFreshcare.data.actionTypeUserNotes
										? ' value="User Notes" data-id="' + nsFreshcare.data.actionTypeUserNotes + '"'
										: '') +
								'></td></tr>');
				}
				...
				$('#ns1blankspaceMultiUseDialog')
					...
					.dialog(
					{
						...
						resizable: false,
						modal: true,
						closeOnEscape: !bHideCancel,
						title: sTitle,
						open: function()
						{	// bind the date and clear status after error
							// v4.0.016 SUP024214 changed to initDatePicker
							ns1blankspace.util.initDatePicker({select: 'input.ns1blankspaceDate'});
							$('.ui-dialog-titlebar-close').css('display', 'none');
							$('.ui-dialog-buttonset').children().css('font-size', '0.625em').css('text-align', 'center')
							$('.ui-dialog-title').css('font-size', '0.75em');
						},
						buttons: aButtons,
						close: function()
						{	// Occurs if user presses Escape
							$('#ns1blankspaceMultiUseContainer').css( "z-index", "99" ).css('display', 'none');
							$('#ns1blankspaceBootstrapDialog').modal('hide');
						},
					});

				$('#ns1blankspaceMultiUseContainer').css( "z-index", "1020" );		// Make sure it appears on top of dialog
			},
				...
				if (oData != undefined)
				{
					$.ajax(
					{
						...
						success: function(oResponse)
						{
							if (oResponse.status === 'OK')
							{
								$('#' + sDialogElementID).dialog( "destroy" );
								delete(oParam.data);
								if (oParam.functionPostSave)
								{
									var fPostSave = oParam.functionPostSave;
									delete(oParam.functionPostSave);
									fPostSave(oParam);
								}
							}
							else
		...
		emails:
		{
			bind: function(oParam)
			{
				...
				// v3.1.1i Limited to current container
			...
			{	// v3.1.1f Moved from grower.admin.emails
				// v3.2.015 SUP023422 Added COP Based Training
				// v3.4.007 Added actionID for bulk emails (this will display the linked message details)
				var aHTML = [];
				var bAdmin = (nsFreshcare.user.roleID === nsFreshcare.data.roles.admin);
				var sMessage = (oRow.actiontype == ns1blankspace.data.actionTypes.bulkEmail.id) ? oRow['action.action.message'] : oRow.message;
				var iLinkID = (oRow.actiontype == ns1blankspace.data.actionTypes.bulkEmail.id) ? oRow['objectcontext'] : oRow.id;
				...
				{
					aHTML.push('<tr id="ns1blankspaceActionRow-' + oRow.id + '" data-actionID="' + iLinkID + '">');

					if (bAdmin)
					{
						aHTML.push('<td class="ns1blankspaceRow">' +
										'<span id="ns1blankspaceAction_type-' + oRow.id + '" class="ns1blankspaceRow">');
						// v3.4.007 Added bulkEmail
						if (oRow.actiontype === '5' || oRow.actiontype == ns1blankspace.data.actionTypes.bulkEmail.id)		// outgoing email
						{
					...
					aHTML.push('<td id="ns1blankspaceAction_date-' + oRow.id + '" style="font-size:0.75em;"' +
								' class="ns1blankspaceRow ns1blankspaceRowShow" data-actionID="' + iLinkID + '">' +
									oRow.duedate.formatXHTML() + '</td>');

					aHTML.push('<td id="ns1blankspaceAction_subject-' + oRow.id + '" style="font-size:0.75em;"' +
								' class="ns1blankspaceRow ns1blankspaceRowShow" data-actionID="' + iLinkID + '">' +
					...
					{
						aHTML.push('<td class="ns1blankspaceRow">' +
										'<span id="ns1blankspaceAction_options_select-' + oRow.id + '"' +
											' title="View / Edit" data-toggle="tooltip"' + 
		...
		relationships:
		{
			...
			show: function(oParam)
			{
				...
				else
				{
					...
					// v3.2.025 COnditionally display Add button
					if (ns1blankspace.option.preventUpdates != true)
					{
						aHTML.push('<tr><td>' +
										'<span id="ns1blankspaceEntityRelationshipsAdd" class="ns1blankspaceAction" title="Add"></span>' +
										'</td></tr>');
					}
			...
			row: function(oRow)
			{
				...
				// v3.2.025 COnditionally display remove button
				if (ns1blankspace.option.preventUpdates != true)
				{
					// v4.0.001 Bootstrap removed label
					aHTML.push('<span id="ns1blankspaceEntityRelationshipRemove_' + oRow.id + '" ' + 
										'class="ns1blankspaceAction ns1blankspaceEntityRelationshipRemove" ' + 
										'data-rowID="' + oRow.id + '" data-action="remove" title="Remove"></span>&nbsp;');
				}

				aHTML.push('<span id="ns1blankspaceEntityRelationshipShow_' + oRow.id + '" ' + 
									'class="ns1blankspaceAction ns1blankspaceEntityRelationshipShow" ' + 
									'data-rowID="' + oRow.id + '" title="Open"></span>' + 
								'</td>');
			...
			bind: function()
			{
				...
				// v4.0.001 Bootstrap removed labels
			...
			add: 
			{
				show: function(oParam)
				{
					...
					else 
					{
						...
						aHTML.push('<tr><td><span id="ns1blankspaceSummaryUserAddGo" class="ns1blankspaceAction" title="Add User">' +
										'</td></tr>');
				...
				process: function(oParam)
				{
					...
					if (iContactPerson && sUserName && sRole)
					{
						...
						{
							// v3.2.020 SUP023578 Network group title has to be role + ' News'
							var oSearch = new AdvancedSearch();
							oSearch.method = 'SETUP_NETWORK_GROUP_SEARCH';
							oSearch.addField('title');
							oSearch.addFilter('title', 'EQUAL_TO', sRole + ' News');
							oSearch.getResults(function(oResponse) 
							{
								if (oResponse.status === 'OK')
								{
		...
		invoices:
		{
			...
			row: function(oRow)
			{
				...
				aHTML.push('<td id="ns1blankspaceEntityInvoices_select-' + oRow.id + '"' +
								' class="ns1blankspaceRow ns1blankspaceSelect" title="Open">' +
								'</td>');
		...
		save:
		{
			...
			send: function(oParam)
			{
				...
				var aTrainerStateData = ns1blankspace.util.getParam(oParam, 'trainerStateData').value;
				...
				else if (oParam.saveEntityStep === 2)
				{
					if (ns1blankspace.okToSave)
					{
						...
						if ($('#ns1blankspaceMainDetails').html() != '')
						{
							...
							// v3.1.2 Now saves CB No, Self-Cert date & Manual Audit Approval
							if ($('#ns1blankspaceDetailsCertificationBodyNumber').is('*'))
							{
						...
						// v3.1.209b SUP022867 Now saves Trainer State data (Trainer Memberships are saved on Contacts tab)
						if ($('#ns1blankspaceMainAccreditation').html() != '')
						{
							oParam.trainerStateData = [];
							$.each($('#ns1blankspaceAccreditationState span'), function()
							{
								if ($(this).attr('data-rowID') && $(this).css('text-decoration').indexOf('line-through') > -1)
								{	// remove
									oParam.trainerStateData.push({data: 'remove=1&id=' + $(this).attr('data-rowID')});
								}
								else if ($(this).attr('data-rowID') == undefined)
								{	// Add new
									oParam.trainerStateData.push({data: 'state=' + this.id.split('_').pop() + 
						...
						{
							if (oParam.trainerStateData)
							{
								oParam.saveEntityStep = 5;
								nsFreshcare.internal.entity.save.send(oParam);
							}
							else
							{
						...
						$.ajax(
						{
							...
							success: function(oResponse)
							{
								if (oResponse.status === 'OK')
								{
									...
									else {oParam.saveEntityStep = (oParam.trainerStateData ? 5 : 10)}
									nsFreshcare.internal.entity.save.send(oParam);
								}
								else
								{
				...
				// Save Trainer States
				else if (oParam.saveEntityStep == 5)
				{
					if (oParam.trainerStateData && oParam.trainerStateData.length > 0)
					{
						if (oParam.trainerStateIndex == undefined) {oParam.trainerStateIndex = 0}

						if (oParam.trainerStateIndex < oParam.trainerStateData.length)
						{
							$.ajax(
							{
								type: 'POST',
								url: ns1blankspace.util.endpointURI('AGRI_TRAINER_STATE_MANAGE'),
								data: oParam.trainerStateData[oParam.trainerStateIndex].data,
								success: function(oResponse)
								{
									if (oResponse.status == 'OK')
									{
										oParam.trainerStateIndex += 1;
										nsFreshcare.internal.entity.save.send(oParam);
									}
									else
									{
										ns1blankspace.status.error('Unable to save Trainer State: ' + oResponse.error.errornotes);
									}
								}
							})
						}
						...
						{
							delete(oParam.trainerStateData);
							delete(oParam.trainerStateIndex);
	...
	contactBusiness:
	{
		...
		search:
		{
			...
			{
				// v3.3.001 SUP023569 Added Snapshot search functionality
				...
				{							
					...
					// v3.2.001 SUp023328 Added sexeroconactid
					// v3.2.019b SUP023594 removed xero fields from 'standard' search
					// v3.2.025 SUP023569 Conditionally added xero fields back in
					var oSearch = new AdvancedSearch();
					oSearch.endPoint = 'contact';
					oSearch.method = 'CONTACT_BUSINESS_SEARCH';
					...
					{
						oSearch = nsFreshcare.util.system.addSnapShot(oSearch, sSearchContext);
						oSearch.getResults(function(data) {nsFreshcare.util.system.search(oParam, data)});
					}
				}
				else
				{
		...
		layout: function()
		{
			...
			// v3.2.001 SUP022329 Added abilty to control access
			// v3.2.020 SUP023422 Added Training Tab
			// v3.3.001 SUP023569 Added System tab
			var oRoot = ns1blankspace.rootnamespace;
			var aHTML = [];
			...
			if (ns1blankspace.objectContext == -1)
			{
				if (nsFreshcare.util.roleHasAccess({action: 'create', tabs: ['Details', 'Address'], access: oRoot.internal.contactBusiness.access()}))
				{
				...
				{
					aHTML.push('You do not have access to add Businesses');
				}
			}
			else
			{
				...
				if (nsFreshcare.util.roleHasAccess({tabs: ['Groups', 'Relationships','Training'], access: oRoot.internal.contactBusiness.access()}))
				{
					...
					{	aHTML.push('<tr><td id="ns1blankspaceControlTraining" class="ns1blankspaceControl">Training</td></tr>');}
				}						
				aHTML.push('</table>');

				aHTML.push('<table class="ns1blankspaceControl">');
				if (nsFreshcare.util.roleHasAccess({tabs: ['Financial'], access: oRoot.internal.contactBusiness.access()}))
				{
					aHTML.push('<tr><td id="ns1blankspaceControlFinancial" class="ns1blankspaceControl">Financials</td></tr>');
				}
				if (nsFreshcare.util.roleHasAccess({tabs: ['Xero'], access: oRoot.internal.contactBusiness.access()}))
				{
					aHTML.push('<tr><td id="ns1blankspaceControlXero" class="ns1blankspaceControl">Xero</td></tr>');
				}
				if (nsFreshcare.util.roleHasAccess({tabs: ['System'], access: oRoot.internal.contactBusiness.access()}))
				{
					aHTML.push('<tr><td>&nbsp;</td></tr>');
					aHTML.push('<tr><td id="ns1blankspaceControlSystem" class="ns1blankspaceControl">System</td></tr>');
				}
				aHTML.push('</table>');		
			
				aHTML.push('<table class="ns1blankspaceControl">');
			
				if (nsFreshcare.util.roleHasAccess({tabs: ['Emails', 'Actions', 'Attachments'], access: oRoot.internal.contactBusiness.access()}))
				{
			...
			{
				ns1blankspace.show({selector: '#ns1blankspaceMainAddress'});
				// v4.0.001 Changed from ns1blankspace version 
				nsFreshcare.admin.grower.address({suburbUpper: true});
			});
			...
			$('#ns1blankspaceControlFinancial').click(function(event)
			{
				...
				ns1blankspace.contactBusiness.financials.show(); 		// 3.4.007 financials() has ben deprecated
			});
			
			$('#ns1blankspaceControlXero').click(function(event)
			{
				ns1blankspace.show({selector: '#ns1blankspaceMainXero'});
				nsFreshcare.internal.contactBusiness.xero();
			});
			...
			$('#ns1blankspaceControlSystem').click(function(event)
			{
				...
				nsFreshcare.util.system.search({xhtmlElementID: 'ns1blankspaceMainSystem', nameSpace: ns1blankspace});
			});
		...
		show:  function (oParam, oResponse)
		{
			...
			else
			{
				...
				// v3.2.025 Use app.context instead of directly disabling 
		...
		summary: function (oParam, oResponse)		
		{
			...
			else
			{
				if (oResponse == undefined)
				{
					...
					// v3.4.001 SUP023639 Added COnvert to Member button
					aHTML.push('<tr><td id="ns1blankspaceConvertToMember" class="ns1blankspaceSummaryCaption"' +
								'>&nbsp;</td></tr>');	
					...
					$('#ns1blankspaceConvertToMember')
					.button({label: 'Convert To Member'})
					.css('font-size', '0.75em')
					.css('width', '150px')
					.on('click', function()
					{
						nsFreshcare.internal.contactBusiness.convertToMember();
					});
				...
				else
				{
					...
					$('#ns1blankspaceFavourite').html('<input type="checkbox" ' + (bFavourite?'checked="checked" ':'') + 'id="ns1blankspaceContactBusinessFavourite"/>' +
							'<label for="ns1blankspaceContactBusinessFavourite" style="font-size:0.75em; width:150px;">&nbsp;</label>');
		...
		convertToMember: function(oParam)
		{
			// SUP023639 Allow user to covnert the Business to a Member if they aren't already 
			oParam = oParam || {};
			if (oParam.convertStep == undefined) {oParam.convertStep = 0}

			if (oParam.convertStep == 0)
			{
				ns1blankspace.okToSave = true;

				ns1blankspace.container.confirm(
				{
					title: 'Convert Business to Member',
					html: 'Are you sure you want to convert ' + ns1blankspace.objectContextData.tradename.formatXHTML() + ' to a Member?',
					buttons:
					[
						{
							text: 'Yes',
							...
							{
								$('#ns1blankspaceBootstrapDialog').modal('hide');
								oParam.convertStep = 1;
								nsFreshcare.internal.contactBusiness.convertToMember(oParam);
							}
						},
						{
							text: 'No',
			...
			// Check to see if Business has a addresses - cannot proceed if not
			else if (oParam.convertStep == 1)
			{
				if ((ns1blankspace.objectContextData.streetaddress1 + 
					ns1blankspace.objectContextData.streetaddress2 +
					ns1blankspace.objectContextData.streetsuburb +
					ns1blankspace.objectContextData.streetpostcode +
					ns1blankspace.objectContextData.streetstate +
					ns1blankspace.objectContextData.streetcountry).length == 0)
				{
					ns1blankspace.status.message('Business does not have a street address. Street Address is required for a Member')
					ns1blankspace.okToSave = false;
				}
				else
				{
					if (ns1blankspace.objectContextData.streetaddress1 == '')
					{
						ns1blankspace.status.message('The first line of the Street address is required to convert to a Member.');
						ns1blankspace.okToSave = false;
					}
					if (ns1blankspace.okToSave && ns1blankspace.objectContextData.streetsuburb == '')
					{
						ns1blankspace.status.message('The suburb of the Street address is required to convert to a Member.');
						ns1blankspace.okToSave = false;
					}
					if (ns1blankspace.okToSave && ns1blankspace.objectContextData.streetstate == '')
					{
						ns1blankspace.status.message('The state of the Street address is required to convert to a Member.');
						ns1blankspace.okToSave = false;
					}
					if (ns1blankspace.okToSave && ns1blankspace.objectContextData.streetpostcode == '')
					{
						ns1blankspace.status.message('The post code of the Street address is required to convert to a Member.');
						ns1blankspace.okToSave = false;
					}
					if (ns1blankspace.okToSave && ns1blankspace.objectContextData.streetcountry == '')
					{
						ns1blankspace.status.message('The country of the Street address is required to convert to a Member.');
						ns1blankspace.okToSave = false;
					}
				}

				if (ns1blankspace.okToSave &&
					(ns1blankspace.objectContextData.mailingaddress1 + 
					ns1blankspace.objectContextData.mailingaddress2 +
					ns1blankspace.objectContextData.mailingsuburb +
					ns1blankspace.objectContextData.mailingpostcode +
					ns1blankspace.objectContextData.mailingstate +
					ns1blankspace.objectContextData.mailingcountry).length == 0)
				{
					ns1blankspace.status.message('Business does not have a mailing address. Mailing Address is required for a Member');
				}
				else
				{
					if (ns1blankspace.okToSave && ns1blankspace.objectContextData.mailingaddress1 == '')
					{
						ns1blankspace.status.message('The first line of the Mailing address is required to convert to a Member.');
						ns1blankspace.okToSave = false;
					}
					if (ns1blankspace.okToSave && ns1blankspace.objectContextData.mailingsuburb == '')
					{
						ns1blankspace.status.message('The suburb of the Mailing address is required to convert to a Member.');
						ns1blankspace.okToSave = false;
					}
					if (ns1blankspace.okToSave && ns1blankspace.objectContextData.mailingstate == '')
					{
						ns1blankspace.status.message('The state of the Mailing address is required to convert to a Member.');
						ns1blankspace.okToSave = false;
					}
					if (ns1blankspace.okToSave && ns1blankspace.objectContextData.mailingpostcode == '')
					{
						ns1blankspace.status.message('The post code of the Mailing address is required to convert to a Member.');
						ns1blankspace.okToSave = false;
					}
					if (ns1blankspace.okToSave && ns1blankspace.objectContextData.mailingcountry == '')
					{
						ns1blankspace.status.message('The country of the Mailing address is required to convert to a Member.');
						ns1blankspace.okToSave = false;
					}

					if (ns1blankspace.okToSave)
					{
						oParam.convertStep = 2;
						nsFreshcare.internal.contactBusiness.convertToMember(oParam);
					}
				}
			}

			// Search for Member Business Group
			else if (oParam.convertStep == 2)
			{
				var oSearch = new AdvancedSearch();
				oSearch.method = 'CONTACT_BUSINESS_GROUP_SEARCH';
				oSearch.addField('contactbusiness,group');
				oSearch.addFilter('contactbusiness', 'EQUAL_TO', ns1blankspace.objectContext);
				...
				oSearch.getResults(function(oResponse)
				{
					if (oResponse.status == 'OK')
					{
						if (oResponse.data.rows.length > 0)
						{
							ns1blankspace.status.message('Business already has Member group so cannot be converted to a Member.');
						}
						else
						{
							ns1blankspace.status.working('Checking if Primary Contact exists');
							oParam.convertStep = 3;
							nsFreshcare.internal.contactBusiness.convertToMember(oParam);
						}
					}
					else
					{
						ns1blankspace.status.error('Error determining if Business is already a Member: ' + oResponse.error.errornotes);
					}
				});
			}

			// Determine if Member has a contact and if so, is there a primary contact set
			// Update primary contact with mandatory fields if it exists
			else if (oParam.convertStep == 3)
			{
				var oSearch = new AdvancedSearch();
				oSearch.method = 'CONTACT_PERSON_SEARCH';
				oSearch.addField('contactperson.primarycontactfor.id,customerstatus,gender,title');
				oSearch.addFilter('contactbusiness', 'EQUAL_TO', ns1blankspace.objectContext);
				oSearch.rows = 500;
				oSearch.getResults(function(oResponse)
				{
					if (oResponse.status == 'OK')
					{
						// No contacts exist - user must add one prior to converting to Member
						if (oResponse.data.rows.length == 0)
						{
							ns1blankspace.status.message('Business has no contacts. Business must have a primary contact to be converted to a Member.');
						}
						else
						{
							// Check if Primary Contact has correct default values
							oParam.primaryContact = $.grep(oResponse.data.rows, function(x) {return x['contactperson.primarycontactfor.id'] == ns1blankspace.objectContext}).shift();
							if (oParam.primaryContact != undefined)
							{
								oParam.convertStep = 4;
								oParam.onComplete = nsFreshcare.internal.contactBusiness.convertToMember;
								if (oParam.primaryContact.customerstatus != nsFreshcare.data.contactStatusActive
									|| oParam.primaryContact.gender == '' || oParam.primaryContact.title == '')
								{
									var oData = 
									{
										id: oParam.primaryContact.id,
										customerstatus: nsFreshcare.data.contactStatusActive
									};
									if (oParam.primaryContact.gender == '') {oData.gender = '1'}
									if (oParam.primaryContact.title == '') {oData.title = '6'}
									$.ajax(
									{
										type: 'POST',
										url: ns1blankspace.util.endpointURI('CONTACT_PERSON_MANAGE'),
										...
										success: function(oResponse)
										{
											if (oResponse.status == 'OK')
											{
												ns1blankspace.status.working('Getting next Company ID');
												nsFreshcare.admin.grower.save.getNextReference(oParam);
											}
											else
											{
												ns1blankspace.status.error('Error updating primary contatc details: ' + oResponse.error.errornotes);
											}
										}
									});
								}
							...
							{
								ns1blankspace.status.message('A primary contact has not been selected for the Business. ' +
												'Please choose a primary contact to convert to a Member.');
							}
						}
					}
					else
					{
						ns1blankspace.status.error('Error checkng for Primary Contact: ' + oResponse.error.errornotes);
					}
				});
			}

			// Change reference number and ensure fields such as Status are set correctly
			else if (oParam.convertStep == 4)
			{
				if (oParam.nextReference)
				{
					// Set values that MUST be set for a member
					var oData = 
					{
					...
					oData['se' + nsFreshcare.data.sendPrintedCertificatesId] = $.map($.grep(nsFreshcare.data.sendPrintedCertificatesOptions, 
											function(x) {return x.title == 'No'}), function(y) {return y.id}).shift();
					...
					$.ajax(
					{
						...
						success: function(oResponse)
						{
							if (oResponse.status == 'OK')
							{
								oParam.convertStep = 5;
								nsFreshcare.internal.contactBusiness.convertToMember(oParam);
							}
							else
							{
								ns1blankspace.status.error('Error updating business reference: ' + oResponse.error.errornotes);
							}
						}
					});
				}
				...
				{
					ns1blankspace.status.error('Error finding business reference.');
				}
			}

			// Add Member business group
			else if (oParam.convertStep == 5)
			{
				$.ajax(
				{
					type: 'POST',
					url: ns1blankspace.util.endpointURI('CONTACT_BUSINESS_GROUP_MANAGE'),
					data: {contactbusiness: ns1blankspace.objectContext, group: nsFreshcare.data.businessGroupGrowerID},
					success: function(oResponse)
					{
						if (oResponse.status == 'OK')
						{
							oParam.convertStep = 6;
							ns1blankspace.status.message('Business converted to Member');
							nsFreshcare.internal.contactBusiness.convertToMember(oParam);
						}
						else
						{
							ns1blankspace.status.error('Error adding Member business group: ' + oResponse.error.errornotes);
						}
					}
				});			
			}

			// Check if Member person group exists and add if not
			else if (oParam.convertStep == 6)
			{
				var oSearch = new AdvancedSearch();
				oSearch.method = 'CONTACT_PERSON_GROUP_SEARCH';
				oSearch.addField('contactperson,group');
				oSearch.addFilter('contactperson', 'EQUAL_TO', oParam.primaryContact.id);
				oSearch.addFilter('group', 'EQUAL_TO', nsFreshcare.data.grower.categoryGrower);
				oSearch.getResults(function(oResponse)
				{
					if (oResponse.status == 'OK')
					{
						if (oResponse.data.rows.length == 0)
						{
							$.ajax(
							{
								type: 'POST',
								url: ns1blankspace.util.endpointURI('CONTACT_PERSON_GROUP_MANAGE'),
								data: {contactperson: oParam.primaryContact.id, group: nsFreshcare.data.grower.categoryGrower},
								success: function(oResponse)
								{
									if (oResponse.status == 'OK')
									{
										oParam.convertStep = 7;
										ns1blankspace.status.message('Member Person Group added.');
										nsFreshcare.internal.contactBusiness.convertToMember(oParam);
									}
									else
									{
										ns1blankspace.status.error('Error adding Member person group: ' + oResponse.error.errornotes);
									}
								}
							});
						}
					...
					{
						ns1blankspace.status.error('Error searching for Person groups: ' + oResponse.error.errornotes);
					}
				})
			}
			
			// Add sites from business street address 
			else if (oParam.convertStep == 7)
			{
				var oData =
				{
					type: '1',
					status: '1',
					address1: ns1blankspace.objectContextData.streetaddress1,
					address2: ns1blankspace.objectContextData.streetaddress2,
					addresssuburb: ns1blankspace.objectContextData.streetsuburb,
					addressstate: ns1blankspace.objectContextData.streetstate,
					addresspostcode: ns1blankspace.objectContextData.streetpostcode,
					addresscountry: ns1blankspace.objectContextData.streetcountry
				};
				$.ajax(
				{
					type: 'POST',
					...
					success: function(oResponse)
					{
						if (oResponse.status == 'OK')
						{
							...
							$.ajax(
							{
								...
								success: function(oResponse)
								{
									if (oResponse.status == 'OK')
									{
										oParam.convertStep = 10;
										nsFreshcare.internal.contactBusiness.convertToMember(oParam);
									}
									else
									{
										ns1blankspace.status.error('Error adding Site Address Link: ' + oResponse.error.errornotes);
									}
								}
							});
						}
						...
						{
							ns1blankspace.status.error('Error adding Site Address: ' + oResponse.error.errornotes);
						}
					}
				});
			}
			...
			// Finish up
			else if (oParam.convertStep == 10)
			{
				
				ns1blankspace.status.clear();
				nsFreshcare.admin.grower.init({id: ns1blankspace.objectContext});
				ns1blankspace.container.confirm(
				{
					title: "Action Required!",
					html: "Member is not complete. Please add a Membership to this Member to ensure data integrity.",
				});
			}
		},
		...
		details: function()
		{
			// v3.1.204 SUP023015 Forked so that could display both customer & supplier status
			var aHTML = [];
		...
		{							
			...
			{	
				...
				if (oResponse == undefined)
				{
					...
					// v3.2.0.25 Conditionally allow adding
					if (ns1blankspace.option.preventUpdates == true)
					{
						aHTML.push('<tr><td>' +
										'<span id="ns1blankspaceContactBusinessGroupsAdd" class="ns1blankspaceAction" title="Add to group"></span>' +
										'</td></tr>');	
					}												
					aHTML.push('</table>');					
					...
					$('#ns1blankspaceContactBusinessGroupsAdd')
					.button()
					.click(function() 
					{
						ns1blankspace.container.position(
					...
					else
					{
						...
						{	
							if (this.grouptext != '')
							{
								...
								aHTML.push('<td style="width:30px;text-align:right;" class="ns1blankspaceRow">' +
												'<span id="ns1blankspaceGroups_remove-' + this.id + '" title="Remove Gruop"' +
												' class="ns1blankspaceRow ns1blankspaceGroupsRemove"></span></td>');
				...
				show: 		function (oParam, oResponse)
				{
					...
					if (oResponse == undefined)
					{
						...
						oSearch.getResults(function(oResponse)
						{
							if (oResponse.status == 'OK')
							{
								ns1blankspace.contactBusiness.groups.search.show(oParam, oResponse);
							}
							else
							{
								ns1blankspace.status.error(oResponse.error.errornotes);
		...
		xero: function()
		{
			...
			if ($('#ns1blankspaceMainXero').attr('data-loading') == '1')
			{
				...
				aHTML.push('<tr class="ns1blankspaceCaption">' +
								...
								'<tr class="ns1blankspace">' +
								'<td class="ns1blankspaceText">' +
								'<input id="ns1blankspaceXeroContactID" class="ns1blankspaceText">' +
								'</td></tr>');			

				// v3.2.005 SUP023329 Did not have date class
				aHTML.push('<tr class="ns1blankspaceCaption">' +
								'<td class="ns1blankspaceCaption">' +
								...
								'<tr class="ns1blankspace">' +
								'<td class="ns1blankspaceText">' +
								'<input id="ns1blankspaceXeroContactUpdated" class="ns1blankspaceDate">' +
								'</td></tr>');			
				...
				if (ns1blankspace.objectContext != -1)
				{
					$('#ns1blankspaceXeroContactID').val(ns1blankspace.objectContextData.sexerocontactid.formatXHTML());
					$('#ns1blankspaceXeroContactUpdated').val(ns1blankspace.objectContextData.sexerocontactupdated.formatXHTML());
				}

				if (!nsFreshcare.util.roleHasAccess({tabs: ['Xero'], action: 'update', access: oRoot.internal.contactBusiness.access()}))
				{
		...
		save:
		{
			send: function()
			{
				...
				if ($('#ns1blankspaceMainXero').html() != '' && nsFreshcare.util.roleHasAccess({tabs: ['Xero'], action: 'update', access: oRoot.internal.contactBusiness.access()}))
				{
					if (ns1blankspace.objectContext == -1 || $('#ns1blankspaceXeroContactID').val() != ns1blankspace.objectContextData.sexerocontactid.formatXHTML())
					{	sData += '&sexerocontactid=' + ns1blankspace.util.fs($('#ns1blankspaceXeroContactID').val());}
					if (ns1blankspace.objectContext == -1 || $('#ns1blankspaceXeroContactUpdated').val() != ns1blankspace.objectContextData.sexerocontactupdated.formatXHTML())
					{	sData += '&sexerocontactupdated=' + ns1blankspace.util.fs($('#ns1blankspaceXeroContactUpdated').val())};
				}
				...
				$.ajax(
				{
					...
					data: sData,
					dataType: 'json',
					success: nsFreshcare.internal.contactBusiness.save.process
				});
			},

			process: function(oResponse)
			{	// v3.4.001 SUP023639 Added
				if (oResponse.status == 'OK')
				{
