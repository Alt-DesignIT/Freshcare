...
nsFreshcare.external.grower = 
{
	...
	details: 	function ()
	{
		...
		if ($('#ns1blankspaceMainDetails').attr('data-loading') == '1')
		{
			...
			// 3.0.1 Added this section for admin
			if (nsFreshcare.user.role.toLowerCase() === 'admin' && ns1blankspace.objectContext != -1)
			{
				...
				aHTML.push('<tr class="ns1blankspaceCaption ns1blankspaceHideOnNew ns1blankspaceHideOnNewTrainee">' +
								...
								'Training Course' +
								'</td></tr>' +
								'<tr id="ns1blankspaceDetailsRowTrainingCourse" class="ns1blankspace ns1blankspaceHideOnNew">' +
								'<td id="ns1blankspaceDetailsTrainingCourse" class="nsFreshcareReadOnly">&nbsp;' +
								'</td></tr>');			
				aElements.push("TrainingCourse");
			...
			if ((nsFreshcare.user.role.toLowerCase() === 'trainer' && ns1blankspace.objectContext === -1) 
				|| (nsFreshcare.user.role.toLowerCase() === 'admin' && ns1blankspace.objectContext != -1))
			{
				// If Trainer logon and a new Grower, we need to choose the Package &  training course
				var aTrainerMemberships = (nsFreshcare.user.role.toLowerCase() === 'trainer')
				...
				// v3.1.1 SUP022473  methodFilter was missing a pipe causing filters to not be executed
				aHTML.push('<tr class="ns1blankspaceCaption">' +
							'<td class="ns1blankspaceCaption ns1blankspaceCaptionMandatory">' +
							((ns1blankspace.objectContext === -1) ? 'Training Course' : '&nbsp;') +
							'</td></tr>' +
							'<tr class="ns1blankspace" id="ns1blankspaceDetailsRowTrainingCourseUpdate">' +
							'<td class="ns1blankspaceText">' +
							'<input id="ns1blankspaceDetailsTrainingCourseUpdate" class="ns1blankspaceSelect"' +
								' data-mandatory="1" data-caption="Training Course"' +
								' data-parent="ns1blankspaceDetails' + ((nsFreshcare.user.role.toLowerCase() === 'trainer') ? 'TrainingPackage' : 'Membership') + 'Update"' +
								' data-parent-search-id="agritrainingcourse.package' + ((nsFreshcare.user.role.toLowerCase() === 'trainer') ? '' : 'membership') + '"' +
								...
								' data-click="nsFreshcare.external.grower.setTrainingDate">' +
							'</td></tr>');		
				...
				aHTML.push('<tr class="ns1blankspace">' +
								...
								'<input id="ns1blankspaceDetailsTrainingDateUpdate" class="ns1blankspaceDate" data-mandatory="1" data-caption="Training Date">' +
								'</td></tr>');
			...
			aHTML.push('<tr class="ns1blankspaceCaption">' +
							...
							((nsFreshcare.user.role.toLowerCase() === 'grower') ? ' data-mandatory="1"' : '') + 
							'>' +
							'</td></tr>');
			...
			if (ns1blankspace.objectContext != -1 && nsFreshcare.user.role.toLowerCase() === 'auditor') 
			{
				aHTML.push('<tr class="ns1blankspaceCaption">' +
								'<td class="ns1blankspaceCaption">' +
								'&nbsp;' +
			...
			// v2.0.2 Now only shown on Details tab when adding new (only for trainer so far)
			// 3.0.1 Added condition for admin
			if ((ns1blankspace.objectContext === -1 && nsFreshcare.user.role.toLowerCase() === 'trainer')
				|| (nsFreshcare.user.role.toLowerCase() === 'admin' && ns1blankspace.objectContext != -1))
			{
				aHTML.push('<tr class="ns1blankspace ns1blankspaceHideOnNew">' +
								'<td class="ns1blankspaceCaption">' +
				...
				{
					aHTML2.push('<tr id="ns1blankspaceScopeRow_' + this.id + '">' + 
									'<td id="nsFreshcareScope_' + this.id + '" data-groupid="' + this.group + '"' + 
			...
			else if (ns1blankspace.objectContext === -1 && nsFreshcare.user.role.toLowerCase() === 'trainer') {
			...
			if (ns1blankspace.objectContextData)
			{
				...
				if (nsFreshcare.user.role.toLowerCase() === 'auditor' || nsFreshcare.user.role.toLowerCase() === 'admin')
				{
					$('#ns1blankspaceDetailsNotes').html(ns1blankspace.objectContextData["contactbusiness.notes"].formatXHTML().replace(/\r\n/g, '<br />'));
				}

				// 3.0.1 Populate Training Fields if admin user
				if (nsFreshcare.user.role.toLowerCase() === 'admin' && $('#ns1blankspaceDetailsTrainingCourseUpdate').is('*'))
				{
					...
					$('#ns1blankspaceDetailsTrainingCourseUpdate').val(ns1blankspace.objectContextData.coursetext.formatXHTML());
					$('#ns1blankspaceDetailsTrainingCourseUpdate').attr('data-id', ns1blankspace.objectContextData.course);
					$('#ns1blankspaceDetailsTrainingCourse').html(ns1blankspace.objectContextData.coursetext.formatXHTML());
					$('#ns1blankspaceDetailsTrainingCourse').attr('data-id', ns1blankspace.objectContextData.course);
					$('#ns1blankspaceDetailsTrainingDateUpdate').val(ns1blankspace.objectContextData.joindate.formatXHTML());
					$('#ns1blankspaceDetailsTrainingDate').html(ns1blankspace.objectContextData.joindate.formatXHTML());
				}
				...
				// List the Scopes when non-admin adding new record and for admin when on "New Growers fron Trainees" page
				// v3.1.0 for admin (new growers from trainers) use scopeValues, not personGroups
				// v3.1.2 SUP022744 Now use scopeVallues for all
				if ((nsFreshcare.user.role.toLowerCase() === 'admin' && ns1blankspace.objectContext != -1))
				{
					aValues = [];
				...
				// List the Categories (Product groups). Only for new growers when in admin logon
				if (ns1blankspace.objectContext != -1 && nsFreshcare.user.role.toLowerCase() === 'admin')
				{
					aValues = [];
			...
			else 
			{
				...
				if (nsFreshcare.user.role.toLowerCase() === 'trainer') 
				{
					...
					if (ns1blankspace.data.grower && ns1blankspace.data.grower.trainingCourseText && ns1blankspace.data.grower.trainingCourse) {
						$('#ns1blankspaceDetailsTrainingCourseUpdate').val(ns1blankspace.data.grower.trainingCourseText.formatXHTML());
						$('#ns1blankspaceDetailsTrainingCourseUpdate').attr('data-id', ns1blankspace.data.grower.trainingCourse);
					}
				}
			...
			if ((ns1blankspace.objectContext === -1 && nsFreshcare.user.role.toLowerCase() === 'trainer')
				|| (ns1blankspace.objectContext != -1 && nsFreshcare.user.role.toLowerCase() === 'admin'))
			{
				if (nsFreshcare.user.role.toLowerCase() === 'trainer'
			...
			// Bind the Scope area
			$('#ns1blankspaceMainDetails .nsFreshcareScope').each(function() 
			{
				// Make the values column the same width as the product group column above
				$('#' + this.id).width($('#ns1blankspaceProductGroup_1').width());
	...
	manageMultiRows: 	function(sXHTMLElementID, iMembership) 
	{
		...
		var sFieldName;
		var bNewTrainee = (ns1blankspace.objectContext === -1 && ns1blankspace.rootNameSpaceText === 'nsFreshcare' && nsFreshcare.user.role.toLowerCase() === 'trainer') ||
						  (ns1blankspace.objectContext === -1 && ns1blankspace.rootNameSpaceText === 'nsECA' && nsFreshcare.user.role.toLowerCase() === 'auditor');
		...
		if ($('#' + sXHTMLElementID).hasClass("nsFreshcareSelected") === false) 
		{
			...
			else 
			{
				...
				var bAddBR = ($('#ns1blankspace' + ((iMembership === '') ? 'Details' : 'Membership') + iMembership.substr(1) + sFieldName).html() != '');
				if ($('#ns1blankspace' + ((iMembership === '') ? 'Details' : 'Membership') + iMembership.substr(1) + sFieldName).html() === 'None')
				{
					$('#ns1blankspace' + ((iMembership === '') ? 'Details' : 'Membership') + iMembership.substr(1) + sFieldName).html('');
					$('#ns1blankspace' + ((iMembership === '') ? 'Details' : 'Membership') + iMembership.substr(1) + sFieldName).height(0);
					bAddBR = false;
				}

				// v2.0.3 Added font-size style for Membership Sites
				$('#ns1blankspace' + ((iMembership === '') ? 'Details' : 'Membership') + iMembership.substr(1) + sFieldName)
				.append('<span id="nsFreshcare' + sFieldName + 'List' + iMembership + '_' + sRowID + '" ' + 
						...
						'class="nsFreshcare' + sFieldName + 'List">' + ((bAddBR) ? '<br />' : '') + 
							$('#nsFreshcare' + sFieldName + iMembership + '_' + sRowID).html() + 
						'</span>');
			}
		}
		else 
	...
	contacts: 	
	{
		search: function (oParam, oResponse) 
		{
			...
			var iContactPerson = ns1blankspace.user.contactPerson;
			var iStep = 1;

			if (oParam) {
				iContactBusiness = (oParam.contactBusiness) ? oParam.contactBusiness : iContactBusiness;
				iContactPerson = (oParam.contactPerson) ? oParam.contactPerson : iContactPerson;
			...
			if (iStep === 1 && iContactBusiness && ns1blankspace.objectContextData) {
				...
				oSearch.getResults(function(data) {
					...
					nsFreshcare.external.grower.contacts.search(oParam, data);
				});
			}
			else if (iStep === 2) {
				if (oResponse.status === "OK") {
					...
					nsFreshcare.external.grower.contacts.search(oParam, oResponse);
				}
			}
			else if (iStep === 3) {
		...
		show: 	function (oParam, oResponse) 
		{
			...
			if (oParam)
			{
				if (oParam.contactBusiness == undefined) {oParam.contactBusiness = ns1blankspace.objectContextData["contactperson.contactbusiness"]}
			} 
			else {oParam = {contactBusiness: ns1blankspace.objectContextData["contactperson.contactbusiness"]};}
			...
			if ($('#ns1blankspaceMainContacts').attr('data-loading') == '1')
			{
				...
				else 
				{
					...
					aHTML.push('<tr><td class="ns1blankspaceHeaderCaption">Name</td>' + 
							        ...
							        ((nsFreshcare.user.role.toLowerCase() == 'trainer') ? '<td class="ns1blankspaceHeaderCaption">Trainer ID</td>' : '') +
							        '<td class="ns1blankspaceHeaderCaption">&nbsp;</td>' +
							    '</tr>');
					
					$.each(oResponse.data.rows, function() 
					{	//v3.1.205 Added inactive class for growers
						var sClass = (ns1blankspace.objectName == 'grower' && this.customerstatus == nsFreshcare.data.contactStatusInactive) 
										? " nsFreshcareInactive" : "";

						aHTML.push('<tr id="ns1blankspaceContactRow_' + this.id + '" class="ns1blankspaceContactRow">');
						aHTML.push('<td class="ns1blankspaceRow ns1blankspaceContactRowEdit' + sClass + '"' + 
						...
						// v3.1.205 Only used for FreshcareProfile, not growers form
						if (nsFreshcare.user.role.toLowerCase() === 'trainer' && ns1blankspace.objectName != 'grower')
						{
							aHTML.push('<td class="ns1blankspaceRow' + sClass + '"' + 
											' id="ns1blankspaceContactValue_reference_' + this.id + 
											'">' +
												this['contactperson.reference'] + 
											'</td>');
						}
					...
					//aHTML.push('<tr><td><span id="ns1blankspaceContactAdd" class="ns1blankspaceAction" colspan="2">Add Contact</span>');
					...
					$('.ns1blankspaceContactRowEdit').click(function(event) {
						nsFreshcare.util.elementEdit.start({xhtmlElementID: event.target.id,
															save: (nsFreshcare.user.role.toLowerCase() === 'admin'),
	...
	address: 	
	{
		show:	function (oParam, oResponse)
		{
			...
			if ($('#ns1blankspaceMainAddress').attr('data-loading') == '1')
			{
				...
				{
					nsFreshcare.admin.grower.siteAddress.search({contactBusiness: ns1blankspace.objectContext,
																 onComplete: nsFreshcare.external.grower.address.show});
				}
				else 
				{
		...
		manage: function(oParam)
		{
				...
				if ($('#ns1blankspaceAddressManageDetails').attr('data-id') === '-1')
				{
					if (!bAdd)
					{
						ns1blankspace.container.confirm(
						{
							...
							buttons: 
							[
								{text: 'OK', click: function() {bContinue = true; $(this).dialog('close')}},
								{text: 'Cancel', click: function() {bContinue = false; $(this).dialog('close')}}
							]
						});
					}
					else
			...
			if (bContinue)
			{
				...
				aHTML.push('<tr>' + 
							'<td id="ns1blankspaceAddressManageColumn1" class="ns1blankspaceColumn1" style="width:75%;"></td>' +
				...
				if (bEdit) 
				{
					aHTML.push('<span id="ns1blankspaceAddressSave" class="ns1blankspaceAction" colspan="2">Done</span><br /><br />');
				}
				aHTML.push('<span id="ns1blankspaceAddressCancel" class="ns1blankspaceAction" colspan="2">Cancel</span><br />');
				aHTML.push('</td></tr></table>');
		...
		search: function (oParam, oResponse) 
		{
			...
			if (iStep === 1 && iContactBusiness && ns1blankspace.objectContextData) 
			{
				...
				oSearch.addField('agrisubscription.membership,agrisubscription.membershiptext,agrisubscription.status,agrisubscription.statustext,agrisubscription.firstcertified' + 
								...
								',agrisubscription.harvestmonth,agrisubscription.crop,agrisubscription.agricertificate.sent,agrisubscription.laststatuschangedate' +
								...
								',agrisubscription.agricertificate.audit.auditbusiness.se' + nsFreshcare.data.jasanzDateId + ',agrisubscription.agricertificate.audit.actualdate' +
								',agrisubscription.agrilastaudit.actualdate,agrisubscription.agrilastaudit.type,agrisubscription.agrilastaudit.typetext' +
		...
		search: function (oParam, oResponse) 
		{
			...
			if (iStep === 1 && iContactBusiness && ns1blankspace.objectContextData) {
				...
				oSearch.addField('audit.reference,audit.title,audit.scheduleddate,audit.actualdate,audit.resultstatus,audit.resultstatustext' + 
								...
								',audit.auditbusiness,audit.auditperson,audit.paid');
		...
		show: 	function(oParam) 
		{
			...
			else
			{
				if ($('#ns1blankspaceMainMembership' + iMembership).attr('data-loading') == '1')
				{
					...
					// v3.1.2 SUP022693 Only show Re-Cert Audit Due if CB is JASANZ
					// Only Gorwers & Trainers not allowed to update
					if (dJASANZ <= dToday)
					{
						aHTML.push('<tr class="ns1blankspace">' +
						...
						if (nsFreshcare.user.role.toLowerCase() == 'trainer' || nsFreshcare.user.role.toLowerCase() == 'grower')
						{
					...
					{
						$.each(nsFreshcare.data.scopeOptions, function() {
							aHTML.push('<tr id="ns1blankspaceScopeRow-' + iMembership + '_' + this.id + '">' + 
											'<td id="nsFreshcareScope-' + iMembership + '_' + this.id + '"' + 
											' class="nsFreshcareScope nsFreshcareSelectable">' + this.title + '</td>' + 
											'</tr>');
						});
					}
						...
						if (this.scopetext != "") 
						{
							aValues.push('<span id="nsFreshcareScopeList-' + iMembership + '_' + this.scope + '" ' + 
								...
								'data-rowID="' + this.id + '">' + 
									this.scopetext + 
								'</span>');
							$('#nsFreshcareScope-' + iMembership + '_' + this.scope).addClass("nsFreshcareSelected");
						}
					});
						...
						if (this.productcategorytext != "") {
							aValues.push('<span id="nsFreshcareProductGroupList-' + iMembership + '_' + this.productcategory + '" ' + 
								...
								'data-rowID="' + this.id + '">' + 
									this.productcategorytext + 
								'</span>');
							$('#nsFreshcareProductGroup-' + iMembership + '_' + this.productcategory).addClass("nsFreshcareSelected");
						}
					});
						...
						if (sAddress != "") {
							aValues.push('<span id="nsFreshcareMembershipSiteList-' + iMembership + '_' + this.id + '" ' + 
								...
								'data-rowID="' + this['address.addresslink.id'] + '">' + 
									sAddress + 
								'</span>');
							$('#nsFreshcareMembershipSite-' + iMembership + '_' + this.id).addClass("nsFreshcareSelected");
						}
					});
					...
					// Bind the Scope area
					$('#ns1blankspaceMembership' + iMembership + 'RowScope .nsFreshcareScope').click(function() {
					...
					oParam.auditBusiness = (nsFreshcare.user.role.toLowerCase() != 'admin') ? ns1blankspace.user.contactBusiness : undefined;
	...
	save: 		
	{
		constructEmail: function(oParam) 
		{
			...
			if (aMessageHTML.length == 0)
			{
				if ($('#ns1blankspaceMainDetails').html() != '') 
				{
					...
					/*if ($('input[name="radioPreferredCommunication"]:checked').val() != $('#ns1blankspaceDetailsPreferredCommunication').attr('data-id')) {
						aMessageHTML.push(nsFreshcare.external.grower.save.addToEmail({label: 'Preferred Communication Method',
										oldValue: ((ns1blankspace.objectContext != -1) ? $('#ns1blankspaceDetailsPreferredCommunication').html() : ''),
				...
				$.each(ns1blankspace.objectContextData.memberships, function(index)
				{
					...
					if (1 == 0 && sInvalidCrops != '')
					{
						...
						{
							aMessageToSourceHTML.push('<p>Grower ' + ns1blankspace.data.contactBusinessText.formatXHTML() + ' has been updated recently but still contains errors.</p>');
						}
			...
			else if (aMessageHTML.length > 0) 
			{
				...
				else
				{
					...
					oEmailParam.subject 			= (sSource.toLowerCase() == 'auditor' ? 'CB' : sSource) + ' Update : [Fields Updated] ' +  sChangedFieldsList ;						
		...
		sendEmailOnDemand: 	function(oParam) 
		{	// 3.1.0b reverted to ondemand version as cannot send from email without account
			...
			sMessageData = 'subject=' + ns1blankspace.util.fs(sSubject) +
						  ...
						  '&fromemail=' + ns1blankspace.util.fs(sFrom) // +		// v3.1.0e SUP022238 removed as was causing the word 'from' to be replaced wit NaN
						 // '&from=' +  + ns1blankspace.util.fs(sFrom);
			...
			{
				if (iContactBusiness && iContactBusiness != -1)
				{
					sMessageData += '&contactbusiness=' + ns1blankspace.objectContext;
				}
				...
				if (ns1blankspace.objectContext === -1) 
				{
					...
					sMessageData += '&object=' + ns1blankspace.util.fs(ns1blankspace.data.object.business) +
									'&objectcontext=' + ns1blankspace.util.fs(ns1blankspace.user.contactBusiness);
				} 
				else 
				{
					// Store against current record
					sMessageData += '&object=' + ns1blankspace.util.fs(ns1blankspace.object) +
									'&objectcontext=' + ns1blankspace.util.fs(ns1blankspace.objectContext);
				}
			}
			...
			$.ajax({
				...
				success: function(oResponse) {
					...
					if (oResponse.status === 'OK') {
						...
						if (oParam.wait != undefined && oParam.wait) 
						{
		...
		{	// 3.1.0i Now uses rpc version as siteFrom has been set on admin account
			...
			if (sMessageOverride === '') 
			{
				var sUpdatedBy = nsFreshcare.user.role + ' ' + ns1blankspace.user.contactBusinessText + 
			...
			if (bSaveEmail)		// v3.1.0 SUP022053 save if we're asked to
			{
				...
				if (iContactBusiness && iContactBusiness != -1)
				{
					sMessageData += '&saveagainstcontactbusiness=' + ns1blankspace.objectContext;
				}
				...
				if (ns1blankspace.objectContext === -1) 
				{
					...
					sMessageData += '&saveagainstobject=' + ns1blankspace.util.fs(ns1blankspace.data.object.business) +
									'&saveagainstobjectcontext=' + ns1blankspace.util.fs(ns1blankspace.user.contactBusiness);
				} 
				else 
				{
					// Store against current record
