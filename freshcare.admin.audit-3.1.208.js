...
nsFreshcare.admin.audit = 
{
	data: 
	{
		...
		[
			...
			{name: 'audit.type', caption: 'Title', display: 'audit.titletext'},
			{name: 'audit.contactbusiness', caption: 'Grower Business', display: 'audit.contactbusinesstext'},
	...
	init: 		function (oParam) 
	{
		...
		if (nsFreshcare.user.role.toLowerCase() != 'admin' && bNew && oParam.contactBusiness === undefined) 
		{
			nsFreshcare.auditor.grower.init({"new": true});
			ns1blankspace.status.message('Inform Freshcare of unregistered Grower.');
		}
			...
			if (oResponse.status === 'OK')
			{
				if (oResponse.data.rows.length == 0)
				{
					aHTML.push('<table id="ns1blankspaceMostLikely">' +
									...
									(nsFreshcare.user.role.toLowerCase() != 'reviewer' 
										? 'To add a new Audit, search for the Grower and click Register Audit.'
										: 'No Audits found.') +
									'</td></tr>' +
									'</table>');
				}
				...
				{
					var sCertBodyColumn = (nsFreshcare.user.role.toLowerCase() != 'admin') ? 'audit.auditpersontext' : 'audit.auditbusinesstext';
					...
					aHTML.push('<tr>' + 
								...
								'<td class="ns1blankspaceCaption ns1blankspaceHeaderSort"' +
									...
									'>' + ((nsFreshcare.user.role.toLowerCase() != 'admin') ? 'Auditor' : 'Cert Body') + '</td>' +
								'</tr>');
	...
	search: 	
	{
		send: 	function (sXHTMLElementId, oParam)
		{
			...
			if (sSearchContext != undefined && iSource != ns1blankspace.data.searchSource.browse)
			{
				...
				{
					if (nsFreshcare.user.role.toLowerCase() != 'auditor') {delete(nsFreshcare.data.auditor);}
			...
			else
			{
				...
				if (sSearchText.length >= iMinimumLength || iSource == ns1blankspace.data.searchSource.browse)
				{
					...
					if ((sRole.toLowerCase() != 'admin' && sRole.toLowerCase() != 'jasanz')
						|| (nsFreshcare.user.role.toLowerCase() == 'jasanz' && nsFreshcare.data.viewFilter.certificationBody)) 
					{
		...
		process: 	function (oParam, oResponse)
		{
			...
			{	
				...
				{
					columns: 'audit.contactbusinesstext,audit.resultstatustext,audit.actualdate',
	...
	layout: 	function ()
	{
		...
		{
			oContext['new'] = true;
			oContext.action = (nsFreshcare.user.role.toLowerCase() != 'auditor');
		...
		{
			if (nsFreshcare.user.role.toLowerCase() === 'auditor' && ns1blankspace.objectContextData)
			{
		...
		{
			if (nsFreshcare.user.role.toLowerCase() === 'reviewer') {oParam.canEdit = false}
			if (nsFreshcare.user.role.toLowerCase() === 'reviewer') {oParam.canRemove = false}
			if (nsFreshcare.user.role.toLowerCase() === 'reviewer') {oParam.canAdd = false}
		...
		$('#ns1blankspaceControlAttachments').click(function(event)
		{
			...
			var sRole = nsFreshcare.user.role.toLowerCase();
			var aAttachmentTypes = (sRole == 'admin' || sRole == 'auditor' || sRole == 'reviewer' || sRole == 'jasanz' || sRole == 'internal auditor') 
			...
			nsFreshcare.admin.grower.attachments.search(
			   	{
					...
					maxFiles: 5, 
					attachmentType: (aAttachmentTypes != undefined && sRole != 'admin') ? $.map(aAttachmentTypes, function(x) {return x.type}).join(',') : undefined,
					...
					helpNotes: "Documents of type 'Audit Report' are viewable by Growers. Please upload PDF documents only to prevent modification of audit reports"
			   	});
		});
		...
		$('#ns1blankspaceControlNotes').click(function(event)
		{
			...
			var sActionTypes = (nsFreshcare.user.role.toLowerCase() == 'grower' || nsFreshcare.user.role.toLowerCase() == 'trainer' || nsFreshcare.user.role.toLowerCase() == 'customer') 
	...
	show: 		function (oParam, oResponse)
	{
		...
		if (oParam.showStep == undefined)
		{
			...
			else
			{
				...
				oParam.showStep = (nsFreshcare.user.role.toLowerCase() == 'jasanz') ? 1 : 10;
			...
			ns1blankspace.history.control(
				{
					...
					xhtmlElementID: (nsFreshcare.user.role.toLowerCase() == 'reviewer' ? 'ns1blankspaceControlSummary' : undefined)
				});
		}
	},	
		...
		else
		{
			...
			else
			{
					...
					&& ns1blankspace.objectContextData['audit.teamleadercontactperson'] === '')
				{
					...
					&& ns1blankspace.objectContextData['audit.teamleadercontactperson'] != '' 
					&& (nsFreshcare.user.role.toLowerCase() === 'admin' || nsFreshcare.user.role.toLowerCase() === 'auditor') )
				{
	...
	summaryBind: function()
	{
		...
		$('#ns1blankspaceAttachDocuments')
			...
			.on('click', function()
			{
				...
				var aAttachmentTypes = (nsFreshcare.user.role.toLowerCase() != 'reviewer')
									 ? nsFreshcare.data.audit.auditorAttachments.concat(nsFreshcare.data.audit.reviewerRequiredAttachmentTypes)
									 : nsFreshcare.data.audit.reviewerRequiredAttachmentTypes;
				...
				$('#ns1blankspaceMainAttachments')
					.html('<table><tr>' +
							...
							'<td id="ns1blankspaceAttachmentsColumn2" class="ns1blankspaceColumn2" style="width:100px; font-size:0.75em;">' +
								"Documents of type 'Audit Report' are viewable by Growers. Please upload PDF documents only to prevent modification of audit reports" +
							'</td></tr>' +
							'</table>');
				...
				ns1blankspace.attachments.add(
				{
					...
					actions: {add: true},
					maxFiles: 5, 
					canRemoveAll: ((nsFreshcare.user.role.toLowerCase() === 'reviewer') ? false : true),
					canSetTypeAll: ((nsFreshcare.user.role.toLowerCase() === 'reviewer') ? false : true),
					attachmentTypes: aAttachmentTypes,
		...
		$('#ns1blankspaceAssignReviewer')
			...
			{
				nsFreshcare.admin.audit.assignReviewer();
			});
		...
		$('#ns1blankspaceApprove')
			...
			.on('click', function()
			{
				// First validate (by checking if Tech review Checklist has been attached) and then save
				nsFreshcare.admin.audit.save.send({resultStatus: nsFreshcare.data.audit.resultStatusApproved, onComplete: nsFreshcare.admin.audit.save.send});
			});
		...
		$('#ns1blankspaceReject')
			...
			.on('click', function()
			{
				var oRoot = ns1blankspace.rootnamespace;
				// First validate (by checking if Tech review Checklist has been attached) and then save
				nsFreshcare.admin.audit.save.send({buttonElement: this, resultStatus: nsFreshcare.data.audit.resultStatusRejected, onComplete: oRoot.admin.audit.save.send});
				//nsFreshcare.admin.audit.addActionReasonNote({buttonElement: this, resultStatus: nsFreshcare.data.audit.resultStatusRejected, onComplete: oRoot.admin.audit.save.send});
			});
		...
		$('#ns1blankspaceConditional')
			...
			.on('click', function()
			{
				var oRoot = ns1blankspace.rootnamespace;
				// First validate (by checking if Tech review Checklist has been attached) and then save
				nsFreshcare.admin.audit.save.send({buttonElement: this, resultStatus:  oRoot.data.audit.resultStatusConditionalApproval, onComplete: oRoot.admin.audit.save.send});
				//nsFreshcare.admin.audit.addActionReasonNote({buttonElement: this, resultStatus: oRoot.data.audit.resultStatusConditionalApproval, onComplete: oRoot.admin.audit.save.send});
			});
	},
		...
		if ($('#ns1blankspaceMainDetails').attr('data-loading') == '1')
		{
			...
			aHTML.push('<tr class="ns1blankspaceCaption">' +
							...
							'<input id="ns1blankspaceDetailsTitle" class="ns1blankspaceSelect"' +
								...
								(((dJASANZ > dToday || (dAudit && dAudit < dJASANZ)) && nsFreshcare.user.role.toLowerCase() == 'auditor') 
			...
			$('#ns1blankspaceDetailsActualDate').change(function() 
			{
				if ($('#ns1blankspaceDetailsActualDate').val() != '' && isValidDate($('#ns1blankspaceDetailsActualDate').val(), 'dd mmm yyyy')) 
				{
						...
						&& $('#ns1blankspaceDetailsResultStatus').attr('data-methodFilter') == undefined)
					{
						...
						if (nsFreshcare.user.role.toLowerCase() == 'auditor') {$('#ns1blankspaceDetailsResultStatusDate').attr('disabled', false);}
					}
					else if (dJASANZ <= dToday && dAudit >= dJASANZ)
					{
						...
						if (nsFreshcare.user.role.toLowerCase() == 'auditor') {$('#ns1blankspaceDetailsResultStatusDate').attr('disabled', true);}
					}
				}
			});
			...
			else 
			{
				...
				if (dJASANZ <= dToday || nsFreshcare.user.role.toLowerCase() != 'auditor')		// JASANZ Accredited - always set to Conducted if admin user as don't have JASANZ date
				{
	...
	setMembershipId: function(oResponse, oParam) 
	{
		...
		if (oResponse === undefined) {
			...
			if ($('#ns1blankspaceDetailsMembership').attr('data-id') != undefined) {
				...
				oSearch.getResults(nsFreshcare.admin.audit.setMembershipId);
			}
		}
		else 
		{
			if (oParam === undefined) 
			{
				if (oResponse.status === 'OK') 
				{
					if (oResponse.data.rows.length > 0) 
					{
						...
						oSearch.getResults(function(oResponse) {nsFreshcare.admin.audit.setMembershipId(oResponse, {step: 2})});
					}
				}
			}
			else 
				...
				else 
				{
					...
					$('.ns1blankspaceRowAuditCAR').click(function(event) {
						...
						nsFreshcare.admin.audit.cars.manage({add: false, 
															edit: (ns1blankspace.objectContextData['audit.membershipstatus'] != nsFreshcare.data.grower.subscriptionStatusIN 
																	&& ns1blankspace.objectContextData['audit.membershipstatus'] != nsFreshcare.data.grower.subscriptionStatusCE
																	&& bCanEdit), 
															rowID: sElementId});
	...
	evaluation: function()
	{
		var aHTML = [];
		var dToday = new Date();
		var bMandatory = (nsFreshcare.user.role.toLowerCase() != 'admin' && ns1blankspace.objectContext === -1);
		var bDisabled = (nsFreshcare.user.role.toLowerCase() != 'admin' && ns1blankspace.objectContext != -1);
	...
	save: 		
	{
		validate: 	function(oParam) 
		{
			...
			{
				if (nsFreshcare.user.role.toLowerCase() != 'auditor' && $('ns1blankspaceDetailsCertBody').val() != '')
				{
			...
			else if (oParam.auditValidateStep === 1)
			{
				...
				if (ns1blankspace.okToSave)
				{
					...
					if (dJASANZ <= dToday && dActual >= dJASANZ)
					{
						...
						else if (ns1blankspace.objectContext != -1 && iResultStatus == nsFreshcare.data.audit.resultStatusNoResult && nsFreshcare.user.role.toLowerCase() != 'admin')
						{
							ns1blankspace.okToSave = false;
				...
				if (ns1blankspace.okToSave && ns1blankspace.objectContext != -1 && nsFreshcare.user.role.toLowerCase() != 'admin'
					...
					&& ns1blankspace.data.audit.subscriptionStatus === nsFreshcare.data.grower.subscriptionStatusWD) 
				{
					if (nsFreshcare.user.role.toLowerCase() === 'admin') 
					{
						ns1blankspace.status.message('WARNING! Grower is Withdrawn.');
					}
					else
					{
						ns1blankspace.okToSave = false;
			...
			else if (oParam.auditValidateStep === 2)
			{
				...
				oSearch.getResults(function(oResponse)
				{
					if (oResponse.status === 'OK')
					{
						...
						{
							ns1blankspace.okToSave = false;
							ns1blankspace.status.message('Grower has not completed appropriate training for this Membership' + 
		...
		process: function(oParam) 
		{
			...
			if (nsFreshcare.user.role.toLowerCase() != 'admin' && bNew 
