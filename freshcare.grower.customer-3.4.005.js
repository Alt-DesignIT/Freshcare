...
nsFreshcare.grower.customer = 
{
	...
	home: 		function (oParam, oResponse)
	{
		...
		if (oResponse == undefined)
		{
			...
			oSearch.addBracket(")");
			// v3.3.001 SUP023508 Added filter so that it won't show disabled customers
			// v3.4.005 Was filtering out if grower was disabled, not customer
			oSearch.addFilter("contactbusiness.relationshipotherbusiness.contactperson.user.disabled", 'NOT_EQUAL_TO', 'N');
		...
		send: 		function (sXHTMLElementId, oParam)
		{
			...
			else
			{
				...
				if (sSearchText.length >= iMinimumLength || iSource == ns1blankspace.data.searchSource.browse)
				{
					...
					oSearch.addFilter("contactbusiness.relationshipotherbusiness.enddate", 'IS_NULL');
					oSearch.addBracket(")");
					// v3.3.001 SUP023508 Added filter so that it won't show inactive customers
					oSearch.addFilter("contactbusiness.contactperson.user.disabled", 'NOT_EQUAL_TO', 'N');
