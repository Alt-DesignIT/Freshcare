...
// v3.2.015 SUP023421 COnvert 'Grower' to 'Member'
...
nsFreshcare.admin.certificationbody = 
{
		...
		search:
		{
			detail:
			{
				fields: 'contactbusiness.tradename,contactbusiness.legalname,contactbusiness.abn,contactbusiness.reference,contactbusiness.webaddress,contactbusiness.notes' + 
						...
						',contactbusiness.se' + nsFreshcare.data.certificationBodyNumberId + ',contactbusiness.se' + nsFreshcare.data.selfCertificationDateId
			}
		}
	},
	...
	home: 		function (oParam, oResponse)
	{
		...
		else
		{
			...
			{
				// v4.0.001 Bootstrap
				aHTML.push('<div class="ns1blankspaceCaption" style="padding-left:8px; margin-bottom:10px; font-weight:500;">MOST RECENTLY UPDATED</div>');
	...
	layout: 	function ()
	{
		...
		else
		{
			...
			if (nsFreshcare.option.qualifyingTraining && nsFreshcare.user.roleID != nsFreshcare.data.roles.jasanz)		// SUP022859
			{	
				...
				{
					aHTML.push('<tr><td id="ns1blankspaceControlJASANZ" class="ns1blankspaceControl">' +
									'JASANZ</td></tr>');
					aHTML.push('<tr><td id="ns1blankspaceControlIOAS" class="ns1blankspaceControl">' +
									'IOAS</td></tr>');
				}
			}
		...
		aHTML.push('<div id="ns1blankspaceMainJASANZ" class="ns1blankspaceControlMain"></div>');
		aHTML.push('<div id="ns1blankspaceMainIOAS" class="ns1blankspaceControlMain"></div>');
		...
		{
			// v3.3.001 SUP023603 Now shows Auditor Memberships
		...
		$('#ns1blankspaceControlJASANZ').click(function(event)
		{
			ns1blankspace.show({selector: '#ns1blankspaceMainJASANZ', context: oContext});
			nsFreshcare.admin.certificationbody.jasanz.show({xhtmlElementID: 'ns1blankspaceMainJASANZ'});
		});

		$('#ns1blankspaceControlIOAS').click(function(event)
		{
			ns1blankspace.show({selector: '#ns1blankspaceMainIOAS', context: oContext});
			nsFreshcare.admin.certificationbody.ioas.show({xhtmlElementID: 'ns1blankspaceMainIOAS'});
		});
	...
	jasanz:
	{
		...
		{
			var iCertBody = ns1blankspace.util.getParam(oParam, 'business', {'default': ns1blankspace.objectContext}).value;

			// v4.0.019 SUP024339 Now only searches for jasanz accredited memberships
			var oSearch = new AdvancedSearch();
			oSearch.method = 'AGRI_MEMBERSHIP_TRAINER_SEARCH';
			oSearch.addField('membership,membershiptext,trainercontactbusiness,trainercontactbusinesstext,notes' +
							',agrimembershiptrainer.membership.jasanzaccredited,seaccreditation');
			oSearch.addFilter('trainercontactbusiness', 'EQUAL_TO', iCertBody);
			oSearch.addFilter('trainercontactperson', 'EQUAL_TO', nsFreshcare.data.dummyCertBodyPerson);
			oSearch.addFilter('seaccreditation', 'EQUAL_TO', 'jasanz');
			oSearch.addFilter('agrimembershiptrainer.membership.jasanzaccredited', 'EQUAL_TO', 'Y');
			oSearch.getResults(function(oResponse)
			{
				if (oResponse.status == 'OK')
				{
					ns1blankspace.objectContextData.jasanzDates = oResponse.data.rows;
				...
				{
					ns1blankspace.status.error('Error finding JASANZ dates: ' + oResponse.error.errornotes);
				}
			});
		},
		...
		show: function(oParam)
		{
			oParam = oParam || {};
			var aHTML = [];
			var bCanEdit = (nsFreshcare.user.roleID == nsFreshcare.data.roles.admin);

			if (ns1blankspace.objectContextData.jasanzDates == undefined)
			{
				oParam.onComplete = nsFreshcare.admin.certificationbody.jasanz.show;
				nsFreshcare.admin.certificationbody.jasanz.search(oParam);
			}

			else
			{

				$('#ns1blankspaceMainJASANZ').attr('data-loading', '');
				
				aHTML.push('<table class="ns1blankspaceContainer">' +
								'<tr class="ns1blankspaceContainer">' +
								'<td id="ns1blankspaceJASANZColumn1" class="ns1blankspaceColumn1"></td>' +
								'</tr></table>');					
				
				$('#ns1blankspaceMainJASANZ').html(aHTML.join(''));
				...
				aHTML.push('<tr class="ns1blankspaceCaption">' +
								...
								'JASANZ Accreditation Dates' +
								'</td><td>&nbsp;</td></tr>'); 

				$.each(nsFreshcare.data.memberships, function(i, membership) 
				{
					if (membership.jasanzaccredited == 'Y')
					{
						aHTML.push('<tr class="ns1blankspace">' +
										'<td class="ns1blankspaceReadOnly">' +  
										membership.title.formatXHTML() + '</td>' +
									'<td class="ns1blankspaceText">' +
										'<input id="ns1blankspaceJASANZMembership-' + membership.id + '"' +
											' class="ns1blankspaceDate"></td>' +
										' </tr>');
					}
				});
				aHTML.push('</table>');

				$('#ns1blankspaceJASANZColumn1').html(aHTML.join(''));

				// v4.0.016 SUP024214 changed to initDatePicker
				ns1blankspace.util.initDatePicker({select: 'input.ns1blankspaceDate'});

				$.each(ns1blankspace.objectContextData.jasanzDates, function()
				{
					$('#ns1blankspaceJASANZMembership-' + this.membership)
						.val(this.notes)
				...
				if (!bCanEdit)
				{
					$('#ns1blankspaceMainJASANZ .ns1blankspaceDate')
	...
	ioas:
	{
		// v4.0.019 SUP024339 New - show IOAS acreditation
		search: function(oParam)
		{
			var iCertBody = ns1blankspace.util.getParam(oParam, 'business', {'default': ns1blankspace.objectContext}).value;
			...
			oSearch.method = 'AGRI_MEMBERSHIP_TRAINER_SEARCH';
			oSearch.addField('membership,membershiptext,trainercontactbusiness,trainercontactbusinesstext,notes' +
								',agrimembershiptrainer.membership.seioasaccredited,seaccreditation');
			oSearch.addFilter('trainercontactbusiness', 'EQUAL_TO', iCertBody);
			oSearch.addFilter('trainercontactperson', 'EQUAL_TO', nsFreshcare.data.dummyCertBodyPerson);
			oSearch.addFilter('seaccreditation', 'EQUAL_TO', 'ioas');
			oSearch.addFilter('agrimembershiptrainer.membership.seioasaccredited', 'EQUAL_TO', 'Y');
			oSearch.getResults(function(oResponse)
			{
				if (oResponse.status == 'OK')
				{
					ns1blankspace.objectContextData.ioasDates = oResponse.data.rows;
					if (oParam.onComplete)
					{
						ns1blankspace.util.onComplete(oParam);
					}
				...
				{
					ns1blankspace.status.error('Error finding IOAS dates: ' + oResponse.error.errornotes);
				}
			});
		},
		...
		show: function(oParam)
		{
			...
			if (ns1blankspace.objectContextData.ioasDates == undefined)
			{
				oParam.onComplete = nsFreshcare.admin.certificationbody.ioas.show;
				nsFreshcare.admin.certificationbody.ioas.search(oParam);
			}

			else
			{

				$('#ns1blankspaceMainIOAS').attr('data-loading', '');
				
				aHTML.push('<table class="ns1blankspaceContainer">' +
								'<tr class="ns1blankspaceContainer">' +
								'<td id="ns1blankspaceIOASColumn1" class="ns1blankspaceColumn1"></td>' +
								'</tr></table>');					
				
				$('#ns1blankspaceMainIOAS').html(aHTML.join(''));
				...
				aHTML.push('<tr class="ns1blankspaceCaption">' +
								'<td class="ns1blankspaceCaption">' +
								'IOAS Accreditation Dates' +
								'</td><td>&nbsp;</td></tr>'); 

				$.each(nsFreshcare.data.memberships, function(i, membership) 
				{
					if (membership.seioasaccredited == 'Y')
					{
						aHTML.push('<tr class="ns1blankspace">' +
										'<td class="ns1blankspaceReadOnly">' +  
										membership.title.formatXHTML() + '</td>' +
									'<td class="ns1blankspaceText">' +
										'<input id="ns1blankspaceIOASMembership-' + membership.id + '"' +
											' class="ns1blankspaceDate"></td>' +
										' </tr>');
					}
				});
				...
				$('#ns1blankspaceIOASColumn1').html(aHTML.join(''));

				ns1blankspace.util.initDatePicker({select: 'input.ns1blankspaceDate'});

				$.each(ns1blankspace.objectContextData.ioasDates, function()
				{
					$('#ns1blankspaceIOASMembership-' + this.membership)
						.val(this.notes)
						.attr('data-id', this.id);
				...
				if (!bCanEdit)
				{
					$('#ns1blankspaceMainIOAS .ns1blankspaceDate')
						.addClass('nsFreshcareDisabled')
						.attr('disabled', true);
				}
			}
	...
	save:
	{
		validate: function(param, response)
		{
			// v4.0.019 Can't have both IOAS and JASANZ - must be one or the other
			// Only check if at least one of JASANZ or IOAS tabs have been clicked on
			param.validateStep = param.validateStep || 1;
			var getValuesFromElements = function(elements)
			{
				var filledElements = $.grep(elements, function(x) {return $(x).val() != ''});
				var values = $.map(filledElements, function(x) 
				{
					var o = 
					{
						id: $(x).attr('data-id'),
						membership: $(x).attr('id').split('-').pop(),
						notes: $(x).val()
					};
					return o;
				});
			...
			// Determine if need to get data for either IOAS or JASANZ tab
			// v4.0.019a SUP024339 Was not calling appropriate code if accreditation dates were missing
			if (param.validateStep == 1)
			{
				if ($('#ns1blankspaceMainJASANZ .ns1blankspaceDate').length || $('#ns1blankspaceMainIOAS .ns1blankspaceDate').length > 0)
				{
					if ($('#ns1blankspaceMainJASANZ .ns1blankspaceDate').length == 0)
					{
						param.ioasValues = getValuesFromElements($('#ns1blankspaceMainIOAS .ns1blankspaceDate'));
						param.validateStep = 2;
						param.setValues = 'jasanz';
						param.onComplete = nsFreshcare.admin.certificationbody.save.validate;
						nsFreshcare.admin.certificationbody.jasanz.search(param);
					}
					else if ($('#ns1blankspaceMainIOAS .ns1blankspaceDate').length == 0)
					{
						param.jasanzValues = getValuesFromElements($('#ns1blankspaceMainJASANZ .ns1blankspaceDate'));
						param.validateStep = 2;
						param.setValues = 'ioas';
						param.onComplete = nsFreshcare.admin.certificationbody.save.validate;
						nsFreshcare.admin.certificationbody.ioas.search(param);
					}
					else
					{
						param.ioasValues = getValuesFromElements($('#ns1blankspaceMainIOAS .ns1blankspaceDate'));
						param.jasanzValues = getValuesFromElements($('#ns1blankspaceMainJASANZ .ns1blankspaceDate'));
						param.validateStep = 3;
						nsFreshcare.admin.certificationbody.save.validate(param);
					}
				}
				else
				{
					param.validateStep = 10;
					nsFreshcare.admin.certificationbody.save.validate(param);
				}
			}

			else if (param.validateStep == 2)
			{
				// v4.0.019b SUP024339 Was looking for response instead of objectContextData
				if (ns1blankspace.objectContextData[param.setValues + 'Dates'])
				{
					param[param.setValues + 'Values'] = ns1blankspace.objectContextData[param.setValues + 'Dates'];
					param.validateStep = 3;
					nsFreshcare.admin.certificationbody.save.validate(param);
				}
				else
				{
					ns1blankspace.status.error('Unable to validate accreditations. Please contact support.');
					ns1blankspace.okToSave = false;
				}
			}

			else if (param.validateStep == 3)
			{
				if (param.ioasValues.length > 0 && param.jasanzValues.length > 0)
				{
					ns1blankspace.okToSave = false;
					ns1blankspace.status.error('Certification Bodies cannot have both JASANZ and IOAS accreditation. Please remove dates as applicable.');
				}
				else
				{
					param.validateStep = 10;
			...
			else if (param.validateStep == 10)
			{
				delete(param.validateStep);
				if (ns1blankspace.okToSave)
				{
					nsFreshcare.admin.certificationbody.save.send(param);
				}
			}
		...
		send: function(param) 
		{
			...
			else if (param.saveStep == 2)
			{
				nsFreshcare.internal.entity.save.send(
				{
					businessGroup: nsFreshcare.data.businessGroupAuditor,
					functionPostSave: nsFreshcare.admin.certificationbody.save.jasanzDates
				});
			}
		},

		jasanzDates: function(oParam)
		{
			// v3.3.005 SUP023801 Now saves JASANZ Dates tab
			oParam = oParam || {};
			oParam.jasanzIndex = oParam.jasanzIndex || 0;
			var aJASANZRows = $('#ns1blankspaceMainJASANZ .ns1blankspaceDate');
			var oData = {};

			if (oParam.jasanzIndex < aJASANZRows.length)
			{
				var oElement = aJASANZRows[oParam.jasanzIndex];

				if ($(oElement).attr('data-id') == undefined && $(oElement).val() == '') 
				{
					oParam.jasanzIndex += 1;
					nsFreshcare.admin.certificationbody.save.jasanzDates(oParam);
				}
				else
				{
					oData.id = $(oElement).attr('data-id');
					if (oData.id && $(oElement).val() == '')
					{
						oData.remove = '1'
					}
					else
					{
						oData.notes = $(oElement).val();
						oData.seaccreditation = 'jasanz';
						oData.contactbusiness = ns1blankspace.objectContext;
						oData.contactperson = nsFreshcare.data.dummyCertBodyPerson;	// Use this until can remove mandatory for contactperson
						oData.membership = $(oElement).attr('id').split('-').pop();
					}
					...
					$.ajax(
					{
						...
						success: function(oResponse)
						{
							...
							{
								ns1blankspace.status.error('Unable to save JASANZ Dates: ' + oResponse.error.errornotes);
							}
						}
					});
				}
			...
			{
				// All done, refresh Cert Body / save IOAS dates
				nsFreshcare.admin.certificationbody.save.ioasDates(oParam);
				//nsFreshcare.admin.certificationbody.search.send('-' + ns1blankspace.objectContext);
			}
		},

		ioasDates: function(oParam)
		{
			// v4.0.019 SUP024339 Also saves IOAS Dates tab
			oParam = oParam || {};
			oParam.ioasIndex = oParam.ioasIndex || 0;
			var aIOASRows = $('#ns1blankspaceMainIOAS .ns1blankspaceDate');
			var oData = {};

			if (oParam.ioasIndex < aIOASRows.length)
			{
				var oElement = aIOASRows[oParam.ioasIndex];
				...
				else
				{
					...
					$.ajax(
					{
						...
						success: function(oResponse)
						{
							...
							{
								ns1blankspace.status.error('Unable to save IOAS Dates: ' + oResponse.error.errornotes);
							}
						}
					});
				}
