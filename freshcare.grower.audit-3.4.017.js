...
nsFreshcare.grower.audit = 
{
	...
	{
		// v3.3.001 SUP023456 Now lists Critical CARs
		oParam = oParam || {};
		...
		else
		{
			...
			else
			{
				...
				aHTML.push('<tr>' + 
								...
								'<td class="ns1blankspaceCaption">CARs (Critical)</td>' +
				...
				$.each(oResponse.data.rows, function()
				{
					...
					aCARs.push({audit: this.id, allCARs: 0, majorCARs: 0, criticalCARs: 0});
					...
					aHTML.push('<td id="ns1blankspaceMostLikely_date-' + this.id + '" class="ns1blankspaceMostLikely">' +
								...
								'<td id="ns1blankspaceMostLikely_CriticalCARs-' + this.id + '" class="ns1blankspaceMostLikely">' +
											ns1blankspace.xhtml.loadingSmall + '</td>' +
	...
	carsCount: 	function(oParam, oResponse) 
	{
			...
			oSearch.getResults(function(oResponse) {
				...
				if (oResponse.status === 'OK') {
					...
					$.each(oResponse.data.rows, function() {
						...
						var iCount = Number(this.allcars);
			...
			oSearch.getResults(function(oResponse) {
				...
				if (oResponse.status === 'OK') {
					...
					$.each(oResponse.data.rows, function() {
						...
						var iCount = Number(this.majorcars);
						var iAudit = this["auditissue.audit"];
		...
		else if (iCARsStep === 3) 
		{
			...
			var oSearch = new AdvancedSearch();
			oSearch.method = "AUDIT_ISSUE_SEARCH";
			oSearch.addField('auditissue.audit,count(auditissue.id) criticalcars');
			oSearch.addFilter('auditissue.audit', 'IN_LIST', $.map(aCARs, function(a) {return a.audit}).join(','));
			oSearch.addFilter('type', 'EQUAL_TO', nsFreshcare.data.auditCAR.severityCritical);
			oSearch.rows = 40;
			oSearch.getResults(function(oResponse) {
				...
				if (oResponse.status === 'OK') 
				{
					$.each(oResponse.data.rows, function() 
					{
						var iCount = Number(this.criticalcars);
						var iAudit = this["auditissue.audit"];
			...
			$.each(aCARs, function() 
			{
				...
				{
					aZeroes.push({audit: this.audit, allCARs: true, majorCARs: true, criticalCARs: true});
				}
				...
				if (this.criticalCARs > 0)
				{
					$('#ns1blankspaceMostLikely_CriticalCARs-' + this.audit).html(this.criticalCARs);
				}

				if (this.allCARs > 0) 
				{
					aZeroes.push({audit: this.audit, allCARs: false, majorCARs: (this.majorCARs == 0), criticalCARs: (this.criticalCARs == 0)});
				}
			});
			...
			$.each(aZeroes, function() 
			{
				...
				if (this.criticalCARs) {
					$('#ns1blankspaceMostLikely_CriticalCARs-' + this.audit).html('');
				}
			});
	...
	show: 		function (oParam, oResponse)
	{
		...
		else
		{
			...
			$('#ns1blankspaceControlContext').click(function(event) 
			{
				...
				oRoot.grower.grower.init({showHome: false});
				oRoot.grower.grower.search.send('-' + $(this).attr('data-id'));
			});
	...
	summary: 	function ()
	{
		...
		else
		{
			...
			// v3.4.008 Only show Audit report if audit has been paid for
			else
			{
	...
	cars: 	
	{
		show: 	function(oParam, oResponse) 
		{
			...
			{
				// v3.4.017 SUP023975 Added sort order
				...
				if (oResponse.data.rows.length === 0) {
					...
					aHTML.push('<tr><td class="ns1blankspaceHeaderCaption">Element</td>' +
									'<td class="ns1blankspaceHeaderCaption">Severity</td>' + 
					...
					$.each(oResponse.data.rows, function() 
					{
						...
						// v3.4.017 SUP023975 Added Issue Type prefix + code
						aHTML.push('<td class="ns1blankspaceRow ns1blankspaceRowAuditCAR"' + 
										' id="ns1blankspaceCARType_' + this.id + 
										'">' +
											this["auditissue.sintype.prefix"] + this["auditissue.sintype.code"] + 
										'</td>');
