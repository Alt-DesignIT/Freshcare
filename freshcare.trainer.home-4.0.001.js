...
nsFreshcare.trainer.home = 
{
	...
	courseStatus: 	
	{
		search: function(oParam, oResponse) 
		{
			...
			// v3.2.015 SUP023469 Added eLearning Filter

			oParam = oParam || {};
			var sCourseStatusFilter = ns1blankspace.util.getParam(oParam, 'courseStatus').value;;
			var iELearningFilter = ns1blankspace.util.getParam(oParam, 'eLearningFilter').value;
			var bCount = ns1blankspace.util.getParam(oParam, 'count', {'default': false}).value;;
			var dToday = new Date();
			dToday = new Date(dToday.toString('dd MMM yyyy'));
			var iScheduledStep = ns1blankspace.util.getParam(oParam, 'scheduledStep').value;;
			...
			if (!bCount) 
			{
				$('.ns1blankspaceCourseStatus').each(function() {
					if ($(this).hasClass('nsFreshcareSelected')) {
						sCourseStatusFilter = this.id.split('_').pop();
					}
				});
			}

			if (sCourseStatusFilter == undefined && bCount)
			{
				sCourseStatusFilter = $('.ns1blankspaceCourseStatus').first().attr('id').split('_').pop()
			}	
			else if (sCourseStatusFilter == undefined) 
			{
				sCourseStatusFilter = $('.ns1blankspaceCourseStatus.nsFreshcareSelected').attr('id').split('_').pop();
			}

			if (iELearningFilter == undefined && !bCount && $('.ns1blankspaceCourseELearning.nsFreshcareSelected').length == 1)
			{
				iELearningFilter = $('.ns1blankspaceCourseELearning.nsFreshcareSelected').attr('id').split('_').pop();
			}	
			...
			if (iScheduledStep === undefined) 
			{
				...
				{	// v3.2.015 SUP023469 Allow filtering on eLearning if admin and user has chosen to filter
					if (iELearningFilter && iELearningFilter != '3')
					{
						oSearch.addFilter('trainercontactbusiness', (iELearningFilter == '2' ? 'NOT_' : '') + 'EQUAL_TO', nsFreshcare.data.eLearningBusiness);
					}
				}

				if (sCourseStatusFilter && sCourseStatusFilter.split('-').length > 1) 
				{
					...
					if (!bCount) 
					{
						sScheduledFilter = sCourseStatusFilter.split('-').pop();
		...
		{
			// v3.2.015 Added eLearning Filter
			var bShow = false;
			...
			if ($('#' + sXHTMLElementID).attr('data-loading') == '1') 
			{
				...
				// v4.0.001 Bootstrap
				aHTML = [];
				aHTML.push('<span id="ns1blankspaceHomeCourseSearchHandle" style="height:25px" title="Search Criteria"></span>');
				$('#ns1blankspaceHomeCourseSearchRibbon').html(aHTML.join(''));

				// Filtering area
				aHTML = [];
				aHTML.push('<table class="ns1blankspaceColumn2" style="width:100%">');
				aHTML.push('<tr><td id="ns1blankspaceHomeCourseStatusSearch" class="ns1blankspaceAction" title="Search" style="width: 75px;">' +
							'</td></tr>');
				...
				// v3.2.015 SUP023422 Only show eLearning filter for Admin
				if (nsFreshcare.user.roleID == nsFreshcare.data.roles.admin)
				{
					aHTML.push('<tr><td>&nbsp;</td></tr>');
					aHTML.push('<tr><td class="ns1blankspaceCaption" style="font-size:0.825em;">eLearning Filter</td></tr>');
					aHTML.push('<tr><td id="ns1blankspaceCourseELearning_1" ' +
										'class="ns1blankspaceCourseELearning nsFreshcareSelectable">eLearning Only</td></tr>');
					aHTML.push('<tr><td id="ns1blankspaceCourseELearning_2" ' +
										'class="ns1blankspaceCourseELearning nsFreshcareSelectable nsFreshcareSelected">Exclude eLearning</td></tr>');
					aHTML.push('<tr><td id="ns1blankspaceCourseELearning_3" ' +
										'class="ns1blankspaceCourseELearning nsFreshcareSelectable">All</td></tr>');
				}
				...
				$('.ns1blankspaceCourseELearning').click(function() {
					...
					if ($(this).hasClass('nsFreshcareSelected')) {
						...
						$('.ns1blankspaceCourseELearning').removeClass('nsFreshcareSelected');
						$(this).addClass('nsFreshcareSelected');
					}
				});
	...
	initData: 	function (oParam)
	{
		...
		ns1blankspace.report.dictionary =
			[ 
				...
				{name: "agritrainingcourseattendee.traineecontactperson.agrisubscription.statustext", caption: nsFreshcare.data.growerText + " Status"},
