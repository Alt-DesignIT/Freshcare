...
nsFreshcare.auditor.jasanzexport = 
{
	data: 
	{
		headers:
		{
			...
			[
				...
				{name: 'status', caption: 'Status', inView: true, field: 'agrisubscription.jasanzStatus'},
	...
	show: 	function (oParam, oResponse)
	{
		...
		var aMemberships = ns1blankspace.util.getParam(oParam, 'memberships', {'default': nsFreshcare.auditor.jasanzexport.data.jasanzMemberships}).value;
		var dToday = new Date();
		...
		if (oParam.searchStep === 1) 
		{
			...
			// v3.4.004 Added so that we only pick up JASANZ memberships for which the selected CB is accredited
			nsFreshcare.auditor.jasanzexport.data.jasanzMemberships = $.grep(nsFreshcare.auditor.jasanzexport.data.cbJASANZDates, function(x)
			{
				if (x.trainercontactbusiness == iCertBody)
				{
					var dJASANZ = (x.notes != '') ? new Date(x.notes) : '';
					return (dJASANZ ? dJASANZ <= dToday : false);
				}
				else
				{
					return false;
		...
		{
			ns1blankspace.status.working('Finding Core Details');
			// v3.1.215 Was filtering on certificate.enddate instead of subscription.enddate
			// v3.3.001 Now that audits all have seJASANZAudit, use this to determine if a JASANZ audit
			...
			oSearch.addField('agrisubscription.contactbusiness.reference,agrisubscription.agricertificate.dateissued,agrisubscription.agricertificate.enddate' +
							...
							',agrisubscription.status,agrisubscription.statustext,agrisubscription.membership');
			...
			oSearch.addFilter('agrisubscription.agricertificate.audit.sejasanzaudit', 'EQUAL_TO', 'Y');
			oSearch.addBracket('(');
			...
			// v3.4.004 Now filters on membership that they're JASANZ accredited to
			if (nsFreshcare.auditor.jasanzexport.data.jasanzMemberships.length > 0)
			{
				oSearch.addFilter('agrisubscription.membership', 'IN_LIST', $.map(nsFreshcare.auditor.jasanzexport.data.jasanzMemberships, 
									function(x) {return x.membership}).join(','));
			}
			if (iCertBody)
			{
		...
		{
			ns1blankspace.status.working('Collating data');
			...
			$.each(nsFreshcare.auditor.jasanzexport.data.subscriptions, function(i, subscription)
			{
				...
				// v3.4.004 Need to map statuses as per email from Angela and send in place of Status: active
				switch (subscription['agrisubscription.status'])
				{
					case nsFreshcare.data.grower.subscriptionStatusIN:
					case nsFreshcare.data.grower.subscriptionStatusCE:
						subscription['agrisubscription.jasanzStatus'] = "Active";
						break;
					...
					case nsFreshcare.data.grower.subscriptionStatusDR:
						 subscription['agrisubscription.jasanzStatus'] = "Suspended";
						 break;

					case nsFreshcare.data.grower.subscriptionStatusSP:
					case nsFreshcare.data.grower.subscriptionStatusWD:
						subscription['agrisubscription.jasanzStatus'] = 'Withdrawn';
						break;

					case nsFreshcare.data.grower.subscriptionStatusIP:
					case nsFreshcare.data.grower.subscriptionStatusTI:
						subscription['agrisubscription.jasanzStatus'] = 'Hidden';
						break;
					...
					case nsFreshcare.data.grower.subscriptionStatusRM:
						...
						console.log(subscription['agrisubscription.contactbusiness.tradename'] + ': Status: ' + 
										subscription['agrisubscription.statustext'] + '   Expiry Date: ' + 
										subscription['agrisubscription.agricertificate.enddate']);
						var dExpiry = new Date(subscription['agrisubscription.agricertificate.enddate']);
						if (dExpiry >= (new Date(dToday.toString('dd MMM yyyy'))))
						{
							subscription['agrisubscription.jasanzStatus'] = "Active";
						}
						else
	...
	summary: 	function (oParam)
	{
		...
		else if (nsFreshcare.auditor.jasanzexport.data.cbJASANZDates == undefined)
		{
			// v3.4.004 Get the list of JASANZ Dates for all or current CB
			if (nsFreshcare.user.roleID != nsFreshcare.data.roles.admin)
			{
				nsFreshcare.internal.data.certificationBodies = $.grep(nsFreshcare.internal.data.certificationBodies, function(x)
				{
					return x.id == ns1blankspace.user.contactBusiness;
				});
			}

			var aCBs = $.map(nsFreshcare.internal.data.certificationBodies, function(x) {return x.id});
			var oSearch = new AdvancedSearch();
			...
			oSearch.addFilter('trainercontactbusiness', 'IN_LIST', aCBs.join(','));
			...
			oSearch.getResults(function(oResponse)
			{
				if (oResponse.status == 'OK')
				{
					nsFreshcare.auditor.jasanzexport.data.cbJASANZDates	= oResponse.data.rows;
					nsFreshcare.auditor.jasanzexport.summary(oParam);
				}
				else
				{
			...
			if ($('#ns1blankspaceSummary').attr('data-loading') == undefined)
			{
				...
				$('#ns1blankspaceSummaryGenerateData')
					...
					.on('click', function(event) 
					{
						...
						if (nsFreshcare.user.roleID == nsFreshcare.data.roles.admin && $('#ns1blankspaceSummarySelectCertBody .nsFreshcareSelected').length == 0)
						{
						...
						else
						{
							...
							// v3.2.017 SUP023493 was comparing to nsFreshcare.user.roleID
							if (nsFreshcare.user.roleID == nsFreshcare.data.roles.admin) 
							{
	...
	renderDataTab: 	function(oParam) 
	{
		...
		{
			ns1blankspace.status.working('Formatting data..');
