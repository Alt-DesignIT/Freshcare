...
nsFreshcare.admin.certificate = 
{
	...
	printIndividualCertificate: function(oParam) 
	{
		...
		if (oParam.printIndividualCertStep === 1 && sXHTMLElementId != ns1blankspace.xhtml.divID) 
		{
			...
			if (nsFreshcare.user.roleID === nsFreshcare.data.roles.grower || nsFreshcare.user.roleID === nsFreshcare.data.roles.trainer || nsFreshcare.user.roleID === nsFreshcare.data.roles.customer)
			{
		...
		if (iCertificate) 
		{
					...
					if (oResponse.status === 'OK') 
					{
						if (oResponse.data.rows.length > 0)
						{
							...
							if (oParam.certificateData === undefined) 
							{
								...
								// v3.2.017 SUP023494 JasanzAccredited needs to be based on date CB started JASANZ as well
			...
			else if (iCertificateGetDataStep === 7)
			{
				...
				else
				{
					var dIssued = new Date(oParam.certificateDateIssued);
					var dSelfCertDate = (nsFreshcare.data.auditor.selfCertificationDate != '') ? new Date(nsFreshcare.data.auditor.selfCertificationDate) : dToday;
					var dJASANZStart = new Date((nsFreshcare.data.auditor.jasanzDate === '') ? dTodayShort : nsFreshcare.data.auditor.jasanzDate);
					var dAudit = new Date(oParam.certificateData.auditDate);

					if (dIssued >= dSelfCertDate)		// If we're on or after the self Cert date
					{
						// v3.2.017 SUP023494 If JASANZ Accredited, but CB hasn't started JASANZ as at Audit date, set JASANZ Accredited back to N
						if (oParam.certificateData.jasanzAccredited == 'Y' && dAudit < dJASANZStart)
						{
