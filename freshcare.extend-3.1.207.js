...
nsFreshcare.extend = 
{
	...
	format:
	{
		render: function (oParam)
		{
			...
			if (sTemplate && ns1blankspace.xhtml.templates[sTemplate + 'lineitem'])
			{
				sXHTMLLineItemTemplate = '<div id="ns1blankspaceTemplatePage2" style="display:none;">' + ns1blankspace.xhtml.templates[sTemplate + 'lineitem'] + '</div>';
		...
		process: function (oParam, aRows)
		{
			...
			if (ns1blankspace.data.financial.invoice == undefined) {ns1blankspace.data.financial.invoice = {maxPage1Items: 5, maxPage2Items: 30}}

			var sTemplate = ns1blankspace.util.getParam(oParam, 'template').value;
			var oXHTML = document;
			var iMaxPage1Items = ns1blankspace.data.financial.invoice.maxPage1Items;		
			var iMaxPage2Items = ns1blankspace.data.financial.invoice.maxPage2Items;	
			...
			if (sTemplate && ns1blankspace.xhtml.templates[sTemplate + 'lineitem']) 		// Lineitem template exists
			{
	...
	setup:
	{
		...
		{
			home: function (oParam, oResponse)
			{	// v3.1.0e SUP022234 Added Invoice Line item template
				$(ns1blankspace.xhtml.container).hide(ns1blankspace.option.hideSpeedOptions);
				$('#ns1blankspaceViewControlAction').button({disabled: false});
				...
				aHTML.push('<tr><td id="ns1blankspaceControlInvoiceLineItemTemplate" class="ns1blankspaceControl">' +
								'Invoice Line Items</td></tr>');
				...
				aHTML.push('<div id="ns1blankspaceMainTemplate_invoicelineitem" class="ns1blankspaceControlMain"></div>');
				...
				$('#ns1blankspaceControlInvoiceLineItemTemplate').click(function(event)
				{
					var oRoot = ns1blankspace.rootnamespace;
					ns1blankspace.show({selector: '#ns1blankspaceMainTemplate_invoicelineitem'});
					ns1blankspace.xhtml.templates.source.invoicelineitem = '/site/' + oRoot.site + '/1blankspace.setup.financial.invoicelineitem-1.0.0.html';
					ns1blankspace.setup.financial.template.init({template: 'invoicelineitem', object: 5});
				});
			...
			template:
			{
				show: function(oParam)
				{
					...
					if ($('#ns1blankspaceMainTemplate_' + sTemplate).attr('data-loading') == '1')
					{
						...
						// v3.1.2 Now calls generic init function
						...
						success: function()
							{
								...
								if ($('#ns1blankspaceMainTemplate_invoicelineitem').html() != '') {ns1blankspace.setup.financial.template.save({template: 'invoicelineitem'})}
