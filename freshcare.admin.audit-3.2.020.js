...
nsFreshcare.admin.audit = 
{
	data: 
	{
		...
		[	/* v3.1.215 changed titletext to typetext */
			...
			{name: 'audit.sequalifyingtraininglink', caption: 'Qualifying Training', display: 'audit.sequalifyingtraininglink.attendee.attendingtrainee'}
		]
	},
	...
	search: 	
	{
		send: 	function (sXHTMLElementId, oParam)
		{
			...
			if (sSearchContext != undefined && iSource != ns1blankspace.data.searchSource.browse)
			{
				...
				// v3.2.020 SUP023422 Added seQualifyingTraining
				var oSearch = new AdvancedSearch();
				oSearch.method = 'AUDIT_SEARCH';
				...
				{
					oSearch.addField('audit.sequalifyingtraininglink,audit.sequalifyingtraininglink.attendee.attendingtrainee,audit.codeofpractice.copbasedtraining' +
	...
	summary: 	function ()
	{
		...
		else
		{
			...
			else
			{
				...
				// v3.2.020 SUP023422 List Qualifying Training
				if (nsFreshcare.option.qualifyingTraining)
				{
					aHTML.push('<tr><td class="ns1blankspaceSummaryCaption">Qualifying Training</td></tr>' +
									'<tr><td class="ns1blankspaceSummary">');
					if (ns1blankspace.objectContextData['audit.sequalifyingtraininglink'] != '' 
						&& ns1blankspace.objectContextData['audit.sequalifyingtraininglink'] != '0') 
					{
						aHTML.push('<table class="ns1blankspace">');
						...
						aHTML.push('<td class="ns1blankspaceRow"><span id="ns1blankspaceSummaryTraineeLinkReset"></td>')
					...
					{
						aHTML.push('<tr><td colspan="5" class="ns1blankspace">No Qualifying Training registered</td><td><span id="ns1blankspaceSummaryTraineeLinkAdd"></span>')
					} 
				}
	...
	summaryBind: function()
	{
		...
		if (nsFreshcare.user.roleID == nsFreshcare.data.roles.admin)
		{
			...
			.button({icons: {primary: 'ui-icon-play'}, text: 'View Training Attendance'})
			.css('width', '25px')
			.css('height', '25px')
			.on('click', function()
			{
			...
			$('#ns1blankspaceSummaryTraineeLinkReset')
			.button({icons: {primary: 'ui-icon-arrowrefresh-1-s'}, text: 'Reset Qualifying Training'})
			.css('width', '25px')
			.css('height', '25px')
			.on('click', function()
			{
				nsFreshcare.admin.audit.save.send({qualifyingTraining: '', fromSummary: true});
			});

			$('#ns1blankspaceSummaryTraineeLinkAdd')
			.button({icons: {primary: 'ui-icon-plus'}, text: 'Link Training Attendance'})
			.css('width', '25px')
			.css('height', '25px')
			.on('click', function()
			{
				var oParam = {qualifyingCOPs: [], fromSummary: true, functionPostAddLink: nsFreshcare.admin.audit.save.send};
				oParam.copBasedTraining = ($('#ns1blankspaceMainDetails').html() != '') 
											? ($('#ns1blankspaceDetailsCOP').attr('data-copBasedTraining') == 'Y')
											: (ns1blankspace.objectContextData['audit.codeofpractice.copbasedtraining'] == 'Y');

				nsFreshcare.admin.audit.save.qualifyingTrainee[(oParam.copBasedTraining) ? 'searchCOP' : 'showDialog'](oParam);
			});
		}
	},
			...
			if (oParam.setMembershipStep == 1) 
			{
				if (oResponse.status === 'OK') 
				{
					if (oResponse.data.rows.length > 0) 
					{
						...
						oSearch.addField('description,isdefault,copbasedtraining');
	...
	save: 		
	{
		validate: 	function(oParam) 
		{
			...
			// v3.2.020 SUP023422 Display dialog for user to search for qualifying trainee
			else if (oParam.auditValidateStep === 3)
			{
				...
				// First, search AgriCOPQualifyingTraining for list of qualifying COP's and then allow user to search for
				// the trainee who completed this training in auditValidateStep = 2
				// v3.2.020 SUP023422 Now uses AgriCOPQualifyingTraining to find COPs, but only if seQualifyingTraining is null 
				// 			and not Cert Recommended or changing to Cert Reccommended
				if (nsFreshcare.option.qualifyingTraining && iMembership && 
					(ns1blankspace.objectContextData == -1
						|| ((ns1blankspace.objectContextData['audit.sequalifyingtraininglink'] == '' 
							|| ns1blankspace.objectContextData['audit.sequalifyingtraininglink'] == '0')
						    && (iResultStatus != nsFreshcare.data.audit.resultStatusCompleted 
						    	|| (iResultStatus == nsFreshcare.data.audit.resultStatusCompleted 
						    		&& ns1blankspace.objectContextData['audit.resultstatus'] != nsFreshcare.data.audit.resultStatusCompleted)
				...
				{
					oParam.qualifyingCOPs = [];
					oParam.copBasedTraining = (($('#ns1blankspaceMainDetails').html() != '' 
													? $('#ns1blankspaceDetailsCOP').attr('data-copBasedTraining') 
													: ns1blankspace.objectContextData['audit.codeofpractice.copbasedtraining'])
												== 'Y');

					nsFreshcare.admin.audit.save.qualifyingTrainee[(oParam.copBasedTraining ? 'searchCOP' : 'showDialog')](oParam);
				}
				// QualifyingTraining already chosen, make sure course is completed if setting to Cert Recommended
				else if (nsFreshcare.option.qualifyingTraining && ns1blankspace.objectContext != -1 
						 && (ns1blankspace.objectContextData['audit.sequalifyingtraininglink'] != '' 
							 || ns1blankspace.objectContextData['audit.sequalifyingtraininglink'] != '0')
						 && (iResultStatus == nsFreshcare.data.audit.resultStatusCompleted 
						     && ns1blankspace.objectContextData['audit.resultstatus'] != nsFreshcare.data.audit.resultStatusCompleted)
		...
		qualifyingTrainee:
		{
			searchCOP: function(oParam)
			{
				...
				oSearch.addFilter('codeofpractice', 'EQUAL_TO', $('#ns1blankspaceDetailsCOP').attr('data-id'));
				oSearch.getResults(function(oResponse)
				{
					if (oResponse.status == 'OK')
					{
						oParam.qualifyingCOPs = $.map(oResponse.data.rows, function(x) {return x.qualifyingcodeofpractice});
						nsFreshcare.admin.audit.save.qualifyingTrainee.showDialog(oParam);
					}
					else
					{
						ns1blankspace.status.error(oResponse.error.errornotes);
			...
			showDialog: function(oParam)
			{
				var sTitle = 'Search for a Qualifying Trainee';
				var aHTML = [];
				var iTrainer;
				var sTrainer = '';
				var sTrainingDate = '';
				...
				{
					iTrainer = $('#ns1blankspaceEvaluationsTrainer').attr('data-id');
					sTrainer = $('#ns1blankspaceEvaluationsTrainer').val();
					sTrainingDate = $('#ns1blankspaceEvaluationsTrainingDate').val();
				...
				else 
				{
					if ($('#ns1blankspaceMainDetails').html() != '')
					{
						iTrainer = (ns1blankspace.objectContext != -1) ? ns1blankspace.objectContextData['audit.trainercontactbusiness'] : '';
						sTrainer = (ns1blankspace.objectContext != -1) ? ns1blankspace.objectContextData['audit.trainercontactbusinesstext'] : '';
						sTrainingDate = (ns1blankspace.objectContext != -1) ? ns1blankspace.objectContextData['audit.trainingdate'] : '';
						iContactBusiness = $('#ns1blankspaceDetailsGrowerBusiness').attr('data-id');
					}
					else if (ns1blankspace.objectContextData)
					{
						iTrainer = ns1blankspace.objectContextData['audit.trainercontactbusiness'];
						sTrainer = ns1blankspace.objectContextData['audit.trainercontactbusinesstext']
						sTrainingDate = ns1blankspace.objectContextData['audit.trainingdate'];
						iContactBusiness = ns1blankspace.objectContextData['audit.contactbusiness'];
					}
				}
				if ($('#ns1blankspaceMainDetails').html() != '')
				{
					sName = $('#ns1blankspaceDetailsGrowerPerson').val();
					iContactPerson = $('#ns1blankspaceDetailsGrowerPerson').attr('data-id');
				}
				else
				{
					sName = ns1blankspace.objectContextData['audit.contactpersontext'];
					iContactPerson = ns1blankspace.objectContextData['audit.contactperson'];
				}
				sFirstName = sName.split(', ').pop();
				sSurname = sName.split(', ').shift();
				...
				aHTML.push('<tr>' +
							'<td class="ns1blankspaceCaption">Trainer Business</td>' +
							'<td class="ns1blankspaceCaption">Trainee First Name</td>' +
							'<td class="ns1blankspaceCaption">Trainee Surname</td>' +
							'<td class="ns1blankspaceCaption">Training Date</td>' +
							'<td>&nbsp;</td>' +
							'</tr>');

				aHTML.push('<tr>' +
							'<td class="ns1blankspaceText">' +
								'<input class="ns1blankspaceSelect" id="ns1blankspaceSearchTrainer"' +
									(iTrainer ? ' data-id="' + iTrainer + '"' : '') +
									' value="' + sTrainer + '"' +
									' data-method="CONTACT_BUSINESS_SEARCH"' +
									' data-columns="tradename"' +
									' data-methodFilter="businessgroup-EQUAL_TO-' + nsFreshcare.data.businessGroupTrainer + 
														'|tradename-TEXT_IS_LIKE|legalname-TEXT_IS_LIKE">' +
							'</td>' +
							'<td class="ns1blankspaceText">' +
								'<input class="ns1blankspaceSelect" id="ns1blankspaceSearchFirstName"' +
									' data-method="CONTACT_PERSON_SEARCH"' +
									' data-columns="firstname-space-surname"' +
									' data-methodFilter="contactbusiness-EQUAL_TO-' + iContactBusiness + 
														'|firstname-TEXT_IS_LIKE|surname-TEXT_IS_LIKE"' +
									' data-click="nsFreshcare.admin.audit.save.qualifyingTrainee.setTraineeName"' +
									' data-autoComplete="false"' +
									(sName != '' ? ' value="' + sFirstName + '" data-id="' + iContactPerson + '"' : '') + '>' +
									'</td>' +
							'<td class="ns1blankspaceText"><input class="ns1blankspaceText" id="ns1blankspaceSearchSurname"' +
									(sName != '' ? 'value="' + sSurname + '"' : '') + 
									'></td>' +
							'<td class="ns1blankspaceText"><input class="ns1blankspaceDate" id="ns1blankspaceSearchCourseDate"' +
																' value="' + sTrainingDate + '">' +
							'</td>' +
							'<td class="ns1blankspaceAction"><span id="ns1blankspaceSearchGo"></span>' +
							'</tr>');

				aHTML.push('<tr><td class="ns1blankspaceText" id="ns1blankspaceTraineeSearchCriteriaError" style="font-color: red;" colspan="5"></td></tr>');

				aHTML.push('</table></td></tr><tr><td style="border: 1px solid #d4d4d4;">');

				aHTML.push('<table class="ns1blankspace" id="ns1blankspaceSearchTraineeResults"' +
								' style="overflow-y:auto;">'); 

				aHTML.push('<tr><td class="ns1blankspaceCaption" colspan="4">Matching Trainees</td></tr>');

				aHTML.push('<tr>' +
							'<td class="ns1blankspaceHeaderCaption">Trainer</td>' +
							'<td class="ns1blankspaceHeaderCaption">Trainee (Business)</td>' +
							'<td class="ns1blankspaceHeaderCaption">COP</td>' +
							'<td class="ns1blankspaceHeaderCaption">Training Date</td>' +
							'</tr>');
				...
				$('#ns1blankspaceMultiUseDialog')
					...
					.dialog(
					{
						...
						{ 
							...
							$('.ui-dialog .ui-dialog-buttonset').before('<span id="ns1blankspaceQualifyingTraineeError" style="color: red;"></span>');
						},
						focus: function(event, ui)
						{
							// Remove the error when user focuses on input fields
							//$('#ns1blankspaceMultiUseDialog input').on('focus', function() {$('#ns1blankspaceTraineeSearchCriteriaError').html('')});
						...
						[
							{
								...
								click: function()
								{
									if ($('.ns1blankspaceTrainees.nsFreshcareSelected').length > 0)
									{
										$('#ns1blankspaceQualifyingTraineeError')
											.html('You have selected a Trainee - deselect the Trainee or click "OK"');
									}
									else
									{
										// First check if changing to Cert Recommended - if so, then can't choose 'Trainee not found'
										var iResultStatus = $('#ns1blankspaceDetailsResultStatus').attr('data-id');
										iResultStatus = iResultStatus || ns1blankspace.objectContextData['audit.resultstatus'];
										if (iResultStatus == nsFreshcare.data.audit.resultStatusCompleted
											&& ns1blankspace.objectContextData['audit.resultstatus'] != nsFreshcare.data.audit.resultStatusCompleted)
										{
											$('#ns1blankspaceQualifyingTraineeError')
												.html('A Trainee must be selected if Certification Recommended');

										}
										else
										{
											delete(oParam.searchTraineeStep);
											$('#ns1blankspaceMultiUseContainer').css( "z-index", "99" ).css('display', 'none');
											$(this).dialog('destroy');
											// If user has clicked Add Qualifying Trainee from Summary, don't do anything if Trainee not found
											if (oParam.fromSummary != true)
											{
							...
							},
							{
								...
								click: function() 
								{
									if ($('.ns1blankspaceTrainees.nsFreshcareSelected').length > 0)
									{
										var oSelectedRow = $('.ns1blankspaceTrainees.nsFreshcareSelected').first().parent();
										
										// First check if changing to Cert Recommended - if so, then course MUST be completed
										var iResultStatus = $('#ns1blankspaceDetailsResultStatus').attr('data-id');
										iResultStatus = iResultStatus || ns1blankspace.objectContextData['audit.resultstatus'];
										if (iResultStatus == nsFreshcare.data.audit.resultStatusCompleted
											&& ns1blankspace.objectContextData['audit.resultstatus'] != nsFreshcare.data.audit.resultStatusCompleted
											&& $(oSelectedRow).attr('data-coursestatus') != nsFreshcare.data.trainingCourse.statusCompleted)
										{
											$('#ns1blankspaceQualifyingTraineeError')
												.html('The selected Trainee has not yet <i>completed</i> training.<br />' + 
														'Training must be completed if Certification Recommended.')
										}
										else
										{
											oParam.traineeAttendance = $(oSelectedRow).attr('data-attendee');
											oParam.traineeContactBusiness = $(oSelectedRow).attr('data-contactbusiness');
											oParam.traineeContactPerson = ($('#ns1blankspaceSearchFirstName').attr('data-id') 
																			? $('#ns1blankspaceSearchFirstName').attr('data-id') 
																			: $(oSelectedRow).attr('data-contactperson'));
											delete(oParam.searchTraineeStep);

											$('#ns1blankspaceMultiUseContainer').css( "z-index", "99" ).css('display', 'none');
											$(this).dialog('destroy')
											nsFreshcare.admin.audit.save.qualifyingTrainee.createLink(oParam);
										}
									}
									else
									{
										$('#ns1blankspaceQualifyingTraineeError')
											.html('You must choose a Trainee or click "Trainee not found"');
									}
								}
							}
						]
				...
				$('#ns1blankspaceSearchGo')
					...
					{
						delete(oParam.searchTraineeStep);
						nsFreshcare.admin.audit.save.qualifyingTrainee.search(oParam);
					});
			},

			search: function(oParam)
			{
				oParam.searchTraineeStep = oParam.searchTraineeStep || 1;

				// User has hit search icon on search dialog
				if (oParam.searchTraineeStep == 1)
				{
					// Make sure the user has entered at least something!
					if ($.map($('#ns1blankspaceMultiUseDialog input'), function(x) {return $(x).val()}).join('').length > 0)
					{
						// Search for Attendees who's course is in the past, not cancelled, has the right Memebrship or Qualifying COP's and fits search criteria
						var oSearch = new AdvancedSearch();
						oSearch.method = 'AGRI_EDUCATION_TRAINING_COURSE_ATTENDEE_SEARCH';
						oSearch.addField('agritrainingcourseattendee.course.trainercontactbusinesstext,attendingtrainee,firstname,surname,traineecontactperson' +
										',agritrainingcourseattendee.course.package.membership.code,agritrainingcourseattendee.course.package.membership' +
										',agritrainingcourseattendee.course.package.codeofpractice,agritrainingcourseattendee.course.package.codeofpracticetext' +
										',traineecontactbusiness,traineecontactbusinesstext,agritrainingcourseattendee.course.coursedate' +
										',agritrainingcourseattendee.course.status,agritrainingcourseattendee.course.statustext');
						oSearch.addBracket('(');
						...
						oSearch.addFilter('agritrainingcourseattendee.course.coursedate', 'LESS_THAN', dToday.toString('dd MMM yyyy'));
						oSearch.addBracket(')');
						// If copBasedTraining, then must be in the relevant list of COP's
						if (oParam.copBasedTraining)
						{
							oSearch.addFilter('agritrainingcourseattendee.course.package.codeofpractice', 'IN_LIST', oParam.qualifyingCOPs.join(','));
						}
						else 	// Otherwise, just has to be the same membershpi
						{
							oSearch.addFilter('agritrainingcourseattendee.course.package.membership', 'EQUAL_TO', $('#ns1blankspaceDetailsMembershipValue').html());
						}

						// Now add filters according to search criteria in dialog
						oSearch.addBracket('(');
						oSearch.addFilter('traineecontactbusiness', 'EQUAL_TO', ns1blankspace.data.contactBusiness);
						oSearch.addOperator('or');

						oSearch.addBracket('(');
						var iFilters = oSearch.criteria.filterField.length;
						var bAddCloseBracket = false;

						if ($('#ns1blankspaceSearchTrainer').val() != '')
						{
							if ($('#ns1blankspaceSearchTrainer').attr('data-id'))
							{
								oSearch.addFilter('agritrainingcourseattendee.course.trainercontactbusiness', 'EQUAL_TO', $('#ns1blankspaceSearchTrainer').attr('data-id'));
							}
							else
							{
								oSearch.addFilter('agritrainingcourseattendee.course.trainercontactbusinesstext', 'TEXT_IS_LIKE', $('#ns1blankspaceSearchTrainer').val());
							}

							if ($('#ns1blankspaceSearchFirstName').val() != '' || $('#ns1blankspaceSearchSurname').val() || $('#ns1blankspaceSearchCourseDate').val() != '')
							{
								oSearch.addOperator('and')
								if ($('#ns1blankspaceSearchCourseDate').val() == '') 
								{
									oSearch.addBracket('(');
									bAddCloseBracket = true;
								}
							}
						}

						if ($('#ns1blankspaceSearchCourseDate').val() != '' && isValidDate($('#ns1blankspaceSearchCourseDate').val(), 'dd mmm yyyy'))
						{
							var dCourse = Date.parse($('#ns1blankspaceSearchCourseDate').val());
							oSearch.addFilter('agritrainingcourseattendee.course.coursedate', 'BETWEEN', 
												dCourse.add({days: -7}).toString('dd MMM yyyy'), dCourse.add({days: 14}).toString('dd MMM yyyy'));

							if ($('#ns1blankspaceSearchFirstName').val() != '' || $('#ns1blankspaceSearchSurname').val())
							{
								oSearch.addOperator('and')
								if (!bAddCloseBracket) 
								{
						...
						iFilters = oSearch.criteria.filterField.length;

						if ($('#ns1blankspaceSearchFirstName').val() != '')
						{
							oSearch.addFilter('agritrainingcourseattendee.firstname', 'TEXT_IS_LIKE', $('#ns1blankspaceSearchFirstName').val());
						}

						if ($('#ns1blankspaceSearchSurname').val() != '')
						{
							if (oSearch.criteria.filterField.length > iFilters) {oSearch.addOperator('or')}
							oSearch.addFilter('agritrainingcourseattendee.surname', 'TEXT_IS_LIKE', $('#ns1blankspaceSearchSurname').val());
						}

						if (bAddCloseBracket) {oSearch.addBracket(')')}
						oSearch.addBracket(')');
						oSearch.addBracket(')');
						...
						oSearch.getResults(function(oResponse)
						{
							if (oResponse.status == 'OK')
							{
								oParam.searchTraineeStep = 2;
								oParam.traineeResponse = oResponse;
								nsFreshcare.admin.audit.save.qualifyingTrainee.search(oParam);
							}
							else
							{
								$('#ns1blankspaceTraineeSearchCriteriaError').html('Error finding Trainees: ' + oResponse.error.errornotes);			
							}
						});
					}
					else
					{
						$('#ns1blankspaceTraineeSearchCriteriaError').html('You must enter at least one value into the search fields..');
					}
				}

				// Display results 
				else if (oParam.searchTraineeStep == 2)
				{
					$('#ns1blankspaceSearchTraineeResults tr.ns1blankspaceSearchTrainees').remove();

					$.each(oParam.traineeResponse.data.rows, function()
					{
						var sTrainee = this.firstname + (this.firstname && this.surname ? ' ' : '') + this.surname +
										(this.traineecontactbusinesstext ? ' (' + this.traineecontactbusinesstext + ')' : '');
		
						$('#ns1blankspaceSearchTraineeResults tr').last()
							.after('<tr data-attendee="' + this.id + '" data-contactperson="' + this.traineecontactperson + '"' +
										' data-contactbusiness="' + this.traineecontactbusiness + '" data-coursestatus="' + this['agritrainingcourseattendee.course.status'] + '"' +
										' class="ns1blankspaceSearchTrainees">' +
									'<td class="ns1blankspaceRow ns1blankspaceTrainees">' + this['agritrainingcourseattendee.course.trainercontactbusinesstext'] + '</td>' +
									'<td class="ns1blankspaceRow ns1blankspaceTrainees">' + sTrainee + '</td>' +
									'<td class="ns1blankspaceRow ns1blankspaceTrainees">' + this['agritrainingcourseattendee.course.package.membership.code'] + ' ' + 
																							this['agritrainingcourseattendee.course.package.codeofpracticetext'] + '</td>' +
									'<td class="ns1blankspaceRow ns1blankspaceTrainees">' + this['agritrainingcourseattendee.course.coursedate'] + 
										(this['agritrainingcourseattendee.course.status'] == nsFreshcare.data.trainingCourse.statusCompleted
											? '<br/>(Completed)' : '') +
									'</td>' +
									'</tr>');
					});
					
					if ($('#ns1blankspaceSearchTraineeResults tr.ns1blankspaceSearchTrainees').length == 0)
					{
						$('#ns1blankspaceSearchTraineeResults tr').last()
							.after('<tr class="ns1blankspaceSearchTrainees"><td colspan="4" class="ns1blankspaceRow">' + 
										'No matching trainees found. If searching by name, try shortening the name to increase potential search results.' +
									'</td></tr>');
					}

					$('.ns1blankspaceTrainees')
					.css('cursor', 'pointer')
					.off('click')
					.on('click', function()
					{
						if ($('.ui-dialog .ui-dialog-buttonpane').children('span').length == 0)
						{	$('.ui-dialog .ui-dialog-buttonpane').children('span').remove();}	// remove error message when shown
						if (!$(this).hasClass('nsFreshcareSelected'))
						{
							$('.ns1blankspaceTrainees').removeClass('nsFreshcareSelected');
						}
						$(this).parent().children().toggleClass('nsFreshcareSelected');
					});
			...
			createLink: function(oParam)
			{
				var oData = ns1blankspace.util.getParam(oParam, 'attendanceLinkData', {'default': {}}).value;
				var fFunctionPostSave = ns1blankspace.util.getParam(oParam, 'functionPostAddLink', {'default': nsFreshcare.admin.audit.save.validate}).value;
				oParam.attendanceLinkStep = oParam.attendanceLinkStep || 1;

				if (oParam.attendanceLinkStep == 1)
				{
					// First, see if link exists. If not, then create it
					if (oParam.traineeAttendance)
					{
						oData.attendee = oParam.traineeAttendance;
						oData.contactbusiness = (ns1blankspace.objectContextData) ? ns1blankspace.objectContextData['audit.contactbusiness'] : ns1blankspace.data.contactBusiness;
						oData.contactperson = oParam.traineeContactPerson;
						oData.subscription = (ns1blankspace.objectContextData) ? ns1blankspace.objectContextData['audit.subscription'] : ns1blankspace.data.subscription;

						var oSearch = new AdvancedSearch();
						oSearch.method = 'AGRI_TRAINEE_ATTENDANCE_LINK_SEARCH';
						oSearch.addField('attendee,contactbusiness,contactperson,subscription');
						oSearch.addFilter('attendee', 'EQUAL_TO', oData.attendee);
						oSearch.addFilter('contactbusiness', 'EQUAL_TO', oData.contactbusiness);
						oSearch.addFilter('contactperson', 'EQUAL_TO', oData.contactperson);
						oSearch.addFilter('subscription', 'EQUAL_TO', oData.subscription);
						oSearch.rows = 1;
						oSearch.getResults(function(oResponse)
						{
							if (oResponse.status == 'OK')
							{
								if (oResponse.data.rows.length == 0)
								{	// Link doesn't exist yet so create it
									oParam.attendanceLinkStep = 2;	
									oParam.attendanceLinkData = oData;
									nsFreshcare.admin.audit.save.qualifyingTrainee.createLink(oParam);
								}
								else
								{	// It already exists, set oParam.qualifyingTraining
									delete(oParam.attendanceLinkStep);
									delete(oParam.attendanceLinkData);
									delete(oParam.functionPostAddLink);
									oParam.qualifyingTraining = oResponse.data.rows[0].id;
									fFunctionPostSave(oParam);
								}
							}
							else
							{
								ns1blankspace.status.error('Error finding trainee attendance link: ' + oResponse.error.errornotes);
							}
						});
						// First, search to see if link already exists
					}
				}

				else if (oParam.attendanceLinkStep == 2)
				{
					// Now create link 
					$.ajax(
					{
						...
						success: function(oResponse)
						{
							if (oResponse.status == 'OK')
							{
								oParam.qualifyingTraining = oResponse.id;
								delete(oParam.attendanceLinkStep);
								delete(oParam.attendanceLinkData);
								delete(oParam.functionPostAddLink)
								fFunctionPostSave(oParam);
							...
							{
								ns1blankspace.status.error('Unable to add attendance link: ' + oResponse.error.errornotes); 
							}
						}
					});
				}
			...
			setTraineeName: function(oParam)
			{
				sName = $('#ns1blankspaceSearchFirstName').val();
				sFirst = sName.split(' ').shift();
				sLast = sName.substr(sFirst.length + 1);
				$('#ns1blankspaceSearchFirstName').val(sFirst);
				$('#ns1blankspaceSearchSurname').val(sLast);
			}
		},
		...
		send: 	function(oParam) 
		{
			...
			// v3.2.020 SUP023422
			// user has clicked Reset Qualifying Training from the Summary tab
			else if (iQualifyingTraining != undefined && bFromSummary == true && sData == '')
			{
				ns1blankspace.objectContextData['audit.sequalifyingtraininglink'] == iQualifyingTraining;	// Need to set here so will validate again if removed
				oParam.data = 'id=' + ns1blankspace.objectContext + '&sequalifyingtraininglink=' + iQualifyingTraining;
				oParam.newAudit = true;		// This will make the audit re-display
				oParam.saveAuditStep = 4;		// We only need to save the data, no other data is saved when auditreviewer is passed
				nsFreshcare.admin.audit.save.send(oParam);
			}
			...
			else if (oParam.saveAuditStep === 3 && ns1blankspace.okToSave) 
			{
				if ($('#ns1blankspaceMainDetails').html() != '') 
				{
					...
					oData.sequalifyingtraininglink = (oParam.qualifyingTraining) ? oParam.qualifyingTraining : ns1blankspace.objectContextData['audit.sequalifyingtraininglink'];
					...
					sData += (oParam.qualifyingTraining) ? '&sequalifyingtraininglink=' + oParam.qualifyingTraining : '';		// v3.2.020 SUP023422
		...
		process: function(oParam) 
		{
			...
			// v3.2.020 We need to refresh if qualifying training link has just been set
			if (!bNew && oParam.qualifyingTraining == undefined) 
			{
