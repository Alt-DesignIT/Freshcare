...
nsFreshcare.admin.trainingcourse = 
{
		...
		send: 		function (sXHTMLElementId, oParam)
					{
						...
						else
						{
							...
							if (sSearchText.length >= iMinimumLength || iSource == ns1blankspace.data.searchSource.browse)
							{
								...
								var oSearch = new AdvancedSearch();
								oSearch.method = 'AGRI_EDUCATION_TRAINING_COURSE_SEARCH';
								oSearch.addField('title,location,coursedate,status,statustext');
		...
		process: 	function (oParam, oResponse)
					{
						...
						{	
							...
							{
								columns: 'title-column-coursedate-column-openbracket-statustext-closebracket',
	...
	summary: 	function ()
	{
		...
		else
		{
			...
			{
				aHTML.push('<tr><td><span id="ns1blankspaceCourseAddTrainee" class="ns1blankspaceAction">' +
			...
			if (nsFreshcare.user.role.toLowerCase() === 'admin')
			{
				...
				{
					aHTML.push('<tr><td><span id="ns1blankspaceCourseProcessNewTrainees" class="ns1blankspaceAction">' +
				...
				{
					aHTML.push('<tr><td><span id="ns1blankspaceCourseCreateInvoice" class="ns1blankspaceAction">' +
				...
				{
					aHTML.push('<tr><td><span id="ns1blankspaceCourseViewInvoice" class="ns1blankspaceAction">' +
			...
			.click(function() 
			{
				nsFreshcare.admin.newgrowers.init(oParam);
			});
	...
	details: function()
	{
		...
		if ($('#ns1blankspaceMainDetails').attr('data-loading') == '1')
		{
			...
			{	
				...
				if (nsFreshcare.user.role.toLowerCase() === 'trainer' 
					&& $('#ns1blankspaceDetailsStatus').attr('data-id') === nsFreshcare.data.trainingCourse.statusCompleted) {
				...
				var dToday = new Date();
				// v3.1.1a Was erroring when user didn't star from Training Package page
	...
	{		
		...
		send: 	function(oParam) 
		{
			...
			if (oParam.courseSaveStep === 1)
			{
				...
				oParam = ns1blankspace.util.setParam(oParam, 'onComplete', nsFreshcare.admin.trainingcourse.save.send);
	...
	createInvoice: function(oParam)
	{
		...
		// Get the id for financial account code 049
		else if (oParam.courseCreateInvoiceStep === 3)
		{
			...
			{
				oParam.financialAccount = $.map($.grep(ns1blankspace.financial.data.accounts, function(x) {return x.code === '049' && x.type === '2'}),
			...
			else
			{
				...
				oSearch.addFilter('code', 'EQUAL_TO', '049');
				...
				oSearch.getResults(function(oResponse)
				{
					...
					else
					{
						if (oResponse.data.rows.length === 0)
						{
							ns1blankspace.status.error('Invoice cannot be created. Financial Account Code 049 cannot be found.');
						}
						else
						{
		...
		{
			sData = 'description=' + ns1blankspace.util.fs('Royalty Payments for Training Course ' + ns1blankspace.objectContextData['agritrainingcourse.title'] + 
			...
			sData += '&contactbusinesssentto=' + ns1blankspace.objectContextData['agritrainingcourse.trainercontactbusiness'] +
					 '&contactpersonsentto=' + ns1blankspace.objectContextData['agritrainingcourse.trainercontactperson'] +
