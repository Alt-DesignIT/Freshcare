...
nsFreshcare.admin.newgrower = 
{
	...
	home: 		function (oParam, oResponse)
	{
		...
		if (oResponse == undefined)
		{
			...
			oSearch.addFilter('coursetext', 'IS_NOT_NULL');
			oSearch.rows = 80;
	...
	search: 	
	{
		send: 		function (sXHTMLElementId, oParam)
		{
			...
			if (sSearchContext != undefined && iSource != ns1blankspace.data.searchSource.browse)
			{
				...
				if (oNewGrowerRoot.data && oNewGrowerRoot.data.search && oNewGrowerRoot.data.search.fields)
				{
					sFields = oNewGrowerRoot.data.search.fields;
				}
			...
			else if (iShowStep == 2)
			{
					...
					|| ns1blankspace.objectContextData.trainercontactbusiness  == undefined || ns1blankspace.objectContextData.trainercontactperson === undefined)
				{
					...
					oSearch.addField('trainercontactbusiness,trainercontactbusinesstext,trainercontactperson,trainercontactpersontext' +
									',agritrainingcourse.package.codeofpractice,agritrainingcourse.package.codeofpracticetext');
	...
	summary: 	function ()
	{
		...
		else
		{
			...
			aHTML.push('<tr><td class="ns1blankspaceSummaryCaption">Training Date</td></tr>' +
						'<tr><td class="ns1blankspaceSummary">' +
						ns1blankspace.objectContextData['joindate'] +
						'</td></tr>');
	...
	save:
	{
		...
		send: function(oParam)
		{
			...
			else if (oParam.saveNewGrowerStep === 2)
			{
				...
				if (oParam.functionCollateData)
				{
					oParam.onComplete = nsFreshcare.admin.newgrower.save.send;
					oParam.functionCollateData(oParam);
				}
				else
				{
					...
					$.ajax(
					{
						...
						success: function(oResponse)
						{
							if (oResponse.status === 'OK')
							{
								...
								nsECA.admin.newgrower.createNewGrower.extendedTasks(oParam);
							}
							else
							{
