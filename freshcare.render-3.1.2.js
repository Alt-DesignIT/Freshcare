	...
	form:
	{
				...
				if (ns1blankspace.objectContext == -1)
				{
					$.each($.grep(aTabs, function(x) {return x.showOnNew === true}), function()
					{
						if (this.displayCondition == undefined
							...
							|| ($.type(this.displayCondition) == 'function' && this.displayCondition() === true))
						{
		...
		tabs: 
		{
			show: function(oParam)
			{
				...
				$('#ns1blankspaceMain' + oTabObject.name).click();
			...
			fill: function(oParam)
			{
				...
				$.each(aFields, function() 
				{
					...
					if (oThisField.type == ns1blankspace.option.typeInput)
					{
						...
						{
							$('#ns1blankspace' + oTabObject.name + oThisField.name).val(ns1blankspace.objectContextData[oThisField.column + 'text'].formatXHTML());
							$('#ns1blankspace' + oTabObject.name + oThisField.name).attr('data-id', ns1blankspace.objectContextData[oThisField.column]);
						}

						else if (oThisField.inputType === 'Radio')
						{
							$('[name="radio' + oThisField.name + '"][value="' + ns1blankspace.objectContextData[oThisField.column] + '"]').attr('checked', true);
						}

						else
						{
							$('#ns1blankspace' + oTabObject.name + oThisField.name).val(ns1blankspace.objectContextData[oThisField.column].formatXHTML());
						}
					}
					...
					else
					{
						...
						{
							$('#ns1blankspace' + oTabObject.name + oThisField.name).html(ns1blankspace.objectContextData[oThisField.column].formatXHTML());
						}
