...
nsFreshcare.admin.user = 
{
	...
	search: 	
	{
		...
		process:	function (oResponse)
		{
			...
			{	
				...
				{
					columns: 'contactbusinesstext-space-contactpersontext',
			...
			{ 
				...
				oSearch.getResults(function(oResponse) 
				{
					...
					{	
						...
						{
							// v3.1.1i SUP022764 Now allows muti-admin roles
							if ($.grep(oResponse.data.rows, function(x) {return x.roletext.toLowerCase() == 'admin'}).length > 0)
							{
								ns1blankspace.data.userRole = $.grep(oResponse.data.rows, function(x) {return x.roletext.toLowerCase() == 'admin'}).shift().role;
								ns1blankspace.data.userRoleText = 'Admin';
								ns1blankspace.data.roles = oResponse.data.rows;
							}
							else
							{
	...
	summary: 	function ()
	{
		...
		else
		{
			...
			if (ns1blankspace.data.userRole != undefined && nsFreshcare.user.role.toLowerCase() == 'admin' 
	...
	details: 	function ()
	{
		...
		if ($('#ns1blankspaceMainDetails').attr('data-loading') == '1')
		{
			...
			if (ns1blankspace.objectContext == -1)
			{
				...
				aHTML.push('<tr><td class="ns1blankspaceSub">Use this section to add external users (Growers,Customers, etc).</td></tr>');
	...
	{			
			...
			if (ns1blankspace.objectContextData != undefined)
			{
				...
				else
				{
						...
						|| (!nsFreshcare.freshcareAdministrator && ns1blankspace.data.userRoleText != 'Admin'))
					{
						...
						aHTML.push('<tr>' +
										'<td class="ns1blankspaceNothing" style="font-weight:600;">Click Add Role to define the user\'s logon, ie: Grower, Trainer, etc.</td></tr>');	
		...
		{
			if (oParam == undefined) {oParam = {userType: 1}}
			var iUserType = 1;
		...
		show:		function (oParam, oResponse)
		{
			...
			if (ns1blankspace.objectContextData != undefined)
			{
				...
				else
				{
					...
					// v3.1.207 SUP023058 Now restricts ermoving network groups
					if (nsFreshcare.freshcareAdministrator || nsFreshcare.supportAdmin
						|| (!nsFreshcare.freshcareAdministrator && ns1blankspace.data.userRoleText != 'Admin'))
					{
	...
	switchIn: 	function() 
	{
		...
		ns1blankspace.user.roles = [{title: ns1blankspace.data.userRoleText, id: ns1blankspace.data.userRole}];
