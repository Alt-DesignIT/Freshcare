	...
	searchGrower: 
	{
		show: function(oParam, oResponse) 
		{
			...
			if (sXHTMLElementID != undefined)
			{
				...
				if (sColumns === undefined)
				{
					...
					// v4.0.006 Was adding extra fields when data-columns specified, causing errors
					if (sMethod == 'CONTACT_PERSON_SEARCH') 	
					{
						var aColumnsToAdd = ['contactbusiness', 'contactbusinesstext', 'contactperson.contactbusiness.reference'];
						if (sColumns)
						{
							$.each(aColumnsToAdd, function(x, column)
							{
								if ($.inArray(column, sColumns.split('-')) == -1)
								{
									sColumns += ',' + column;
								}
							});
		...
		multiSelect: 
		{
			add: 	function(oParam) 
			{
				...
				{
					// v4.0.001 Bootstrap removed Delete label
					aHTML.push('<tr>' +
