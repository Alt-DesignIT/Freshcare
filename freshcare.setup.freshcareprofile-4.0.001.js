...
nsFreshcare.external.freshcareprofile = 
{
		...
		send: 		function (sXHTMLElementId, oParam)
		{
			...
			if (sSearchContext != undefined && iSource != ns1blankspace.data.searchSource.browse)
			{
				...
				oSearch.addField('contactperson.firstname,contactperson.surname,contactperson.title,contactperson.titletext,contactperson.position,contactperson.reference' +
								...
								(nsFreshcare.option.certBodyNumber ? ',contactperson.contactbusiness.se' + nsFreshcare.data.certificationBodyNumberId : ''));
	...
	layout: 	function ()
	{
		...
		{				
			// v3.3.005 Added JASANZ tab for CB's
			aHTML.push('<tr><td id="ns1blankspaceControlCertificateJASANZ" class="ns1blankspaceControl">' +
							'JASANZ</td></tr>');
		...
		$('#ns1blankspaceControlContacts').click(function(event)
		{
			...
			// v3.4.010 SUP023940 Was still looking at roleLower == 'grower'. Also added showCompetencies parameter
			var sStatusField = nsFreshcare.user.roleID == nsFreshcare.data.roles.auditor || nsFreshcare.user.roleID == nsFreshcare.data.roles.grower 
					? 'customer' : 'supplier';
			ns1blankspace.show({selector: '#ns1blankspaceMainContacts', context: {new: true, actionOptions: true, inContext: false}});
			nsFreshcare.external.grower.contacts.show({statusField: sStatusField, hideInactive: true, showCompetencies: (nsFreshcare.user.roleID == nsFreshcare.data.roles.auditor)});
		});
		...
		$('#ns1blankspaceControlAccreditation').click(function(event)
		{
			...
			if (nsFreshcare.user.roleID === nsFreshcare.data.roles.trainer) 
			{
				// v3.1.209b SUP022867 Now uses admin accreditation function
				//nsFreshcare.external.freshcareprofile.accreditation();
				nsFreshcare.admin.trainer.accreditation();
			}
			// v3.4.010 SUP023940 Show Training for Auditors
			else if (nsFreshcare.user.roleID === nsFreshcare.data.roles.auditor) 
			{
				nsFreshcare.internal.entity.training.show(
				{
					xhtmlElementID: 'ns1blankspaceMainAccreditation',
		...
		$('#ns1blankspaceControlCertificateJASANZ').click(function(event)
		{
			ns1blankspace.show({selector: '#ns1blankspaceMainJASANZ', context: {all: true, inContext: false}});
			nsFreshcare.admin.certificationbody.jasanz.show({business: ns1blankspace.objectContextData['contactperson.contactbusiness']});
		});
		
		$('#ns1blankspaceControlCertificateTemplate').click(function(event)
		{
			ns1blankspace.show({selector: '#ns1blankspaceMainCertificateTemplate', context: {all: true, inContext: false}});
			nsFreshcare.external.freshcareprofile.certificateTemplate();
		});
		
		$('#ns1blankspaceControlCARTemplate').click(function(event)
		{
			ns1blankspace.show({selector: '#ns1blankspaceMainCARTemplate', context: {all: true, inContext: false}});
	...
	template:
	{
		find: function(oParam)
		{
			var sTemplateVariable = ns1blankspace.util.getParam(oParam, 'templateVariable').value;
			var iDefaultTemplate = ns1blankspace.util.getParam(oParam, 'defaultTemplate').value;
			...
			var sURLFilter = ns1blankspace.util.getParam(oParam, 'urlFilter', {'default': ''}).value;

			// Templates are stored in a document with the url set to the contactbusiness id of the currently logged on user
			// If this document doesn't exist, get the default document so they can modify and save
			var oSearch = new AdvancedSearch();
			oSearch.method = 'DOCUMENT_SEARCH';
			oSearch.addField('content,type,object,title');
			if (iDocumentId === undefined)
			{
				oSearch.addFilter('url', 'EQUAL_TO', sURLFilter + ns1blankspace.user.contactBusiness);
			}
			else
			{
			...
			oSearch.getResults(function(oResponse)
			{
				...
				{	
					if (oResponse.data.rows.length > 0)
					{
						...
						{
							oParam.document = oResponse.data.rows[0].id;
							nsFreshcare.data[sTemplateVariable] = oParam.document;
							ns1blankspace.format.templates.data[iObject] =
							[
								{
									id: oParam.document,
									title: oResponse.data.rows[0].title.formatXHTML(),
									xhtml: oParam.documentContent,
									content: oResponse.data.rows[0].content 
								}
							];
							// Check if object & type are set correctly and 'convert' if not
							if (oResponse.data.rows[0].object != iObject || oResponse.data.rows[0].type != '10')
							{
								nsFreshcare.external.freshcareprofile.template.convert(oParam);
							}
							else
							{
								ns1blankspace.util.onComplete(oParam);
						...
						{
							oParam.document = iDefaultTemplate;
							ns1blankspace.util.onComplete(oParam);
						}
					}
					else
			...
			if (iDocumentId)
			{
				$.ajax(
				{
					...
					success: function(oResponse)
					{
						...
						{
							ns1blankspace.status.error('Unable to update template: ' + oResponse.error.errornotes);
						}
					}
				});
			}
	...
	certificateTemplate: function(oParam)
	{
		...
		if (oParam)
		{
			...
			if (oParam.document) {iDocumentId = oParam.document}
		}	
		else {oParam = {}}
		...
		if ($('#ns1blankspaceMainCertificateTemplate').attr('data-loading') == '1')
		{
			...
			aHTML.push('<table class="ns1blankspaceContainer">' +
						...
						'</tr>' + 
						'<tr class="ns1blankspaceContainer">' +
						'<td id="ns1blankspaceMainTemplate_Certificate" class="ns1blankspaceColumn1" data-loading="1"></td>' +
						'</tr>' + 
						'<tr class="ns1blankspaceContainer">' +
			...
			{
				nsFreshcare.external.freshcareprofile.template.find(
				{
					templateVariable: 'documentEmailCertificateTemplate',
					document: iDocumentId,
					object: nsFreshcare.objectCertificate,
					defaultTemplate: nsFreshcare.data.documentCertificateTemplateDefault,
					onComplete: nsFreshcare.external.freshcareprofile.certificateTemplate
				})
			}
			else
			{
				...
				// v2.0.4 SUP021408 Adedd email preview (send to CB)
				...
				{
					aHTML = [];
					aHTML.push('<table><tr><td style="width:75%;" id="ns1blankspaceProfileCertificateTemplateTitle">' +
								'</td>' +
					...
					$('#ns1blankspaceCertificateTemplatePreview')
						...
						.click(function()
						{
							// v3.2.005 SUP023190 Added replyto and fromemail
							var sData = 'to=' + ns1blankspace.util.fs(ns1blankspace.user.email) +
										...
										'&fromemail=' + ns1blankspace.util.fs(nsFreshcare.data.emailFromAuditor);
							$.ajax(
							{
								type: 'POST',
								...
								success: function(oResponse)
								{
									...
									{
										// v3.2.017 Was trying to show ns1blankspace.error.
										ns1blankspace.status.error('Email not sent: ' + oResponse.error.errornotes);
									}
								}
							});
						});
				...
				ns1blankspace.setup.financial.template.show(
				{
					document: iDocumentId,
					documentDefault: nsFreshcare.data.documentCertificateTemplateDefault,
					template: 'Certificate',
					//templateActual: 'Certificate Email Template - ' + ns1blankspace.user.contactBusinessText,
					object: nsFreshcare.objectCertificate,
					bracketCount: 1,
					url: ns1blankspace.user.contactBusiness
				})

				var sTitle = (iDocumentId == nsFreshcare.data.documentCertificateTemplateDefault)
								? 'Certificate Email Template - ' + ns1blankspace.user.contactBusinessText
								: sTitle;
				$('#ns1blankspaceProfileCertificateTemplateTitle').html('<input id="ns1blankspaceTemplateTitle_Certificate' +  + ns1blankspace.counter.editor + '"' +
						' class="ns1blankspaceText" style="display:none;"' +
						' value="' + sTitle.formatXHTML() + '">');
				...
				aHTML.push('<tr><td class="ns1blankspaceSummary" style="font-size:75%;"><br />' +
								((oParam.document === nsFreshcare.data.documentCertificateTemplateDefault) 
									...
									: '') + 
								'You can also insert TAGS into your email by clicking in the document where you<br />' +
								'want to insert the tag and then clicking on the tag.<br />' +
							'</td></tr></table>');
	...
	carTemplate: function(oParam)
	{
		...
		if ($('#ns1blankspaceMainCARTemplate').attr('data-loading') == '1')
		{
			...
			aHTML.push('<table class="ns1blankspaceContainer">' +
						...
						'</tr>' + 
						'<tr class="ns1blankspaceContainer">' +
						'<td id="ns1blankspaceMainTemplate_CAR" class="ns1blankspaceColumn1" data-loading="1"></td>' +
						'</tr>' + 
						'<tr class="ns1blankspaceContainer">' +
			...
			if (sDocumentContent == undefined || sDocumentContent === '')
			{
				...
				nsFreshcare.external.freshcareprofile.template.find(
				{
					urlFilter: 'carTemplate-', 
					templateVariable: 'documentEmailCARTemplate',
					document: iDocumentId,
					object: nsFreshcare.objectAuditIssue,
					defaultTemplate: nsFreshcare.data.documentCARTemplateDefault,
					onComplete: nsFreshcare.external.freshcareprofile.carTemplate
				})
			}
			else
			{
				...
				{
					aHTML = [];
					aHTML.push('<table><tr><td style="width:75%;" id="ns1blankspaceProfileCARTemplateTitle">&nbsp;</td>' +
				...
				{
					document: iDocumentId,
					documentDefault: nsFreshcare.data.documentCARTemplateDefault,
					template: 'CAR',
					object: nsFreshcare.objectAuditIssue,
					bracketCount: 1,
					url: 'carTemplate-' + ns1blankspace.user.contactBusiness
				})

				var sTitle = (iDocumentId == nsFreshcare.data.documentCARTemplateDefault)
								? 'CAR Email Template - ' + ns1blankspace.user.contactBusinessText
								: ns1blankspace.format.templates.data[nsFreshcare.objectAuditIssue][0].title;
				$('#ns1blankspaceProfileCARTemplateTitle').html('<input id="ns1blankspaceTemplateTitle_CAR' +  + ns1blankspace.counter.editor + '"' +
						' class="ns1blankspaceText" style="display:none;"' +
						' value="' + sTitle.formatXHTML() + 
						'">');
				...
				aHTML.push('<tr><td class="ns1blankspaceSummary" style="font-size:75%;"><br />' +
								((oParam.document === nsFreshcare.data.documentCARTemplateDefault) 
	...
	save: 		
	{
		...
		replaceContentImages: function(oParam, oResponse)
		{
			...
			var aContent = [];
			var oRoot = ns1blankspace.rootnamespace;
			var sURL = (ns1blankspace.isLab) 
						? window.location.origin 
						: 'https://app.1blankspace.com';
		...
		send: 	function (oParam)
		{
			...
			// v4.0.001 No longer saved here as there's now a button on the tab
			...
			*/
			...
			else if (ns1blankspace.okToSave && oParam.profileSaveStep === 5) 	
			{
				...
				{
					// v3.3.001 SUP023422: Note which user did the updates
					aMessageHTML.unshift(nsFreshcare.external.grower.save.addToEmail(
										{label: 'Record updated by user:',
										 oldValue: ns1blankspace.user.commonName, 
										 newValue: '(' + ns1blankspace.user.logonName + ')'
										}));
