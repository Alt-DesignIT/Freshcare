...
nsFreshcare.admin.certificate = 
{
	...
	certificatesToBeCreated: 
	{
		search: function(oParam) 
		{
			...
			else if (oParam.certificatesToBeCreatedStep === 1)
			{
				...
				oSearch.addField('audit.reference,audit.agrisubscription.membership.code,audit.auditbusiness,audit.auditbusinesstext,audit.contactbusinesstext,audit.contactpersontext,audit.type' +
								...
								',audit.agrisubscription.firstcertified');
		...
		validateCertificateData: function(oParam)
		{
			...
			else if (oParam.validateCertificateDataStep == 6)
			{
				...
				//&& oParam.validateCertificateData.anniversaryCheck == undefined)
						...
						[
							{
								...
								click: function()
								{
									...
									sVersionNumber = (Number(sPreviousVersion.split('-').pop()) + 1).toString();
									...
									oParam.certificateData.versionNumber = sPreviousVersion.split('-').shift() + '-' + 
																			sPreviousVersion.split('-').pop() + '-' + sVersionNumber;
	...
	getDifferencesHTML: function(oBefore, oAfter, sKey)
	{
		...
		var sBefore = (sKey != 'sites') ? oBefore[sKey] : $.map(oBefore.sites, function(x) {return x.streetCombined + ' ' + x.streetLocation}).join('<br />');
		var sAfter = (sKey != 'sites') ? oAfter[sKey] : $.map(oAfter.sites, function(x) {return x.streetCombined + ' ' + x.streetLocation}).join('<br />');
		var sHTML = '<tr>' +
	...
	certificateGetExpiry: function(oParam)
	{
		...
		{
			// SUP022693 If the Audit is in the 60 days before the beginning of the ExpiryAnniversary Month, (v3.3.001 SUP023474 changed from 2 mths to 60 days)
			...
			if (anniversaryStatus) 
			{
				...
				// Months 11 & 12 are when the audit is AFTER the recert audit month
				...
				// These months when the audit is too far BEFORE the anniversary month. Also if month 3 and iDaysToAnniversary > 60 
				else if ((iMonthsToAniversary >= 4 && iMonthsToAniversary <= 9) || iDaysToAnniversary > 60)
				{
					oAnniversary.status = 1;
					oAnniversary.message = 'Audit date is more than 60 days prior to Members set ReCert Audit Month.<br /><br />' +
									'Update to Re-Certification Audit Due required on the Member\'s Membership Page before continuing.<br /><br />' +
									'Contact Freshcare if assistance required.';
				}
				// The audit is 60 days or less before the anniversary month (months 1 or 2) or it's month 12
				else
				{
			...
			if (dAudit >= dAnniversaryStart.add({days: -60}))
		...
		if (iCertificate) 
		{
			...
			if (iCertificateGetDataStep === 1) 
			{
				...
				oSearch.addField('agricertificate.certificatenumber,agricertificate.externalcertificatenumber,agricertificate.enddate,agricertificate.dateissued,agricertificate.audit' +
								...
								',agricertificate.audit.sefullcertdata');
				...
				oSearch.getResults(function(oResponse) 
				{
					...
					if (oResponse.status === 'OK') 
					{
						if (oResponse.data.rows.length > 0)
						{
							...
							// v3.4.015 SUP023981 Added fullCertificateData condition
							oParam.certificateGetDataStep = (oParam.certificateData.auditType == nsFreshcare.data.audit.typeScopeExtension && oParam.certificateData.fullCertificateData != 'Y') ? 31 : 4;
						}
						else
						{
			...
			else if (iCertificateGetDataStep === 7)
			{
				...
				// Get values for auditor.selfCert if undefined
	...
	certificateGetTemplate: function(oParam)
	{
		...
		if (bValidParms)
		{
			...
			// v3.3.001 SUP023424 Now gets page 2 templatae as well
			else 
			{
		...
		if (oParam.html === undefined && oData != undefined) 
		{
			...
			oSearch.getResults(function(oResponse) {
				...
				if (oResponse.status === 'OK' && oResponse.data.rows.length > 0) 
				{
					...
					bSitesOnPg2 = (oData.sites.length > 10 || (oData.sites.length > 5 && oData.crops.length > 300));
