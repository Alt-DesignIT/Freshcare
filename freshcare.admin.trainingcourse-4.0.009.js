...
nsFreshcare.admin.trainingcourse = 
{
		...
		else
		{
			...
			else
			{
				...
				aHTML.push('<div class="ns1blankspaceCaption"' +
						' style="padding-left:8px; margin-bottom:10px; font-weight:500;">MOST RECENTLY SCHEDULED</div>');
	...
	show: 		function (oParam, oResponse)
	{
		...
		if (oParam.showStep == 1)
		{
			...
			else
			{
				...
				// v4.0.001 Replace invalid carriage return
				ns1blankspace.objectContextData['agritrainingcourse.details'] = ns1blankspace.objectContextData['agritrainingcourse.details'].replace(/\/r\/n/g, "\r\n")
	...
	details: function()
	{
		...
		if ($('#ns1blankspaceMainDetails').attr('data-loading') == '1')
		{
			...
			// v4.0.009 SUP024107 "datat-methodFilter" part ws missing
			if (nsFreshcare.user.roleID != nsFreshcare.data.roles.admin 
				&& $.grep(nsFreshcare.data.trainerStates, function(a) {return a.state === nsFreshcare.data.stateDistanceEducation}).length == 0) 
			{
				aHTML.push(' data-methodFilter="id-NOT_IN_LIST-' + nsFreshcare.data.stateDistanceEducation + '"');
			}
			aHTML.push('">' +
							'</td></tr>');

			// v4.0.009 SUP024107 Distance education no longer a selectable option
			aHTML.push('<tr class="ns1blankspaceCaption">' +
							'<td class="ns1blankspaceCaption">' +
							'Location' +
							'</td></tr>' +
							...
							'<input id="ns1blankspaceDetailsLocation" class="ns1blankspaceText"' +
								' data-caption="Location" data-mandatory="1">' +
							'</td></tr>');			
	...
	createInvoice: function(oParam)
	{
		...
		else if (oParam.courseCreateInvoiceStep === 3)
		{
			...
			{
				// v4.0.005 changed from 4-2100
				oParam.financialAccount = $.map($.grep(ns1blankspace.financial.data.accounts, function(x) {return x.code === '5017' && x.type === '2'}),
			...
			else
			{
				...
				oSearch.addFilter('code', 'EQUAL_TO', '5017'); 	// v4.0.004 Changed from 4-2100
				...
				oSearch.getResults(function(oResponse)
				{
					...
					else
					{
						if (oResponse.data.rows.length === 0)
						{
							ns1blankspace.status.error('Invoice cannot be created. Financial Account Code 5017 cannot be found.');
						}
						else
						{
	...
	trainees: 	
	{
			...
			if ($('#ns1blankspaceMainTrainees').attr('data-loading') == '1')
			{
				...
				if (oResponse === undefined && (oParam.response === undefined)) {
					...
					if (nsFreshcare.user.roleID === nsFreshcare.data.roles.admin || 
						...
						aHTML.push('<tr><td><span id="ns1blankspaceCourseTraineeAdd" class="ns1blankspaceAction">Add Trainee</span>');
					}

					aHTML.push('<tr><td><span id="ns1blankspaceCourseTraineesExport" class="ns1blankspaceAction">Export Trainees</span>');
					aHTML.push('</td></tr></table>');
					...
					if (bNew) 
					{
						...
						nsFreshcare.admin.trainingcourse.trainees.details(
						{
							dataTraineeID: -1,
					...
					$('#ns1blankspaceCourseTraineesExport').button(
					{
						...
						icons: {primary: "glyphicon-download"}
					})
					.click(function() {
					...
					$('#ns1blankspaceCourseTraineesAdd').css('width', $('#ns1blankspaceCourseTraineesExport').width() + 10);
		...
		row: 	function(oRow) 
		{
			...
			{
				aHTML.push('<td width><span id="ns1blankspaceTraineeRemove_' + oRow.id + '" ' + 
								...
								'class="ns1blankspaceAction ns1blankspaceTraineeOpen" title="Open"' + 
		...
		bind: 	function() 
		{
			...
			$('.ns1blankspaceRowCourseTrainee:visible').click(function(event) 
			{
			...
			if (bCanRemove) 
			{
				$('.ns1blankspaceTraineeRemove:visible').button(
				{
			...
			$('.ns1blankspaceTraineeOpen:visible').button(
			{
				icons: {primary: "glyphicon-play"}
			})
			.click(function(event) 
			{
				if (nsFreshcare.user.roleID === nsFreshcare.data.roles.trainer) {
