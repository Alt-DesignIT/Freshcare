...
nsFreshcare.admin.grower = 
{
	...
	home: 		function (oParam, oResponse)
	{
		...
		else
		{
			...
			else
			{
				...
				// Need to remove the duplicates caused by referencing persongroup
				var iPreviousGrower = '';
				...
				$.each(ns1blankspace.objectContextData.memberships, function()
				{
					...
					if ($('#ns1blankspaceMainMembership' + iSubscriptionId).html() != '')
					{
						...
						if (oSubscription['agrisubscription.status'] != $('#ns1blankspaceMembership' + iSubscriptionId + 'Status').attr('data-id'))
						{
							...
							// v3.1.2 SUP022442 If Memberhsip is being reinstated and user is disabled, re-enable them. 
							// v3.1.2 SUP022966 Also remove endddate from subscription
							else if ($('#ns1blankspaceMembership' + iSubscriptionId + 'Status').attr('data-id') === nsFreshcare.data.grower.subscriptionStatusRM 
										&& oSubscription['agrisubscription.status'] != nsFreshcare.data.grower.subscriptionStatusRM)
							{
								if (ns1blankspace.objectContextData['contactbusiness.contactperson.user.id'] != ''
								...
								// v3.1.2 SUP022433 Remove End date when reinstating
	...
	membership: 
	{
		...
		{
			var sStatusElmentID = $('.nsFreshcareShowOnChangeStatus').last().children().first().children().first().attr('id').replace('ChangedByPerson', '');
			var iMembershipID = sStatusElmentID.split('ns1blankspaceMembership').pop();
			iMembershipID = iMembershipID.split('Status').shift();
			var sSavedStatus = ''
			...
			if (iMembershipID != undefined && iMembershipID != '')
			{
				...
				{
					if ($('#' + sStatusElmentID).attr('data-id') != sSavedStatus)
					{
						$('.nsFreshcareShowOnChangeStatus').show();
					}
				}
			}
		},
		...
		defaultCodeOfPractice: function(oParam)
		{
			...
			if (iMembershipID === undefined) 
			{
				...
				sXHTMLElementID = ns1blankspace.xhtml.divID;
			}
			...
			if (iMembershipID)
			{
				...
				oSearch.getResults(function(oResponse)
				{
					...
					{
						$('#' + sXHTMLElementID + 'COP').attr('data-id', oResponse.data.rows[0].id);
						$('#' + sXHTMLElementID + 'COP').val(oResponse.data.rows[0].description);
					}
				});
			}
		},
				...
				$.ajax(
				{
					...
					success: function(oResponse)
					{
						...
						if (oResponse.status == 'OK')
						{
							...
							if (oMembershipTransactions.length > 1 && oMembership['agrisubscription.status'] != oMembershipTransactions[1].tostatus)
							{
