...
nsFreshcare.admin.report = 
{
	...
	initData: function(oParam)
	{
		...
		if (ns1blankspace.report.reports === undefined || ns1blankspace.report.reports.length === 0)
		{
			...
			[
				...
				{	/* FSQ Report 
					v3.1.2 SUP022744 Added AgriScope to this report */
					name: 'FSQ Report',
					group: 1,
					object: nsFreshcare.objectSubscription,
					...
					returnParameters: 'agrisubscription' +
								...
								',agrisubscription.codeofpractice' +
								',agrisubscription.agricertificate' +
								',agrisubscription.agriobjectscope' +
								',agrisubscription.status',
					functionSearch: nsFreshcare.admin.grower.search.send,
					windowOpen: '/#/nsFreshcare-admin.grower/id:',
					summary: 'Report of Growers, Audits and Training Courses',
					scriptOnNewPage: nsFreshcare.admin.report.newPage.fsqReport,
					showSort: true,
					customExportFormat:
					{
						headers:
						...
						[
							{
								...
								type: 'Check',
								caption: 'Training Course Mandatory?',
								mandatory: false,
								"default": false
							},
							{
					...
					fixedParameters:
					{
						fields:
						[
							{name: 'agrisubscription.contactbusiness.id', hidden: true},
							{name: 'agrisubscription.membership.id', hidden: true},
							{name: 'agrisubscription.contactbusiness.tradename'},
							{name: 'agrisubscription.contactbusiness.contactperson.firstname'},
							{name: 'agrisubscription.contactbusiness.contactperson.surname'},
							{name: 'agrisubscription.contactbusiness.addresslink.address.address1'},
							...
							{name: 'agrisubscription.agricertificate.enddate'},
							{
								name: 'agrisubscription.trainingcodeofpracticetext',
								headerOnly: true,
								processFunction: nsFreshcare.admin.report.newPage.fsqReport,
								headerSortDisabled: true
							},
							{
								name: 'agrisubscription.trainingtrainercontactbusinesstext',
								headerOnly: true,
								processFunction: nsFreshcare.admin.report.newPage.fsqReport,
								headerSortDisabled: true
							},
							{
								name: 'agrisubscription.trainingtrainercontactpersontext',
								headerOnly: true,
								processFunction: nsFreshcare.admin.report.newPage.fsqReport,
								headerSortDisabled: true
							},
							...
							{name: 'agrisubscription.codeofpracticetext'},
							{
								name: 'agrisubscription.auditactualdate',
								headerOnly: true,
								processFunction: nsFreshcare.admin.report.newPage.fsqReport,
								headerSortDisabled: true
							},
							{
								name: 'agrisubscription.auditstatustext',
								headerOnly: true,
								processFunction: nsFreshcare.admin.report.newPage.fsqReport,
								headerSortDisabled: true
							},
							{
								name: 'agrisubscription.auditauditbusinesstext',
								headerOnly: true,
								processFunction: nsFreshcare.admin.report.newPage.fsqReport,
								headerSortDisabled: true
							},
							{
								name: 'agrisubscription.auditauditpersontext',
								headerOnly: true,
								processFunction: nsFreshcare.admin.report.newPage.fsqReport,
								headerSortDisabled: true
							}
					...
					selectableParameters:
					{
						fields:
						[
							{name: 'agrisubscription.membershiptext', hidden: true},
							{name: 'agrisubscription.contactbusiness.tradename', hidden: true}
						],
						sort: {name: 'agrisubscription.contactbusinesstext', direction: 'asc'}
					}
				},
				{
					name: 'CARs',
		...
		if (ns1blankspace.report.dictionary === undefined || ns1blankspace.report.dictionary.length == 0)
		{
			...
			[
				...
				{name: "contactbusiness.contactperson.mailingaddress1", caption: "Mail Address 1"},
				{name: "contactbusiness.contactperson.mailingaddress2", caption: "Mail Address 2"},
				{name: "contactbusiness.contactperson.mailingsuburb", caption: "Mail Suburb"},
				{name: "contactbusiness.contactperson.mailingstate", caption: "Mail State"},
				{name: "contactbusiness.contactperson.mailingpostcode", caption: "Mail Post Code"},
				{name: "contactbusiness.contactperson.mailingcountry", caption: "Mail Country"},
				...
				{name: "contactbusiness.agrisubscription.agricertificate.sent", caption: "Certificate Sent?"},
				...
				{name: 'agrisubscription.agricertificate.certificatenumber', caption: 'Freshcare Certification Number'},
	...
	initUnsentCertificatesReport: function(oParam)
	{
		...
		if ($.grep(ns1blankspace.report.reports, function(x) {return x.name === 'Unsent Mailed Certificates'}).length === 0)
		{
			ns1blankspace.report.reports.push(
				{
					...
					/*customExportFormat: {name: "Export Certificate Data"},*/
					fixedParameters:
					{
						fields:
						[
			...
			(
				...
				{name: "agricertificate.subscription.contactbusiness.contactperson.mailingaddresscombined", caption: 'Mailing Address'},
	...
	subscriptionPerformanceSummary:
	{
		data: 
		{
			...
			[
				...
				{id: '2', label: 'Trainer', column: 'agrisubscription.contactbusiness.relationshipotherbusiness.contactbusinesstext'},
				{id: '3', label: 'Certification Body', column: 'agrisubscription.contactbusiness.relationshipotherbusiness.contactbusinesstext'}
			]
		},
		...
		search: function(oParam, oResponse)
		{
			...
			if (oParam.reportSearchStep === 1)
			{
				if (oResponse === undefined)
				{
					...
					if (ns1blankspace.okToSave === true)
					{
						...
						ns1blankspace.status.working('Finding subscription records. Rows 1 to 500...');
						...
						{
							case '1':
								// By State: Change this for business-centric
								oSearch.addField('agrisubscription.contactperson.streetstate,count(agrisubscription.contactperson.streetstate) GroupCount');
								oSearch.addFilter('agrisubscription.contactbusiness.id', 'IS_NOT_NULL');
								//oSearch.addFilter('agrisubscription.addresslink.address.addressstate,count(agrisubscription.addresslink.address.addressstate) GroupCount');
								//oSearch.addFilter('agrisubscription.addresslink.address.status', 'EQUAL_TO', '1');		// We only want the prinary address
								break;

							case '2':
								// By Trainer
								// Search for all subscriptions & related trainers before the enddate - then process these records in step 2
								oSearch.addField('contactbusiness,contactbusinesstext,contactperson,contactpersontext,membership,codeofpractice' +
												',agrisubscription.contactbusiness.relationshipotherbusiness.contactbusiness' +
												',agrisubscription.contactbusiness.relationshipotherbusiness.contactbusinesstext');
								oSearch.addFilter('agrisubscription.contactbusiness.id', 'IS_NOT_NULL');
								oSearch.addFilter("agrisubscription.contactbusiness.relationshipotherbusiness.type", "EQUAL_TO", nsFreshcare.data.relationshipTrainer);
								oSearch.addFilter("agrisubscription.contactbusiness.relationshipotherbusiness.startdate", "LESS_THAN_OR_EQUAL_TO", dEnd.toString("dd MMM yyyy"));
								oSearch.addBracket("(");
								oSearch.addFilter("agrisubscription.contactbusiness.relationshipotherbusiness.enddate", "GREATER_THAN_OR_EQUAL_TO", dEnd.toString("dd MMM yyyy"));
								oSearch.addOperator("or");
							...
							case '3':
								// By CB
								// Search for all subscriptions & related CB's before the enddate and group by CB
								oSearch.addField('agrisubscription.contactbusiness.relationshipotherbusiness.contactbusinesstext,count(id) GroupCount');
				...
				else
				{
					if (oResponse.status === 'OK')
					{
						...
						{
							oParam.reportSearchStep = (iOption == '2') ? 2 : 10;
			...
			// If Trainers, we need to do a second search to find the trainer who trained them for the current membership 
			else if (oParam.reportSearchStep === 2)
			{
				if (oParam.resultIndex === undefined) {oParam.resultIndex = 0; nsFreshcare.admin.report.subscriptionPerformanceSummary.data.traineeData = []}
				var aContactBusinesses = [];

				if (oParam.resultIndex < nsFreshcare.admin.report.subscriptionPerformanceSummary.data.searchResults.length)
				{
					if (oResponse == undefined)
					{
						// Get the list of contactbusinesses we need to search for
						for (var i = 0; i < 500; i++)
						{
							if (oParam.resultIndex + i < nsFreshcare.admin.report.subscriptionPerformanceSummary.data.searchResults.length)
							{
								aContactBusinesses.push(nsFreshcare.admin.report.subscriptionPerformanceSummary.data.searchResults[i + oParam.resultIndex].contactbusiness);
							}
							else {i = 500;}
						}

						ns1blankspace.status.working('Finding trainers who trained growers. Rows ' + oParam.resultIndex + ' to ' + (oParam.resultIndex + 500) + '...');

						// Now find the trainer who last trained the grower for the current membership
						var oSearch = new AdvancedSearch();
						oSearch.method = 'AGRI_EDUCATION_TRAINING_COURSE_ATTENDEE_SEARCH ';
						oSearch.addField('agritrainingcourseattendee.course.trainercontactbusiness,agritrainingcourseattendee.course.trainercontactbusinesstext,' +
										'traineecontactbusiness,traineecontactbusinesstext,agritrainingcourseattendee.course.coursedate,agritrainingcourseattendee.course.package.membership');
						oSearch.addFilter('traineecontactbusiness', 'IN_LIST', aContactBusinesses.join(','));
					...
					else
					{
						if (oResponse.status === 'OK')
						{
							$.each(oResponse.data.rows, function() {nsFreshcare.admin.report.subscriptionPerformanceSummary.data.traineeData.push(this)});
						...
						{
							ns1blankspace.status.error('Error finding trainers: ' + oResponse.error.errornotes);
						}
					}

				}
				// We have all the data we need so let's go to the next step
				else
				{
			...
			// Let process the Trainers data. 
			else if (oParam.reportSearchStep === 3)
			{
				// First, remove duplicate training course rows from traineeData - we only want to know about the most recent training for each subscription
				var iPreviousSubscription = '';
				nsFreshcare.admin.report.subscriptionPerformanceSummary.data.uniqueTrainees = [];
				$.each(nsFreshcare.admin.report.subscriptionPerformanceSummary.data.traineeData, function()
				{
					if (this.traineecontactbusiness + '+' + this['agritrainingcourseattendee.course.package.membership'] != iPreviousSubscription)
					{
						nsFreshcare.admin.report.subscriptionPerformanceSummary.data.uniqueTrainees.push(this);
					}
					iPreviousSubscription = this.traineecontactbusiness + '-' + this['agritrainingcourseattendee.course.package.membership'];
				});

				// Now loop thru searchResults. If grower exists in uniqueTrainees with the same trainer, then add to outputData
				nsFreshcare.admin.report.subscriptionPerformanceSummary.data.outputData = [];
				$.each(nsFreshcare.admin.report.subscriptionPerformanceSummary.data.searchResults, function()
				{
					var oThisSubscription = this;
					if ($.grep(nsFreshcare.admin.report.subscriptionPerformanceSummary.data.uniqueTrainees, function(x)
					{
						return x.traineecontactbusiness === oThisSubscription.contactbusiness 
							&& x['agritrainingcourseattendee.course.trainercontactbusiness'] === oThisSubscription['agrisubscription.contactbusiness.relationshipotherbusiness.contactbusiness'];
					}).length > 0)
					{
						nsFreshcare.admin.report.subscriptionPerformanceSummary.data.outputData.push(oThisSubscription);
					}
				});

				// Now we need to group the outputData into summaryData
				nsFreshcare.admin.report.subscriptionPerformanceSummary.data.summaryData = [];
				$.each(nsFreshcare.admin.report.subscriptionPerformanceSummary.data.outputData, function()
				{
					var oThisSubscription = this;
					var iIndex = 0;

					// First check if we need to add this trainer to summaryData
					if ($.grep(nsFreshcare.admin.report.subscriptionPerformanceSummary.data.summaryData, function(x) 
					{
						return x['agrisubscription.contactbusiness.relationshipotherbusiness.contactbusinesstext'] == oThisSubscription['agrisubscription.contactbusiness.relationshipotherbusiness.contactbusinesstext'];
					}).length === 0)
					{
						nsFreshcare.admin.report.subscriptionPerformanceSummary.data.summaryData.push(
						{
							'agrisubscription.contactbusiness.relationshipotherbusiness.contactbusinesstext': oThisSubscription['agrisubscription.contactbusiness.relationshipotherbusiness.contactbusinesstext'],
							groupcount: 0							
						});
						iIndex = nsFreshcare.admin.report.subscriptionPerformanceSummary.data.summaryData.length - 1;
					}
					else		// Find the index of the curent trainer's row in summaryData
					{
						iIndex = $.grep($.map(nsFreshcare.admin.report.subscriptionPerformanceSummary.data.summaryData, function(x, index) {return {index: index, trainer: x['agrisubscription.contactbusiness.relationshipotherbusiness.contactbusinesstext']}}),
							function(y) {return y.trainer === oThisSubscription['agrisubscription.contactbusiness.relationshipotherbusiness.contactbusinesstext']}).shift().index;
					}

					nsFreshcare.admin.report.subscriptionPerformanceSummary.data.summaryData[iIndex].groupcount += 1;
				});

				// Now sort SummaryData by Trainer name
				nsFreshcare.admin.report.subscriptionPerformanceSummary.data.summaryData.sort(ns1blankspace.util.sortBy('agrisubscription.contactbusiness.relationshipotherbusiness.contactbusinesstext'));
		...
		process: function(oParam)
		{
			...
			//ns1blankspace.report.data.resultRows = oResponse.data.rows;		// v3.1.1 SUP022435 Need to export directly from this data
	...
	subscriptionPerformance:
	{
			...
			else if (oParam.reportSearchStep === 2)
			{
				...
				if (oParam.monthIndex < Object.keys(aMonths).length)
				{
					...
					oSearch.getResults(function(oResponse)
					{
						...
						{
							$.each(oResponse.data.rows, function() 
							{nsFreshcare.admin.report.subscriptionPerformance.data.extraData["month" + aMonths[oParam.monthIndex].year + '-' + aMonths[oParam.monthIndex].month].push(this)});
		...
		process: function(oParam)
		{
			...
			var aMonthsTotal = {id: '', reference: 'NEW', title: 'NEW Members'};
			$.each(aMonthsToSearch, function(monthIndex, month)
			{
				var sMonthRef = "month" + month.year + '-' + month.month;
				aMonthsTotal[month.year.toString() + month.month.toString()] = $.grep(nsFreshcare.admin.report.subscriptionPerformance.data.extraData[sMonthRef], function(x)
									{
...
nsFreshcare.admin.setup = {
	...
	exports: 	function() 
	{
		...
		if ($.grep(ns1blankspace.setup.file["export"].formats, function(x) {return x.name === 'Export Certificate Data'}).length === 0) 
		{
