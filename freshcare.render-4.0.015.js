	...
	form:
	{
		...
		processSearchObject: function(oSearchParms)
		{
			...
			if (oSearchParms)
			{
				...
				// v4.0.015 No longer processes applytosubsearch parameter
				if (oSearchParms.filters)
				{
					$.each(oSearchParms.filters, function()
					{
						...
						oSearch.addFilter(this.name, this.comparison, this.value1, this.value2, this.value4)
		...
		home: function(oParam, oResponse)
		{
			...
			else
			{
				...
				{
					// v4.0.001 Bootstrap
					aHTML.push('<div class="ns1blankspaceCaption" style="padding-left:8px; margin-bottom:10px; font-weight:500;">' + 
							(oDisplay.titleText ? oDisplay.titleText : 'MOST LIKELY.') + '</div>');
		...
		tabs: 
		{
				...
				if (aFields != undefined)
				{
					...
					if (bContinue)
					{
						...
						$.each(aColumns, function()
						{
							...
							// v3.4.019 SUP023960 Added type as a function
							$.each($.grep(aFields, function(x) {return x.displayColumn == iColumn}), function()
							{
								...
								var iType = ($.type(this.type) == 'function' ? this.type() : this.type);

								if (bDisplay)
								{
									// Input field
									if (iType == ns1blankspace.option.typeInput)
									{
										// Caption
										if (oThisField.label)
										{
									...
									// Display only field
									else if (iType == ns1blankspace.option.typeReadOnly)
									{
										// Caption
										if (oThisField.label)
										{
									...
									// Buttons / actions
									else if (iType == ns1blankspace.option.typeButton)
									{
										aHTML.push('<tr><td id="ns1blankspace' + oTabObject.name + oThisField.name + '"' +
									...
									else if (iType == ns1blankspace.option.typeFiller)
									{
							...
							{
								// v4.0.009 SUP024114 CHanged initialisation of datetiem
			...
			fill: function(oParam)
			{
				...
				$.each(aFields, function() 
				{
					...
					// Input field
					// v3.4.019 SUP023690 Now accepts a funciton for type
		...
		save:
		{
			...
			send: function(oParam)
			{
				...
				else if (oParam.saveStep == 2)
				{
					...
					// v3.2.010 Only save if it's allowed to be displyed
					$.each($.grep(aTabs, function(x) {return x.save && x.save.root === true}), function(i, tab)
					{
						var bDisplay = ($.type(tab.displayCondition) == 'function' ? tab.displayCondition() : true);
						if (bDisplay)
						{
