...
nsFreshcare.external.grower = 
{
	details: 	function ()
	{
		...
		if ($('#ns1blankspaceMainDetails').attr('data-loading') == '1')
		{
			...
			if (bAuditorGrower) 
			{
				...
				// SUP023894 New fields required for new Growers from Auditor
				else
				{
					...
					aHTML.push('<tr class="ns1blankspaceCaption">' +
									...
									'<input id="ns1blankspaceDetailsMembershipsUpdate" class="ns1blankspaceSelect"' +
										' data-multiSelect="true"' +
										' data-method="AGRI_MEMBERSHIP_SEARCH"' +
										' data-methodFilter="status-EQUAL_TO-' + nsFreshcare.data.membershipStatusActive + '">' +
									'</td></tr>');			
					...
					aHTML.push('<tr class="ns1blankspaceCaption">' +
									...
									'</td></tr>' +
									'<tr><td class="ns1blankspaceText">' +
									'<input id="ns1blankspaceDetailsCertificateNumberUpdate" class="ns1blankspaceText">' +
									'</td></tr>');
					...
					aHTML.push('<tr class="ns1blankspaceCaption">' +
									...
									'</td></tr>' +
									'<tr><td class="ns1blankspaceText">' +
									'<input id="ns1blankspaceDetailsStateUpdate" class="ns1blankspaceSelect"' +
										' data-method="SITE_STATE_SEARCH"' +
										' data-methodFilter="title-NOT_EQUAL_TO-Distance Education">' +
									'</td></tr>');
					...
					aHTML.push('<tr class="ns1blankspaceCaption">' +
									...
									'Has audit Occurred?' +
									'</td></tr>' +
									'<tr><td id="ns1blankspaceDetailsAuditOccurredUpdate" class="ns1blankspaceRadio">' +
									'<input type="radio" id="radioAuditOccurred1" name="radioAuditOccurred" data-text="Yes" value="Y"/>Yes&nbsp;' +
									'<input type="radio" id="radioAuditOccurred2" name="radioAuditOccurred" data-text="No" value="N"/>No&nbsp;&nbsp;&nbsp;' +
									'<span id="ns1blankspaceDetailsAuditDateLabel" class="ns1blankspaceDetailsAuditDate"' +
										' style="display:none;">Date</span>&nbsp;' +
									'<input id="ns1blankspaceDetailsAuditDateUpdate" class="nsFreshcareDate ns1blankspaceDetailsAuditDate"' +
			...
			ns1blankspace.util.initDatePicker({select: 'input.nsFreshcareDate'});

			// v4.0.006 SUP023894 bind Audit Occurred field for AuditorGrower
			$('[name="radioAuditOccurred"').on('click', function()
			{
				if ($(this).val() == 'Y')
				{
					$('.ns1blankspaceDetailsAuditDate').show();
				}
				else
				{
					$('.ns1blankspaceDetailsAuditDate').hide();
					$('#ns1blankspaceDetailsAuditDateUpdate').val('');
				}
			});
	...
	postMembershipClick: function(oParam)
	{
		...
		// v4.0.001 Was erroring when xhtml.divID was blank
		var iMembership = ns1blankspace.util.getParam(oParam, 'membership', 
						{'default': (ns1blankspace.xhtml.divID ? $('#' + ns1blankspace.xhtml.divID.replace('#', '')).attr('data-id') : '')}).value;
	...
	address: 	
	{
		show:	function (oParam, oResponse)
		{
			...
			if ($('#ns1blankspaceMainAddress').attr('data-loading') == '1')
			{
				...
				else 
				{
					...
					// v4.0.001 Bootstrap added tooltip
					...
					aHTML.push('<tr class="ns1blankspaceCaption">' +
									...
									'<span id="ns1blankspaceSiteAdd" class="ns1blankspaceAction"' +
										' title="Add Site Address" data-toggle="tooltip">&nbsp;</span>' +
									'</td></tr>');
					...
					if (ns1blankspace.objectContextData != undefined)
					{
						...
						if (oNS == undefined || !(oNS.readOnly == true))
						{
							...
							// v4.0.001 Removed label, added title
							if (bCanChangeStatus)
							{
								$('.ns1blankspaceGrowerAddressChangeStatus').each(function()
								{
									...
									{
										$(this)
										.button({text: false, icons: {primary:'glyphicon-minus'}})
										.attr('title', 'Inactivate');
									}
								});
				...
				if ($('#ns1blankspaceAddressManageDetails').attr('data-id') === '-1')
				{
					if (!bAdd)
					{
						...
						ns1blankspace.container.confirm(
						{
							...
							buttons: 
							[
								{text: 'OK', name: 'btnOK', click: function() {bContinue = true; $('#ns1blankspaceBootstrapDialog').modal('hide')}},
								{text: 'Cancel', name: 'btnCancel', click: function() {bContinue = false; $('#ns1blankspaceBootstrapDialog').modal('hide')}}
							]
						});
					}
					else
		...
		row: function(oParam)
		{
			...
			if (oRow.status != '1' && bCanChangeStatus)
			{
				aHTML.push('<td class="ns1blankspaceAction ns1blankspaceGrowerAddressChangeStatus"' +
							...
							' title="' + (bNew ? 'Remove' : (oRow.status === '2') ? 'Inactivate' : 'Activate') + '" data-toggle="tooltip"' + 
							'></td>');
				...
				aHTML.push('<td class="ns1blankspaceAction ns1blankspaceGrowerAddressChangeStatus"' +
							...
							' title="' + ((oRow.status === '2') ? 'Remove' : 'Remove') + '"' +
							'></td>');
		...
		manageRow: function(oParam) 
		{
			...
			{
				// v4.0.001 Bootstrap updates
				$('#ns1blankspaceSiteRow_' + sID).children()
					.removeClass('nsFreshcareInactive')
					.css('text-decoration', '')
					.css('cursor', 'pointer')
					.addClass('ns1blankspaceGrowerAddressRowEdit');
	...
	memberships: 
	{
		search: function (oParam, oResponse) 
		{
			...
			if (iStep === 1 && iContactBusiness && ns1blankspace.objectContextData) 
			{
				...
				// v4.0.012 Sup024144 Added extensionmonths and type for certificate audit
				...
				oSearch.addField('agrisubscription.membership,agrisubscription.membershiptext,agrisubscription.status,agrisubscription.statustext,agrisubscription.firstcertified' + 
								...
								',agrisubscription.agrilastaudit.resultstatus,agrisubscription.agricertificate.audit.seextensionmonths' +
								',agrisubscription.agricertificate.audit.type' +
								sAddField);
