...
nsFreshcare.report = 
{
	init: function (oParam)
	{
		...
		// v3.2.019a Was showing for Trainers & Customers!!! 
		if (nsFreshcare.option.exportToXero && nsFreshcare.user.roleID == nsFreshcare.data.roles.xeroSuperUser)
		{
		...
		if ($.grep(ns1blankspace.report.reports, function(x) {return x.name == 'Saved Emails and SMS'}).length > 0)
		{
			$.each(ns1blankspace.report.reports, function(i, report)
			{
				if (report.name == 'Saved Emails and SMS')
				{
					if (report.selectableParameters && report.selectableParameters.fields)
					{
						if ($.grep(report.selectableParameters.fields, function(x) {return x.name == 'action.createddate'}).length == 0)
						{
							report.selectableParameters.fields.push({name: 'action.createddate', caption: 'Date Sent / Saved'});
							report.selectableParameters.fields.push({name: 'action.createdusertext', caption: 'Sent / Saved By'});
						}
					}
				}
			});
