...
nsFreshcare.customer.grower = 
{
		...
		else
		{
			...
			if (ns1blankspace.objectContextData.memberships && ns1blankspace.objectContextData.memberships.length > 0) 
			{
				...
				$.each(ns1blankspace.objectContextData.memberships, function() 
				{
					...
					var bShowCert = !(this['agrisubscription.status'] === nsFreshcare.data.grower.subscriptionStatusWD 
									|| this['agrisubscription.status'] === nsFreshcare.data.grower.subscriptionStatusFM
									|| this['agrisubscription.status'] === nsFreshcare.data.grower.subscriptionStatusSP);
				...
				if (bAuditPaidNote)
				{
					// v3.3.006 SUP023828 Changed text
					aHTML.push('<tr><td class="ns1blankspaceCaption" style="font-size: 0.75em;">' +
									'* The Certification Body has not yet released the certificate for this business. ' +
									'The business must follow-up with their Certification Body directly to have this resolved.' +
								'</td></tr>');
				}
			} 
			else {
		...
		if ($('#ns1blankspaceMainDetails').attr('data-loading') == '1')
		{
			...
			aHTML.push('<tr class="ns1blankspaceCaption">' +
							...
							'<input id="ns1blankspaceDetailsSupplierBusiness" class="nsFreshcareSelectGrower"' +
								...
								' data-method="CONTACT_BUSINESS_SEARCH"' +
								' data-columns="tradename-space-hyphen-space-contactbusiness.addresslink.address.addresssuburb"' +
			...
			aHTML.push('<table id="ns1blankspaceSummaryActions" class="ns1blankspace"');

			aHTML.push('<tr><td><span id="ns1blankspaceDetailsSearchSupplier" style="font-size:0.75em;width:100px;">Search</span></td>');
			aHTML.push('<tr><td style="width:10px;">&nbsp;</td></tr>');
			aHTML.push('<tr><td><span id="ns1blankspaceDetailsClearSupplier" style="font-size:0.75em;width:100px;">Clear</span></td></tr>');
			aHTML.push('<tr><td style="width:10px;">&nbsp;</td></tr>');
			aHTML.push('<tr><td id="ns1blankspaceDetailsLinkMessage" style="color:red;"></td></tr>');
			aHTML.push('</table>');
	...
	setGrowerDetails: 	function() 
	{
		...
		// v3.3.001 SUP023608 Now checks to see if link request already exists
		nsFreshcare.data.grower.email = '';		// v1.0.22 Clear grower email before searching
		
		$('#ns1blankspaceDetailsLinkMessage').html('');
		...
		if ($('#ns1blankspaceDetailsSupplierPerson').attr('data-id') != undefined) 
		{
			...
			oSearch.addField('contactperson.agrisubscription.agricertificate.certificatenumber,contactperson.contactbusiness.addresslink.address.addresssuburb' +
				...
				',contactperson.contactbusiness.seallowlinkrequests');
			...
			oSearch.getResults(function(oResponse) 
			{
				if (oResponse.status === 'OK') 
				{
					if (oResponse.data.rows.length >= 1) 
					{
						...
						$('#ns1blankspaceDetailsSupplierBusiness').attr('data-allowRequests', oResponse.data.rows[0]['contactperson.contactbusiness.seallowlinkrequests']);
						...
						oSearch.addField('contactbusiness,othercontactbusiness,contactbusinesstext,othercontactbusinesstext');
						...
						oSearch.addFilter('linktype', 'EQUAL_TO', nsFreshcare.data.relationshipSupplier);
						oSearch.getResults(function(oResponse)
						{
							if (oResponse.status == 'OK')
							{
								if (oResponse.data.rows.length > 0)
								{
									$('#ns1blankspaceDetailsLinkMessage').html('A request for you to be linked to this business has already been added' +
														' and is pending the ' + nsFreshcare.data.growerText + "'s approval.");
								}
							}
						});
					}
				...
				{
					ns1blankspace.status.error('Unable to find Business information: ' + oResponse.error.erronotes);
				}
			});
		}
	},
		...
		if ($(ns1blankspace.xhtml.container).is(':visible')) {
			...
			if (iStep === 1 && oResponse === undefined) 
			{
				...
				oSearch.addField('contactbusiness.contactperson.firstname,contactbusiness.contactperson.surname,contactbusiness.tradename,contactbusiness.contactperson.id' +
								',contactbusiness.reference,contactbusiness.addresslink.address.addresssuburb,seallowlinkrequests' +
			...
			else if (iStep === 2 && oResponse) 
			{
				...
				// v3.3.001 SUP023608 Now searches for and sets allowlinkrequests
				var aHTML = [];
				aHTML.push('<table class="ns1blankspaceSearchMedium">');
					...
					$.each(oResponse.data.rows, function() {
						...
						if (sPreviousId != this.id) {
							aHTML.push('<tr id="ns1blankspaceSupplierSearchRow_' + this['contactbusiness.contactperson.id'] + '"' +
										' data-allowRequests="' + this.seallowlinkrequests + '">');
				...
				$('td.ns1blankspaceSearch').click(function() {
					...
					var sBusinessId = this.id.split('_').pop();
					var sPersonId = $(this).parent().attr('id').split('_').pop();
					...
					if (sBusinessId) {
						...
						$('#ns1blankspaceDetailsSupplierBusiness').attr('data-allowRequests', $(this).parent().attr('data-allowRequests'));
	...
	save: 		
	{
		validate: 	function() 
		{
			...
			// v3.3.001 SUP023608 Now checks that link request doesn't already exist
			if ($('#ns1blankspaceDetailsLinkMessage').html() != '')
			{
				ns1blankspace.status.error('Link has already been requested.');
		...
		send: 		function (oParam)
		{
				...
				if (ns1blankspace.okToSave) 
				{
					...
					oSearch.getResults(function(oResponse) 
					{
						if (oResponse.status === 'OK') 
						{
							...
							$.each(oResponse.data.rows, function() 
							{
								// v3.3.001 BEFORE CHANGING THIS SECION, MAKE SURE THE SCENARIO IS VALID IF allowLinkRequests is false
								if (this.type === nsFreshcare.data.relationshipSupplier) 
								{
									...
									{
										// v3.3.001 SUP023608 Not really necessary as generally don't have relationships starting in the future
										// 			If one exists, allow it to go ahead as the grower would have authorised it prior to v3.3.001
										bValid = false;
										bEmailGrower = false;
								...
								{
									// v3.2.022 SUP023620 had 'bEmailAdmin - false'
									// v3.2.022 SUP023608 Changed text of error
									bValid = false;
									bEmailGrower = false;
									bEmailAdmin = false;
									sReason = '';
									sSubject = '';
									ns1blankspace.okToSave = false;
									ns1blankspace.status.error('Link request has been declined by supplier. Please contact supplier to request they ask the Freshcare Office to remove the decline.');
								}

								if (!bValid) {return false;}		// Quit out of loop
							});

							// v3.3.001 SUP023608 If link is valid but grower doesn't allow LinkRequests, let the supplier know
							if (bValid && $('#ns1blankspaceDetailsSupplierBusiness').attr('data-allowRequests') != 'Y')
							{
								bValid = false;
								bEmailGrower = false;
								bEmailAdmin = false;
								...
								sSubject = '';
								ns1blankspace.okToSave = false;
								ns1blankspace.status.error('Member has requested not to be linked.  Please contact the member to request they instruct the Freshcare Office to allow link.');
							}
			...
			{		// v1.0.22 Was previously just an if - code continued without proper values
				...
				if (ns1blankspace.okToSave) 
				{
					aMessageHTML.push('<table><tr><td>' + nsFreshcare.user.role + ' ' + ns1blankspace.user.contactBusinessText + ' has requested a link to grower ' +
					...
					aGrowerMessageHTML.push('<table><tr><td>' +
											...
											'<br /><br />' +
											ns1blankspace.user.contactBusinessText.formatXHTML() + ' may be your direct customer; or they may wish to include you on their approved list as they regularly purchase your product from one of your direct customers, for supply to an end customer that requires supply only from certified growers.' +
											'<br /><br />' +
											...
											'<br /><br />' +
											'No action is required by you if you consent to be linked to ' + ns1blankspace.user.contactBusinessText.formatXHTML() + '. This link will be established after 7 days from the date of this email.' +
											'<br /><br />' +
					...
					var oParam = 
					{	
						message: aGrowerMessageHTML.join(''),
		...
		process: 	function (oParam, oResponse)
		{	
			// v3.2.022 SUP023620 Now saves against Member / Customer as applicable
			oParam = oParam || {};
			oParam.saveCustomerStep = oParam.saveCustomerStep || 1;
			...
			if (oResponse.status == 'OK') 
			{
				...
				{
					// Send email to grower from Admin, but only if they have a valid email address ver 1.0.20
						...
						&& nsFreshcare.data.grower.email.indexOf('@') > -1) 
					{
						...
						oParam.subject = oParam.subject || "Freshcare Customer requesting Grower to be added to their list";
						oParam.to = nsFreshcare.data.grower.email;
						oParam.from = nsFreshcare.data.emailFromAdmin;
						oParam.contactBusiness = $('#ns1blankspaceDetailsSupplierBusiness').attr('data-id');
						oParam.contactPerson = $('#ns1blankspaceDetailsSupplierPerson').attr('data-id');
				...
				else 
				{
					...
					oParam.subject = (oParam.subject == "Freshcare Customer requesting Grower to be added to their list") ? undefined : oParam.subject;
					oParam.message = oParam.step2Message;
					delete(oParam.step2Message);
					oParam.subject = oParam.subject || 'A new supplier link has been requested';
					...
					if (oParam.emailAdmin)
					{
						...
						oParam.contactPerson = ns1blankspace.user.contactPerson;
						nsFreshcare.external.grower.save.sendEmail(oParam);
					}
					else
					{
