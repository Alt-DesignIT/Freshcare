...
nsFreshcare.trainer.grower = 
{
	init: function (oParam) 
	{
		...
		// v3.2.020 SUP022975 Now defaults COP and Training Date when adding anew grower from Training Course
		if (oParam != undefined)
		{
			...
			if (oParam.codeOfPractice != undefined) {ns1blankspace.data.grower.codeOfPractice = oParam.codeOfPractice;}
			if (oParam.codeOfPracticeText != undefined) {ns1blankspace.data.grower.codeOfPracticeText = oParam.codeOfPracticeText;}
			if (oParam.trainingDate != undefined) {ns1blankspace.data.grower.trainingDate = oParam.trainingDate;}
		...
		//  v3.2.017 SUP023498 Trainer's aren't allowed to add growers from Grower form
		ns1blankspace.app.context({inContext: false, 'new': true, action: true, actionOptions: true});
	...
	layout: function ()
	{
		...
		{
			ns1blankspace.show({selector: '#ns1blankspaceMainSummary', context: {new: true, actionOptions: true, inContext: false}});
	...
	show: function (oParam, oResponse)
	{
		...
		if (iStep === 1) 
		{
			...
			// v3.2.017 SUP023498 Trainer's aren't allowed to add growers from Grower form
			ns1blankspace.app.context({inContext: false, 'new': true, action: false, actionOptions: true});
		}
	...
	postPackageClick: function(oResponse) 
	{
		...
		if (oResponse === undefined) {
			...
			if (oResponse.status === 'OK') 
			{
				...
				$('#ns1blankspaceMembershipHarvestMonthsCaption').html((oRow.membership === nsFreshcare.data.membershipSCS ? 'Operational' : 'Harvest') + ' Months');

				// v3.3.001 SUP023456 Set Valid scopes
				nsFreshcare.admin.grower.membership.defaultScopeOptions(
				{
					xhtmlElementID: "ns1blankspaceDetailsScopeUpdate", 
					membership: oRow.membership
				});
				...
				{
					// Enable crops field
					// v3.3.001 SUP023456 Apply relevant filter to crop list
					nsFreshcare.admin.grower.membership.defaultCropTable({membership: oRow.membership});
	...
	membership: 
	{
		...
		{	// v3.3.001 SUP023097 Now shows training rows
			var oRoot = ns1blankspace.rootnamespace;
			var aHTML = [];
	...
	save: 	
	{
		validate: 	function(oParam) 
		{
			...
			if (ns1blankspace.objectContext != -1) 
			{
				...
				if (oParam.validateSaveStep === 1)
				{
					if (oParam.membershipShowIndex === undefined) 
					{
						...
						if ($('#ns1blankspaceMainDetails').html() == '')
						{
							...
							nsFreshcare.trainer.grower.details();
				...
				else if (oParam.validateSaveStep === 2)
				{
					if ($('#ns1blankspaceMainDetails').html() != '') 
					{
						...
						// v3.3.001 SUP023471 At least one of phone, mobile or email must be added
						if (ns1blankspace.okToSave &&
						...
						{
							ns1blankspace.okToSave = false;
							ns1blankspace.status.message('Please provide at least one of phone, mobile or email.');
						}
					}
			...
			if (ns1blankspace.objectContext === -1)
			{
				if ($('#ns1blankspaceMainDetails').html() != '') 
				{
					...
					// v3.3.001 SUP023471 Valid email to be entered as per existing record
					if (ns1blankspace.okToSave && $('#ns1blankspaceDetailsEmailUpdate').val() != '' && !nsFreshcare.util.validateEmail($('#ns1blankspaceDetailsEmailUpdate').val()))
					{
						ns1blankspace.okToSave = false;
						ns1blankspace.status.message('Please provide a valid email address for the ' + nsFreshcare.data.growerText);
		...
		send: 		function (oParam)
		{
			...
			var iGrower = $.map($.grep(nsFreshcare.data.businessGroupGrowerText, function(x) {return x.title == nsFreshcare.data.growerText}),
								function(y) {return y.id}).shift();
				...
				else 	// A new trainee - add to AGRI_TEMP_TRAINEE
				{
					...
					if (oParam.step === 1) 
					{
						...
						if ($('#ns1blankspaceMainDetails').html() != '') 
						{
							...
							sDataTrainee += '&course=' + ns1blankspace.util.fs($('#ns1blankspaceDetailsTrainingPackageUpdate').attr('data-course')); 
							...
							sDataTrainee += '&sestatus=1';		// v3.3.001 SUP023422 For new trainees
					...
					else if (oParam.step === 3)
					{
						...
						{	
							...
							{
								html: 'The new ' + nsFreshcare.data.growerText + ' will appear on your training course once approved by Freshcare.', 
								title: 'Please note:',
								buttons: 
								[
									{text: "OK",
