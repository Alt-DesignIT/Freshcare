	...
	requestSearch: 
	{
		send: function(oParam, oResponse)
			{	// Area to use for ad-hoc advanced searches
				...
				aHTML.push('<tr><td class="ns1blankspace">Apply to SubSearch Join?</td>' + 
							'<td><input id="ns1blankspaceHomeTestFilterApplyToSubSearch" class="ns1blankspaceText"></td><tr>');
				...
				if (nsFreshcare.systemadmin.params) 
				{
					...
					{ 	
						...
						{
							aFilters.push('<span id="filterItem_' + iFilterCount + '"></span>' + this);
						});
						$('#ns1blankspaceHomeFiltersList').html(aFilters.join('<br />'))
					}
				...
				$('#ns1blankspaceHomeFilterAdd')
					...
					.click(function() {
						...
						if (gbOkToSave) {
							...
							$('#ns1blankspaceHomeFiltersList').html($('#ns1blankspaceHomeFiltersList').html() +
																	 '<span id="filterItem_' + iFilterCount + '"></span>' +
																	 ...
																	 '^^' + (($('#ns1blankspaceHomeTestFilterApplyToSubSearch').val() != '') ? 'Y' : '') +  
																	 '<br />');
							$('#ns1blankspaceHomeTestFilterName').val('');
							...
							$('#ns1blankspaceHomeTestFilterValue2').val('');
							$('#ns1blankspaceHomeTestFilterValue3').val('');
							$('#ns1blankspaceHomeTestFilterApplyToSubSearch').val('');
							
							$('#filterItem_' + iFilterCount)
							.button(
							{	icons: 
								{
							...
							$('#filterItem_' + iFilterCount).click(function(event)
							{
				...
				$('#ns1blankspaceHomeTestRun')
					...
					.click(function() {
						...
						if (gbOkToSave) {
							...
							nsFreshcare.systemadmin.params.filters = $('#ns1blankspaceHomeFiltersList').html().split('<br>');
							...
							{
								var aFilters = $('#ns1blankspaceHomeFiltersList').html().split('<br>');
							
								$.each(aFilters, function() 
								{
									var sFilter = this.split('</span>').pop();
									...
									var sApplyToSubSearch = (aThisFilter.length > 5) ? aThisFilter[5]: undefined;
									if (aThisFilter[0] != '') {
									
										oSearch.addFilter(aThisFilter[0], aThisFilter[1].toUpperCase(), sValue1, sValue2, sValue3, sApplyToSubSearch);
									}
								});
							}
