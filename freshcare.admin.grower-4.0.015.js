...
nsFreshcare.admin.grower = 
{
	data: 
	{
		...
		[
			...
			},
			{
				...
				groupSearch: 
				{
					method: 'SETUP_CONTACT_PERSON_GROUP_SEARCH',
					fields: 'title',
					sort: {name: 'title', direction: 'asc'}
				},
				contactManage:
		...
		[
			...
			{name: 'contactbusiness.seallowlinkrequests', caption: 'Allow Link Requests'},
			{name: 'contactbusiness.primarycontactpersontext', caption: 'Primary Contact Person'},
		...
		subscription:
		{
			historyFields: 
			[
				{name: 'membershiptext', caption: 'Membership'},
				{name: 'contactbusinesstext', caption: 'Member'},
				{name: 'codeofpracticetext', caption: 'COP'},
				{name: 'startdate', caption: 'Start Date'},
				{name: 'enddate', caption: 'End Date'},
				{name: 'statustext', caption: 'Membership Status'},
				{name: 'laststatuschangedate', caption: 'Status Changed on'},
				{name: 'firstcertified', caption: 'First Certified'},
				{name: 'crop', caption: 'Crops'},
				{name: 'harvestmonth', caption: 'Harvest Month'},
				{name: 'expirymonth', caption: 'Re-Cert Audit Due'},
				{name: 'expirychangeddate', caption: 'Re-Cert Audit Due Changed on'},
				{name: 'expirychangedbyusertext', caption: 'Re-Cert Audit Due Changed by'},
				{name: 'expirychangereason', caption: 'Re-Cert Audit Due Change Reason'}
			]
		},
		...
		certificate: 
		{
			historyFields:
			[
				{name: 'certificatenumber', caption: 'Certificate Number'},
				{name: 'dateissued', caption: 'Certificate Issued'},
				{name: 'enddate', caption: 'Certificate Expires'},
				{name: 'sent', caption: 'Certificate Sent?'},
				{name: 'sentdate', caption: 'Certificate Sent on'},
				{name: 'audittext', caption: 'Certificate Audit'}
			]
		},
		...
		site:
		{
			historyFields:
			[
				{name: 'address1', caption: 'Site Address 1'},
				{name: 'address2', caption: 'Site Address 2'},
				{name: 'addresssuburb', caption: 'Suburb'},
				{name: 'addressstate', caption: 'State'},
				{name: 'addresspostcode', caption: 'Post Code'},
				{name: 'statustext', caption: 'Status'},
			]
		}
	},
	...
	home: 		function (oParam, oResponse)
	{
		...
		if (oResponse == undefined)
		{
			...
			{
				$('#ns1blankspaceControl').html('<i style="color:#ccc; font-size:2em; margin-top:12px;' +
			...
			oSearch.addField('contactbusiness.contactperson.firstname,contactbusiness.contactperson.surname,contactbusiness.tradename,contactbusiness.legalname' +
							',contactbusiness.agrisubscription.agricertificate.certificatenumber,contactbusiness.agrisubscription.membership.code');
		...
		else
		{
			...
			else
			{
				...
				aHTML.push('<div class="ns1blankspaceCaption"' +
							' style="padding-left:8px; margin-bottom:10px; font-weight:500;">' +
							'MOST RECENTLY UPDATED</div>');
	...
	search: 	
	{
		send: 		function (sXHTMLElementId, oParam)
		{
			...
			if (sSearchContext != undefined && iSource != ns1blankspace.data.searchSource.browse)
			{
				...
				var oSearch = new AdvancedSearch();		//
				oSearch.method = 'CONTACT_BUSINESS_SEARCH';
				oSearch.addField('contactbusiness.phonenumber,contactbusiness.faxnumber,contactbusiness.contactperson.customerstatus,contactbusiness.seallowlinkrequests' +
								...
								',contactbusiness.contactperson.user.id,contactbusiness.contactperson.user.username' +
								',contactbusiness.contactperson.user.disabled,contactbusiness.contactperson.user.se' + nsFreshcare.data.userPasswordId + 
				...
				if (!bSystemSearch)
				{
					...
					oSearch.addFilter("contactbusiness.contactperson.id", 'EQUAL_TO', 'field:contactbusiness.primarycontactperson');
				}
				else
				{
			...
			else
			{
				...
				if (sSearchText.length >= iMinimumLength || iSource == ns1blankspace.data.searchSource.browse)
				{
					...
					var oSearch = new AdvancedSearch();
					oSearch.method = 'CONTACT_BUSINESS_SEARCH';
					oSearch.addField('contactbusiness.contactperson.firstname,contactbusiness.contactperson.surname'+
									',contactbusiness.tradename,contactbusiness.legalname,contactbusiness.contactperson.id' +
									',contactbusiness.agrisubscription.membership.code');
		...
		validate: function(oParam)
		{
			...
			if (oParam.removeGrowerStep === undefined) 
			{
				...
				ns1blankspace.container.confirm({title: 'Delete ' + nsFreshcare.data.growerText + '?', html: 'Are you sure you want to delete this ' + nsFreshcare.data.growerText + '?',
													buttons: 
													[
														{text: "Yes", name: "btnYes", icons: {primary: 'ui-icon-check'},
															click: function() 
															{
														...
														{text: "No", name: "btnNo", icons: {primary: 'ui-icon-close'}, 
															click: function() 
															{
																$('#ns1blankspaceBootstrapDialog').modal('hide');
	...
	layout: 	function ()
	{
		...
		{
			// v3.3.002 SUP023422 Asked to sort by training date descending
		...
		$('#ns1blankspaceControlNewMembership').on('click', function(event)
		{
	...
	summary: 	function ()
	{
		...
		else
		{
			...
			if (!nsFreshcare.admin.grower.readOnly == true)
			{
				...
				// v3.4.004 SUP023867 No longer required to show here - only on Membership tab
				...
				$('#ns1blankspaceGrowerSumaryAddUser')
					...
					.on("click", function()
					{
						nsFreshcare.internal.entity.user.add.show(
						{
							...
							role: nsFreshcare.data.growerText,
	...
	details: 	function ()
	{
		...
		if ($('#ns1blankspaceMainDetails').attr('data-loading') == '1')
		{
			...
			// SUP023471 Removed mandatory condition for ABN
			aHTML.push('<tr class="ns1blankspaceCaption">' +
							'<td class="ns1blankspaceCaption">' +
							'ABN' +
							'</td></tr>' +
			...
			$('#ns1blankspaceDetailsABN').mask('99 999 999 999');
			...
			else
			{
				...
				$('[name="radioReceiveLinkRequests"][value="Y"]').attr('checked', true);
			}
		}	
	},
	...
	address: function (oParam)
	{
		...
		if ($('#ns1blankspaceMainAddress').attr('data-loading') == '1')
		{
			...
			if (ns1blankspace.objectContextData != undefined)
			{
				// v4.0.013 SUP024160 remove contactbusiness alias for Businesses form
				if (ns1blankspace.objectContextData['contactbusiness.streetaddress1'] == undefined 
					&& ns1blankspace.objectContextData.streetaddress1 != undefined)
				{
					ns1blankspace.objectContextData['contactbusiness.streetaddress1'] = ns1blankspace.objectContextData.streetaddress1;
					ns1blankspace.objectContextData['contactbusiness.streetaddress2'] = ns1blankspace.objectContextData.streetaddress2;
					ns1blankspace.objectContextData['contactbusiness.streetsuburb'] = ns1blankspace.objectContextData.streetsuburb;
					ns1blankspace.objectContextData['contactbusiness.streetpostcode'] = ns1blankspace.objectContextData.streetpostcode;
					ns1blankspace.objectContextData['contactbusiness.streetstate'] = ns1blankspace.objectContextData.streetstate;
					ns1blankspace.objectContextData['contactbusiness.streetcountry'] = ns1blankspace.objectContextData.streetcountry;
				}
				if (ns1blankspace.objectContextData['contactbusiness.mailingaddress1'] == undefined 
					&& ns1blankspace.objectContextData.mailingaddress1 != undefined)
				{
				...
				$('#ns1blankspaceAddressStreetAddress1').val(ns1blankspace.objectContextData['contactbusiness.streetaddress1'].formatXHTML());
				$('#ns1blankspaceAddressStreetSuburb').val(ns1blankspace.objectContextData['contactbusiness.streetsuburb'].formatXHTML());
				$('#ns1blankspaceAddressStreetState').val(ns1blankspace.objectContextData['contactbusiness.streetstate'].formatXHTML());
				$('#ns1blankspaceAddressStreetPostCode').val(ns1blankspace.objectContextData['contactbusiness.streetpostcode'].formatXHTML());
				$('#ns1blankspaceAddressStreetCountry').val(ns1blankspace.objectContextData['contactbusiness.streetcountry'].formatXHTML());
				$('#ns1blankspaceAddressMailingAddress1').val(ns1blankspace.objectContextData['contactbusiness.mailingaddress1'].formatXHTML());
				$('#ns1blankspaceAddressMailingSuburb').val(ns1blankspace.objectContextData['contactbusiness.mailingsuburb'].formatXHTML());
				$('#ns1blankspaceAddressMailingState').val(ns1blankspace.objectContextData['contactbusiness.mailingstate'].formatXHTML());
				$('#ns1blankspaceAddressMailingPostCode').val(ns1blankspace.objectContextData['contactbusiness.mailingpostcode'].formatXHTML());
				$('#ns1blankspaceAddressMailingCountry').val(ns1blankspace.objectContextData['contactbusiness.mailingcountry'].formatXHTML());

				if (bTwoLineAddress) 
				{
					$('#ns1blankspaceAddressStreetAddress2').val(ns1blankspace.objectContextData['contactbusiness.streetaddress2'].formatXHTML());
					$('#ns1blankspaceAddressMailingAddress2').val(ns1blankspace.objectContextData['contactbusiness.mailingaddress2'].formatXHTML());
				}
			}
		}	
	},
	...
	emails: 	
	{
		configure: function(oParam)
		{
			...
			// v3.4.007 SUP022931 No longer necessary to filter on person as well
			...
			//{
				...
				// v3.4.007 Added bulkemail to filter
				oFilters.push({operation: 'addFilter', name: 'actiontype', comparison: 'IN_LIST', value1: '5,9,10,' + ns1blankspace.data.actionTypes.bulkEmail.id});
				...
				// v4.0.009 SUP024112 No longer be necessary as now consistently saving against business when saved to person (fingers crossed!)
				/*
				oFilters.push({operation: 'addOperator', operator: 'or'});
				oFilters.push({operation: 'addBracket', bracket: '('});
		...
		detail: 
		{
			search: function(oParam)
			{
				...
				if (oParam)
				{
					...
					iLinkID = oParam.linkID || iActionId;
				...
				if (oParam.emailDetailStep === 1)
				{
					...
					oSearch.addFilter('action', 'EQUAL_TO', iLinkID);
					oSearch.rows = 40;
					oSearch.getResults(function(oResponse)
					{
						if (oResponse.status === 'OK')
				...
				else if (oParam.emailDetailStep === 2)
				{
					...
					oSearch.addFilter('objectcontext', 'EQUAL_TO', iLinkID);
					oSearch.rows = 40;
					oSearch.getResults(function(oResponse)
					{
						if (oResponse.status === 'OK')
			...
			show: function(oParam)
			{
				...
				if (oParam)
				{
					if (oParam.xhtmlElementID) {iActionId = oParam.xhtmlElementID.split('-').pop()}
					if (oParam.emailDetailStep === undefined) {oParam.emailDetailStep = 1}
					iLinkID = $('#' + oParam.xhtmlElementID).attr('data-actionID') || iActionId;
					oParam.linkID = iLinkID;
				}

				if (oParam.message === undefined)
				{
					oParam.message = $('#ns1blankspaceAction_Message-' + iActionId).html();
					// v3.0.1a SUP021899 if undefined, this errors
		...
		configure: function(oParam)
		{
			...
			//{
				...
				var iActionTypes = $.map($.grep(nsFreshcare.data.actionTypes, function(x)
											{
												return x.id != '5' && x.id != '9' && x.id != '10' && x.id != ns1blankspace.data.actionTypes.bulkEmail.id;
											}),
				...
				// v3.2.025 COnditionally show Add button
				nsFreshcare.internal.entity.actions.show(
				{
					...
					filters: oFilters,
					actions: {add: (ns1blankspace.option.preventUpdates != true)},
					functionProcess: nsFreshcare.internal.entity.actions.process,
	...
	attachments: 
	{
		search: function(oParam)
		{
			...
			if (oActions.add)
			{
				...
				.click(function() 
				{
					...
					delete (oParam.xhtml);
					ns1blankspace.attachments.add.file(oParam); 		// v3.4.006 New version of 1bs.control
				});
		...
		row: function(oRow, oParam)
		{
			...
			// v3.2.025 COnditinally prevent updates
			if (ns1blankspace.option.preventUpdates == true)
			{
				bCanRemoveOwn = false;
				bCanRemoveAll = false;
				bCanSetTypeOwn = false;
				bCanSetTypeAll = false;
			}
			...
			{	// v1.0.1a SUP021910 Added ns1blankspaceNoUnloadWarn class
				aHTML.push('<td id="ns1blankspaceAttachment_' + this.name + '-' + oRow.id + '" ' +
								'class="ns1blankspaceRow ns1blankspaceNoUnloadWarn"' +
								(this.name === 'filename' ? 'data-filename="' + oRow.filename + '"' : '') + '>' + 
			...
			{
				aHTML.push('<span id="ns1blankspaceAttachment_options_attachmenttype-' + oRow.attachment + '"' +
							' data-id="' + oRow.id + '" class="ns1blankspaceAttachmentsType" title="Set Attachment Type"></span>');
			}
		...
		bind: function(oParam)
		{
			...
			if (ns1blankspace.data.attachmentTypes && ns1blankspace.data.attachmentTypes.length > 0)
			{
				...
				{
					text: false,
					icons: {primary: "glyphicon-menu-hamburger"}
				})
				.on('click', function()
				{
					...
					else 
					{
						...
						$.each(ns1blankspace.data.attachmentTypes, function()
						{
							aHTML.push('<tr><td><span id="ns1blankspaceAttachmentType_' + this.type + '"' +
										' class="ns1blankspaceSetAttachmentType ns1blankspaceRow"' +
										' data-attachmentid="' + $('#' + ns1blankspace.xhtml.divID).attr('data-id') + '"' +
						...
						{ 
							...
							left: $('#' + ns1blankspace.xhtml.divID).offset().left + 20 /*$('#' + ns1blankspace.xhtml.divID).width()*/
						});
						...
						$('.ns1blankspaceSetAttachmentType')
							...
							{
								oParam.attachmentId = $(this).attr('data-attachmentid');
								oParam.element = this;
		...
		// v3.4.008 SUP023064 Moved to own functiont o that can validate any changes to Attachment Type
		manageType: function(oParam)
		{
			oParam = oParam || {};
			var oElement = ns1blankspace.util.getParam(oParam, 'element').value;
			var sAttachmentType = $(oElement).attr('id').split('_').pop();
			var oParentElement = $('#' + $(oElement).attr('data-parentid'));
			var sAttachmentRowID = $(oParentElement).attr('id').split('-').pop();
			oParam.manageTypeStep = oParam.manageTypeStep || 1;

			
			if (sAttachmentRowID != '' && sAttachmentRowID != undefined)
			{
				// Validate type if required
				if (oParam.manageTypeStep == 1)
				{
					$(ns1blankspace.xhtml.container).html('');
					$(ns1blankspace.xhtml.container).hide();
					
					ns1blankspace.okToSave = true;
					oParam.manageTypeStep = 2;

					if ($.type(oParam.functionValidateType) == 'function')
					{
						oParam.functionValidateType(oParam);
					}
					else
					{
						nsFreshcare.admin.grower.attachments.manageType(oParam);
				...
				// Update the type
				else if (oParam.manageTypeStep == 2)
				{
					delete(oParam.manageTypeStep);
					if (ns1blankspace.okToSave)
					{
						$.ajax(
						{
							...
							data: 'type=' + sAttachmentType + '&id=' + sAttachmentRowID,
							success: function(oResponse)
							{
								if (oResponse.status === 'OK')
								{
			...
			{
				ns1blankspace.status.error("Attachment does not have a valid ID. Please contact Support");
			}
		}
	},
	...
	save: 		
	{
		validate: function(oParam)
		{
			...
			else if (oParam.validateGrowerStep === 1)
			{
				if ($('#ns1blankspaceMainDetails').html() != '')
				{
					// v3.4.001 SUP023639 Default Join Date to Start Date of any new Memberships being added if not populated
					if (ns1blankspace.objectContextData && $('#ns1blankspaceMainNewMembership').html() != '')
					{
						if (ns1blankspace.objectContextData['contactbusiness.agribusiness.joindate'] == '')
						{
							$('#ns1blankspaceDetailsJoinDate').val($('#ns1blankspaceMembershipStart').val());
						}
						if (ns1blankspace.objectContextData['contactbusiness.agribusiness.prioritymembership'] == '')
						{
							$('#ns1blankspaceDetailsPriorityMembership').val($('#ns1blankspaceMembership').val());
							$('#ns1blankspaceDetailsPriorityMembership').attr('data-id', $('#ns1blankspaceMembership').attr('data-id'));
						}
					}
					...
					// v3.3.001 SUP023471 No longer required to enter fax
					if (ns1blankspace.okToSave && $('#ns1blankspaceDetailsPhone').val() == "" 
				...
				if (ns1blankspace.okToSave && $('#ns1blankspaceMainAddress').html() != '')
				{
					...
					if (ns1blankspace.okToSave)
					{
						...
						{
							// v3.3.001 SUP023706 now checks for inactivated addresses as well
							...
							if ($('#ns1blankspaceSiteValue_streetAddress1-' + iAddressID).is('*') &&
								...
								|| $('#ns1blankspaceSiteValue_streetAddress1-' + iAddressID).hasClass('nsFreshcareInactive'))
							{
								$.each(ns1blankspace.objectContextData.memberships, function()
								{
									...
									{	// We need to cancel the remove as well
										...
										ns1blankspace.status.message('An address you are trying to remove/inactivate is linked to a Membership. Please remove it from the Membership first');
										return false;
									}
								});
								...
								if (ns1blankspace.okToSave && $('#ns1blankspaceMainNewMembership').html() != '')
								{
									...
									{
										ns1blankspace.okToSave = false;
										ns1blankspace.status.message('An address you are trying to remove/inactivate is linked to the new Membership. Please remove it from the Membership and then remove from the ' + nsFreshcare.data.growerText);
										return false;
									}
								}
							}
			...
			else if (oParam.validateGrowerStep === 2)
			{
				if (ns1blankspace.okToSave)
				{
					oParam.newGrower = (ns1blankspace.objectContext == -1)	// 3.4.001 Added as it wasn't getting new ref for new Members
		...
		send: function (oParam)
		{
			...
			else if (oParam.saveGrowerStep === 2)
			{
				...
				{
					// v3.4.001 We don't need to get the next reference number if we're not creating a new Member
					// v3.4.002 SUP022964 Using newGrower, not newMember
					if ((oParam.configure && oParam.configure.isMember != true && oParam.configure.createBusiness != true) 
						|| (oParam.newGrower == false))
					{
						oParam.nextReference = -1;
					}
					else
					{
			...
			else if (oParam.saveGrowerStep === 3)
			{
				...
				if (ns1blankspace.objectContext != -1 && ns1blankspace.objectContext != '')
				{
					...
					{
						if (iThisId && ns1blankspace.objectContextData)
						{
							...
							{	// Find the correct existing record in the array
								oStoredData = $.grep(oStoredData, function(x) 
									{
										var sPrefix = (sStoredDataPrefix.split('.').length > 2) ? sStoredDataPrefix : '';
										return x[sPrefix + 'id'] === iThisId;
									}).shift();
							}
						}
					}
				...
				{
					// Address - we need add / update the link
					if (oSaveObject.object === 'address') 
					{
						oManageDataPost = {};
						// This is a new grower or new membership - add the link to the subscription
					...
					if ((oUpdatedData.remove === true || oUpdatedData.remove === '1') && oUpdatedData.id)
					{
						...
						if (oSaveObject.object == 'objectscope' || oSaveObject.object == 'agriproductgroupcategory' || oSaveObject.object == 'addresslink')
						{
							oParam.historyData = 
							{
								object: nsFreshcare.objectSubscription,
								objectContext: oUpdatedData.objectContext,
								seauditobject: oSaveObject.object,
								description: oUpdatedData.itemtext,
								text: ns1blankspace.objectContextData['contactbusiness.tradename'],
								createddate: oUpdatedData.createddate,
								createduser: oUpdatedData.createduser
							}
						}
						...
						$.each(oUpdatedData, function(key, value)
						{
								...
								&& oParam.contactbusiness.id)
							{
								...
								{	
									...
									// v4.0.001 Was not checking oPram.configure.creatBusiness
									if ((oParam.newGrower || oParam.newMembership === true)
										|| (oSaveObject.object == 'address' && oParam.configure 
											&& oParam.configure.createBusiness && oParam.configure.isMember)) 
									{
										oSubscriptionSitesLinkData.objectcontext = '';
									}
								}
								else
							...
							else if (key.split('.').length > 1 && oUpdatedData.id === undefined)
							{
								...
								/*if ((oParam.newGrower === true || oParam.newMembership === true)
									|| (oSaveObject.object == 'addresslink' && oParam.configure && oParam.configure.isMember)) 
								{
									oSubscriptionSitesLinkData[key.split('.').pop()] = value;
								}*/
							}
						...
						|| (oManageData.id && Object.keys(oManageData).length > 1))
					{
						...
						// v4.0.010 SUP024126 Re-apply the check as this only applied for Eggs
						...
						$.ajax(
						{
							...
							success: function(oResponse)
							{
								if (oResponse.status === 'OK')
								{
									...
									if (ns1blankspace.objectContext === -1)
									{
										oParam.newGrower = ((oParam.configure && oParam.configure.createMember) || oParam.configure == undefined);
										if (oSaveObject.method == 'CONTACT_BUSINESS_MANAGE')
									...
									{
										// v3.4.010 SUP023755 Now adds history data if specified rows are removed
										if (oParam.historyData)
										{
											oParam.onComplete = fFunctionPostSave;
											nsFreshcare.util.system.actionHistory.save(oParam);
										}

										else if (fFunctionPostSave)
										{
											fFunctionPostSave(oParam);
										}
									}
					...
					{
						if (oSaveObject.object == 'contactbusiness' && oParam.contactbusiness == undefined)
						{
							oParam.contactbusiness = {id: ns1blankspace.objectContext};
						}
						if (aUpdatedData != undefined && aUpdatedData.length > 0)
						{
							oParam[oSaveObject.saveSource] = aUpdatedData;
			...
			// v3.4.005 If converting existing business and no primary contact set, we update it here
			else if (oParam.saveGrowerStep === 4)
			{
				if (oParam.newGrower == true || (oParam.configure && oParam.configure.hasPrimaryContact == false)
					&& oParam.contactperson.id)
				{
					$.ajax(
					{
						type: 'POST',
					...
					$.ajax(
					{
						...
						success: function(oResponse)
						{
							if (oResponse.status === 'OK')
							{
								...
								oParam.saveGrowerStep = ((oParam.functionSaveMore) ? 6: 7);
								nsFreshcare.admin.grower.save.send(oParam);
							}
							else
							{
			...
			// v3.3.005 SUP023800 Save Re-Cert Audit month to Audit. Non-standard save
			else if (oParam.saveGrowerStep == 52)
			{
				if (oParam.subscriptionAuditData && oParam.subscriptionAuditData.length > 0)
				{
					oParam.auditIndex = oParam.auditIndex || 0;
					if (oParam.auditIndex < oParam.subscriptionAuditData.length)
					{
						var oAuditData = oParam.subscriptionAuditData[oParam.auditIndex];
						$.ajax(
						{
							type: 'POST',
							...
							success: function(oResponse)
							{
								if (oResponse.status == 'OK')
								{
									ns1blankspace.status.message('Re-Cert Audit Month Saved');
									oParam.auditIndex += 1;
									nsFreshcare.admin.grower.save.send(oParam);
								}
								else
								{
									ns1blankspace.status.error('Error updaing Re-Cert audit month: ' + oResponse.error.errornotes);
								}
							}
						});
					}
			...
			// v3.3.005 SUP023800 Display message that user must re-print certificate when re-cert audit due month has changed
			else if (oParam.saveGrowerStep == 7)
			{
				...
				if (oParam.subscriptionAuditData && oParam.subscriptionAuditData.length > 0)
				{
					ns1blankspace.container.confirm(
					{
						title: 'Certificate Data changed!',
						html: 'You have changed the Re-Certification Audit Due Month. Please re-print the Certificate.',
						buttons: 
						[{
							text: 'OK',
							name: 'btnOK',
			...
			else if (oParam.saveGrowerStep === 10)
			{
				...
				{
					// v3.4.001 SUP022964 Now sends isMember & personID
					fFunctionSearch(
					{
						xhtmlElementID: '-' + ns1blankspace.objectContext, 
						id: ns1blankspace.objectContext, 
						isMember: (oParam.configure ? oParam.configure.createMember || oParam.configure.isMember : true), 
						personID: (oParam.contactperson ? oParam.contactperson.id : undefined)
					});
				}
				else
				{
		...
		getNextReference: function(oParam)
		{
			...
			// v3.3.001 Now filters on last 365 days
			// v3.4.010 SUP023639 Now does an additional search to see if referrnce number created already exists
			oParam = oParam || {}
			var d365DaysAgo = new Date();
			d365DaysAgo = new Date(d365DaysAgo.setDate(d365DaysAgo.getDate() - 365));
			oParam.nextReferenceStep = oParam.nextReferenceStep || 1;

			var setNextReference = function(value)
			{
				return (Number(value.replace("BUS", '')) + 1).toString();
			}

			// Find next reference from most recently created businesses
			if (oParam.nextReferenceStep == 1)
			{
				var oSearch = new AdvancedSearch();
				oSearch.method = 'CONTACT_BUSINESS_SEARCH';
				oSearch.addField('reference');
				oSearch.addFilter('createddate', 'BETWEEN', d365DaysAgo.toString('dd MMM yyyy') + ' 00:00:00', dToday.toString('dd MMM yyyy' + ' 23:59:59'));
				oSearch.addFilter('businessgroup', 'EQUAL_TO', nsFreshcare.data.businessGroupGrowerID);
				oSearch.sort('createddate', 'desc');
				oSearch.rows = 50;
				oSearch.getResults(function(oResponse)
				{
					...
					if (oResponse.status === 'OK')
					{
						if (oResponse.data.rows.length > 0)
						{
							var oThis = oResponse.data.rows.shift();
							while (oThis && !nsFreshcare.util.isNumeric({value: oThis.reference.replace("BUS", ''), zeroValid: false}))
							{
								oThis = oResponse.data.rows.shift();
							}
							if (oThis)
							{
								sNextReference = setNextReference(oThis.reference);
							}
							else
							{
								sNextReference = '0000';
			...
			// Check to see if reference number already exists
			else if (oParam.nextReferenceStep == 2)
			{
				var oSearch = new AdvancedSearch();
				oSearch.method = 'CONTACT_BUSINESS_SEARCH';
				oSearch.addField('reference');
				oSearch.addFilter('reference', 'EQUAL_TO', oParam.nextReference);
				oSearch.getResults(function(oResponse)
				{
					if (oResponse.status == 'OK')
					{
						// This reference already exists - add 1 and check again until not found
						if (oResponse.data.rows.length > 0)
						{
							oParam.nextReference = setNextReference(oParam.nextReference);
							nsFreshcare.admin.grower.save.getNextReference(oParam);
						}
						// The reference doesn't already exist so we're good to continue
						else
						{
							delete(oParam.resultsList);
							delete(oParam.nextReferenceStep);
							ns1blankspace.util.onComplete(oParam);
						}
					}
					else
					{
						ns1blankspace.status.error('Error checking next Company Id: ' + oResponse.status.error);
					}
				});
			}
		},
		...
		details: function(oParam)
		{
			...
			oBusinessData = $.extend(true, oBusinessData, 
			{
				...
				seallowlinkrequests: $('input[name="radioReceiveLinkRequests"]:checked').val(),
		...
		membership: function(oParam)
		{
			...
			var aSubscriptionCertificateData = ns1blankspace.util.getParam(oParam, 'subscriptionCertificateData', {"default": []}).value;
			var aSubscriptionsWithdrawn = ns1blankspace.util.getParam(oParam, 'subscriptionsWithdrawn', {"default": []}).value;
			var aSubscriptionAuditData = ns1blankspace.util.getParam(oParam, 'subscriptionAuditData', {"default": []}).value;
			...
			if (ns1blankspace.objectContext != -1)
			{
				$.each(ns1blankspace.objectContextData.memberships, function()
				{
					...
					if ($('#ns1blankspaceMainMembership' + iSubscriptionId).html() != '')
					{
						...
						if ($('#ns1blankspaceMembership' + iSubscriptionId + 'ExpiryChangeReason').is('*'))
						{
							aSubscriptionsData[aSubscriptionsData.length-1].expirychangereason = $('#ns1blankspaceMembership' + iSubscriptionId + 'ExpiryChangeReason').val();
						}
						...
						if (!bWithdrawn)
						{
									...
									|| $('#ns1blankspaceMembership' + iSubscriptionId + 'Expires').val() != '' || $('#ns1blankspaceMembership' + iSubscriptionId + 'SentDate').val() != ''))
							{
								...
								// v3.3.001 SUP023563 This question no longer asked since we store the expiry month on the audit as well
								// v3.3.005 SUP023800 Still want question, but push value to audit if user answers Yes
								if ($('input[name="radio' + iSubscriptionId + 'UpdateCertificateExpiry"]:checked').val() == 'Y')
								{
									...
									// v3.3.005 SUP023800 Save re-cert Audit Month to Audit if affects certificate expiry
									if ($('#ns1blankspaceMembership' + iSubscriptionId + 'Audit').attr('data-id'))
									{
										aSubscriptionAuditData.push( 
										{
											id: $('#ns1blankspaceMembership' + iSubscriptionId + 'Audit').attr('data-id'),
											serecertauditmonth: $('#ns1blankspaceMembership' + iSubscriptionId + 'ExpiryMonth').attr('data-id')
										});
									}
								}
						...
						{	
							...
							if (bRemove || ($(this).hasClass('nsFreshcareSelected') && $(this).attr('data-id') == undefined))
							{
								aSubscriptionScopeData.push(
								{
									...
									createduser: $(this).attr('data-createduser')
								});
							}
						});
						...
						// v3.3.001 SUP023456 Only applies if Scope Extension audit is still open
						if (oSubscription['agrisubscription.agrilastaudit.type'] == nsFreshcare.data.audit.typeScopeExtension  
							&& oSubscription['agrisubscription.agrilastaudit.resultstatus'] != nsFreshcare.data.audit.resultStatusCompleted
			...
			oParam.subscriptionsWithdrawn = aSubscriptionsWithdrawn;
			oParam.subscriptionAuditData = aSubscriptionAuditData;
	...
	membership: 
	{
				...
				$('#ns1blankspaceControl-' + this.id).click(function(event)
				{
					ns1blankspace.show({selector: '#ns1blankspaceMainMembership' + (this.id).split('-').pop(),
										context: 
										{
											new: true, 
											action: oNS.readOnly,
											actionOptions: (nsFreshcare.user.roleID != nsFreshcare.data.roles.admin), 
											inContext: false
										}});
		...
		'new': function(oParam)
		{
			...
			// v3.4.001 Added data-loading check as was re-binding when validation failed
			if ($('#ns1blankspaceMainNewMembership').attr('data-loading') == '1')
			{
				$('#ns1blankspaceMainNewMembership').attr('data-loading', '');
				...
				aHTML.push('<tr class="ns1blankspaceCaption">' +
								...
								'<input id="ns1blankspaceMembership" class="ns1blankspaceSelect"' +
									...
									' data-click="nsFreshcare.admin.grower.membership.postMembershipClick"' + 
									'>' +
								'</td></tr>');		
				...
				aHTML.push('<tr class="ns1blankspace">' +
								'<td class="ns1blankspaceCaption" style="text-align:left;">' +
									'<span id="ns1blankspaceMembershipHarvestMonthsCaption" style="text-align:left;">Harvest Months</span>&nbsp; ' + 
				...
				// v3.3.001 Need to set scope values, etc after setting membership
				if ($('#ns1blankspaceMembership').attr('data-id'))
				{
					nsFreshcare.admin.grower.membership.postMembershipClick({membership: $('#ns1blankspaceMembership').attr('data-id')});
				}
				...
				$('#nsFreshcareMembershipHarvestMonthsAll')
					.click(function(event)
					{
						if ($(this).html() === '[ All ]')
						{
							$.each($('#ns1blankspaceMainNewMembership .nsFreshcareHarvestMonthsUpdate'), function()
							{
				...
				$('#ns1blankspaceMainNewMembership .nsFreshcareProductGroup').click(function() 
				{
					nsFreshcare.external.grower.manageMultiRows(this.id)
				});

				// Bind the Membership Sites area
				$('#ns1blankspaceMainNewMembership .nsFreshcareMembershipSite').click(function() 
				{
					nsFreshcare.external.grower.manageMultiRows(this.id)
				});
			}
		...
		show: 	function(oParam) 
		{
			...
			else
			{
				if ($('#ns1blankspaceMainMembership' + iMembership).attr('data-loading') == '1')
				{
					...
					// v3.4.001 Was setting Details tab data-loading to null. Doh!
					...
					$('#ns1blankspaceMainMembership' + iMembership)
						.html(aHTML.join(''))
						.attr('data-subscriptionid', iMembership);
					...
					aHTML.push('<tr class="ns1blankspaceCaption">' +
									...
									'<input id="ns1blankspaceMembership' + iMembership + 'COP" class="ns1blankspaceSelect"' +
										...
										' data-subscriptionid="' + oMembership.id + '"' +
										'>' +
									'</td></tr>');		
					...
					// v3.3.001 SUP023456 Set to 'Operational Months' is Supply Chain Standard
					aHTML.push('<tr class="ns1blankspace">' +
									'<td class="ns1blankspaceCaption" style="text-align:left;">' +
										'<span style="text-align:left;">' +
										(oMembership['agrisubscription.membership'] == nsFreshcare.data.membershipSCS ? 'Operational': 'Harvest') + ' Months</span>&nbsp; ' + 
										'<span style="font-size:0.825em;text-align:right;">' + 
					...
					// v3.3.001 SUP023456 Now looks at membership's seCropsLookup field to determien which crop list to use
					aHTML.push('<tr class="ns1blankspace">' +
									'<td class="ns1blankspaceCaption">' +
									'Crops' +
									'</td></tr>' +
					...
					if (!nsFreshcare.admin.grower.readOnly == true)
					{
						if (oMembership['agrisubscription.status'] != nsFreshcare.data.grower.subscriptionStatusWD)
						{
								...
								&& oMembership['agrisubscription.status'] != nsFreshcare.data.grower.subscriptionStatusSP)
							{
								...
								// v4.0.012 SUP024144 Added Extend Certificate option if option is available
								if (nsFreshcare.option.certificateExtension.is)
								{
									aHTML.push('<tr><td>&nbsp;</td></tr>');

									aHTML.push('<tr><td><span id="ns1blankspaceGrowerMembershipCertificateExtendExpiry-' + iMembership + '"' +
													' class="ns1blankspaceAction" style="width:145px;"' +
													' data-certificate="' + oMembership['agrisubscription.agricertificate.id'] + '"' +
													' data-extensionmonths="' + oMembership['agrisubscription.agricertificate.audit.seextensionmonths'] + '">' +
												'</span></td></tr>');
								}
							...
							if (oMembership['agrisubscription.agricertificate.id'] != '')
							{
								aHTML.push('<tr><td><span id="ns1blankspaceCertificateRemove-' + oMembership['agrisubscription.agricertificate.id'] + '"' +
												' class="ns1blankspaceAction" style="text-align: right;width: 145px;">Remove Certificate' +
												'</span>');
							}
					...
					aHTML.push('<tr><td>&nbsp;</td>' +
								'<td class="nsFreshcareSystem" id="nsFreshcareSystemSubscription-' + iMembership + '" style="text-align:right; width:145px;">' +
					...
					if (oMembership["agrisubscription.agricertificate.id"] != '')
					{
						...
						aHTML.push('<tr><td>&nbsp;</td>' +
									'<td class="nsFreshcareSystem" id="nsFreshcareSystemCertificate-' + oMembership['agrisubscription.agricertificate.id'] + '" style="text-align:right;width:145px;">' +
					...
					aHTML.push('<tr><td colspan="2">&nbsp;</td></tr>');
					aHTML.push('<tr><td>&nbsp;</td>' +
								'<td class="nsFreshcareSystem" id="nsFreshcareSystemSubRows-' + iMembership + '" style="text-align:right; width:145px;">' +
									'<span style="font-size:1.125em;">Sites, Scope & Category&nbsp;</span>' +
									'<span class="ns1blankspaceSystemAction"></span>' +
								'</td></tr>');		
					...
					$('#ns1blankspaceMembership' + iMembership + 'COP').attr('data-id', oMembership["agrisubscription.codeofpractice"]);
					$('#ns1blankspaceMembership' + iMembership + 'COP').attr('data-subscriptionid', oMembership["id"]);
					...
					if (!nsFreshcare.admin.grower.readOnly == true)
					{
						...
						// Bind the Category (Product Groups) area
						$('#ns1blankspaceMainMembership' + iMembership + ' .nsFreshcareProductGroup').click(function() {
							
							nsFreshcare.external.grower.manageMultiRows(this.id, iMembership)
						});

						// Bind the Membership Sites area
						$('#ns1blankspaceMainMembership' + iMembership + ' .nsFreshcareMembershipSite').click(function() {
					...
					// v3.3.001 SUP023569 Bind System Subscription and Certificate areas
					// v4.0.001 Bootstrap - changed icon
					$('#ns1blankspaceMainMembership' + iMembership + ' .ns1blankspaceSystemAction')
						.button({icons: {primary: 'glyphicon-resize-full'}})
						//.css('font-size', '0.75em')
						//.css('height', '18px')
						//.css('width', '18px');

					$('#nsFreshcareSystemSubscription-' + iMembership)
						.on('click', function()
						{
							$('#ns1blankspaceMultiUseContainer').css('width', '720');
							ns1blankspace.container.show(
							{
								xhtmlElement: $('#nsFreshcareSystemSubscription-' + iMembership), 
								xhtmlElementID: 'ns1blankspaceMultiUseContainer', 
								leftOffset: (707 - $('#nsFreshcareSystemSubscription-' + iMembership).width()) * -1, 
								topOffset: $('#nsFreshcareSystemSubscription-' + iMembership).height() * -1, 
								onShow: nsFreshcare.admin.grower.membership.getHistory,
								membership: iMembership,
								object: nsFreshcare.objectSubscription,
								historyFields: nsFreshcare.admin.grower.data.subscription.historyFields,
								auditFieldsFrom: oMembership,
								tablePrefix: 'agrisubscription',
								xhtmlElementSystemID: 'nsFreshcareSubscriptionSystem',
								xhtml: '<table style="background-color: #EFEFEF; border: 2px solid #B8B8B8">' +
											'<tr>' +
												'<td class="ns1blankspaceSummaryCaption">Membership Subscription History</td>' +
												'<td style="width: 25px; padding: 5px;"><span class="ns1blankspaceSystemClose"></span></td>' +
											'</tr>' + 
											'<tr>' +
												'<td style="padding: 5px;height: 300px;" colspan="2"' +
													' id="nsFreshcareSubscriptionSystem">' + ns1blankspace.xhtml.loading + '</td>' +
											'</tr></table>'
							});
						});

					$('#nsFreshcareSystemCertificate-' + oMembership['agrisubscription.agricertificate.id'])
						.on('click', function()
						{
							$('#ns1blankspaceMultiUseContainer').css('width', '720');
							ns1blankspace.container.show(
							{
								xhtmlElement: $('#nsFreshcareSystemCertificate-' + oMembership['agrisubscription.agricertificate.id']), 
								xhtmlElementID: 'ns1blankspaceMultiUseContainer', 
								leftOffset: (707 - $('#nsFreshcareSystemCertificate-' + oMembership['agrisubscription.agricertificate.id']).width()) * -1, 
								topOffset: $('#nsFreshcareSystemCertificate-' + oMembership['agrisubscription.agricertificate.id']).height() * -1, 
								onShow: nsFreshcare.admin.grower.membership.getHistory,
								membership: iMembership,
								object: nsFreshcare.objectCertificate,
								historyFields: nsFreshcare.admin.grower.data.certificate.historyFields,
								auditFieldsFrom: oMembership,
								tablePrefix: 'agrisubscription.agricertificate',
								xhtmlElementSystemID: 'nsFreshcareCertificateSystem',
								xhtml: '<table style="background-color: #EFEFEF; border: 2px solid #B8B8B8">' +
											'<tr>' +
												'<td class="ns1blankspaceSummaryCaption">Membership Certificate History</td>' +
												'<td style="width: 25px; padding: 5px;"><span class="ns1blankspaceSystemClose"></span></td>' +
											'</tr>' + 
											'<tr>' +
												'<td style="padding: 5px;height: 300px;" colspan="2"' +
													' id="nsFreshcareCertificateSystem">' + ns1blankspace.xhtml.loading + '</td>' +
											'</tr></table>'
							});
						});

					$('#nsFreshcareSystemSubRows-' + iMembership)
						.on('click', function()
						{
							// First display screen, then get values to populate it for each of scope, sites & category
							$('#ns1blankspaceMultiUseContainer').css('width', '720');
							ns1blankspace.container.show(
							{
								xhtmlElement: $('#nsFreshcareSystemSubRows-' + iMembership), 
								xhtmlElementID: 'ns1blankspaceMultiUseContainer', 
								leftOffset: (707 - $('#nsFreshcareSystemSubRows-' + iMembership).width()) * -1, 
								topOffset: $('#nsFreshcareSystemSubRows-' + iMembership).height() * -1, 
								onShow: nsFreshcare.admin.grower.membership.getScopeHistory,
								object: nsFreshcare.objectSubscription,
								objectcontext: iMembership,
								auditFieldsFrom: oMembership,
								xhtmlElementSystemID: 'nsFreshcareSubRowsSystem',
								xhtml: '<table style="background-color: #EFEFEF; border: 2px solid #B8B8B8">' +
											'<tr>' +
												'<td class="ns1blankspaceSummaryCaption">Membership Scopes History</td>' +
												'<td style="width: 25px; padding: 5px;"><span class="ns1blankspaceSystemClose"></span></td>' +
											'</tr>' + 
											'<tr>' +
												'<td style="padding: 5px;height: 300px;" colspan="2"' +
													' id="nsFreshcareSubRowsSystem">' + ns1blankspace.xhtml.loading + '</td>' +
											'</tr></table>'
							});
		...
		getHistory: function(oParam, oResponse)
		{
			...
			oParam.systemStep = 2;
			oParam.stripPrefix = true;

			// v4.0.001 Bootstrap - changed icon
			$('.ns1blankspaceSystemClose')
				.button({icons: {primary: 'glyphicon-resize-small'}})
				//.css('font-size', '0.75em')
				//.css('height', '18px')
				//.css('width', '18px')
				.on('click', function()
			...
			var oSearch = new AdvancedSearch();
			oSearch.method = 'AGRI_' + (iObject == nsFreshcare.objectSubscription ? 'SUBSCRIPTION': 'CERTIFICATE') + '_SEARCH';
		...
		getScopeHistory: function(oParam)
		{
			// v3.4.010 SUP023755 Get any audit trail actions and display along with existing data
			var oMembership = ns1blankspace.util.getParam(oParam, 'auditFieldsFrom').value;
			var oXHTMLElementID = ns1blankspace.util.getParam(oParam, 'xhtmlElementSystemID').value;
			oParam.scopeHistoryStep = oParam.scopeHistoryStep || 1;

			// Get scope history rows
			if (oParam.scopeHistoryStep == 1)
			{
				oParam.onComplete = nsFreshcare.admin.grower.membership.getScopeHistory;
				oParam.scopeHistoryStep = 2;
				oParam.attachToObject = oMembership;
				nsFreshcare.util.system.actionHistory.search(oParam);
			}

			// Sort history rows and intersperse them into Sites, Category & Scope rows for display
			else if (oParam.scopeHistoryStep == 2)
			{
				// We map each one so that we don't modify the original list (and we get a date to sort by)
				var aHistorySites = $.map(oMembership.membershipSites, function(x) 
									{	
										var y = 
										{
											actionDate : new Date(x['address.addresslink.createddate']),
											action: "Added",
											actionUser: x['address.addresslink.createdusertext'],
											value: x.address1 + ' ' + (x.address2 ? x.address2 + ' ' : '') + x.addresssuburb + ' ' + 
												   x.addressstate + ' ' + x.addresspostcode + ' ' + x.addresscountry
										}
										return y;
									});
				var aHistoryScope = $.map(oMembership.scopeValues, function(x)
									{	
										var y = 
										{
											actionDate : new Date(x.createddate),
											action: "Added",
											actionUser: x.createdusertext,
											value: x.scopetext
										}
										return y;
									});
				var aHistoryCategory = $.map(oMembership.productGroups, function(x) 
									{	
										var y = 
										{
											actionDate : new Date(x.createddate),
											action: "Added",
											actionUser: x.createdusertext,
											value: x.productcategorytext
										}
										return y;
									});
				var historyRow = function(oRow)
				{
					var a = [];
					aHTML.push('<tr><td class="ns1blankspaceRow" width="100px">' + oRow.actionDate.toString('dd MMM yyyy hh:mm:ss') + 
								'<br />' + oRow.actionUser.split('@').shift() + '@<br />' + oRow.actionUser.split('@').pop() + '</td>');
					aHTML.push('<td class="ns1blankspaceRow">' + oRow.action + '</td>');
					aHTML.push('<td class="ns1blankspaceRow">' + oRow.value + '</td></tr>');
					return a.join('');
				}

				$.each(oMembership.auditTrail, function()
				{
					var dAction = new Date(this.duedatetime);
					var sAction = (this.priority == '1') ? 'Added' : 'Removed';
					var oThisRow = 
					{
						actionDate: dAction,
						action: sAction,
						actionUser: this.actionbytext,
						value: this.description
					};

					if (this.seauditobject == 'agriproductgroupcategory')
					{
						aHistoryCategory = aHistoryCategory.concat(oThisRow);
					}
					else if (this.seauditobject == 'objectscope')
					{
						aHistoryScope = aHistoryScope.concat(oThisRow);
					}
					else if (this.seauditobject == 'addresslink')
					{
						aHistorySites = aHistorySites.concat(oThisRow);
					}
				});
				aHistorySites.sort(ns1blankspace.util.sortBy('actionDate', 'desc'));
				aHistoryScope.sort(ns1blankspace.util.sortBy('actionDate', 'desc'));
				aHistoryCategory.sort(ns1blankspace.util.sortBy('actionDate', 'desc'));
				...
				var aHTML = [];
				aHTML.push('<table id="ns1blankspaceSystemHistory-category">')
				aHTML.push('<tr><td colspan="3" class="ns1blankspaceSummaryCaption">Category History</td></tr>');
				aHTML.push('<tr><td class="ns1blankspaceHeaderCaption">Date / Who</td>');
				aHTML.push('<td class="ns1blankspaceHeaderCaption">Action</td>');
				aHTML.push('<td class="ns1blankspaceHeaderCaption">Value</td>');
				$.each(aHistoryCategory, function()
				{
					aHTML.push(historyRow(this));
				});
				aHTML.push('</tr></table><p>&nbsp;</p>');

				aHTML.push('<table id="ns1blankspaceSystemHistory-scope">')
				aHTML.push('<tr><td colspan="3" class="ns1blankspaceSummaryCaption">Membership Scope History</td></tr>');
				aHTML.push('<tr><td class="ns1blankspaceHeaderCaption">Date / Who</td>');
				aHTML.push('<td class="ns1blankspaceHeaderCaption">Action</td>');
				aHTML.push('<td class="ns1blankspaceHeaderCaption">Value</td>');
				$.each(aHistoryScope, function()
				{
					aHTML.push(historyRow(this));
				});
				aHTML.push('</tr></table><p>&nbsp;</p>');

				aHTML.push('<table id="ns1blankspaceSystemHistory-site">')
				aHTML.push('<tr><td colspan="3" class="ns1blankspaceSummaryCaption">Membership Site History</td></tr>');
				aHTML.push('<tr><td class="ns1blankspaceHeaderCaption">Date / Who</td>');
				aHTML.push('<td class="ns1blankspaceHeaderCaption">Action</td>');
				aHTML.push('<td class="ns1blankspaceHeaderCaption">Value</td>');
				$.each(aHistorySites, function()
				{
					aHTML.push(historyRow(this));
				});
				aHTML.push('</tr></table>');

				$('#' + oXHTMLElementID).html(aHTML.join(''))
			}
		},
		...
		bind: function(oParam)
		{
			...
			$('#ns1blankspaceGrowerMembershipCertificatePrint-' + iMembership)
				...
				.click(function(event)
				{
					// v3.4.010 SUP023761 Added showDifferences param
			...
			$('#ns1blankspaceGrowerMembershipCertificateExport-' + iMembership)
				.button(
				{
					...
					icons: {primary: 'glyphicon-chevron-down'}
				})
				.click(function()
				{
					// v3.4.010 SUP023761 Added showDifferences param
			...
			// v4.0.012 SUP024144 Added Certificate Extension button
			$('#ns1blankspaceGrowerMembershipCertificateExtendExpiry-' + iMembership)
				.button(
				{
					label: 'Extend Certificate Expiry',
					icons: {primary: 'glyphicon-calendar'}
				})
				.click(function()
				{
					// Don't allow Certificate Expiry extension on Scope Extension audit
					if (oMembership['agrisubscription.agricertificate.audit.type'] == nsFreshcare.data.audit.typeScopeExtension)
					{
						ns1blankspace.container.confirm(
						{
							title: 'Certificate Expiry not permitted',
							html: 'This Member\'s most recent audit is a Scope Extension.<br /><br />' +
									'Presently, the system is unable to process Certificate Expiry extensions for these audits. ' +
									'If a certificate expiry extension is required, please contact the Freshcare for assistance.'
						});
					}
					else
					{
						nsFreshcare.admin.grower.membership.extendCertificate({membership: oMembership});
					}
				});
			...
			$('#ns1blankspaceAuditAdd' + iMembership)
				...
				.click(function() 
				{
					...
					// v3.3.001 SUP023795 Make sure we check if any changes before registering audit
					...
					{	
						...
						ns1blankspace.rootnamespace.admin.audit.init(
						{
							contactBusiness: ns1blankspace.data.contactBusiness,
							contactPerson: ns1blankspace.data.contactPerson,
							contactBusinessText: ns1blankspace.data.contactBusinessText,
			...
			$('#ns1blankspaceCertificateRemove-' + oMembership['agrisubscription.agricertificate.id'])
				.button(
				{
					icons: {primary: "ui-icon-close"}
				})
				.click(function() 
				{
					var iCertificate = $(this).attr('id').split('-').pop();
					ns1blankspace.container.confirm(
					{
						title: 'Remove Certificate',
						 html: 'Are you sure you want to remove the certificate for the ' + oMembership['agrisubscription.membershiptext'] + ' membership?',
						 buttons: 
						 [
							{text: "Yes", name: "btnYes", icons: {primary: 'ui-icon-check'},
								click: function() 
								{
									$('#ns1blankspaceBootstrapDialog').modal('hide');
									nsFreshcare.admin.grower.membership.certificate.remove(iCertificate);
								}
							},
							{text: "No", name: "btnNo", icons: {primary: 'ui-icon-close'}, 
								click: function() 
		...
		expiryMonthChangeReason: function()
		{
			...
			if ($('#' + sXHTMLElementID).attr('data-id') != oMembership['agrisubscription.expirymonth'])
			{
				...
				// v3.3.005 Check to see if a certificate exists otherwise the question isn't necessary
				if (!$('#ns1blankspaceMembership' + iMembership + 'ExpiryChangeCertificateUpdate').is('*')
					&& $('#ns1blankspaceMembership' + iMembership + 'CertificateNumber').is('*'))
				{
			...
			else
			{
				...
				if (nsFreshcare.user.roleID == nsFreshcare.data.roles.admin && $('#ns1blankspaceMainMembership' + iMembership + ' .ns1blankspaceExpiryMonthChange:visible').length > 0
					&& $('#' + sXHTMLElementID.replace('ExpiryMonth', 'ExpiryChangeReasonCaption')).html() == 'Re-Certification Due Change Reason'
				...
				else if (nsFreshcare.user.roleID == nsFreshcare.data.roles.admin && oMembership['agrisubscription.expirychangereason'] != '') {}
				else
				{
					$('#ns1blankspaceMainMembership' + iMembership + ' .ns1blankspaceExpiryMonthChange').hide();
				}
		...
		{
			// v3.3.001 SUP023456 Sets Valid scope options plus defaults the COP, plus sets Harvest Months caption
			...
			$('#ns1blankspaceMembershipHarvestMonthsCaption').html((iMembershipID == nsFreshcare.data.membershipSCS ? 'Operational' : 'Harvest') + ' Months');
			nsFreshcare.admin.grower.membership.defaultCodeOfPractice({membership: iMembershipID});
			nsFreshcare.admin.grower.membership.defaultScopeOptions({membership: iMembershipID, subscription: $('.ns1blankspaceControlMain:visible').attr('data-subscriptionid')});
			nsFreshcare.admin.grower.membership.defaultCropTable({membership: iMembershipID});
		},
		...
		defaultCodeOfPractice: function(oParam)
		{
			...
			if (iMembershipID)
			{	// v3.3.001 SUP023456 Get default COP from nsFreshcare.data.memberships 
				var oMembership = $.grep(nsFreshcare.data.memberships, function(x) {return x.id == iMembershipID}).shift();
				var oCodeOfPractice = $.grep(oMembership.codesOfPractice, function(x) {return x.isdefault == 'Y'}).shift();
				if (oCodeOfPractice)
				{
					var sElementID = ($('#' + sXHTMLElementID + 'COP').is('*')) ? sXHTMLElementID + 'COP' : sXHTMLElementID.replace('Membership', 'COP');
					
					$('#' + sElementID).attr('data-id', oCodeOfPractice.id);
					$('#' + sElementID).val(oCodeOfPractice.description);
				}
			}
		},

		defaultScopeOptions: function(oParam)
		{	// v3.3.001 Sets Scope Options based on Membership or COP selected
			var sXHTMLElementID = ns1blankspace.util.getParam(oParam, 'xhtmlElementID', {'default': 'ns1blankspaceMembershipScopeUpdate'}).value;	
			var iMembershipID = ns1blankspace.util.getParam(oParam, 'membership', {'default': $('#' + sXHTMLElementID).attr('data-id')}).value;	
			var iSubscriptionID = ns1blankspace.util.getParam(oParam, 'subscription', {'default': ''}).value;
			var iCodeOfPractice = ns1blankspace.util.getParam(oParam, 'codeOfPractice').value;	
			...
			if (iMembershipID)
			{
				...
				{
					if (iCodeOfPractice == undefined)
					{
						iCodeOfPractice = $('#' + ns1blankspace.xhtml.divID).attr('data-id');
					}
					var oSourceObject = $.grep(oMembership.codesOfPractice, function(x) {return x.id == iCodeOfPractice}).shift();
					iSubscriptionID = iSubscriptionID || $('#' + ns1blankspace.xhtml.divID).attr('data-subscriptionid');
				}
				else
				{
					var oSourceObject = oMembership;
				}

				if (oSourceObject)
				{
					aHTML = ['<table class="ns1blankspaceMembership' + iSubscriptionID + 'ScopeUpdateOptions" style="margin-bottom:7px;">'];
					$.each(oSourceObject.validScopes, function() 
					{
						aHTML.push('<tr id="ns1blankspaceScopeRow_' + this.scope + '">' +
										'<td id="nsFreshcareScope-' + iSubscriptionID + '_' + this.scope + '" ' + 
										'class="nsFreshcareScope nsFreshcareSelectable">' + this.scopetext + '</td>' + 
										'</tr>');
					});
					aHTML.push('</table>');

					if (iSubscriptionID != '') {sXHTMLElementID = sXHTMLElementID.replace('MembershipScope', 'Membership' + iSubscriptionID + 'Scope')}
					if (!$('#' + sXHTMLElementID).is('*'))
					{
						sXHTMLElementID = 'ns1blankspaceDetailsScopeUpdate';
					}
					$('#' + sXHTMLElementID).html(aHTML.join(''));

					// Bind the Scope area
					$('#' + sXHTMLElementID +' .nsFreshcareScope').on('click', function() 
					{
						nsFreshcare.external.grower.manageMultiRows(this.id, iSubscriptionID);
					});
				}
			}
		},

		defaultCropTable: function(oParam)
		{	// v3.3.001 Sets Crop table based on what's set up against Membership

			var sXHTMLElementID = ns1blankspace.util.getParam(oParam, 'xhtmlElementID', {'default': 'ns1blankspaceMembershipCropsUpdate'}).value;	
			var iMembershipID = ns1blankspace.util.getParam(oParam, 'membership', {'default': $('#' + sXHTMLElementID).attr('data-id')}).value;	
			var oStandard = $.grep(nsFreshcare.data.memberships, function(x) {return x.id == iMembershipID}).shift();

			if (oStandard.secropslookup || $('#' + sXHTMLElementID).attr('data-methodFilter').indexOf(oStandard.secropslookup) == -1)
			{
				var aMethodFilter = $('#' + sXHTMLElementID).attr('data-methodFilter').split('|');
				aMethodFilter = $.map(aMethodFilter, function(filter)
				{
					if (filter.substr(0,7) == 'element')
					{
						filter = 'element-EQUAL_TO-' + oStandard.secropslookup;
					}
					return filter;
				});
				$('#' + sXHTMLElementID).attr('data-methodFilter', aMethodFilter.join('|'));
			}
		},
		...
		defaultWineryValues: function(oParam)
		{
			...
			if (iMembership === nsFreshcare.data.membershipWIN || iMembership === nsFreshcare.data.membershipVIT)
			{
				...
				// v3.4.004 Let Membership setup determine scope values
		...
		extendCertificate: function(oParam)
		{
			var oMembership = ns1blankspace.util.param(oParam, 'membership').value; 
			var iMembership = ns1blankspace.util.param(oMembership, 'id').value;
			var sCertificateAudit = ns1blankspace.util.param(oMembership, 'agrisubscription.agricertificate.audit').value;
			var sExtensionMonths = ns1blankspace.util.param(oMembership, 'agrisubscription.agricertificate.audit.seextensionmonths').value;
			var sExpiry = ns1blankspace.util.param(oMembership, 'agrisubscription.agricertificate.enddate').value;
			var iCertificate = ns1blankspace.util.param(oMembership, 'agrisubscription.agricertificate.id').value;

			// First display the question
			if (oParam.extendCertStep == undefined)
			{
				var aHTML = [];

				aHTML.push('<table class="ns1blankspace" style="width:100%;">');

				aHTML.push('<tr><td class="ns1blankspaceSummaryCaption">Number of  month\'s extension required from ' +
									'current expiry date of ' + sExpiry + 
									(sExtensionMonths != '' 
										? '<br /><span class="ns1blankspaceSubNote">(already extended by ' + sExtensionMonths + ' months)</span>'
										: '') +
								'</td></tr>' +
							'<tr><td class="ns1blankspaceText">' +
								'<input id="ns1blankspaceExtendCertificateMonths" class="ns1blankspaceText">' +
							'</td></tr>');

				aHTML.push('<tr><td class="ns1blankspaceSummaryCaption">Reason for certification extension</td></tr>' +
							'<tr><td class="ns1blankspaceText">' +
								'<textarea rows="5" cols="35" id="ns1blankspaceExtendCertificateReason" class="ns1blankspaceTextMulti"' +
									' style="width: 100%;"></textarea>' +
							'</td></tr>');

				aHTML.push('<tr><td class="ns1blankspaceSummaryCaption">Please attach any supporting evidence</td></tr>' +
							'<tr><td class="ns1blankspaceText">' +
								ns1blankspace.attachments.upload.show(
								{
									showUpload: false,
									attachmentType: nsFreshcare.data.attachmentTypeCertificateExtension,
									object: nsFreshcare.objectAudit,
									objectContext: oMembership['agrisubscription.agricertificate.audit']
								}) +
							'</td></tr>');

				aHTML.push('<tr><td class="ns1blankspaceSummaryCaption" id="ns1blankspaceExtendCertificateError" style="color: red;"></td></tr>');

				aHTML.push('</table>');

				ns1blankspace.container.confirm(
				{
					title: 'Enter details to support your request for a certification extension of up to ' + 
						nsFreshcare.option.certificateExtension.maxExtensionMonths + ' months',
					...
					[
						{
							...
							{
						 		ns1blankspace.okToSave = true;
						 		var iMonths = $('#ns1blankspaceExtendCertificateMonths').val().replace(/,/g, '');
						 		var sReason = $('#ns1blankspaceExtendCertificateReason').val();
						 		var iExtensionMonths = (sExtensionMonths != '') ? parseInt(sExtensionMonths) : 0;

						 		if (iMonths == '')
						 		{
						 			$('#ns1blankspaceExtendCertificateError').html('Please enter the number of months');
						 			ns1blankspace.okToSave = false;
						 		}
						 		else 
						 		{
						 			if (isNaN(iMonths))
						 			{
							 			$('#ns1blankspaceExtendCertificateError').html('Months must be a number');
							 			ns1blankspace.okToSave = false;
						 			}
						 			else if (iMonths.indexOf('.') > -1)
						 			{
							 			$('#ns1blankspaceExtendCertificateError').html('Months must be a whole number');
							 			ns1blankspace.okToSave = false;
						 			}
						 			else 
						 			{
						 				iMonths = parseInt(iMonths);
						 				if (iExtensionMonths + iMonths > nsFreshcare.option.certificateExtension.maxExtensionMonths)
						 				{
								 			$('#ns1blankspaceExtendCertificateError').html('Maximum number of extension months exceeded');
								 			ns1blankspace.okToSave = false;
						 				}
						 			}
						 		}
						 		...
						 		if (ns1blankspace.okToSave)
						 		{
						 			if (sReason == '')
						 			{
							 			$('#ns1blankspaceExtendCertificateError').html('Please enter a reason');
							 			ns1blankspace.okToSave = false;
						 			}
						 		}
						 		...
						 		if (ns1blankspace.okToSave)
						 		{
						 			if ($('#oFile0').val() != '' && $('#oFile0').val() == undefined)
						 			{
						 				$('#ns1blankspaceExtendCertificateError').html('Certificate extension supporting evidence required to be attached');
							 			ns1blankspace.okToSave = false;
						 			}
						 		}
						 		...
						 		if (ns1blankspace.okToSave)
						 		{
							 		$('#ns1blankspaceExtendCertificateError').html('');
							 		$('#ns1blankspaceBootstrapDialog').modal('hide'); 	
									oParam.extendCertStep = 1;
							 		oParam.extension =
							 		{
							 			months: iMonths + iExtensionMonths,
							 			reason: sReason,
							 			fileAttached: ($('#oFile0').val() != '' && $('#oFile0').val() != undefined)
							 		}
							 		nsFreshcare.admin.grower.membership.extendCertificate(oParam);
							 	}
							}
						},
						{
			...
			// Find new expiry date and update Subscription
			else if (oParam.extendCertStep == 1)
			{
				ns1blankspace.status.working();
				
				var dExpiry = nsFreshcare.admin.certificate.certificateGetExpiry(
				{
					auditDate: new Date(oMembership['agrisubscription.agricertificate.audit.actualdate']),
					expiry: new Date(sExpiry),
					expiryMonths: (oMembership['agrisubscription.agricodeofpractice.auditdueafter'] != '' ? parseInt(oMembership['agrisubscription.agricodeofpractice.auditdueafter']) : '0'),
					expiryAnniversary: oMembership['agrisubscription.expirymonth'],
					extensionMonths: oParam.extension.months
				});

				var oData = 
				{
					id: iCertificate,
					enddate: dExpiry.toString('dd MMM yyyy')
				};
				...
				$.ajax(
				{
					...
					success: function(oResponse)
					{
						if (oResponse.status == 'OK')
						{
							ns1blankspace.status.message('Certificate Expiry updated to ' + dExpiry.toString('dd MMM yyyy'));
							oParam.extendCertStep = 2;
							nsFreshcare.admin.grower.membership.extendCertificate(oParam);
						}
						else
						{
							ns1blankspace.status.error('Unable to update certificate expiry: ' + oResponse.error.errornotes);
						}
					}
				});
			}

			// Update Audit - set seextensionmonths
			else if (oParam.extendCertStep == 2)
			{
				var oData = 
				{
					id: oMembership['agrisubscription.agricertificate.audit'],
					seextensionmonths: oParam.extension.months
				};
				...
				$.ajax(
				{
					...
					success: function(oResponse)
					{
						if (oResponse.status == 'OK')
						{
							ns1blankspace.status.message('Extension months updated.');
							oParam.extendCertStep = 3;
							nsFreshcare.admin.grower.membership.extendCertificate(oParam);
						}
						else
						{
							ns1blankspace.status.error('Unable to update extension months: ' + oResponse.error.errornotes);
						}
					}
				});
			}
			...
			// add note
			else if (oParam.extendCertStep == 3)
			{
				sExtensionMonths = (sExtensionMonths == '') ? '0' : sExtensionMonths;
				var oData = 
				{
					object: nsFreshcare.objectAudit,
					objectcontext: oMembership['agrisubscription.agricertificate.audit'],
					actiontype: nsFreshcare.data.actionTypeCertificateExtension,
					duedate: dToday.toString('dd MMM yyyy'),
					description: "Certificate extension of " + (oParam.extension.months - parseInt(sExtensionMonths)) + ' months requested ' +
								'due to ' + oParam.extension.reason,
					priority: '2',
					contactbusiness: ns1blankspace.objectContext,
					status: '1'
				};
				...
				$.ajax(
				{
					...
					success: function(oResponse)
					{
						if (oResponse.status == 'OK')
						{
							ns1blankspace.status.message('Certificate extension note added.');
							oParam.extendCertStep = 4;
							nsFreshcare.admin.grower.membership.extendCertificate(oParam);
						}
						else
						{
							ns1blankspace.status.error('Unable to add certificate extension note: ' + oResponse.error.errornotes);
						}
					}
				});
			}
			...
			// attach file
			else if (oParam.extendCertStep == 4)
			{
				oParam.extendCertStep = 10;
				if (oParam.extension.fileAttached)
				{
					oParam.functionPostUpdate = nsFreshcare.admin.grower.membership.extendCertificate;
					oParam.submit = true;
			...
			// refresh view
			else if (oParam.extendCertStep == 10)
			{
				ns1blankspace.inputDetected = false;
				ns1blankspace.status.working();
				nsFreshcare[nsFreshcare.user.roleLower].grower.search.send({id: ns1blankspace.objectContext, xhtmlElementID: '-' + ns1blankspace.objectContext});
			}
		},

		certificate:
		{
			remove: function(iCertificate)
			{
				var oData = {id: iCertificate, remove: '1'};
				$.ajax(
				{
					type: 'POST',
					url: ns1blankspace.util.endpointURI('AGRI_CERTIFICATE_MANAGE'),
		...
		crops: 
		{
			...
			{
				// v3.3.001 SUP023456 Allow different list of Crops to be used
				var aCropTables = [];

				if (nsFreshcare.data.cropList === undefined || nsFreshcare.data.cropGroups == undefined)
				{

					$.each(nsFreshcare.data.memberships, function(i, x)
					{
						if ($.inArray(x.secropslookup, aCropTables) == -1)
						{
							aCropTables.push(x.secropslookup);
						}
					});

					var oSearch = new AdvancedSearch();
					oSearch.method = 'SETUP_STRUCTURE_ELEMENT_OPTION_SEARCH';
					oSearch.addField('element,title');
					oSearch.addFilter('element', 'IN_LIST', aCropTables.join(','));
					oSearch.rows = 500;
					oSearch.async = false;
					oSearch.sort('element', 'asc');
					oSearch.sort('title', 'asc');
					oSearch.getResults(function(oResponse)
					{
						if (oResponse.status === 'OK')
						{
							nsFreshcare.data.cropList = $.map($.grep(oResponse.data.rows, function(y) {return y.element == nsFreshcare.structureElementCrops}), 
																	function(x) {return {"title": x.title, "id": x.id}});
							nsFreshcare.data.cropGroups = $.map($.grep(oResponse.data.rows, function(y) {return y.element == nsFreshcare.structureElementCropGroups}), 
																	function(x) {return {"title": x.title, "id": x.id}});
				...
				{
					// v3.3.001 Determine whether to use cropList or cropGroups and set crops
					if (oParam.cropsTable == nsFreshcare.structureElementCropGroups)
					{
						nsFreshcare.data.crops = $.map(nsFreshcare.data.cropGroups, function(x) {var o = {id: x.id, title: x.title}; return o;});
					}
					else
					{
						nsFreshcare.data.crops = $.map(nsFreshcare.data.cropList, function(x) {var o = {id: x.id, title: x.title}; return o;});
					}
			...
			show: function(oParam)
			{
				...
				if (oParam.xhtmlElementId) {sXHTMLElementId = oParam.xhtmlElementId}
				oParam.cropsShowStep = oParam.cropsShowStep || 1;

				if (oParam.cropsShowStep == 1)
				{
						// v3.3.001 Always call getList so we're using the correct crop list
					oParam.cropsShowStep = 2;
					oParam.onCompleteWhenCan = oParam.onComplete;
					oParam = ns1blankspace.util.setParam(oParam, 'onComplete', nsFreshcare.admin.grower.membership.crops.show);
					nsFreshcare.admin.grower.membership.crops.getList(oParam);
				}

				else if (oParam.cropsShowStep == 2)
				{
				...
				else
				{
					...
					if (!bShowRows || !$('ns1blankspaceMembership' + iMembership + 'Audits').is('*'))
					{
						...
						aHTML.push('<tr class="ns1blankspaceCaption ns1blankspaceTableCaption">' +
										'<td class="ns1blankspaceCaption" colspan="10">' +
										'Audits' +
										'</td>' +
										'<td style="text-align:right;">' +
											'<span class="ns1blankspaceAction" title="' + (bShowRows ? 'Hide' : 'Show') + ' Audits"' +
												' id="ns1blankspaceAudits' + iMembership + 'Show"></span>' +
										'</td></tr>');

						if ($('ns1blankspaceMembership' + iMembership + 'Audits').is('*'))
						{
							aHTML.push('<tr id="ns1blankspaceAuditsLoading_' + iMembership + '">' +
											'<td colspan="10">' + ns1blankspace.xhtml.loadingSmall + '</td></tr>');
						}
						...
						$('#ns1blankspaceAudits' + iMembership + 'Show')
							.button(
							{
								...
								icons: {primary: (bShowRows ? 'glyphicon-chevron-up' : 'glyphicon-chevron-down')}
							})
							.on('click', function(event)
							{
								...
								else
								{
									...
									$(this).button(
									{
										text: false,
										//label: 'Hide Audits',
										icons: {primary: 'glyphicon-chevron-up'}
									})
									.attr('title', 'Hide Audits');
									if ($('#ns1blankspaceMembership' + iMembership + 'AuditHeader').is('*')) 
									{
					...
					if (bShowRows)
					{
						...
						aHTML.push('<tr class="ns1blankspaceCaption" id="ns1blankspaceMembership' + iMembership + 'AuditHeader">' +
										...
										'<td class="ns1blankspaceHeaderCaption">' + nsFreshcare.data.growerText + ' Contact</td>' +
										'<td class="ns1blankspaceHeaderCaption" title="Register Audit"' +
											' id="ns1blankspaceAuditsRegisterAudit' + iMembership + '"></td>' +
										'</tr>');
						...
						// v3.4.010 SUP023214 Added Lapsed
						$($.grep(ns1blankspace.objectContextData.audits, function (a) {return a["audit.subscription"] == iMembership;})).each( function() 
						{
							aHTML = [];
							aHTML.push('<tr class="ns1blankspaceGrowerMembership' + iMembership + 'Audits">' + 
								   ...
								   '<td class="ns1blankspaceRow" id="ns1blankspaceAudit_resultstatustext-' + this.id + '">' + 
								   		...
								   		(this['audit.membershipstatus'] == '-1' ? '<br /><span class="ns1blankspaceSubNote">(Lapsed)</span>' : '') +
								   		'</td>' +
								   '<td class="ns1blankspaceRow" id="ns1blankspaceAudit_resultstatustext-' + this.id + '">' + 
								   ...
								   '<td class="ns1blankspaceRow" id="ns1blankspaceAudit_contactpersontext-' + this.id + '">' + 
								   	...
								   	'<td class="ns1blankspaceRowSelect ns1blankspaceGrowerMembershipAudits' + iMembership + 'RowSelect"' +
								   			' id="ns1blankspaceGrowerAudit_options_view-' + this.id + '" title="Open"></td>' +
								   '</tr>');
							$('#ns1blankspaceMembership' + iMembership + 'Audits').children().last().after(aHTML.join(''));
						});
			...
			show: function(oParam)
			{
				...
				else
				{
					...
					if (!bShowRows || !$('#ns1blankspaceMembership' + iMembership + 'StatusTransactions').is('*'))
					{
						...
						aHTML.push('<tr class="ns1blankspaceCaption ns1blankspaceTableCaption">' +
										'<td class="ns1blankspaceCaption" colspan="6">' +
										'Status History' +
										'</td><td style="text-align:right;">' +
											'<span class="ns1blankspaceAction" id="ns1blankspaceStatus' + iMembership + 'Show"></span>' +
										'</td></tr>');
						...
						if ($('ns1blankspaceMembership' + iMembership + 'StatusTransactions').is('*'))
						{
							aHTML.push('<tr id="ns1blankspaceStatusTransactionsLoading_' + iMembership + '">' +
											'<td colspan="6">' + ns1blankspace.xhtml.loadingSmall + '</td></tr>');
						}
					...
					if (bShowRows)
					{
						...
						$($.grep(ns1blankspace.objectContextData.statusTransactions, function (a) {return a["subscription"] == iMembership;})).each(function(i, oRow) 
						{
							...
							if (nsFreshcare.user.roleID === nsFreshcare.data.roles.admin && i === 0 && ns1blankspace.option.preventUpdates != true)
							{
		...
		trainingCourses: 
		{
			show: function(oParam)
			{
				// v3.3.001 SUP023422 Now searches for training courses linked to attendees
				var aHTML = [];
				var iMembership = ns1blankspace.util.getParam(oParam, 'membership').value;
				var oMembership = ns1blankspace.util.getParam(oParam, 'membershipData').value;
				...
				else if (bShowRows && ns1blankspace.objectContextData.trainingCourses === undefined) 
				{
					...
					oParam.contactBusiness = ns1blankspace.objectContextData.id;
					oParam.onCompleteWhenCan = oParam.onComplete;
					oParam.sortColumn = 'agritraineeattendancelink.attendee.course.coursedate';
					oParam.sortDirection = 'desc';
					oParam.onComplete = nsFreshcare.admin.grower.membership.trainingCourses.show;
				...
				else
				{
					...
					if (!bShowRows || !$('#ns1blankspaceMembership' + iMembership + 'Training').is('*'))
					{
						...
						aHTML.push('<tr class="ns1blankspaceCaption ns1blankspaceTableCaption">' +
										...
										'</td>' +
										'<td style="text-align:right;">' +
											'<span class="ns1blankspaceAction" id="ns1blankspaceTraining' + iMembership + 'Show"></span>' +
										'</td></tr>');
					...
					if (bShowRows)
					{
						...
						// v3.3.002 SUP023422 id for TrainingRowSelect contained 'undefined'
						// v3.4.001 SUP023826 Show attending trainee in orange if not the same as linked contact
						$($.grep(ns1blankspace.objectContextData.trainingCourses, function (a) 
						{
							return a["agritraineeattendancelink.subscription.membership"] == oMembership['agrisubscription.membership']})).each( function() 
							{
								aHTML = [];
								var sWarningStyle = ' style="color: #ee8f00;"';
								var bShowWarning = (this['agritraineeattendancelink.attendee.surname'] + ', ' + this['agritraineeattendancelink.attendee.firstname']) 
													!= this['agritraineeattendancelink.attendee.traineecontactpersontext']
								...
								aHTML.push('<tr class="ns1blankspaceGrowerMembership' + iMembership + 'Training">' + 
										'<td class="ns1blankspaceRow" id="ns1blankspaceTraining_coursedate-' + this['agritraineeattendancelink.attendee.course'] + '">' +
									   		this["agritraineeattendancelink.attendee.course.coursedate"] + 
									   		'<span class="ns1blankspaceSub"><br />(' + this["agritraineeattendancelink.attendee.course.statustext"] +  ')</span></td>' +
									   '<td class="ns1blankspaceRow" id="ns1blankspaceTraining_business-' + this['agritraineeattendancelink.attendee.course'] + '">' + 
											this['agritraineeattendancelink.attendee.course.trainercontactbusinesstext'] + 
											(this['agritraineeattendancelink.attendee.course.trainercontactpersontext'] != ''
												? '<br /><span class="ns1blankspaceSub">(' + this['agritraineeattendancelink.attendee.course.trainercontactpersontext'] + ')</span>'
												: '') +
									   		'</td>' +
									   '<td class="ns1blankspaceRow" id="ns1blankspaceTraining_course-' + this['agritraineeattendancelink.attendee.course'] + '">' + 
									   		this["agritraineeattendancelink.attendee.course.package.membership.code"] + ' ' + this["agritraineeattendancelink.attendee.course.package.codeofpracticetext"] + '</td>' +
									   '<td id="ns1blankspaceTraining_trainee-' + this['agritraineeattendancelink.attendee.course'] + '" class="ns1blankspaceRow' +
											((bShowWarning) 
												? ' ns1blankspaceLinkedPerson"' + sWarningStyle + ' data-contactpersontext="' + this['agritraineeattendancelink.attendee.traineecontactpersontext'] + '"' 
												: '"') + '>' +
									   		this["agritraineeattendancelink.attendee.firstname"] + ' ' + this["agritraineeattendancelink.attendee.surname"] + 
									   		(this.contactbusiness != this['agritraineeattendancelink.attendee.traineecontactbusiness'] 
									   			? '<span class="ns1blankspaceSub"><br />(' + this['agritraineeattendancelink.attendee.traineecontactbusinesstext'] + ')</span>' 
									   			: '') +
									   		'</td>' +
									   	'<td class="ns1blankspaceRowSelect ns1blankspaceMembership' + iMembership + 'TrainingRowSelect"' +
									   		' id="ns1blankspaceGrowerTraining_options_view-' + this['agritraineeattendancelink.attendee.course'] + '"></td>' +
									   '</tr>');
								$('#ns1blankspaceMembership' + iMembership + 'Training').children().last().after(aHTML.join(''));
							}
						);
						...
						// v3.4.001 SUP023826 Show Linked Trainee on hover
						if (nsFreshcare.user.roleID == nsFreshcare.data.roles.admin)
						{
							$('#ns1blankspaceMembership' + iMembership + 'Training .ns1blankspaceLinkedPerson')
							.hover(
								function() 
								{
		...
		save:
		{
			validate: function(oParam)
			{
					...
					&& oMembership['agrisubscription.expirymonth'] != $('#ns1blankspaceMembership' + iMembership + 'ExpiryMonth').attr('data-id'))
				{
					...
					else if ($('input[name="radio' + iMembership + 'UpdateCertificateExpiry"]').is('*')
						&& $('input[name="radio' + iMembership + 'UpdateCertificateExpiry"]:checked').val() == undefined)
					{
						ns1blankspace.okToSave = false;
	...
	siteAddress: 
	{
		search: function(oParam, oResponse) 
		{
			oParam = oParam || {};
			var iStep = 1;
			var iObject = ns1blankspace.util.getParam(oParam, 'object', {'default': nsFreshcare.objectBusiness}).value;	// v3.4.001 Added

			if (oParam.sitesStep) {iStep = oParam.sitesStep; }

			if (iStep === 1 && ns1blankspace.objectContext && ns1blankspace.objectContext != -1 && ns1blankspace.objectContextData) 
			{
				// SUP02755 Added link's audit fields
				ns1blankspace.objectContextData.sites = [];
				var oSearch = new AdvancedSearch();
				oSearch.method = 'CORE_LOCATION_ADDRESS_SEARCH';
				oSearch.addField('status,statustext,address1,address2,addresssuburb,addressstate,addresspostcode,addresscountry,type,' +
									'address.addresslink.object,address.addresslink.objectcontext,address.addresslink.id' +
									',address.addresslink.createddate,address.addresslink.createduser,address.addresslink.createdusertext');
				oSearch.addFilter("address.addresslink.object", "EQUAL_TO", iObject);
		...
		remove: function(oParam)	
		{
			...
			if (oParam.addressStep === 1)
			{
				...
				oSearch.addField('address,object,objectcontext,addresslink.address.status' +
								',addresslink.address.addresss1,addresslink.address.addresss2,addresslink.address.addressssuburb' +
								',addresslink.address.addresssstate,addresslink.address.addressspostcode,addresslink.address.addressscountry');
			...
			else if (oParam.addressStep === 2)
			{
				...
				if (iAddressIndex < nsFreshcare.admin.grower.data.addressLinks.length)
				{
					...
					// v3.4.010 SUP023755 Add a history row when link is removed
						...
						)
					{
						var oHistoryData = 
						{
							object: oThisLink.object,
							objectcontext: oThisLink.objectcontext,
							seauditobject: ((oThisLink.object === nsFreshcare.objectBusiness) ? 'Member Site' : 'Membership Site'),
							description: oThisLink['addresslink.address.addresss1'] + ' ' + oThisLink['addresslink.address.addresss2'] +
										oThisLink['addresslink.address.addressssuburb'] + ' ' + oThisLink['addresslink.address.addresssstate'] + 
										oThisLink['addresslink.address.addressspostcode'] + oThisLink['addresslink.address.addressscountry'],
							text: ns1blankspace.objectContextData['contactbusiness.tradename'],
							createddate: oThisLink['addresslink.address.createddate'],
							createduser: oThisLink['addresslink.address.createduser']
						};
						...
						$.ajax(
						{
							...
							success: function(oResponse)
							{
								if (oResponse.status === 'OK')
								{
									...
									oParam.onComplete = nsFreshcare.admin.grower.siteAddress.remove;
									nsFreshcare.util.system.actionHistory.save(oParam);
								}
								else
								{
