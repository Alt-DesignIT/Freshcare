...
nsFreshcare.auditor.home = 
{
	...
	{
		// v3.1.2 SUP022693 Only show new task lists if Auditor is rolled-out for JASANZ
	...
	auditStatus: 	
	{
		...
		show: 	function(oParam, oResponse) 
		{
			...
			if ($('#' + sXHTMLElementID).attr('data-loading') == '1') 
			{
				...
				if (bShowFilters && aResultStatusValues == undefined)
				{
					...
					if (dJASANZ > dToday)
					{
						oSearch.addFilter('id', 'IN_LIST', nsFreshcare.data.audit.resultStatusNoResult + ',' + nsFreshcare.data.audit.resultStatusPending + ',' + nsFreshcare.data.audit.resultStatusCompleted);
					}
					oSearch.getResults(function(oResponse)
					{
						if (oResponse.status == 'OK')
				...
				else
				{
					...
					if (bShowFilters)
					{
						...
						aHTML = [];
						aHTML.push('<span id="ns1blankspaceHomeAuditSearchHandle" style="height:25px">Search Criteria</span>');
		...
		bind: 	function(oParam) 
		{
			...
			.click(function()
			{
				nsFreshcare.admin.audit.init({showHome: false});
	...
	auditsDue: 	
	{
		...
		show: 	function(oParam, oResponse) 
		{
			...
			if ($('#' + sXHTMLElementID).attr('data-loading') == '1') 
			{
				...
				// Search criteria bar 'handle' - allows user to get the search criteria div back into view
				aHTML = [];
				aHTML.push('<span id="ns1blankspaceHomeAuditsDueSearchHandle" style="height:25px">Search Criteria</span>');
	...
	pendingGrowers: 
	{
		...
		show: 	function(oParam, oResponse)
		{
			...
			if ($('#' + sXHTMLElementID).attr('data-loading') == '1') {
				...
				// Search criteria bar 'handle' - allows user to get the search criteria div back into view
				aHTML = [];
				aHTML.push('<span id="ns1blankspaceHomeGrowerSearchHandle" style="height:25px">Search Criteria</span>');
			...
			.click(function()
			{
				...
				oSearch.getResults(function(oResponse)
				{
					if (oResponse.status === 'OK')
					{
						if (oResponse.data.rows.length > 0)
						{
							nsFreshcare.admin.audit.init({showHome: false, id:oResponse.data.rows[0].id});
							//nsFreshcare.admin.audit.search.send('-' + oResponse.data.rows[0].id);
						}
						else
						{
...
nsFreshcare.auditor.home.report = 
{
	initData: 	function (oParam)
	{
		...
		[ 
			...
			{name: "auditissue.sintypetext", caption: "CAR Type"},
		...
		{
			// v2.0.4d SUP021492 Added filter for only current memberships as was exporting SaladGap. Also added hidden address2 and now filters on address.status 1,2
			ns1blankspace.report.reportGroups.push({id: 3, name: 'Exports'});
			...
			ns1blankspace.report.reports.push(
				{	/* v3.1.206 SUP023035 Now exports business mailing address */
					...
					returnParameters: 'contactbusiness' +
									 ...
									 ',contactbusiness.addresslink.address' +
									 ',contactbusiness.agrisubscription.agricertificate',
					summary: 'Export recently modified Growers ',
					showSort: false,
					functionSearch: nsFreshcare.auditor.grower.search.send,
					windowOpen: '/#/nsFreshcare-auditor.grower/id:',
					customFilters: 
			...
			ns1blankspace.report.reports.push(
				{
					...
					fixedParameters:
					{
						...
						[
							...
							{name: "audit.contactbusiness.tradename", exportHeader: "Grower Trading Name"},
							{name: "audit.contactbusiness.legalname", exportHeader: "Grower Legal Name"},
							{name: "audit.contactperson.firstname", exportHeader: "Grower Name"},
							{name: "audit.contactperson.surname", exportHeader: "Grower Surname"},
...
nsFreshcare.auditor.setup = 
{
	exports: 	function() 
	{
		...
		if ($.grep(ns1blankspace.setup.file["export"].formats, function(x) {return x.name === 'Grower Export'}).length === 0) {
		...
		if ($.grep(ns1blankspace.setup.file["export"].formats, function(x) {return x.name === 'Grower Export - Sites'}).length === 0) {
			ns1blankspace.setup.file["export"].formats.push(
					{
						name: 'Grower Export - Sites',
						header:
						[
							{
								line: 1,
