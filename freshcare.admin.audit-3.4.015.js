...
nsFreshcare.admin.audit = 
{
	data: 
	{
		...
		[	/* v3.1.215 changed titletext to typetext 
			...
			{
				name: 'audit.sequalifyingtraininglink', 
			...
			if (sSearchContext != undefined && iSource != ns1blankspace.data.searchSource.browse)
			{
				...
				if (nsFreshcare.data.jasanzDateId)
				{
					...
					oSearch.addField('audit.auditbusiness.semanualauditapproval' +  
										',audit.auditbusiness.se' + nsFreshcare.data.selfCertificationDateId + ',audit.sejasanzaudit');
				}
	...
	details: 	function ()
	{
		...
		if ($('#ns1blankspaceMainDetails').attr('data-loading') == '1')
		{
			...
			$('#ns1blankspaceDetailsActualDate').on('change', nsFreshcare.admin.audit.setJASANZAuditFlag);
	...
	setJASANZAuditFlag: function()
	{
		...
		if (ns1blankspace.objectContext == -1)
		{
			if ($('#ns1blankspaceDetailsMembership').attr('data-JASANZMembership') == 'Y')
			{
				if (nsFreshcare.data.auditor)
				{
					var oMembership = $.grep(nsFreshcare.data.auditor.jasanzDates, function(x) 
						{return x.membership == $('#ns1blankspaceDetailsMembershipValue').val()}).shift();

					if (dAudit != '' && oMembership && oMembership.notes)
					{
						dAudit = new Date(dAudit);
						dJASANZ = new Date(oMembership.notes);
						$('[name="radioJASANZ"][value="' + (dAudit >= dJASANZ ? 'Y' : 'N') + '"]').attr('checked', true);
						// v3.3.005 SUP023800 Only disable JASANZ date for ther than admin & auditor
						if (nsFreshcare.user.roleID != nsFreshcare.data.roles.admin 
							&& nsFreshcare.user.roleID != nsFreshcare.data.roles.auditor)
						{
							$('[name="radioJASANZ"]').attr('disabled', (dAudit < dJASANZ));
						}
					}
				}

				if (dJASANZ == undefined)
				{	// we don't know if they need to specify. Set to N and enable
					$('[name="radioJASANZ"][value="N"]').attr('checked', true);
					$('[name="radioJASANZ"]').attr('disabled', false);
				}
			}
			// Membership is not JASANZ accredited - always set to No
			else
			{
				$('[name="radioJASANZ"][value="N"]').attr('checked', true);
			}
	...
	setMembershipId: function(oParam, oResponse) 
	{
		...
		if (oResponse === undefined) 
		{
			...
			if ($('#ns1blankspaceDetailsMembership').attr('data-id') != undefined) 
			{
				...
				oSearch.addField('membership,status,membershiptext,expirymonth,codeofpractice');
			...
			if (oParam.setMembershipStep == 1) 
			{
				if (oResponse.status === 'OK') 
				{
					if (oResponse.data.rows.length > 0) 
					{
						...
						$('#ns1blankspaceDetailsMembership').attr('data-expirymonth', oResponse.data.rows[0].expirymonth);
						...
						{
							var aFilters = $('#ns1blankspaceDetailsAuditor').attr('data-methodFilter').split('|');
						...
						// v3.3.001 SUP023474 Sets JASANZMembership Flag and shows relevant fields as applicable
						if (nsFreshcare.option.jasanzAccreditation)
						{
							if (oMembership)
							{
	...
	trainee: 
	{
		...
		bind: function(oParam)
		{
			...
			if (ns1blankspace.objectContext != -1 && ns1blankspace.objectContextData['audit.sequalifyingtraininglink'] != '')
			{
				$.each($('#ns1blankspaceMainTrainee tr.ns1blankspaceRow'), function()
				{
					if ($(this).attr('id').split('_').pop() == ns1blankspace.objectContextData['audit.sequalifyingtraininglink'])
					{
		...
		getTraineeDetails: function()
		{
			...
			if ($('#ns1blankspaceMainTrainee tr.ns1blankspaceRow .nsFreshcareSelected').length == 0)
			{
				...
				aHTML.push('<tr>' +
							...
							'<tr class="ns1blankspace">' +
								'<td class="ns1blankspaceText">' +
								'<input id="ns1blankspaceTraineeDetailsTrainer" class="nsFreshcareSelectGrower"' +
				...
				// v3.3.001 Don't need to filter on relationship as it may not yet have been added
				...
				.dialog(
				{
					...
					{ 
						...
						$('.ui-dialog .ui-dialog-buttonset').before('<span id="ns1blankspaceTraineeDetailsError" style="color: red;"></span>');
					},
					...
					[
						{
							...
							click: function()
							{
								...
								$(this).dialog('destroy');
								nsFreshcare.admin.audit.trainee.getTraineeDetails();
							}
						},
						{
							text: 'No',
		...
		getTraineeData: function(bNew)
		{
			...
			if ($('#ns1blankspaceMainTrainee').html() != '')
			{
				...
				{
					iQualifyingTraining = ns1blankspace.objectContextData['audit.sequalifyingtraininglink'];
				}
		...
		search: function (oParam, oResponse) 
		{
			...
			if (iStep === 1 && iAudit && ns1blankspace.objectContextData) 
			{
				...
				oSearch.addField('auditissue.audit,auditissue.reference,auditissue.details,auditissue.datecompleted,auditissue.resolution,auditissue.modifieddate' + 
								',auditissue.sintype,auditissue.sintypetext,auditissue.sintype.code,auditissue.status,auditissue.statustext,auditissue.type,auditissue.typetext');
				...
				else 
				{
					...
					$.each(oResponse.data.rows, function() 
					{
						...
						aHTML.push('<td class="ns1blankspaceRow ns1blankspaceRowAuditCAR"' + 
										...
										'">' +
											this["auditissue.sintype.code"] + ' - ' + this["auditissue.sintypetext"] + 
										'</td>');
				...
				else
				{
					...
					$.each(oResponse.data.rows, function() 
					{
						...
						aHTML.push('<td style="font-weight:bold;' + sCertificateStyle + '>' +
										...
										'<td style="' + sCertificateStyle + '>' + oCertificateData.reCertificationDue.formatXHTML() + '</td>' +
										'</tr>');
	...
	save: 		
	{
		validate: 	function(oParam) 
		{
			...
			else if (oParam.auditValidateStep === 1)
			{
				...
				if ($('#ns1blankspaceMainDetails').html() != '')
				{
					...
					if (ns1blankspace.okToSave && nsFreshcare.option.jasanzAccreditation
						&& $('input[name="radioJASANZ"]:checked').val() === undefined) {
				...
				// We need to set Date Result Achieved. Latest of Date Approved or Date all Major CARs closed out
				...
				if (ns1blankspace.okToSave && ns1blankspace.objectContext != -1)
				{
					...
					{
						// v3.3.001 SUP023474 Commented out as now sets resultstatusdate in save.send as button only available for reviewers
					...
					)
				{
					...
					if (iAuditType != nsFreshcare.data.audit.typeScopeExtension)
					{
						...
						if (oAnniversary.status == 1 || oAnniversary.status == 2)
						{
							...
							oSearch.addFilter('object', 'EQUAL_TO', nsFreshcare.objectAudit);
							oSearch.addFilter('objectcontext', 'EQUAL_TO', ns1blankspace.objectContext);
							oSearch.addFilter('actiontype', 'IN_LIST', nsFreshcare.data.actionTypeRecertAuditLate + ',' + nsFreshcare.data.actionTypeOtherReason);
							oSearch.getResults(function(oResponse)
							{
								if (oResponse.status == 'OK')
								{
					...
					[
						{
							...
							click: function()
							{
								...
								$.extend(oParam,
								{
									...
									hidePriority: true,
									hideCancel: true,
									actionType: nsFreshcare.data.actionTypeRecertAuditLate,
			...
			else if (oParam.auditValidateStep === 2)
			{
				...
				if (ns1blankspace.okToSave && $('#ns1blankspaceMainTrainee').html() != '') 
				{
					...
					if (ns1blankspace.objectContext != -1)
					{
						sSavedQualifyingTraining = ns1blankspace.objectContextData['audit.sequalifyingtraininglink'];
			...
			else if (oParam.auditValidateStep === 3)
			{
					...
					)
				{
					...
					if ((ns1blankspace.objectContextData['audit.sequalifyingtraininglink'] == '' || ns1blankspace.objectContextData['audit.sequalifyingtraininglink'] == '0')
		...
		send: 	function(oParam) 
		{
			...
			if (iResultStatus && sData === '')
			{
				...
				oParam.data = 'id=' + ns1blankspace.objectContext + '&resultstatus=' + iResultStatus + '&resultstatusdate=' + dToday.toString('dd MMM yyyy');
			...
			else if (oParam.saveAuditStep === 3 && ns1blankspace.okToSave) 
			{
				...
				if ($('#ns1blankspaceMainTrainee').html() != '' )
				{
					...
					if (oParam.qualifyingTraining != undefined)
					{
						...
						oData['audit.sequalifyingtraininglink'] = oParam.qualifyingTraining;
			...
			else if (oParam.saveAuditStep == 6)
			{
				...
				if (oParam.scopeExtensionIndex < oParam.scopeExtensions.length)
				{
					...
					oData['audit.id'] = ns1blankspace.objectContext;
					oData['audit.subscription'] = oParam.subscription;
					$.ajax(
					{
						type: 'POST',
						url: ns1blankspace.util.endpointURI('AGRI_CERTIFICATE_UPGRADE_MANAGE'),
