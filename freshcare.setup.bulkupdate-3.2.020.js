...
nsFreshcare.setup.bulkupdate = 
{
	...
	show: 	function (oParam) 
	{
			...
			*/
			aHTML.push('<tr class="ns1blankspaceControl">' +
							'<td id="ns1blankspaceControlPopulateAttendanceLinks" class="ns1blankspaceControl ns1blankspaceControlHome">' +
								'Populate Attendance Links table</td>' +
							'</tr>');
	...
	bind: 	function ()
	{
		...
		*/
		$('#ns1blankspaceControlPopulateAttendanceLinks').click(function(event)
		{
			...
			nsFreshcare.setup.bulkupdate.data.userContinue = 2;
			oParam = nsFreshcare.setup.bulkupdate.populateAttendanceLinks.setParams(oParam);
			nsFreshcare.setup.bulkupdate.showUpdate(oParam);
		});
		},
	...
	populateAttendanceLinks:
	{
		...
		setParams: function(oParam)
		{
			...
			oParam.functionUpdate = nsFreshcare.setup.bulkupdate.populateAttendanceLinks.update;
		...
		update: function(oParam)
		{
			...
			else
			{
				...
				if (iDataIndex < nsFreshcare.setup.bulkupdate.data.attendeeData.length)
				{
					...
					if (oThisRow['agritrainingcourseattendee.traineecontactbusiness.agrisubscription.id'] != '' && this.traineecontactbusinesstext != '')
					{
						...
						$.ajax(
						{
							...
							success: function(oResponse)
							{
								...
								{
									aHTML.push('<tr><td>' + (iDataIndex + oParam.startRow) + ' ' + sContactBusinessText + 
												' (' + oThisRow.traineecontactpersontext.formatXHTML() + ') - ' + oThisRow['agritrainingcourseattendee.course.coursedate'] + '</td></tr>');
								}
								else
								{
									aHTML.push('<tr><td>' + (iDataIndex + oParam.startRow) + ' ' + sContactBusinessText + 
					...
					{
						oParam.dataIndex = iDataIndex + 1;
						aHTML.push('<tr><td>' + (iDataIndex + oParam.startRow) + ' ' + sContactBusinessText + ' does not have a ' + 
										(oThisRow.traineecontactbusinesstext == '' ? 'business' : 'subscription') + '..<br />' +
										oThisRow.trainercontactpersontext + ' ' + oThisRow['agritrainingcourseattendee.course.coursedate'] + ' ' + 
