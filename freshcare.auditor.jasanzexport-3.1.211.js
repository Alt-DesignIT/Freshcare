...
nsFreshcare.auditor.jasanzexport = 
{
	data: 
	{
		headers:
		{
			coreDetails:
			[
				{name: 'companyid', caption: 'ID Number', inView: true, field: 'agrisubscription.contactbusiness.reference'},
			...
			exportSites:
			[
				{name: 'companyid', caption: 'ID Number', inView: true, field: 'companyid'},
			...
			[
				{name: 'companyid', caption: 'ID Number', inView: true, field: 'agrisubscription.contactbusiness.reference'},
				{name: 'codevalue', caption: 'Code Value', inView: true, field: 'agrisubscription.agricertificate.certificatenumber'}
			]
		}
	},
	...
	layout: 	function ()
	{
		...
		{			
			...
			aHTML.push('<tr><td id="ns1blankspaceControlCodes" class="ns1blankspaceControl">' +
							'Codes</td></tr>');
		}
		...
		aHTML.push('<div id="ns1blankspaceMainCodes" class="ns1blankspaceControlMain"></div>');
		...
		$('#ns1blankspaceControlCodes').click(function(event)
		{
			ns1blankspace.show({selector: '#ns1blankspaceMainCodes', refresh: true});
			nsFreshcare.auditor.jasanzexport.codes();
		});
	},
	...
	show: 	function (oParam, oResponse)
	{
		...
		if (oParam.searchStep === 1) 
		{
			...
			nsFreshcare.auditor.jasanzexport.data.codes = [];
		...
		else if (oParam.searchStep == 2)
		{
			...
			oSearch.addBracket('(');
			oSearch.addFilter('agrisubscription.agricertificate.enddate', 'GREATER_THAN', dToday.toString('dd MMM yyyy'));
			oSearch.addOperator('or');
			oSearch.addFilter('agrisubscription.agricertificate.enddate', 'IS_NULL');
			oSearch.addBracket(')');
		...
		else if (oParam.searchStep == 5)
		{
			$.each(nsFreshcare.auditor.jasanzexport.data.subscriptions, function(i, subscription)
			{
				...
				var oCodes = {};
				$.each(nsFreshcare.auditor.jasanzexport.data.headers.codes, function(i, map)
				{
					oCodes[map.caption] = (map.field) ? subscription[map.field].formatXHTML() : map.value;	
				});
				nsFreshcare.auditor.jasanzexport.data.codes.push(oCodes);

			});


			// Add Company ID to the Sites object
			$.each(nsFreshcare.auditor.jasanzexport.data.sites, function(i, site)
			{
				if (site.companyid == undefined)
				{
					...
					site.companyid = oSubscription['agrisubscription.contactbusiness.reference']; 
				}
		...
		else if (oParam.searchStep == 10)
		{
			ns1blankspace.objectContextData = 
			{
				...
				codesRows: nsFreshcare.auditor.jasanzexport.data.codes.length
			};
		...
		else
		{
			if ($('#ns1blankspaceSummary').attr('data-loading') == undefined)
			{
				...
				if (ns1blankspace.objectContext != -1 && ns1blankspace.objectContextData)
				{
					...
					aHTML.push('<tr><td>&nbsp;</td></tr>');
					aHTML.push('<tr><td class="ns1blankspaceSummaryCaption">Codes rows: ' +
									'<span style="font-weight: normal;">' + ns1blankspace.objectContextData.codesRows + '</span></td></tr>');
					...
					aHTML.push('<tr><td class="ns1blankspaceSummaryCaption">Total rows: ' +
									'<span style="font-weight: normal;">' + 
										(ns1blankspace.objectContextData.coreDetailsRows +
										 ...
										 ns1blankspace.objectContextData.codesRows) +
										'</span></td></tr>');
				...
				$('#ns1blankspaceSummaryExportData')
					...
					.on('click', function()
					{
						...
						if ((ns1blankspace.objectContextData.coreDetailsRows +
							 ns1blankspace.objectContextData.sitesRows +
							 ns1blankspace.objectContextData.schemesRows +
							 ns1blankspace.objectContextData.codesRows) > 0)
						{
	...
	codes: function()
	{
		nsFreshcare.auditor.jasanzexport.renderDataTab({tabContext: 'Codes', rowData: 'codes'});
	},
