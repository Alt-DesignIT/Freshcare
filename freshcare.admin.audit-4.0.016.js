	...
	setJASANZAuditFlag: function()
	{
		...
		if (ns1blankspace.objectContext == -1)
		{
			if ($('#ns1blankspaceDetailsMembership').attr('data-JASANZMembership') == 'Y')
			{
				...
				{
					var oJASANZ = $.grep(nsFreshcare.data.auditor.jasanzDates, function(x) 
						{return x.membership == $('#ns1blankspaceDetailsMembershipValue').val()}).shift();

					// v3.4.016 Was enabling JASANZ flag if not accredited
					if (oJASANZ)
					{
						if (oJASANZ.notes)
						{
							if (dAudit != '' && dAudit != undefined)
							{
								...
								dJASANZ = new Date(oJASANZ.notes);
								$('[name="radioJASANZ"][value="' + (dAudit >= dJASANZ ? 'Y' : 'N') + '"]').attr('checked', true);
								// v3.3.005 SUP023800 Only disable JASANZ date for other than admin & auditor
								if (nsFreshcare.user.roleID != nsFreshcare.data.roles.admin 
									&& nsFreshcare.user.roleID != nsFreshcare.data.roles.auditor)
								{
							...
							else // Audit date not specified - don't know if JASANZ accredited as yet
							{
								$('[name="radioJASANZ"][value="N"]').attr('checked', true);
								$('[name="radioJASANZ"]').attr('disabled', false);
							}
						}
						else 	// no date specified - not JASANZ accredited
						{
							$('[name="radioJASANZ"][value="N"]').attr('checked', true);
							$('[name="radioJASANZ"]').attr('disabled', true);
						}
					}
					else 	// No matching JASANZ dates - not JASANZ accredited
					{
						$('[name="radioJASANZ"][value="N"]').attr('checked', true);
						$('[name="radioJASANZ"]').attr('disabled', true);
					}
