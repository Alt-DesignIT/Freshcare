...
nsFreshcare.external.freshcareprofile = 
{
	...
	accreditation: 	function(oParam) 
	{
		...
		if (oParam) {
			if (oParam.accreditationStep) {iStep = oParam.accreditationStep}
		}

		if ($('#ns1blankspaceMainAccreditation').attr('data-loading') === '1')
		{

			if (iStep === 1 && ns1blankspace.objectContextData.states === undefined) {
				nsFreshcare.external.freshcareprofile.trainerStates({onComplete: nsFreshcare.external.freshcareprofile.accreditation,
																	 accreditationStep: 2});
			}
			else iStep = 2;

			if (iStep === 2 && nsFreshcare.data.validStates === undefined) {
				nsFreshcare.external.freshcareprofile.validStates({onComplete: nsFreshcare.external.freshcareprofile.accreditation,
																   accreditationStep: 3});
			}
			else iStep = 3;
