...
nsFreshcare.extend = 
{
	financial:
	{
		...
		actions:
		{
			bind: function(oParam)
			{
				...
				var aFilters = [];
				if (nsFreshcare.option.splitActionsAndEmails)
				{
		...
		invoice:
		{
			...
			{	// v3.1.1f SUP022623 Aliased as board members can't add attachments or actions and need 'read-only' access
				...
				// v3.1.019 SUP023960 Cater for Freshmark - no emails tab
			...
			{	
				...
				"default": function (oParam)
				{
					...
					{
						if (oParam.summaryDefaultStep == undefined && ns1blankspace.data.financial 
							&& ns1blankspace.data.financial[ns1blankspace.objectName] 
							&& $.type(ns1blankspace.data.financial[ns1blankspace.objectName].functionRenderPreProcess) == 'function')
						{
			...
			xero: function(oParam)
			{
				...
				if ($('#ns1blankspaceMainXero').attr('data-loading') == '1')
				{
					...
					// v4.0.008 Now uses util function
			...
			email:
			{
				...
				{	// v3.1.0e SUP022278 Aliased because need lineitem alias on FINANCIAL_ITEM_SEARCH results
					...
					// v3.4.019 (Freshmark) Now called for any object
					...
					if (oResponse == undefined)
					{
						...
						oSearch.getResults(function(data)
						{
							...
							if (ns1blankspace.data.financial && ns1blankspace.data.financial[ns1blankspace.objectName] 
								&& $.type(ns1blankspace.data.financial[ns1blankspace.objectName].functionRenderPreProcess) == 'function')
							{
								oParam.response = data;
								oParam.onComplete = nsFreshcare.extend.financial[ns1blankspace.objectName].email.render;
								ns1blankspace.data.financial[ns1blankspace.objectName].functionRenderPreProcess(oParam);
							}
							else
							{
				...
				show: function (oParam)
				{
					...
					// v3.4.020 SUP023960 Parameterised emailTo with options
					var sEmailTo = (ns1blankspace.rootnamespace.data.financial && ns1blankspace.rootnamespace.data.financial.invoice
									&& ns1blankspace.rootnamespace.data.financial.invoice.emailTo)
									? ($.type(ns1blankspace.rootnamespace.data.financial.invoice.emailTo) == 'function'
										? ns1blankspace.rootnamespace.data.financial.invoice.emailTo()
										: ns1blankspace.rootnamespace.data.financial.invoice.emailTo)
									: ns1blankspace.objectContextData['invoice.contactperson.email'];
					...
					$('#ns1blankspaceEmailTo').val(sEmailTo);
				...
				{	// v3.1.0g SUP022278 Want to use rpc email
					...
					// v3.4.019 (Freshmark) Added sFrom
					if (iMode == 1)
					{
						...
						var sFrom = $('#ns1blankspaceEmailFrom').val();
					}	
					else
					{	
						...
						var sFrom = (nsFreshmark) ? nsFreshmark.data.fromEmail : undefined;
					}	
		...
		expense:
		{
				...
				if (oResponse == undefined)
				{
					...
					if (nsFreshcare.util.roleHasAccess({action: 'pay', tabs: ['Home'], access: aAccess}))
					{
						...
						aHTML.push('<tr class="ns1blankspaceControl">' +
									'<td style="padding-top:15px;" id="ns1blankspaceControlOutstanding" class="ns1blankspaceControl">To Be Paid' +
										'<br /><span class="ns1blankspaceSub" style="font-size:0.75em;">Create a file<br />for payment</span></td>' +
									'</tr>');
			...
			{	// v3.1.1f SUP022623 Now prevents board members from adding attachments / actions
				...
				// v3.4.019 SUP023960 Cater for Freshmark - no email tab
				$('#ns1blankspaceControlActions').click(function(event)
				{	// v3.1.1f SUP022623 Board Members can't add actions
					if (nsFreshcare.option.splitActionsAndEmails)
					{
		...
		receipt:
		{
			...
			home: 	function (oParam, oResponse)
			{
				// v3.4.019 SUP023960 Now use roleHasAccess to determine rights
			...
			details: function()
			{
				...
				if ($('#ns1blankspaceMainDetails').attr('data-loading') == '1')
				{
					...
					aHTML.push('<tr class="ns1blankspaceCaption">' +
									...
									'<tr class="ns1blankspace">' +
									'<td class="ns1blankspaceTextMulti">' +
									'<textarea rows="10" cols="35" id="ns1blankspaceDetailsNotes" class="ns1blankspaceTextMulti"' +
										' data-column="notes"></textarea>' +
									'</td></tr>');		
		...
		journal:
		{
			...
			{	// v3.1.1i SUP022764 readonly access
				...
				$('#ns1blankspaceControlItems').click(function(event)
				{
					ns1blankspace.show(
					{
						selector: '#ns1blankspaceMainItem', 
						refresh: true,
						actions: 
						{
							add: nsFreshcare.util.roleHasAccess(
							{
								action: 'create', 
								access: aAccess, 
								tabs: ['Items']
						...
						options: 
						{
							remove: nsFreshcare.util.roleHasAccess(
							{
								action: 'delete', 
								access: aAccess, 
								tabs: ['Items']
					...
					if (oParam.dataIndex < ns1blankspace.financial.invoicing.data.unsentEmail.length)
					{
						...
						else if (oParam.emailSendStep == 3)
						{
							...
							{	
								...
								sTo = (ns1blankspace.isLab) ? 'cassandra.buono@alt-designit.com.au' : sTo;
								var oData = 
								{
				...
				else
				{
					...
					$.each(oResponse.data.rows, function()
					{
						...
						aHTML.push('<td id="tdAction_options-' + this.id + '" class="ns1blankspaceRows" >' + 
										'<span id="ns1blankspaceActionComplete_' + this.id + '" class="ns1blankspaceAction ns1blankspaceActionComplete" title="Complete"></span>' + 
										'<span id="ns1blankspaceActionPriority_' + this.id + '"' +
											' class="ns1blankspaceAction ns1blankspaceActionPriority"' +
											' data-priority="' + this.priority + '" title="Priority"></span>' +
									'</td>');
	...
	format:
	{
		...
		{	
			...
			$('span.template', oXHTML).each(function(i,e) 
			{
				...
				if (oTemplateTag)
				{
					...
					if (oTemplateTag.sourceFunction && oTemplateTag.sourceMethod == undefined)
					{
						oTemplateTag.sourceMethod = oTemplateTag.sourceFunction;
						oTemplateTag.sourceMethodRootNamespaceText = oTemplateTag.sourceFunctionRootNamespaceText;
					}
					if (oTemplateTag.sourceMethod)
					{
						$(e).attr('data-format-source-method', oTemplateTag.sourceMethod);
						$(e).attr('data-format-source-method-namespace', oTemplateTag.sourceMethodRootNamespaceText);
					}
					...
					if (oTemplateTag.object == iObject && oTemplateTag.type == 1)
					{
						...
						if (sSourceMethod)
						{
							var oRoot = (oTemplateTag.sourceMethodRootNamespaceText) ? window[oTemplateTag.sourceMethodRootNamespaceText] : ns1blankspace;
		...
		{	/* To be removed */
			...
			// v3.4.019 (Freshmark) Now uses ns1blanksapce.object

			if (ns1blankspace.data.financial == undefined) {ns1blankspace.data.financial = {}}
			if (ns1blankspace.data.financial[ns1blankspace.objectName] == undefined) 
			{
				ns1blankspace.data.financial[ns1blankspace.objectName] = {maxPage1ScheduleRows: 5, maxPage2ScheduleRows: 30}
			}

			var sTemplate = ns1blankspace.util.getParam(oParam, 'template').value;
			var oXHTML = document;
			var iMaxPage1Items = ns1blankspace.data.financial[ns1blankspace.objectName].maxPage1ScheduleRows;		
			var iMaxPage2Items = ns1blankspace.data.financial[ns1blankspace.objectName].maxPage2ScheduleRows;	
			...
			{	
				...
				oTRClone.find('[data-format-source]').each(function()
				{
					...
					if (sSourceMethod)
					{
						var oRoot = ($(this).attr('data-format-source-method-namespace') != '') 
											? window[$(this).attr('data-format-source-method-namespace')] 
											: ns1blankspace;
						var fFunction = ns1blankspace.util.toFunction(sSourceMethod, oRoot);
		...
		// v4.0.001 Added Temaplte fields
		templateGrowerFirstname: function(oRow)
		{
			return '[TEMPLATE_GROWERFIRSTNAME]';
		},

		templateGrowerSurname: function(oRow)
		{
			return '[TEMPLATE_GROWERSURNAME]';
		},

		templateTradename: function(oRow)
		{
			return '[TEMPLATE_GROWERTRADINGNAME]';
		},

		templateMembership: function(oRow)
		{
			return '[TEMPLATE_MEMBERSHIP]';
		},

		templateAuditDate: function(oRow)
		{
			return '[TEMPLATE_AUDITDATE]';
		},

		templateCertificateNumber: function(oRow)
		{
			return '[TEMPLATE_CERTIFICATENUMBER]';
		},

		templateCertificateExpiry: function(oRow)
		{
			return '[TEMPLATE_CERTIFICATEEXPIRY]';
		},

		templateAuditorName: function(oRow)
		{
			return '[TEMPLATE_AUDITORNAME]';
		},

		templateOpenCARsAll: function(oRow)
		{
			return '[TEMPLATE_OPENCARSALL]';
		},

		templateOpenCARsMajor: function(oRow)
		{
			return '[TEMPLATE_OPENCARSMAJOR]';
		},

		templateOpenCARsMinor: function(oRow)
		{
			return '[TEMPLATE_OPENCARSMINOR]';
		},
		...
		{	
			init:	function (oParam)
			{
				// v4.0.003 Adjusted plugins to match 1bs
				...
				var sImageSearchParams = ns1blankspace.util.getParam(oParam, 'imageSearchParams').value;
				var sMediaSearchParams = ns1blankspace.util.getParam(oParam, 'mediaSearchParams').value;
				...
				if (ns1blankspace.option.richTextEditing)
				{
					...
					{	
						...
						{	
							...
							if (sImageSearchParams)
							{
								oInit.image_list += sImageSearchParams;
							}
					...
					else if (sVersion == '3')
					{
						...
						if (iObject)
						{
							oInit.TemplateLinkType = iObject;
						}
						if (sImageSearchParams)
						{
							oInit.external_image_list_url = '/ondemand/core/?method=CORE_EDITOR_IMAGE_SEARCH' + sImageSearchParams;
						}
						if (sMediaSearchParams)
						{
							oInit.media_external_list_url = '/ondemand/core/?method=CORE_EDITOR_IMAGE_SEARCH' + sMediaSearchParams;
						}
					}	
	...
	document: 
	{
		// v3.4.016 Removed details, networkGroup and save
	...
	setup:
	{
		codes:
		{
			home:	function (oParam, oResponse)
			{
				...
				else
				{
					...
					aHTML.push('<td class="ns1blankspaceHeaderCaption" style="text-align:right"><span id="ns1blankspaceSetupAdd" title="Add"></span></td>');
					aHTML.push('</tr>');
		...
		financial:
		{
			...
			accounts:
			{
				show: function(oParam, oResponse)
				{
					...
					else if (iStep == 4)
					{
						...
						var sXHTMLElementID;
						// v3.2.001 SUP023329 Added bHasAccess - only for Financial role
						// v3.4.019 SUP023960 Changed to use rootnamespace as Freshmark also uses
						var bHasXeroAccess = ($.grep(ns1blankspace.user.roles, function(x) 
						{
							return x.id == ns1blankspace.rootnamespace.data.roles.xeroSuperUser
						}).length == 1);
						var bHasFinancialAccess = ($.grep(ns1blankspace.user.roles, function(x) 
						{
							return x.id == ns1blankspace.rootnamespace.data.roles.financial
						}).length == 1);
			...
			template:
			{	// v4.0.001 Aliased to allow bracketCount parameter
				show: 	function (oParam)
				{
					var sTemplate = ns1blankspace.util.getParam(oParam, 'template', {"default": 'invoice'}).value;
					...
					var iBracketCount = ns1blankspace.util.getParam(oParam, 'bracketCount', {'default': 2}).value;
							...
							if (this.object == iObject && this.type == 1)
							{
								...
								aHTML.push('<span id="spanInterfaceFormatTag_' + (this.caption).replace(/ /g,'-') + '"' +
										  		' class="interfaceFormatTags" ' +
										   		' data-caption="' + Array(iBracketCount + 1).join('[') + 
										   				(this.caption) + Array(iBracketCount + 1).join(']') + '"' +
										   		' style="cursor: pointer;">' + this.caption + '</span>');
				...
				save:		function (oParam)
				{
					...
					var iDefaultDocument = ns1blankspace.util.getParam(oParam, 'documentDefault').value;
					...
					var oData = {id: (iDefaultDocument == iDocument) ? undefined : iDocument};
					oData.content = tinyMCE.get('ns1blankspaceTemplateText_' + sTemplate + sCounter).getContent();
					oData.type = 10;
					oData.title = sTitle;
					if (sURL) {oData.url = sURL}
					if (iObject) {oData.object = iObject}
		...
		people:
		{
			newPage: function(oParam)
			{
				...
				oSearch.getResults(function(oResponse)
				{
					...
					if (oResponse.status == 'OK')
					{
						$.each(oResponse.data.rows, function(index, group)
						{
							$.each($('tr[data-contactbusiness="' + this.contactbusiness + '"]'), function()
							{
								var oElement = $(this).children().last();
								if ($(oElement).html() == ns1blankspace.xhtml.loadingSmall)
								{
									$(oElement)
										.html(group.grouptext.formatXHTML())
										.attr('data-groupID', group.group);
								}
							});
						});
					}
