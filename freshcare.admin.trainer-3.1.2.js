...
nsFreshcare.admin.trainer = 
{
	...
	accreditation: 	function(oParam) 
	{
		...
		if (oParam.accreditationStep == undefined) {oParam.accreditationStep = 0}

		if ($('#ns1blankspaceMainAccreditation').attr('data-loading') === '1')
		{

			if (oParam.accreditationStep === 1 && ns1blankspace.objectContextData.states === undefined) 
			{
				nsFreshcare.external.freshcareprofile.trainerStates({onComplete: nsFreshcare.admin.trainer.accreditation, accreditationStep: 2});
			}
			else oParam.accreditationStep = 2;

			if (oParam.accreditationStep === 2 && nsFreshcare.data.validStates === undefined) 
			{
				nsFreshcare.external.freshcareprofile.validStates({onComplete: nsFreshcare.admin.trainer.accreditation,
																   accreditationStep: 3});
			}
			else oParam.accreditationStep = 3;

			if (oParam.accreditationStep === 3) {
				aHTML = [];
				...
				$.each(nsFreshcare.data.trainerMemberships, function() {
					aHTML.push('<tr class="ns1blankspace">' +
									'<td class="ns1blankspaceReadOnly">&nbsp;</td></tr>');		
				});
	...
	addTrainingCourse: function(oParam, oResponse)
	{
		var iStep = 1;

		if (oParam) 
		{
			if (oParam.addCourseStep) {iStep = oParam.addCourseStep;}
		}
		else {oParam = {addCourseStep: 1}}
		...
		else 
		{
			...
			if (iStep === 1 && oResponse === undefined) 
			{
				...
				oSearch.addField('title,agritrainingpackage.membership.code,codeofpracticetext');
				...
				oSearch.getResults(function(oResponse) {
					...
					nsFreshcare.admin.trainer.addTrainingCourse({addCourseStep: 2}, oResponse);
				});
			}
			else if (iStep === 2 && oResponse) 
			{
				...
				if (oResponse.status === 'OK') 
				{
					$.each(oResponse.data.rows, function() 
					{
						aHTML.push('<tr id="ns1blankspacePackageSearchRow_' + this.id + '">');
				...
				$('td.ns1blankspaceSearch').click(function() {
					...
					if (sPackageId != undefined) 
					{
						...
						var oParam = {functionDefault: 'nsFreshcare.admin.trainingcourse.details())',
										package: sPackageId,
										trainerBusiness: ns1blankspace.objectContext,
										trainerPerson: ns1blankspace.data.contactPerson,
										trainerBusinessText: ns1blankspace.objectContextData['contactbusiness.tradename'],
										trainerPersonText: ns1blankspace.data.contactPersonText,
										"new": true,
										showHome: false};
