	...
	entity:
	{
			...
			if ($('#ns1blankspaceMainDetails').attr('data-loading') == '1')
			{
				...
				if (ns1blankspace.objectContextData != undefined)
				{
					...
					if (sSourceNamespace == 'certificationbody')
					{
						...
						if ($('#ns1blankspaceDetailsJASANZDate').is('*'))
		...
		people:
		{
			...
			newPage: function(oParam)
			{
				...
				else if (oParam.newPageStep == 3)
				{
					...
					// v3.2.015 SUP023422 Added COP Based Training
					if (oParam.pageObject == 'trainer' && nsFreshcare.user.roleID == nsFreshcare.data.roles.admin)
					{
				...
				else if (oParam.newPageStep === 4)
				{
					...
					oParam.newPageStep = (bShowTraining) ? 5 : 6;
					nsFreshcare.internal.entity.people.newPage(oParam);
				}
			...
			details: function(oParam)
			{
				...
				aHTML.push('<tr class="ns1blankspace ns1blankspaceHideNonAdmin">' +
								...
								(oParam.showTrainingCourses === true 
									...
									: (oParam.showMemberships == true ? 'Training Memberships' :'&nbsp;')) +
								'</td></tr>');								
				...
				{	// v3.1.207 SUP023067 Added .formatXHTML() to non-address / name fields
					...
					{
						aHTML = [];
						aHTML.push('<table id="ns1blankspaceContactTrainerMemberships">');
						...
						$.each(oThisContact.trainerMemberships, function()
						{
						...
						$('#ns1blankspaceContactTrainerMemberships .nsFreshcareSelectable')
							.css('cursor', 'pointer')
							.on('click', function()
							{
					...
					if (sID && sXHTMLElementID)
					{
						...
						else if (oParam.savePrimaryContactStep === 3)
						{
							...
							if (nsFreshcare.user.roleID === nsFreshcare.data.roles.auditor || nsFreshcare.user.roleID === nsFreshcare.data.roles.grower || nsFreshcare.user.roleID === nsFreshcare.data.roles.trainer)
							{
								$.extend(oParam, false,
								{
									...
									subject: nsFreshcare.user.roleID + ' ' + ns1blankspace.user.contactBusinessText + ' has changed a primary contact',
									message: nsFreshcare.user.roleID + ' ' + ns1blankspace.user.contactBusinessText + ' has changed the primary contact against ' + ns1blankspace.data.contactBusinessText, 
									contactBusiness: ns1blankspace.objectContext,
						...
						else if (oParam.savePrimaryContactStep === 4)
						{
							...
							{	// We're viewing a grower page
								// v3.2.015 SUP023422 Added COP Based Training
								if (oRoot[nsFreshcare.user.roleID] && oRoot[nsFreshcare.user.roleID].grower)
								{
									oRoot[nsFreshcare.user.roleID].grower.init({id: ns1blankspace.objectContext});
								}
								else
								{
									// v3.2.015 SUP023422 Added COP Based Training
									nsFreshcare[nsFreshcare.user.roleID].grower.init({id: ns1blankspace.objectContext});
								}
							}
							else
							{
								...
								if (oResponse.status === 'OK')
								{
											...
											|| nsFreshcare.user.roleID === nsFreshcare.data.roles.trainer))
									{
										nsFreshcare.external.grower.save.sendEmail(
										{
											...
											subject: nsFreshcare.user.roleID + ' ' + ns1blankspace.user.contactBusinessText + ' has inactivated a contact',
											message: nsFreshcare.user.roleID + ' ' + ns1blankspace.user.contactBusinessText + ' has inactivated contact ' + sContactName + ' from ' + ns1blankspace.data.contactBusinessText, 
											contactBusiness: ns1blankspace.objectContext
										});
									}
								}
				...
				send: function(oParam)
				{
					...
					else if (oParam.savePersonStep === 10)
					{
						...
						{	// We're viewing a grower page
							...
							if (oRoot[nsFreshcare.user.roleID] && oRoot[nsFreshcare.user.roleID].grower && oRoot[nsFreshcare.user.roleID].grower.save)
							{
								fFunctionPostSave = oRoot[nsFreshcare.user.roleID].grower.save.send;
							}
							else
							{
								fFunctionPostSave = nsFreshcare[nsFreshcare.user.roleID].grower.save.send;
							}
							//oParam = {id: ns1blankspace.objectContext};
						}
					}
		...
		training:
		{
			...
			search: function(oParam)
			{
				...
				oSearch.addField('agritraineeattendancelink.attendee.course.coursedate' +
								...
								',contactbusinesstext,contactpersontext,contactperson,contactbusiness' +
				...
				oSearch.getResults(function(oResponse)
				{
					if (oResponse.status === 'OK')
					{
						ns1blankspace.objectContextData[sStoreColumn] = oResponse.data.rows;	
			...
			show: function(oParam)
			{
				...
				if (ns1blankspace.objectContextData.training === undefined)
				{
				...
				else
				{
					...
					else
					{
						...
						{
							aHTML.push(nsFreshcare.internal.entity.training.row(this));
						});
						...
						ns1blankspace.render.page.show(
						{
							...
							functionNewPage: 'nsFreshcare.internal.entity.training.bind()',
							type: 'json'
						}); 	
			...
			row: function(oRow)
			{
				...
				aHTML.push('<td id="ns1blankspaceTraining_agritraineeattendancelink.attendee.course.coursedate-' + oRow.id + '" class="ns1blankspaceRow">' +
								oRow['agritraineeattendancelink.attendee.course.coursedate'] + '</td>');

				aHTML.push('<td id="ns1blankspaceTraining_agritraineeattendancelink.attendee.course.trainercontactbusinesstext-' + oRow.id + '" class="ns1blankspaceRow">' +
								oRow['agritraineeattendancelink.attendee.course.trainercontactbusinesstext'] + '</td>');

				aHTML.push('<td id="ns1blankspaceTraining_agritraineeattendancelink.attendee.course.trainercontactpersontext-' + oRow.id + '" class="ns1blankspaceRow">' +
								oRow['agritraineeattendancelink.attendee.course.trainercontactpersontext'] + '</td>');

				aHTML.push('<td id="ns1blankspaceTraining_agritraineeattendancelink.attendee.course.package.membership.code-' + oRow.id + '" class="ns1blankspaceRow">' +
		...
		actions:
		{
			...
			row: function(oRow, oParam)
			{
				...
				if (bShowDescription && oRow.description)
				{
				...
				if (oResponse === undefined)
				{
					...
					if (oData != undefined)
					{
						...
						$.ajax(
						{
							...
							data: oSaveData,
							dataType: 'json',
							success: function(data) {fPostSave(oParam, oData, data);}
						});
					}
					else
					{
			...
			send: function (sXHTMLElementId, oParam)
			{
				...
				{							
					...
					oSearch.addField( 'reference,tradename,legalname,phonenumber,faxnumber,industry,industrytext' +
										...
										',webaddress,area,areatext,sexerocontactid,sexerocontactupdated' +
