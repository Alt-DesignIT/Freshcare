...
nsFreshcare.admin.newtrainee = 
{
	...
	home: 		function (oParam, oResponse)
	{
		...
		if (oParam.homeStep == 1)
		{
			...
			// v3.3.002 SUP023800 Added Membership to search fields
			oParam.homeStep = 2;
			...
			oSearch.addField('agritrainingcourseattendee.course.coursedate,agritrainingcourseattendee.course.trainercontactbusinesstext,traineecontactbusinesstext,course,traineecontactbusiness,' +
							...
							'agritrainingcourseattendee.traineecontactbusiness.agrisubscription.id,agritrainingcourseattendee.course.package.membership');	
		...
		else if (oParam.homeStep == 2)
		{
			...
			else
			{
				...
				{
					aHTML.push('<tr class="ns1blankspaceRow nsFreshcarePrimary" data-growerid="' + this.traineecontactbusiness + '"' +
									' data-courseId="' + this.course + '"' +
									' data-traineeid="' + this.id + '"' +
									' data-membershipId="' + this['agritrainingcourseattendee.course.package.membership'] + '"' +
									' data-subscriptionId="' + this['agritrainingcourseattendee.traineecontactbusiness.agrisubscription.id'] + '">');
					
					// v3.3.003 SUP023800 Removed alias from traineecontactbusiness field
				...
				if (oResponse.status == 'OK')
				{
								...
								else
								{
									aHTML.push('<tr class="nsFreshcareSecondary ns1blankspaceRow" data-growerid="' + $(growerRow).attr('data-growerid') + '"' +
													...
													'<td style="vertical-align:middle; text-align: center;"><a href="#" class="nsFreshcareCreateLink"' +
															...
															' data-subscriptionId="' + $(growerRow).attr('data-subscriptionId') + '"' +
															' data-membershipId="' +  $(growerRow).attr('data-membershipId') + '"' +
					...
					$('.nsFreshcareCreateLink')
						...
						.on('click', function()
						{
							...
							var oData = 
							{
								id: $(oElement).attr('data-traineeid'),
								traineecontactperson: $(oElement).attr('data-contactId'),
								firstname: $(oElement).attr('data-contactfirstname'),
								surname: $(oElement).attr('data-contactsurname')
							}
							...
							$.ajax(
							{
								...
								success: function(oResponse)
								{
									if (oResponse.status == 'OK')
									{
										var oCheckLinkParam = 
										{
											onComplete: nsFreshcare.admin.newtrainee.home,
											contactBusiness: $(oElement).attr('data-businessId'),
											contactPerson: $(oElement).attr('data-contactId'),
											subscription: $(oElement).attr('data-subscriptionId'),
											membership: $(oElement).attr('data-membershipId'),
											attendee: $(oElement).attr('data-traineeid')
										};
										nsFreshcare.admin.newtrainee.checkAttendanceLink(oCheckLinkParam);
									}
									else
									{
	...
	checkAttendanceLink: function(oParam)
	{	// v3.3.002 SUP023800 Now checks to see if Attendance Link exists and creates if not
		var iBusiness = ns1blankspace.util.getParam(oParam, 'contactBusiness').value;
		var iPerson = ns1blankspace.util.getParam(oParam, 'contactPerson').value;
		var iSubscription = ns1blankspace.util.getParam(oParam, 'subscription').value;
		var iMembership = ns1blankspace.util.getParam(oParam, 'membership').value;
		var iAttendee = ns1blankspace.util.getParam(oParam, 'attendee').value;

		oParam.checkLinkStep = oParam.checkLinkStep || 1;

		if (oParam.checkLinkStep == 1)
		{
			if (iBusiness && iPerson && iAttendee && iMembership)
			{
				var oSearch = new AdvancedSearch();
				...
				oSearch.addField('attendee,contactbusiness,contactperson,subscription,' + 
									...
									'agritraineeattendancelink.attendee.traineecontactbusiness.agrisubscription.membership');
				oSearch.addFilter('attendee', 'EQUAL_TO', iAttendee);
				oSearch.addFilter('contactbusiness', 'EQUAL_TO', iBusiness);
				oSearch.addFilter('contactperson', 'EQUAL_TO', iPerson);
				oSearch.getResults(function(oResponse)
				{
					if (oResponse.status == 'OK')
					{
						$.each(oResponse.data.rows, function()
						{
							if (this['agritraineeattendancelink.attendee.course.package.membership'] == iMembership
								&& this['agritraineeattendancelink.attendee.traineecontactbusiness.agrisubscription.membership'] == iMembership)
							{
								oParam.linkID = this.id;
							}
						});

						oParam.checkLinkStep = 2;
						nsFreshcare.admin.newtrainee.checkAttendanceLink(oParam);						
					}
				});
			}
			else
			{
				ns1blankspace.status.error('Cannot create Attendance Link: Insufficient information. Contact support');
			}
		}

		else if (oParam.checkLinkStep == 2)
		{
			if (oParam.linkID == undefined)
			{
				var oData = 
				{
					attendee: iAttendee,
					contactbusiness: iBusiness,
				...
				$.ajax(
				{
					...
					success: function(oResponse)
					{
						if (oResponse.status == 'OK')
						{
							ns1blankspace.status.message('Attendance Link added..');
						...
						{
							ns1blankspace.status.error('Attendance link not added: ' + oResponse.error.errornotes);
						}
					}
				})	
			}
			...
			{
				ns1blankspace.status.message('Attendance link already exists..');
				if (oParam.onComplete)
				{
					ns1blankspace.util.onComplete(oParam);
				}
		...
		send: 		function (sXHTMLElementId, oParam)
		{
			...
			if (sSearchContext != undefined && iSource != ns1blankspace.data.searchSource.browse)
			{
				...
				oSearch.addField('agritrainingcourseattendee.course.coursedate,agritrainingcourseattendee.course.trainercontactbusinesstext,traineecontactbusinesstext,course,traineecontactbusiness,' +
							...
							',agritrainingcourseattendee.traineecontactbusiness.agrisubscription.id');		//v3.3.008 Added subscription id
	...
	createNewTrainee:
	{
		...
		send: function(oParam)
		{
			...
			// v3.3.005 SUP023807 Search for existing attendance link
			else if (oParam.createTraineeStep == 5)
			{
				if (nsFreshcare.option.qualifyingTraining)
				{
					var oSearch = new AdvancedSearch();
					oSearch.method = 'AGRI_TRAINEE_ATTENDANCE_LINK_SEARCH';
					oSearch.addField('id,attendee,contactbusiness,contactperson,subscription');
					oSearch.addFilter('attendee', 'EQUAL_TO', ns1blankspace.objectContext);
					// v3.3.008 SUP023842 direction added
					oSearch.sort('id', 'asc');		// We want to update the first link added as this will be the one that matches the training record
					oSearch.getResults(function(oResponse)
					{
						if (oResponse.status == 'OK')
						{
							if (oResponse.data.rows.length > 0)
							{oParam.attendanceLink = oResponse.data.rows[0].id;}
							oParam.createTraineeStep = 6;
							nsFreshcare.admin.newtrainee.createNewTrainee.send(oParam);
						}
						else
						{
			...
			// Add Attendance Link
			else if (oParam.createTraineeStep == 6)
			{
				if (nsFreshcare.option.qualifyingTraining)
				{
					oData.id = oParam.attendanceLink; 	// v3.3.005 Update existing row if there is one
					oData.attendee = ns1blankspace.objectContext;
					oData.contactbusiness = ns1blankspace.objectContextData.traineecontactbusiness;
					oData.contactperson = oParam.contactID;
					oData.subscription = ns1blankspace.objectContextData['agritrainingcourseattendee.traineecontactbusiness.agrisubscription.id'];
