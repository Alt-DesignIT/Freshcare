	...
	home: function (oParam, oResponse)
	{
		if (oResponse == undefined)
		{
			...
			$('#ns1blankspaceControlCustomerSuppliers').click(function(event)
			{
				...
				oParam.columns = [
									{title: 'Grower', name: 'ContactBusinessText', field: 'contactbusinesstext'},
									{title: 'Grower Contact', name: 'ContactPersonText', 
			...
			$('#ns1blankspaceControlByGrowerCertBody').click(function(event)
			{
				...
				oParam = {};
				oParam.xhtmlElementID = 'ns1blankspaceMain';
				oParam.label = 'Cert Body / Grower Links';
				...
				oParam.columns = [
									...
									{title: 'Grower', name: 'OtherContactBusinessText', field: 'othercontactbusinesstext'},
									{title: 'Grower Contact', name: 'OtherContactPersonText', field: 'othercontactpersontext'},
									{name: 'ContactBusiness', field: 'contactbusiness', hidden: true},
		...
		else
		{
			...
			else
			{
				aHTML.push('<table>');
				aHTML.push('<tr>');
				aHTML.push('<td class="ns1blankspaceCaption">MOST RECENTLY MODIFIED</td>');
				aHTML.push('</tr>');
	...
	details: function ()
	{
		...
		if ($('#ns1blankspaceMainDetails').attr('data-loading') == '1')
		{
			...
			else
			{
				...
				{
					if (nsFreshcare.admin.relationships.data.contactBusiness)
					{
	...
	linkRequests: 
	{
		...
		show: function(oParam, oResponse)
		{
			...
			if ($('#' + sXHTMLElementID).attr('data-loading') === '1') 
			{
				...
				aHTML = [];
				aHTML.push('<span id="ns1blankspaceLinks' + sXHTMLContext + 'SearchHandle" style="height:25px">Search Criteria</span>');
				aHTML.push('<span id="ns1blankspaceLinks' + sXHTMLContext + 'Process" style="height:25px">Create Links</span>');
				aHTML.push('<span id="ns1blankspaceLinks' + sXHTMLContext + 'Delete" style="height:25px">Delete Links</span>');
						...
						oSearch.getResults(function(oResponse)
						{
							...
							if (oResponse.status === 'OK')
							{
								...
								else
								{
									...
									{
										sErrorNote = 'The Grower has requested that this Supplier not be linked.';
									}
									else
									{
							...
							$.ajax(
							{
								...
								success: function(oResponse)
								{
									...
									else
									{
										...
										nsFreshcare.admin.certificate.rowStatus({xhtmlElement: oThisElement, 
																	errorMessage: 'Email to Grower could not be sent.',
																	errorNotes: oResponse.error.errornotes});
									}
									nsFreshcare.admin.relationships.linkRequests.process(oParam);
								}
					...
					else if (oParam.linksProcessStep === 10)
					{
						...
						if (oParam.successful)		
						{
							...
							{	// Supplier email sucessful but Grower email was not - put Grower email error back on page
								nsFreshcare.admin.certificate.rowStatus({xhtmlElement: oThisElement, 
															errorMessage: 'Customer email sent but Email to Grower failed.',
							...
							{	// Link create was successful (because oParam.successful) but link remove wasn't - put link remove error back on page
								nsFreshcare.admin.certificate.rowStatus({xhtmlElement: oThisElement, 
															errorMessage: 'Customer & Grower email sent but link request not removed.',
					...
					$('#ns1blankspaceLinksCustomerSupplierLinks_ContactPersonText-' + oRowId)
						...
						.hover(
							function(event) 
							{
								...
								$('#ns1blankspaceMultiUseDialog').html('<table class="ns1blankspaceSearchMedium" style="width:100px;"><tr>' +
												'<td><span class="ns1blankspaceSearch" style="font-size:0.625em;">Grower has no email address!</span></td>' +
												'</tr></table>');
							},
							function() 
							{
